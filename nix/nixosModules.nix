# nix/nixosModules.nix — NixOS module for shaggy-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: shaggy runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $SHAGGY_HOME/.env
# and read by shaggy at startup — no container recreation needed for env changes.
#
# Tool resolution: the shaggy wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.shaggy-agent;
    effectivePackage =
    ...
    shaggy-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-shaggy-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "shaggy-config-attrs";
      description = "Shaggy YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "shaggy-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "shaggy-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'SHAGGY_DOC_EOF'\n${value}\nSHAGGY_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "shaggy-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/shaggy";
    ...
    # shaggy user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "shaggy-container-entrypoint" ''
      set -eu

      SHAGGY_UID="''${SHAGGY_UID:?SHAGGY_UID must be set}"
      SHAGGY_GID="''${SHAGGY_GID:?SHAGGY_GID must be set}"

      # ── Group: ensure a group with GID=$SHAGGY_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$SHAGGY_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="shaggy"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$SHAGGY_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$SHAGGY_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$SHAGGY_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$SHAGGY_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="shaggy"
        TARGET_HOME="/home/shaggy"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$SHAGGY_UID" -g "$SHAGGY_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$SHAGGY_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$SHAGGY_UID:$SHAGGY_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure SHAGGY_HOME is owned by the target user.
      ...
      if [ -n "''${SHAGGY_HOME:-}" ] && [ -d "$SHAGGY_HOME" ]; then
        find "$SHAGGY_HOME" \! -user "$SHAGGY_UID" -exec chown "$SHAGGY_UID:$SHAGGY_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/shaggy-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/shaggy-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/shaggy ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/shaggy
        chmod 0440 /etc/sudoers.d/shaggy
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$SHAGGY_UID" --regid="$SHAGGY_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $SHAGGY_HOME/.env.
    ...
    # Default: /var/lib/shaggy/workspace → /data/workspace.
  ...
  in {
    options.services.shaggy-agent = with lib; {
      enable = mkEnableOption "Shaggy Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = shaggy-agent;
        description = "The shaggy-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/shaggy";
        description = "State directory. Contains .shaggy/ subdir (SHAGGY_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Shaggy config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $SHAGGY_HOME/.env at activation time.
          Shaggy reads this file on every startup via load_shaggy_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $SHAGGY_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $SHAGGY_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `shaggy gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the shaggy user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the shaggy plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Shaggy discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          shaggy_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as shaggy (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in shaggy-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the shaggy CLI to environment.systemPackages and export
          SHAGGY_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.shaggy symlink to the service
            stateDir. These users are automatically added to the shaggy group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.shaggy-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the shaggy CLI to system PATH and export SHAGGY_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.shaggy/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.SHAGGY_HOME = "${cfg.stateDir}/.shaggy";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.shaggy-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the shaggy user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.shaggy-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed shaggy
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure shaggy is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.shaggy        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.shaggy/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.shaggy/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.shaggy/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.shaggy/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.shaggy/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."shaggy-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.shaggy
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.shaggy ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.shaggy ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.shaggy -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.shaggy/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.shaggy/$_subdir"
            chmod 2770 "${cfg.stateDir}/.shaggy/$_subdir"
            find "${cfg.stateDir}/.shaggy/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.shaggy/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.shaggy/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.shaggy/config.yaml
            chmod 0640 ${cfg.stateDir}/.shaggy/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.shaggy/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.shaggy/.managed
          chmod 0644 ${cfg.stateDir}/.shaggy/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.shaggy/.container-mode <<'SHAGGY_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    shaggy_bin=${containerDataDir}/current-package/bin/shaggy
    SHAGGY_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.shaggy/.container-mode
            chmod 0644 ${cfg.stateDir}/.shaggy/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.shaggy/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.shaggy";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.shaggy" ]; then
                  rm -f "${symlinkPath}"
                  echo "shaggy-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.shaggy -> stateDir/.shaggy for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.shaggy";
                target = "${cfg.stateDir}/.shaggy";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "shaggy-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.shaggy/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.shaggy/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.shaggy/auth.json
              fi
            ''}
          ''}
          ...
          # Shaggy reads $SHAGGY_HOME/.env at startup via load_shaggy_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.shaggy/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'SHAGGY_NIX_ENV_EOF'
    ${envFileContent}
    SHAGGY_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.shaggy/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.shaggy/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.shaggy/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.shaggy-agent = {
          description = "Shaggy Agent Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            SHAGGY_HOME = "${cfg.stateDir}/.shaggy";
            SHAGGY_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $SHAGGY_HOME/.env by the activation script. load_shaggy_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/shaggy"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the shaggy group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.shaggy-agent = {
          description = "Shaggy Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              SHAGGY_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              SHAGGY_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env SHAGGY_UID="$SHAGGY_UID" \
                --env SHAGGY_GID="$SHAGGY_GID" \
                --env SHAGGY_HOME=${containerDataDir}/.shaggy \
                --env SHAGGY_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/shaggy gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
