...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      shaggy-agent = self'.packages.default;
      shaggyVenv = shaggy-agent.shaggyVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "shaggy-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${shaggyVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "shaggy-cross-eval" { } (
          if failures != [] then
        ...
        package-contents = pkgs.runCommand "shaggy-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${shaggy-agent}/bin/shaggy || (echo "FAIL: shaggy binary missing"; exit 1)
          test -x ${shaggy-agent}/bin/shaggy-agent || (echo "FAIL: shaggy-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${shaggy-agent}/bin/shaggy version 2>&1 | grep -qi "shaggy" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "shaggy-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in shaggy shaggy-agent shaggy-acp; do
            test -x ${shaggy-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "shaggy-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking shaggy --help ==="
          ${shaggy-agent}/bin/shaggy --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${shaggy-agent}/bin/shaggy --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "shaggy-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${shaggy-agent}/share/shaggy-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${shaggy-agent}/share/shaggy-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "SHAGGY_BUNDLED_SKILLS" ${shaggy-agent}/bin/shaggy || \
            (echo "FAIL: SHAGGY_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: SHAGGY_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "shaggy-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${shaggy-agent}/share/shaggy-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${shaggy-agent}/share/shaggy-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "SHAGGY_BUNDLED_PLUGINS" ${shaggy-agent}/bin/shaggy || \
            (echo "FAIL: SHAGGY_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: SHAGGY_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-tui = pkgs.runCommand "shaggy-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${shaggy-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${shaggy-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "SHAGGY_TUI_DIR" ${shaggy-agent}/bin/shaggy || \
            (echo "FAIL: SHAGGY_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: SHAGGY_TUI_DIR set in wrapper"
        ...
        # Verify SHAGGY_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        shaggy-node = pkgs.runCommand "shaggy-node-version" { } ''
          set -e
          echo "=== Checking SHAGGY_NODE in wrapper ==="
          grep -q "SHAGGY_NODE" ${shaggy-agent}/bin/shaggy || \
            (echo "FAIL: SHAGGY_NODE not set in wrapper"; exit 1)
          echo "PASS: SHAGGY_NODE present in wrapper"

          SHAGGY_NODE=$(sed -n "s/^export SHAGGY_NODE='\(.*\)'/\1/p" ${shaggy-agent}/bin/shaggy)
          test -x "$SHAGGY_NODE" || (echo "FAIL: SHAGGY_NODE=$SHAGGY_NODE not executable"; exit 1)
          echo "PASS: SHAGGY_NODE executable at $SHAGGY_NODE"

          NODE_MAJOR=$("$SHAGGY_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All SHAGGY_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify SHAGGY_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "shaggy-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(SHAGGY_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking SHAGGY_MANAGED guards ==="
          check_blocked "config set" ${shaggy-agent}/bin/shaggy config set model foo
          check_blocked "config edit" ${shaggy-agent}/bin/shaggy config edit
        ...
        extra-python-packages = let
          ...
          shaggyWithExtra = shaggy-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "shaggy-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${shaggyWithExtra}/bin/shaggy || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${shaggyWithExtra}/bin/shaggy || \
          ...
          if grep -q "PYTHONPATH" ${shaggy-agent}/bin/shaggy; then
        ...
        extra-dependency-groups = let
          shaggyWithGroups = shaggy-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "shaggy-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${shaggyWithGroups}"
          echo "venv: ${shaggyWithGroups.shaggyVenv}"
        ...
        in pkgs.runCommand "shaggy-config-roundtrip" {
          ...
          merge_and_load() {
            ...
            ${configMergeScript} ${nixSettings} "$shaggy_home/config.yaml"
            ${shaggyVenv}/bin/python3 -c '
import json, sys
