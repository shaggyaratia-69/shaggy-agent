...
Auto-tracks and cleans up ephemeral files created during Shaggy Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$SHAGGY_HOME` and `/tmp/shaggy-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `SHAGGY_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `SHAGGY_HOME` or `/tmp/shaggy-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$SHAGGY_HOME/disk-cleanup/` is itself excluded
- `$SHAGGY_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
