"""disk_cleanup — ephemeral file cleanup for Shaggy Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under SHAGGY_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly SHAGGY_HOME and /tmp/shaggy-*
Never touches: ~/.shaggy/logs/ or any system directory.
"""
...
try:
    ...
    def get_shaggy_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("SHAGGY_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".shaggy").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$SHAGGY_HOME/logs/``."""
    return get_shaggy_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$SHAGGY_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under SHAGGY_HOME or ``/tmp/shaggy-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    shaggy_home = get_shaggy_home()
    try:
        path.resolve().relative_to(shaggy_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/shaggy-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("shaggy-"):
        return True
    return False
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside SHAGGY_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under SHAGGY_HOME (but leave SHAGGY_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "shaggy-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(shaggy_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == shaggy_home:
                continue
            try:
                rel_parts = dirpath.relative_to(shaggy_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    shaggy_home = get_shaggy_home()
    try:
        rel = path.resolve().relative_to(shaggy_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "shaggy-agent",
        }:
            return None
        ...
        # Path isn't under SHAGGY_HOME (e.g. /tmp/shaggy-*) — fall through.
        pass
