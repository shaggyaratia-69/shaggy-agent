...
Optional Shaggy observability plugin that maps Shaggy observer hooks to
...
not replace Shaggy Agent's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Shaggy emit NeMo Relay lifecycle events for
the work Shaggy already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Shaggy Agent can:

- Preserve Shaggy execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
shaggy plugins enable observability/nemo_relay
```

The `SHAGGY_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `SHAGGY_HOME` that the
agent run will use:

```bash
env SHAGGY_HOME=/tmp/shaggy-nemo-relay-test \
  shaggy plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`SHAGGY_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`SHAGGY_HOME` is the Shaggy profile/config home used by both
`shaggy plugins enable ...` and the later `shaggy chat ...` run. If unset,
Shaggy uses the user's default home, usually `~/.shaggy`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export SHAGGY_HOME=/tmp/shaggy-nemo-relay-test
shaggy plugins enable observability/nemo_relay
shaggy chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `shaggy` command you run is built from the
...
uv run shaggy plugins enable observability/nemo_relay
uv run shaggy chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/shaggy_agent-*.whl
...
The plugin can configure exporters directly from `SHAGGY_NEMO_RELAY_*`
...
```bash
export SHAGGY_NEMO_RELAY_ATOF_ENABLED=1
export SHAGGY_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export SHAGGY_NEMO_RELAY_ATIF_ENABLED=1
export SHAGGY_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `SHAGGY_NEMO_RELAY_ATOF_FILENAME`
- `SHAGGY_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `SHAGGY_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `SHAGGY_NEMO_RELAY_ATIF_AGENT_NAME`
- `SHAGGY_NEMO_RELAY_ATIF_AGENT_VERSION`
- `SHAGGY_NEMO_RELAY_ATIF_MODEL_NAME`
- `SHAGGY_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point Shaggy at it:

```bash
export SHAGGY_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
agent_name = "Shaggy Agent"
agent_version = "local"
```

When `SHAGGY_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`SHAGGY_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`SHAGGY_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Shaggy does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Shaggy keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, Shaggy routes LLM and tool
...
For the full generic Shaggy middleware contract, see
...
export SHAGGY_HOME=/tmp/shaggy-nemo-relay-docs/shaggy-home
mkdir -p "$SHAGGY_HOME"

cat > "$SHAGGY_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Shaggy session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export SHAGGY_NEMO_RELAY_ATOF_ENABLED=1
export SHAGGY_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/shaggy-nemo-relay-docs/subagent/atof
export SHAGGY_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export SHAGGY_NEMO_RELAY_ATOF_MODE=overwrite
export SHAGGY_NEMO_RELAY_ATIF_ENABLED=1
export SHAGGY_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/shaggy-nemo-relay-docs/subagent/atif
export SHAGGY_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export SHAGGY_NEMO_RELAY_ATIF_AGENT_NAME='Shaggy Agent E2E'
export SHAGGY_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export SHAGGY_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"shaggy.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Shaggy Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Shaggy dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/shaggy-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/shaggy-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/shaggy-nemo-relay-docs/workdir/beta.txt
cd /tmp/shaggy-nemo-relay-docs/workdir

export SHAGGY_NEMO_RELAY_ATOF_ENABLED=1
export SHAGGY_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/shaggy-nemo-relay-docs/parallel/atof
export SHAGGY_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export SHAGGY_NEMO_RELAY_ATOF_MODE=overwrite
export SHAGGY_NEMO_RELAY_ATIF_ENABLED=1
export SHAGGY_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/shaggy-nemo-relay-docs/parallel/atif
export SHAGGY_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export SHAGGY_NEMO_RELAY_ATIF_AGENT_NAME='Shaggy Agent E2E'
export SHAGGY_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Shaggy sessions map to `agent` scopes.
- Shaggy API request hooks map to `llm` scope start/end events.
- Shaggy tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses Shaggy execution middleware to hand
...
Enable it for Shaggy:

```bash
export SHAGGY_NEMO_RELAY_PLUGINS_TOML=/tmp/shaggy-middleware-test/plugins.toml
```

When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, Shaggy routes execution
through these boundaries:

```text
Shaggy provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> Shaggy provider adapter next_call(...)

Shaggy tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> Shaggy tool dispatcher next_call(...)
```
...
middleware for a local Shaggy run. This path requires a NeMo Relay runtime that
...
```bash
export SHAGGY_HOME=/tmp/shaggy-middleware-test/shaggy-home
mkdir -p "$SHAGGY_HOME" /tmp/shaggy-middleware-test/nemo-relay
...
cat > /tmp/shaggy-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/shaggy-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/shaggy-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "Shaggy Middleware E2E"
agent_version = "local"
...
export SHAGGY_NEMO_RELAY_PLUGINS_TOML=/tmp/shaggy-middleware-test/nemo-relay/plugins.toml
