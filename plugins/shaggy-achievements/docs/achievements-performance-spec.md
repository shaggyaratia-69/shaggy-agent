# Shaggy Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: shaggy-achievements plugin
...
### `GET /api/plugins/shaggy-achievements/achievements`
...
### `POST /api/plugins/shaggy-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/shaggy-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/shaggy-achievements/overview`
...
- Persist plugin data under:
  - `~/.shaggy/plugins/shaggy-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.shaggy/plugins/shaggy-achievements/scan_checkpoint.json`
...
- `~/.shaggy/plugins/shaggy-achievements/`
