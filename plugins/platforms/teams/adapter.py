"""
Microsoft Teams platform adapter for Shaggy Agent.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Shaggy"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        shaggy_action = data.get("shaggy_action", "")
        session_key = data.get("session_key", "")

        if not shaggy_action or not session_key:
        ...
        choice = choice_map.get(shaggy_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="shaggy_approve",
                    data={**btn_data_base, "shaggy_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="shaggy_approve",
                    data={**btn_data_base, "shaggy_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="shaggy_approve",
                    data={**btn_data_base, "shaggy_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="shaggy_approve",
                    data={**btn_data_base, "shaggy_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Shaggy\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.shaggy/.env")
