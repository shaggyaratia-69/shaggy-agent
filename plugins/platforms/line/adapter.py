"""
LINE Messaging API platform adapter for Shaggy Agent.
...
to Shaggy Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``SHAGGY_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            shaggy_home = Path.home().joinpath(".shaggy").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``shaggy status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``shaggy status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``shaggy setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.shaggy/.env`` via ``shaggy_cli.config``.
    """
    print()
    ...
    try:
        from shaggy_cli.config import get_env_var, set_env_var
    except ImportError:
        print("shaggy_cli.config not available; set LINE_* vars manually in ~/.shaggy/.env")
        return
