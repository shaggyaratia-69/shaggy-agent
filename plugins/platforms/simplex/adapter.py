"""SimpleX Chat platform adapter (Shaggy plugin).
...
This adapter ships as a Shaggy platform plugin under
``plugins/platforms/simplex/``. The Shaggy plugin loader scans the
...
discoverable and `shaggy setup` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Shaggy — no
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        payload = {
            "corrId": f"shaggy-snd-{int(time.time() * 1000)}",
            "cmd": cmd_str,
        }
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``shaggy setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / home channel.
    Writes to ``~/.shaggy/.env`` via ``shaggy_cli.config``.
    """
    print()
    ...
    try:
        ...
        print("shaggy_cli.config not available; set SIMPLEX_* vars manually in ~/.shaggy/.env")
        return
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # env-only setups show up in `shaggy gateway status` without
