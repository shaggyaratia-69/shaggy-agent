...
# ``model_tools`` import time, so ``shaggy status``, ``shaggy chat``, etc.
...
# ``shaggy_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Shaggy is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a shaggy MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${SHAGGY_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _shaggy_home = _get_shaggy_home()
        except (ModuleNotFoundError, ImportError):
            _shaggy_home = _Path.home() / ".shaggy"
        self._thread_count_store = _ThreadCountStore(
            _shaggy_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Shaggy is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("SHAGGY_HOME", str(_Path.home() / ".shaggy"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${SHAGGY_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a shaggy MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Shaggy is thinking…' marker message.
        ...
        "Shaggy is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Shaggy is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Shaggy is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Shaggy is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Shaggy is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``shaggy setup``.

    The setup wizard at ``shaggy_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.shaggy/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. shaggy-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.shaggy/.env")
    print_info("Restart the gateway: shaggy gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``shaggy cron`` running as a
    separate process from ``shaggy gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Shaggy plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'shaggy-agent[google_chat]'",
        ...
        # the "Shaggy is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Shaggy is thinking…' marker message "
