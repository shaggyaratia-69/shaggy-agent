...
Credential storage mirrors every other Shaggy channel:

    * runtime SDK creds  -> ``~/.shaggy/.env``  (``PHOTON_PROJECT_ID`` =
      spectrumProjectId, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.shaggy/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a shaggy dependency
...
# registers Shaggy as its own client_id.
...
# Default name of the project Shaggy provisions for the operator.
DEFAULT_PROJECT_NAME = "Shaggy Agent"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of shaggy-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.shaggy/auth.json`` honouring the active Shaggy profile."""
    try:
        from shaggy_constants import get_shaggy_home
        return Path(get_shaggy_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.shaggy")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.shaggy/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.shaggy/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.shaggy/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: shaggy_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `shaggy photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `shaggy photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `shaggy photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `shaggy photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `shaggy photon setup`)"
