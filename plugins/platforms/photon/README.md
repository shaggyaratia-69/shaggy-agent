...
This plugin connects Shaggy Agent to iMessage (and other Spectrum
...
TypeScript-only, Shaggy runs it inside a small supervised Node sidecar and
...
- **Outbound**: `send` / `send_typing` are loopback POSTs to the sidecar,
  authenticated with a shared `X-Shaggy-Sidecar-Token`.
...
There is no separate `login` command; like every other Shaggy channel,
...
Run `shaggy photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.shaggy/.env` (the same place every other
...
Management metadata lives in `~/.shaggy/auth.json` under `credential_pool`:
