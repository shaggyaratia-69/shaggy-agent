...
Small Node helper that bridges Shaggy Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Shaggy is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Shaggy-Sidecar-Token`)
...
The Shaggy plugin's `shaggy photon setup` command runs `npm install`
here automatically.
