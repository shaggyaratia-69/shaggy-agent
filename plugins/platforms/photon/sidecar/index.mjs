// Shaggy Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Shaggy): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Shaggy -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Shaggy-Sidecar-Token: ${TOKEN}`):
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-shaggy-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Shaggy supplied them so a known-good
