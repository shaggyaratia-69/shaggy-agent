...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: shaggy)
...
Or via $SHAGGY_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so Shaggy can degrade gracefully instead of repeatedly trying to start
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $SHAGGY_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_shaggy_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "shaggy": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "shaggy"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this Shaggy config."""
    profile = config.get("profile", "shaggy")
    return str(profile or "shaggy")
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active Shaggy profile name (from agent_identity)
      {workspace} — Shaggy workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``shaggy-{user}`` with no user becomes ``shaggy``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Shaggy Agent and the User"
    ...
    def save_config(self, values, shaggy_home):
        """Write config to $SHAGGY_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(shaggy_home) / "hindsight"
    ...
    def post_setup(self, shaggy_home: str, config: dict) -> None:
        ...
        from shaggy_cli.memory_setup import _curses_select
        ...
        provider_config.setdefault("bank_id", "shaggy")
        ...
        self.save_config(provider_config, shaggy_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(shaggy_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(shaggy_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "shaggy"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: shaggy-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Shaggy Agent and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "shaggy"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "shaggy"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = cfg_get(self._config, "banks", "shaggy", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "shaggy")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Shaggy Agent and the User")
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        document. See shaggy-agent#6672.
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When Shaggy created/used that client on the shared async loop,
