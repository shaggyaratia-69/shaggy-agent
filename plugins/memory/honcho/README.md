...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/shaggy>
...
```bash
shaggy honcho setup    # full interactive wizard (cloud or local)
shaggy memory setup    # generic picker, also works
```

Or manually:
```bash
shaggy config set memory.provider honcho
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$SHAGGY_HOME/honcho.json` | Profile-local (isolated Shaggy instances) |
| 2 | `~/.shaggy/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Shaggy profile: `shaggy` (default) or `shaggy.<profile>`.
...
| 4 | `per-session` strategy | Shaggy session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`shaggy-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`shaggy`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-shaggy-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening shaggy in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Shaggy session ID (timestamp + hex). Every `shaggy` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Shaggy profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`shaggy`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Shaggy profile: `shaggy` (default) or `shaggy.<profile>` (e.g. `shaggy -p coder` → host key `shaggy.coder`).
...
| `SHAGGY_HONCHO_HOST` | Host key override |

## CLI Commands

| Command | Description |
|---------|-------------|
| `shaggy honcho setup` | Full interactive setup wizard |
| `shaggy honcho status` | Show resolved config for active profile |
| `shaggy honcho enable` / `disable` | Toggle Honcho for active profile |
| `shaggy honcho mode <mode>` | Change recall or observation mode |
| `shaggy honcho peer --user <name>` | Update user peer name |
| `shaggy honcho peer --ai <name>` | Update AI peer name |
| `shaggy honcho tokens --context <N>` | Set context token budget |
| `shaggy honcho tokens --dialectic <N>` | Set dialectic max chars |
| `shaggy honcho map <name>` | Map current directory to a session name |
| `shaggy honcho sync` | Create host blocks for all Shaggy profiles |
