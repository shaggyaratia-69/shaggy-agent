...
Config file: `$SHAGGY_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `shaggy` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `shaggy-{identity}` → `shaggy-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per Shaggy profile:

```json
{
  "container_tag": "shaggy-{identity}"
}
```

For a profile named `coder`, this resolves to `shaggy-coder`. The default profile resolves to `shaggy-default`. Without `{identity}`, all profiles share the same container.
