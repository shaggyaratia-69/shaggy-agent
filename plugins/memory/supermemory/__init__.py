...
def _load_supermemory_config(shaggy_home: str) -> dict:
    config = _default_config()
    config_path = Path(shaggy_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, shaggy_home: str) -> None:
    config_path = Path(shaggy_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `shaggy memory setup`.
        # All other options are documented for $SHAGGY_HOME/supermemory.json
    ...
    def save_config(self, values, shaggy_home):
        ...
        _save_supermemory_config(sanitized, shaggy_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from shaggy_constants import get_shaggy_home
        self._shaggy_home = kwargs.get("shaggy_home") or str(get_shaggy_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._shaggy_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "shaggy", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "shaggy_memory", "target": target, "type": "explicit_memory"},
