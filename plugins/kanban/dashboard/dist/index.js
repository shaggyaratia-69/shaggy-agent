/**
 * Shaggy Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.shaggy/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__SHAGGY_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__SHAGGY_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "shaggy-kanban-dot-triage",
    todo: "shaggy-kanban-dot-todo",
    ready: "shaggy-kanban-dot-ready",
    running: "shaggy-kanban-dot-running",
    blocked: "shaggy-kanban-dot-blocked",
    done: "shaggy-kanban-dot-done",
    archived: "shaggy-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-shaggy-task";
  ...
  const DOCS_URL = "https://shaggy-agent.cherriesandco.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://shaggy-agent.cherriesandco.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "shaggy.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="shaggy-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "shaggy-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "shaggy-kanban-md",
  ...
  // standard `drop` event and our `shaggy-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("shaggy-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("shaggy-kanban-column--drop");
          if (target) target.classList.add("shaggy-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("shaggy-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("shaggy-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("shaggy-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__SHAGGY_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "shaggy-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "shaggy-kanban-attention",
        "shaggy-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "shaggy-kanban-attention-bar" },
        h("span", { className: "shaggy-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "shaggy-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "shaggy-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "shaggy-kanban-attention-row",
                  "shaggy-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "shaggy-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "shaggy-kanban-attention-row-id" }, task.id),
                h("span", { className: "shaggy-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "shaggy-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "shaggy-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "shaggy-kanban-diag-action-btn",
      action.suggested ? "shaggy-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " shaggy-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".shaggy-kanban-drawer-comment-row input, .shaggy-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "shaggy-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("shaggy-kanban-diag", sevClass) },
      h("div", { className: "shaggy-kanban-diag-header" },
        h("span", { className: "shaggy-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "shaggy-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "shaggy-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "shaggy-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "shaggy-kanban-diag-data-row" },
                  h("span", { className: "shaggy-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "shaggy-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "shaggy-kanban-diag-data-row" },
                h("span", { className: "shaggy-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "shaggy-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "shaggy-kanban-diag-reassign-row" },
            h("span", { className: "shaggy-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "shaggy-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "shaggy-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "shaggy-kanban-diag-msg",
              msg.ok ? "shaggy-kanban-diag-msg--ok" : "shaggy-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "shaggy-kanban-section" },
      h("div", { className: "shaggy-kanban-section-head-row" },
        h("span", { className: "shaggy-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "shaggy-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "shaggy-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "shaggy-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "shaggy-kanban-docs-link",
      title: "Open Shaggy Kanban docs in a new tab",
      "aria-label": "Shaggy Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `shaggy kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "shaggy-kanban-msg-ok" : "shaggy-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "shaggy-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "shaggy-kanban-boardswitcher" },
      h("div", { className: "shaggy-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "shaggy-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "shaggy-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "shaggy-kanban-dialog-title" },
        ...
        h("div", { className: "shaggy-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Shaggy profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "shaggy-kanban-bulk" },
      h("span", { className: "shaggy-kanban-bulk-count" },
      ...
      h("div", { className: "shaggy-kanban-bulk-priority",
      ...
      h("div", { className: "shaggy-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Shaggy profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "shaggy-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("shaggy-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("shaggy-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "shaggy-kanban-trash",
        dragOver ? "shaggy-kanban-trash--drop" : "",
        props.draggingTaskId ? "shaggy-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "shaggy-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "shaggy-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".shaggy-kanban-card");
      if (!card) return;
    ...
    return h("div", { className: "shaggy-kanban-columns", onDragStart: handleDragStart, onDragEnd: handleDragEnd },
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("shaggy-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("shaggy-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "shaggy-kanban-column",
        dragOver ? "shaggy-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "shaggy-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "shaggy-kanban-col-check",
        ...
        h("span", { className: cn("shaggy-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "shaggy-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "shaggy-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "shaggy-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "shaggy-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "shaggy-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "shaggy-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "shaggy-kanban-lane" },
                  h("div", { className: "shaggy-kanban-lane-head" },
                    h("span", { className: "shaggy-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "shaggy-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "shaggy-kanban-card--stale-red";
    if (age >= tier.amber) return "shaggy-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".shaggy-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "shaggy-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "shaggy-kanban-card",
        props.selected ? "shaggy-kanban-card--selected" : "",
        props.failed ? "shaggy-kanban-card--failed" : "",
        props.draggingSource ? "shaggy-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "shaggy-kanban-card-content" },
          h("div", { className: "shaggy-kanban-card-row" },
            h("label", {
              className: "shaggy-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "shaggy-kanban-card-check",
            ...
            h("span", { className: "shaggy-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or shaggy kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "shaggy-kanban-warning-badge",
                    "shaggy-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "shaggy-kanban-needs-assignee",
          ...
          h("div", { className: "shaggy-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "shaggy-kanban-card-row shaggy-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "shaggy-kanban-assignee",
                            title: `Assigned to Shaggy profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "shaggy-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "shaggy-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "shaggy-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "shaggy-kanban-inline-create" },
      h("textarea", {
        value: title,
      ...
      h("div", { className: "flex gap-2" },
        h(Input, {
          ...
          title: props.columnName === "triage"
            ? "Shaggy profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
            : "Shaggy profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "shaggy-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "shaggy-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "shaggy-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "shaggy-kanban-drawer-close",
        ...
        data ? h("div", { className: "shaggy-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "shaggy-kanban-section" },
      h("div", { className: "shaggy-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "shaggy-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "shaggy-kanban-drawer-body" },
      h("div", { className: "shaggy-kanban-drawer-title" },
        h("span", { className: cn("shaggy-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "shaggy-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "shaggy-kanban-drawer-meta" },
      ...
      t.result ? h("div", { className: "shaggy-kanban-section" },
        h("div", { className: "shaggy-kanban-section-head" }, tx(i18n, "result", "Result")),
      ...
      h("div", { className: "shaggy-kanban-section" },
        h("div", { className: "shaggy-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "shaggy-kanban-comment" },
                h("div", { className: "shaggy-kanban-comment-head" },
                  h("span", { className: "shaggy-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "shaggy-kanban-comment-ago" },
      ...
      h("div", { className: "shaggy-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "shaggy-kanban-event",
              isDiag ? "shaggy-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "shaggy-kanban-event-header" },
                  h("span", { className: "shaggy-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "shaggy-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "shaggy-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "shaggy-kanban-event-header-plain" },
                  h("span", { className: "shaggy-kanban-event-kind" }, e.kind),
                  h("span", { className: "shaggy-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "shaggy-kanban-event-phantom-row" },
                  h("span", { className: "shaggy-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "shaggy-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "shaggy-kanban-event-payload" },
    ...
    return h("div", { className: "shaggy-kanban-section" },
      h("div", { className: "shaggy-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "shaggy-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `shaggy-kanban-run--${r.outcome || r.status || "ended"}`
          : "shaggy-kanban-run--active";
        return h("div", { key: r.id, className: cn("shaggy-kanban-run", outcomeClass) },
          h("div", { className: "shaggy-kanban-run-head" },
            h("span", { className: "shaggy-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "shaggy-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "shaggy-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "shaggy-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "shaggy-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "shaggy-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "shaggy-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "shaggy-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "shaggy-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "shaggy-kanban-pre shaggy-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "shaggy-kanban-meta-row" },
      h("span", { className: "shaggy-kanban-meta-label" }, props.label),
      h("span", { className: "shaggy-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "shaggy-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "shaggy-kanban-meta-row" },
        h("span", { className: "shaggy-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "shaggy-kanban-meta-value shaggy-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "shaggy-kanban-meta-row" },
        h("span", { className: "shaggy-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "shaggy-kanban-meta-value shaggy-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "shaggy-kanban-section" },
      h("div", { className: "shaggy-kanban-section-head-row" },
        h("span", { className: "shaggy-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "shaggy-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "shaggy-kanban-section" },
      h("div", { className: "shaggy-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "shaggy-kanban-deps-row" },
        h("span", { className: "shaggy-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "shaggy-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "shaggy-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "shaggy-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "shaggy-kanban-dep-chip-x",
      ...
      h("div", { className: "shaggy-kanban-deps-row" },
        h("span", { className: "shaggy-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "shaggy-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "shaggy-kanban-actions" },
        specifyButton,
        decomposeButton,
      ...
      specifyMsg ? h("div", {
        className: specifyMsg.ok
          ? "shaggy-kanban-msg-ok"
          : "shaggy-kanban-msg-err",
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "shaggy-kanban-section" },
      ...
      h("div", { className: "shaggy-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "shaggy-kanban-home-sub shaggy-kanban-home-sub--on"
              : "shaggy-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__SHAGGY_PLUGINS__ && typeof window.__SHAGGY_PLUGINS__.register === "function") {
    window.__SHAGGY_PLUGINS__.register("kanban", KanbanPage);
  }
})();
