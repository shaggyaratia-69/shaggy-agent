...
``{title, url, description, position}`` rows every other Shaggy web
provider produces.
...
prefers Shaggy-managed xAI Grok OAuth (via ``shaggy auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.shaggy/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``shaggy tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `shaggy auth` to sign in with "
