...
A standards-compliant OpenID Connect Relying Party for the ``shaggy dashboard``
OAuth gate. Unlike the bundled ``nous`` provider (which encodes Cherries Portal's
...
:class:`~shaggy_cli.dashboard_auth.DashboardAuthProvider` methods and touches
...
``shaggy_cli/dashboard_auth/routes.py``; this provider only:
  ...
  4. verifies the **ID token** (RS256/ES256) against the discovered
     ...
     onto a :class:`~shaggy_cli.dashboard_auth.Session`.
...
provider verifies its *access* token because Cherries Portal mints a custom JWT
...
same precedence convention as the ``nous`` plugin)::
    ...
    dashboard:
      oauth:
        ...
        self_hosted:
          issuer: https://auth.example.com/application/o/shaggy/   # required
          client_id: shaggy-dashboard                              # required
          scopes: "openid profile email"                           # optional

    # Environment overrides (Docker/Fly secret injection)
    SHAGGY_DASHBOARD_OIDC_ISSUER
    SHAGGY_DASHBOARD_OIDC_CLIENT_ID
    SHAGGY_DASHBOARD_OIDC_SCOPES        # optional; defaults to "openid profile email"
...
class SelfHostedOIDCProvider(DashboardAuthProvider):
    ...
    def _session_from_tokens(
        ...
        *,
        ...
        OAuth access token is intentionally NOT stored — Shaggy does not call
...
def register(ctx) -> None:
    ...
    client_id are configured (via ``SHAGGY_DASHBOARD_OIDC_*`` env vars or the
    ...
    issuer = _resolve_setting(
        "SHAGGY_DASHBOARD_OIDC_ISSUER", oidc_cfg.get("issuer")
    )
    client_id = _resolve_setting(
        "SHAGGY_DASHBOARD_OIDC_CLIENT_ID", oidc_cfg.get("client_id")
    )
    scopes = (
        _resolve_setting("SHAGGY_DASHBOARD_OIDC_SCOPES", oidc_cfg.get("scopes"))
        or _DEFAULT_SCOPES
    )
    ...
    if not issuer or not client_id:
        LAST_SKIP_REASON = (
            ...
            "(SHAGGY_DASHBOARD_OIDC_ISSUER + SHAGGY_DASHBOARD_OIDC_CLIENT_ID) "
