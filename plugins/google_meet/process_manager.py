...
session-scoped state file under ``$SHAGGY_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $SHAGGY_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_shaggy_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this shaggy install, leave it first —
    ...
    env = os.environ.copy()
    env["SHAGGY_MEET_URL"] = url
    env["SHAGGY_MEET_OUT_DIR"] = str(out)
    env["SHAGGY_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["SHAGGY_MEET_HEADED"] = "1"
    if auth_state:
        env["SHAGGY_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["SHAGGY_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if SHAGGY_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["SHAGGY_MEET_MODE"] = mode
    if realtime_model:
        env["SHAGGY_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["SHAGGY_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["SHAGGY_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["SHAGGY_MEET_REALTIME_KEY"] = realtime_api_key
