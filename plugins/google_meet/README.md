...
Let the shaggy agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where shaggy runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `shaggy meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `shaggy meet` CLI |
...
| `cli.py` | `shaggy meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$SHAGGY_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `shaggy meet node {run,list,approve,remove,status,ping}` |
...
shaggy meet install                                      # pip + Chromium
shaggy meet setup                                        # preflight
shaggy meet auth                                         # optional
shaggy meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
shaggy meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.shaggy/.env
shaggy meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
shaggy meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
shaggy meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.shaggy/.env
shaggy meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, shaggy will **not** switch your system audio input automatically — the
...
shaggy plugins enable google_meet
shaggy meet node run --display-name my-mac --host 0.0.0.0 --port 18789
