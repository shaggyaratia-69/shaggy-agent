...
## Client ID
- Need to register a "Desktop app" OAuth client on a Cherries and Co GCP project
...
## Token Lifecycle
- Store at `~/.shaggy/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
2. `shaggy_cli/auth.py` — Add ProviderConfig for "gemini" with auth_type="oauth_google"
3. `shaggy_cli/models.py` — Add Gemini model catalog
4. `shaggy_cli/runtime_provider.py` — Add gemini branch (read OAuth token, build OpenAI client)
5. `shaggy_cli/main.py` — Add `_model_flow_gemini()`, add to provider choices
6. `shaggy_cli/setup.py` — Add gemini auth flow (trigger browser OAuth)
...
## Prerequisites
- Cherries and Co GCP project with Desktop OAuth client registered
- OR: accept user-provided client_id via SHAGGY_GEMINI_CLIENT_ID env var
...
- shaggy-agent Copilot OAuth: `shaggy_cli/main.py` `_copilot_device_flow()` (different flow type but same lifecycle pattern)
