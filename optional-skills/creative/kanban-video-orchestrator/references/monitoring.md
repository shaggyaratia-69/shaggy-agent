...
shaggy kanban watch --tenant <project-slug>

# Snapshot of the board
shaggy kanban list --tenant <project-slug>
shaggy kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
shaggy kanban stats --tenant <project-slug>

# Visual dashboard (browser)
shaggy dashboard

# Inspect a specific task (includes comments + events)
shaggy kanban show <task-id>

# Follow a single task's event stream
shaggy kanban tail <task-id>
```

Verify available subcommands with `shaggy kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `shaggy kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `shaggy kanban block <id>` then `shaggy kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `shaggy kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `shaggy kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `shaggy profile list` (profile exists?), `shaggy status` (gateway/dispatcher up?), and verify tenant. |
...
shaggy kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
shaggy kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(shaggy kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
shaggy kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
shaggy kanban block <task-id>
shaggy kanban archive <task-id>

# Diagnose what happened
shaggy kanban show <task-id>      # task body, comments, recent events
shaggy kanban tail <task-id>      # follow the live event stream
shaggy kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `shaggy kanban create` for the director
...
while true; do
    clear
    shaggy kanban list --tenant <slug>
    echo "---"
    shaggy kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `shaggy kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `shaggy profile list` and start
