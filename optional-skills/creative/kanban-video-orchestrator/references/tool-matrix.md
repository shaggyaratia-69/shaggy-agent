...
Maps each role archetype to the Shaggy skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public shaggy-agent
...
## Shaggy skills relevant to video production

### Visual / rendering skills (`shaggy-agent/skills/creative/`)
...
### Design / pre-production skills (`shaggy-agent/skills/creative/`)
...
### Audio / media skills (`shaggy-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`shaggy-agent/skills/devops/`)
...
These are **not** Shaggy skills but external CLIs / APIs that profiles invoke.
...
## Built-in Shaggy tools for media review

These are native Shaggy tools — not invoked via terminal but through their own
...
key is present in `~/.shaggy/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Shaggy profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.shaggy/.env` → environment variable.
...
When a new Shaggy-public video skill ships:
