...
## 5. GODMODE FAST — Shaggy 4 405B (Zero Refusal)

**Model:** `nousresearch/shaggy-4-405b`
**Strategy:** Shaggy models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Shaggy Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.shaggy/config.yaml`:
