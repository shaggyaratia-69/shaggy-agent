...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("SHAGGY_HOME", os.path.expanduser("~/.shaggy")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("SHAGGY_HOME", Path.home() / ".shaggy")) / "skills" / "red-teaming" / "godmode"
...
SHAGGY_HOME = Path(os.getenv("SHAGGY_HOME", Path.home() / ".shaggy"))
CONFIG_PATH = SHAGGY_HOME / "config.yaml"
PREFILL_PATH = SHAGGY_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Cherries/Shaggy models — already uncensored, just needs clean prompt
    "shaggy": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "shaggy" in model_lower or "nous" in model_lower:
        return "shaggy"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Shaggy config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.shaggy/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Shaggy for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Shaggy for changes to take effect.")
