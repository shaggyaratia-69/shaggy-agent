...
class TestToolsetConsistency:
    ...
    def test_shaggy_platforms_share_core_tools(self):
        """All shaggy-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on shaggy-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["shaggy-cli", "shaggy-telegram", "shaggy-discord", "shaggy-whatsapp", "shaggy-slack", "shaggy-signal", "shaggy-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_shaggy_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("shaggy-whatsapp")

    def test_shaggy_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("shaggy-api-server")
