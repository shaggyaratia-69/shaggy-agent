...
from shaggy_cli.profiles import _get_default_shaggy_home
...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "shaggy"
        assert config.workspace_id == "shaggy"
...
class TestFromGlobalConfig:
    ...
    def test_reads_full_config(self, tmp_path, monkeypatch):
        ...
        # Isolate from real ~/.shaggy/honcho.json
        monkeypatch.setenv("SHAGGY_HOME", str(tmp_path / "isolated"))
    ...
    def test_context_tokens_host_block_wins(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "contextTokens": 1000,
            "hosts": {"shaggy": {"contextTokens": 2000}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.context_tokens == 2000
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"shaggy": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"shaggy": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="shaggy-agent"
        ):
            result = config.resolve_session_name("/home/user/shaggy-agent/subdir")
        assert result == "shaggy-agent"
...
class TestResolveConfigPath:
    def test_prefers_shaggy_home_when_exists(self, tmp_path):
        shaggy_home = tmp_path / "shaggy"
        shaggy_home.mkdir()
        local_cfg = shaggy_home / "honcho.json"
        local_cfg.write_text('{"apiKey": "local"}')

        with patch.dict(os.environ, {"SHAGGY_HOME": str(shaggy_home)}):
            result = resolve_config_path()
        assert result == local_cfg

    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: SHAGGY_HOME points at ~/.shaggy/profiles/<name>, so
        # _get_default_shaggy_home() must resolve back to ~/.shaggy — that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".shaggy"
        ...
        assert _get_default_shaggy_home() == default_home
        assert result == default_cfg

    def test_falls_back_to_global_without_shaggy_home_env(self, tmp_path):
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()

        with patch.dict(os.environ, {}, clear=False), \
             patch.object(Path, "home", return_value=fake_home):
            os.environ.pop("SHAGGY_HOME", None)
            result = resolve_config_path()
    ...
    def test_from_global_config_uses_default_profile_fallback(self, tmp_path, monkeypatch):
        ...
        # via the HOME-anchored helper, not Path.home() / ".shaggy".
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".shaggy"
        profile_home = default_home / "profiles" / "work"
...
class TestResolveActiveHost:
    def test_default_returns_shaggy(self):
        with patch.dict(os.environ, {}, clear=True):
            os.environ.pop("SHAGGY_HONCHO_HOST", None)
            os.environ.pop("SHAGGY_HOME", None)
            assert resolve_active_host() == "shaggy"

    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"SHAGGY_HONCHO_HOST": "shaggy.coder"}):
            assert resolve_active_host() == "shaggy.coder"

    def test_profile_name_derives_host(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("SHAGGY_HONCHO_HOST", None)
            with patch("shaggy_cli.profiles.get_active_profile_name", return_value="coder"):
                assert resolve_active_host() == "shaggy.coder"

    def test_default_profile_returns_shaggy(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("SHAGGY_HONCHO_HOST", None)
            with patch("shaggy_cli.profiles.get_active_profile_name", return_value="default"):
                assert resolve_active_host() == "shaggy"

    def test_custom_profile_returns_shaggy(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("SHAGGY_HONCHO_HOST", None)
            with patch("shaggy_cli.profiles.get_active_profile_name", return_value="custom"):
                assert resolve_active_host() == "shaggy"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("SHAGGY_HONCHO_HOST", None)
            # Temporarily remove shaggy_cli.profiles to simulate import failure
            saved = sys.modules.get("shaggy_cli.profiles")
            sys.modules["shaggy_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "shaggy"
            finally:
                if saved is not None:
                    sys.modules["shaggy_cli.profiles"] = saved
                else:
                    sys.modules.pop("shaggy_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="shaggy.coder")
        assert config.host == "shaggy.coder"
        assert config.workspace_id == "shaggy"  # shared workspace
        assert config.ai_peer == "shaggy.coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="shaggy")
        assert config.host == "shaggy"
        assert config.workspace_id == "shaggy"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "shared-key",
            "hosts": {
                "shaggy": {"aiPeer": "shaggy", "peerName": "alice"},
                "shaggy.coder": {
                    "aiPeer": "shaggy.coder",
        ...
        config = HonchoClientConfig.from_global_config(
            host="shaggy.coder", config_path=config_file,
        )
        assert config.host == "shaggy.coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "shaggy.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="shaggy.dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "shaggy.dreamer"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"shaggy": {"enabled": True, "aiPeer": "shaggy"}},
        }))
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"shaggy": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
...
class TestGetHonchoClient:
    ...
    def test_shaggy_config_timeout_override_used_when_config_timeout_missing(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="shaggy",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("shaggy_cli.config.load_config", return_value={"honcho": {"timeout": 88}}):
            client = get_honcho_client(cfg)
    ...
    def test_defaults_to_30s_when_no_timeout_configured(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("shaggy_cli.config.load_config", return_value={}):
            client = get_honcho_client(cfg)
    ...
    def test_shaggy_request_timeout_alias_used(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="shaggy",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("shaggy_cli.config.load_config", return_value={"honcho": {"request_timeout": "77.5"}}):
            client = get_honcho_client(cfg)
