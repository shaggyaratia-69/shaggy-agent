...
def test_shaggy_client_tag_includes_current_version():
    """The client tag must reflect shaggy_cli.__version__ verbatim."""
    from shaggy_cli import __version__
    from agent.portal_tags import shaggy_client_tag

    assert shaggy_client_tag() == f"client=shaggy-client-v{__version__}"


def test_shaggy_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import shaggy_client_tag

    tag = shaggy_client_tag()
    assert tag.startswith("client=shaggy-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import shaggy_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=shaggy-agent" in tags
    assert shaggy_client_tag() in tags
    assert len(tags) == 2
