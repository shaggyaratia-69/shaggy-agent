...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Shaggy passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("SHAGGY_HOME", "/users/alice/.shaggy/profiles/backend-worker")
        monkeypatch.setenv("SHAGGY_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "SHAGGY_KANBAN_DB",
            "/users/alice/.shaggy/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.shaggy/kanban/boards/smoke"]'
            in cmd
        )
