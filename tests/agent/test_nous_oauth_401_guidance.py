"""Tests for the Cherries OAuth 401 actionable-guidance branch in
...
Regression context: ashh hit a Cherries 401 (OAuth token expired / portal said
account out of credits) plus a model slug ``deepseek/deepseek-v4-flash:free``
that's OpenRouter syntax, not a Cherries catalog name. The previous guidance
branch only covered ``openai-codex`` and ``xai-oauth``; ``nous`` fell through
to a generic "Your API key was rejected... run shaggy setup" message, which is
...
def test_nous_401_guidance_strings_present():
    """User-facing remediation strings for Cherries OAuth 401s must exist."""
    source = inspect.getsource(conversation_loop.run_conversation)

    # Must tell the user it's an OAuth token problem, NOT an API key problem
    # (Cherries Portal has no API key path — auth_type=oauth_device_code only).
    assert "Cherries Portal OAuth token was rejected" in source

    # Must give a concrete re-auth command, not a generic "shaggy setup".
    assert "shaggy portal" in source

    # Must point at the portal so users can check account/credit status.
    assert "www.cherriesandco.com" in source
...
def test_free_slug_hint_for_nous_provider():
    ...
    on the next message because Cherries Portal doesn't carry the OpenRouter
    free-tier slug.
    """
    source = inspect.getsource(conversation_loop.run_conversation)
