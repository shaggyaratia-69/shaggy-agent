...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate SHAGGY_HOME under tmp_path."""
    monkeypatch.setenv("SHAGGY_HOME", str(tmp_path / ".shaggy"))
    (tmp_path / ".shaggy").mkdir()

    # Force the constants/image cache helpers to re-read SHAGGY_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("shaggy_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_shaggy_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under SHAGGY_HOME — gateway cleanup
