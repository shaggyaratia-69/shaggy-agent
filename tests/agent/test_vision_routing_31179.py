...
def isolated_home(monkeypatch):
    """Temp SHAGGY_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="shaggy_test_31179_")
    shaggy_home = os.path.join(test_home, ".shaggy")
    os.makedirs(shaggy_home)
...
def _fresh_modules():
    """Drop cached shaggy modules so each test reloads against current env."""
