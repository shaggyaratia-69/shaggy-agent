...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_shaggy_env(self):
        """Shaggy' own .env inside SHAGGY_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: shaggy internal .env is in ~/.shaggy/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.shaggy/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.shaggy/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal Shaggy cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        shaggy_home = tmp_path / ".shaggy"
        cache = shaggy_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._shaggy_home_path", return_value=shaggy_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Shaggy cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        shaggy_home = tmp_path / ".shaggy"
        hub = shaggy_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_shaggy_home(self, tmp_path):
        """The env basename guard does not depend on SHAGGY_HOME resolution."""
        shaggy_home = tmp_path / ".shaggy"
        shaggy_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        shaggy_home = tmp_path / ".shaggy"
        cache = shaggy_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
