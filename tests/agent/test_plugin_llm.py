...
""",
        ...
        from shaggy_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        shaggy_home = tmp_path / ".shaggy"
        shaggy_home.mkdir()
        (shaggy_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("SHAGGY_HOME", str(shaggy_home))
        from shaggy_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from shaggy_cli.plugins import PluginContext, PluginManifest, PluginManager
