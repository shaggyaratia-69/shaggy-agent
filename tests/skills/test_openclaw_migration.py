...
"""
    ...
    assert "Tyler Williams > Active Projects: Shaggy Agent" in entries
...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `shaggy cron` to recreate scheduled tasks" in notes_text
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    # shaggy_config_mod  — print statements in the post-migration summary that
    #                      tell the user to *review* ~/.shaggy/config.yaml;
    ...
    KNOWN_FALSE_POSITIVES = {"agent_config_mod", "python_os_environ", "shaggy_config_mod"}
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    # Mixed-case / capitalized matches → capital-H ``Shaggy``.
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "Shaggy prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told Shaggy to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "Shaggy config is great"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "Shaggy uses tools well"
    # All-lowercase matches → lowercase ``shaggy``; this preserves the
    # real filesystem path ``~/.shaggy`` (Shaggy home) when rebranding
    # memory entries that reference ``~/.openclaw`` or ``openclaw`` prose.
    assert mod.rebrand_text("openclaw should always respond concisely") == "shaggy should always respond concisely"


def test_rebrand_text_replaces_legacy_bot_names():
    mod = load_module()
    # Same case-preservation rule as above.
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "Shaggy remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "shaggy prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "Shaggy was configured for Spanish"
    assert mod.rebrand_text("moltbot uses Python") == "shaggy uses Python"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "Shaggy said to ask Shaggy about Shaggy settings"


def test_rebrand_text_preserves_filesystem_path_casing():
    """Lowercase matches — especially ``.openclaw`` filesystem paths — must
    rewrite to lowercase ``.shaggy`` (the real Shaggy home), not the broken
    ``.Shaggy``.
    ...
    rewritten to ``~/.Shaggy/config.yaml`` — a path that doesn't exist —
    and the agent kept trying to read it.
    """
    mod = load_module()
    assert mod.rebrand_text("config is at ~/.openclaw/config.yaml") == \
        "config is at ~/.shaggy/config.yaml"
    assert mod.rebrand_text("use .openclaw directory") == "use .shaggy directory"
    assert mod.rebrand_text("Path.home() / '.openclaw'") == "Path.home() / '.shaggy'"
    # Sentence with both lowercase path and capitalized prose.
    assert mod.rebrand_text("openclaw config path: ~/.openclaw/") == \
        "shaggy config path: ~/.shaggy/"
...
def test_migrate_soul_rebrands_content(tmp_path):
    ...
    assert "OpenClaw" not in result
    assert "You are Shaggy" in result
