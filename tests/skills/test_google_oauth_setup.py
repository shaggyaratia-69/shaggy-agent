...
class TestShaggyConstantsFallback:
    """Tests for _shaggy_home.py fallback when shaggy_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_shaggy_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _shaggy_home.py with shaggy_constants blocked."""
        monkeypatch.setitem(sys.modules, "shaggy_constants", None)
        spec = importlib.util.spec_from_file_location("_shaggy_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module

    def test_fallback_uses_shaggy_home_env_var(self, monkeypatch, tmp_path):
        """When shaggy_constants is missing, SHAGGY_HOME comes from env var."""
        monkeypatch.setenv("SHAGGY_HOME", str(tmp_path / "custom-shaggy"))
        module = self._load_helper(monkeypatch)
        assert module.get_shaggy_home() == tmp_path / "custom-shaggy"

    def test_fallback_defaults_to_dot_shaggy(self, monkeypatch):
        """When shaggy_constants is missing and SHAGGY_HOME unset, default to ~/.shaggy."""
        monkeypatch.delenv("SHAGGY_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_shaggy_home() == Path.home() / ".shaggy"

    def test_fallback_ignores_empty_shaggy_home(self, monkeypatch):
        """Empty/whitespace SHAGGY_HOME is treated as unset."""
        monkeypatch.setenv("SHAGGY_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_shaggy_home() == Path.home() / ".shaggy"

    def test_fallback_display_shaggy_home_shortens_path(self, monkeypatch):
        """Fallback display_shaggy_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("SHAGGY_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_shaggy_home() == "~/.shaggy"

    def test_fallback_display_shaggy_home_profile_path(self, monkeypatch):
        """Fallback display_shaggy_home() handles profile paths under ~/."""
        monkeypatch.setenv("SHAGGY_HOME", str(Path.home() / ".shaggy/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_shaggy_home() == "~/.shaggy/profiles/coder"

    def test_fallback_display_shaggy_home_custom_path(self, monkeypatch):
        """Fallback display_shaggy_home() returns full path for non-home locations."""
        monkeypatch.setenv("SHAGGY_HOME", "/opt/shaggy-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_shaggy_home() == "/opt/shaggy-custom"

    def test_delegates_to_shaggy_constants_when_available(self):
        """When shaggy_constants IS importable, _shaggy_home delegates to it."""
        spec = importlib.util.spec_from_file_location(
            "_shaggy_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        import shaggy_constants
        assert module.get_shaggy_home is shaggy_constants.get_shaggy_home
        assert module.display_shaggy_home is shaggy_constants.display_shaggy_home
