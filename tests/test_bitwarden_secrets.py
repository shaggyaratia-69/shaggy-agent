...
binary download are exercised manually by `shaggy secrets bitwarden
...
def shaggy_home(tmp_path, monkeypatch):
    """Point Shaggy at an isolated home directory."""
    home = tmp_path / ".shaggy"
    home.mkdir()
    monkeypatch.setenv("SHAGGY_HOME", str(home))
    # Some modules cache get_shaggy_home; clear if needed.
    import shaggy_constants
    if hasattr(shaggy_constants, "_SHAGGY_HOME_CACHE"):
        shaggy_constants._SHAGGY_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(shaggy_home, monkeypatch):
...
def test_install_bws_happy_path(shaggy_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(shaggy_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(shaggy_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from shaggy_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under shaggy_home/cache/."""
    home = tmp_path / ".shaggy"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
