...
Override the image with ``SHAGGY_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("SHAGGY_TEST_IMAGE", "shaggy-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``SHAGGY_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("SHAGGY_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged shaggy user
# ---------------------------------------------------------------------------
#
# Background: every Shaggy runtime path inside the container drops to UID
# 10000 (the ``shaggy`` user) via ``s6-setuidgid shaggy``. ``docker exec``
...
# fails (PermissionError swallowed) for shaggy, so a test that ran as root
...
# helpers below run every probe under ``-u shaggy`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: shaggy).
