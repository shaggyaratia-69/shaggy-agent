"""Harness: dashboard opt-in via SHAGGY_DASHBOARD.

Today (tini): dashboard starts once when SHAGGY_DASHBOARD=1; if it crashes
...
Every ``docker exec`` here runs as the unprivileged ``shaggy`` user
...
def test_dashboard_not_running_by_default(
    ...
    """Without SHAGGY_DASHBOARD, no dashboard process should be running."""
    subprocess.run(
    ...
    time.sleep(5)
    r = docker_exec(container_name, "pgrep", "-f", "shaggy dashboard")
    # pgrep exits non-zero when no match found
    assert r.returncode != 0, (
        "Dashboard should not be running without SHAGGY_DASHBOARD"
    )
...
def test_dashboard_slot_reports_down_when_disabled(
    ...
    """Without SHAGGY_DASHBOARD, s6-svstat should report the dashboard
    slot as DOWN (not up-with-sleep-infinity, which would
    false-positive `shaggy doctor` and any other health check).
    ...
    SHAGGY_DASHBOARD is unset, so the slot reflects reality.
    """
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name, built_image,
         "sleep", "60"],
    ...
    assert "down" in r.stdout, (
        f"Dashboard slot should be 'down' without SHAGGY_DASHBOARD; "
...
def test_dashboard_slot_reports_up_when_enabled(
    ...
    """Symmetry: with SHAGGY_DASHBOARD=1, s6-svstat reports the slot as up."""
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         ...
         # SHAGGY_DASHBOARD_OAUTH_CLIENT_ID in this test env), start_server
...
def test_dashboard_opt_in_starts(
    ...
    """With SHAGGY_DASHBOARD=1, a dashboard process should be visible."""
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "SHAGGY_DASHBOARD=1",
    ...
    ok, _ = _poll(
        container_name, "pgrep -f 'shaggy dashboard'", deadline_s=30.0,
    )
    assert ok, "Dashboard should be running with SHAGGY_DASHBOARD=1"


def test_dashboard_port_override(
    built_image: str, container_name: str,
) -> None:
    """SHAGGY_DASHBOARD_PORT changes the dashboard's listen port."""
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "SHAGGY_DASHBOARD=1", "-e", "SHAGGY_DASHBOARD_PORT=9120",
         # Default bind is 0.0.0.0; pin insecure opt-in so the auth gate
...
def test_dashboard_restarts_after_crash(
    ...
    for _attempt in range(10):
        first_pid_result = docker_exec(
            container_name, "pgrep", "-f", "shaggy dashboard",
        )
    ...
    # Kill the dashboard. The dashboard process runs as shaggy, so the
    # shaggy user can kill it (same UID).
...
# whenever `SHAGGY_DASHBOARD_HOST` was non-loopback, silently disabling
...
def test_dashboard_oauth_gate_engages_on_non_loopback_bind(
    ...
    provider activates whenever ``SHAGGY_DASHBOARD_OAUTH_CLIENT_ID`` is
    set).
    ...
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "SHAGGY_DASHBOARD=1",
         "-e", "SHAGGY_DASHBOARD_HOST=0.0.0.0",
         "-e", "SHAGGY_DASHBOARD_OAUTH_CLIENT_ID=agent:test-instance",
         built_image, "sleep", "120"],
        check=True, capture_output=True, timeout=30,
    )
    ...
    assert "nous" in provider_names, (
        ...
        f"SHAGGY_DASHBOARD_OAUTH_CLIENT_ID is set. Got: {payload!r}"
    )
    ...
    assert status_code == 401, (
        ...
        "when a provider is registered and SHAGGY_DASHBOARD_INSECURE "
...
def test_dashboard_insecure_env_var_opts_out_of_gate(
    ...
    """``SHAGGY_DASHBOARD_INSECURE=1`` re-enables the legacy no-gate mode
    ...
    assert status.get("auth_required") is False, (
        "SHAGGY_DASHBOARD_INSECURE=1 must disable the auth gate (explicit "
