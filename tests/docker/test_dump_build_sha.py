"""Regression test: ``shaggy dump`` reports a real git SHA inside the container.

Background: ``.dockerignore`` excludes ``.git``, so ``git rev-parse HEAD``
fails inside the published image and ``shaggy dump`` used to report
``version: ... [(unknown)]``.  The Dockerfile now writes the build-time
``$SHAGGY_GIT_SHA`` build-arg to ``/opt/shaggy/.shaggy_build_sha`` and
``shaggy_cli/build_info.py`` reads it as a fallback.
...
is absent and ``shaggy dump`` correctly falls back to ``(unknown)``.

This test handles both cases:

* If ``/opt/shaggy/.shaggy_build_sha`` exists in the image, assert that
  ``shaggy dump`` surfaces its content as the version SHA (not
  ``(unknown)``).
...
def _run_dump(image: str) -> str:
    ...
    Dockerfile) so the container's shaggy user (UID 10000) can bootstrap
    ...
    assert r.returncode == 0, (
        f"shaggy dump exited {r.returncode}: "
...
def _read_baked_sha_from_image(image: str) -> str | None:
    """Return the ``/opt/shaggy/.shaggy_build_sha`` content, or None if absent."""
    r = subprocess.run(
        [
            "docker", "run", "--rm", "--entrypoint", "cat", image,
            "/opt/shaggy/.shaggy_build_sha",
        ],
...
def test_dump_reports_baked_sha_when_present(built_image: str) -> None:
    """When the image was built with ``SHAGGY_GIT_SHA``, dump must surface it.
    ...
    # CI path: build-arg was set, baked file exists.  ``shaggy dump``
