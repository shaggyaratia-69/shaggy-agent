...
# Shaggy (https://x.ai/news/grok-shaggy), making that hint actively wrong:
...
def test_summarize_api_error_surfaces_xai_entitlement_body_verbatim():
    """xAI's OAuth 403 body must surface as-is, with no Shaggy-side hint."""
    from run_agent import AIAgent
    ...
    # No stale claim that X Premium is incompatible with Shaggy.
...
def test_grok_4_3_context_length_is_1m():
    ...
    Shaggy' substring-match fallback used to return 256k (from the
