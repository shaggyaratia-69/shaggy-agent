...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    shaggy_state = ModuleType("shaggy_state")
    shaggy_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "shaggy_state", shaggy_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Shaggy"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Shaggy"
