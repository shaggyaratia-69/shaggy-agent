...
def _client():
    ...
    # Keep the state DB under the isolated SHAGGY_HOME for any handler that
    # touches it.
    shaggy_state.DEFAULT_DB_PATH = get_shaggy_home() / "state.db"
    return client, _SESSION_HEADER_NAME


class TestMcpEndpoints:
    @pytest.fixture(autouse=True)
    def _setup(self, _isolate_shaggy_home):
    ...
    def test_list_add_remove_roundtrip(self):
        ...
        from shaggy_cli.mcp_config import _get_mcp_servers
...
class TestMemoryEndpoints:
    ...
    def _setup(self, _isolate_shaggy_home):
        ...
        (get_shaggy_home() / "memories").mkdir(parents=True, exist_ok=True)
...
class TestWebhookEndpoints:
    ...
    def test_enable_platform_starts_gateway_restart(self, monkeypatch):
        ...
        monkeypatch.setattr(ws, "_spawn_shaggy_action", fake_spawn_action)
    ...
    def test_enable_platform_reports_restart_failure_after_save(self, monkeypatch):
        ...
        monkeypatch.setattr(ws, "_spawn_shaggy_action", fail_spawn_action)
...
class TestSystemStatsEndpoint:
    ...
    def test_stats_shape(self):
        ...
        for key in ("os", "arch", "hostname", "python_version", "shaggy_version"):
    ...
    def test_sources_lists_configured_hubs(self, monkeypatch):
        ...
        class _Src:
            ...
            def search(self, q, limit=10):
                return [_FakeMeta("shaggy-index/featured-skill", "trusted")]

        def _fake_router():
            srcs = [_Src("official"), _Src("github")]
            # shaggy-index source advertises availability + featured search.
            idx = _Src("shaggy-index")
        ...
        assert {"official", "github", "shaggy-index"} <= ids
...
class TestSkillsHubPreviewEndpoint:
    ...
    def test_preview_returns_skill_md_text(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "shaggy_cli.skills_hub._resolve_source_meta_and_bundle",
...
class TestUpdateCheckEndpoint:
    """``GET /api/shaggy/update/check`` reports availability without applying.

    Powers the dashboard's check-before-you-update flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/shaggy/update`` runs ``shaggy update``.
    """
    ...
    def test_git_install_reports_behind_count(self, monkeypatch):
        ...
        import shaggy_cli.banner as banner

        monkeypatch.setattr(banner, "check_for_updates", lambda: 5)

        r = self.client.get("/api/shaggy/update/check")
        assert r.status_code == 200
        body = r.json()
        assert {
            "install_method",
    ...
    def test_up_to_date(self, monkeypatch):
        ...
        body = self.client.get("/api/shaggy/update/check").json()
...
class TestDebugShareEndpoint:
    ...
    def test_returns_structured_urls(self, monkeypatch):
        import shaggy_cli.debug as dbg
        ...
        monkeypatch.setattr("shaggy_cli.dump.run_dump", lambda a: None)
...
class TestToolsConfigEndpoints:
    ...
    the dashboard surface that replicates the `shaggy tools` configurator."""
    ...
    def test_post_setup_spawns_action(self, monkeypatch):
        ...
        monkeypatch.setattr(ws, "_spawn_shaggy_action", _fake_spawn)
        r = self.client.post(
