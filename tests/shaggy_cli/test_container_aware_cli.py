...
def container_env(tmp_path, monkeypatch):
    """Set up a fake SHAGGY_HOME with .container-mode file."""
    shaggy_home = tmp_path / ".shaggy"
    shaggy_home.mkdir()
    monkeypatch.setenv("SHAGGY_HOME", str(shaggy_home))
    monkeypatch.delenv("SHAGGY_DEV", raising=False)

    container_mode = shaggy_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=shaggy-agent\n"
        "exec_user=shaggy\n"
        "shaggy_bin=/data/current-package/bin/shaggy\n"
    )
    return shaggy_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("shaggy_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "shaggy-agent"
    assert info["exec_user"] == "shaggy"
    assert info["shaggy_bin"] == "/data/current-package/bin/shaggy"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("shaggy_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_shaggy_dev(container_env, monkeypatch):
    """Returns None when SHAGGY_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("SHAGGY_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_shaggy_dev_zero(container_env, monkeypatch):
    """SHAGGY_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("SHAGGY_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        shaggy_home = Path(tmpdir) / ".shaggy"
        shaggy_home.mkdir()
        (shaggy_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("shaggy_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_shaggy_home": lambda: shaggy_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("SHAGGY_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=shaggy-custom\n"
        "exec_user=myuser\n"
        "shaggy_bin=/opt/shaggy/bin/shaggy\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "shaggy-custom"
    assert info["exec_user"] == "myuser"
    assert info["shaggy_bin"] == "/opt/shaggy/bin/shaggy"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from shaggy_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "shaggy"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "shaggy-agent" in cmd
    assert "/data/current-package/bin/shaggy" in cmd
    assert "chat" in cmd
