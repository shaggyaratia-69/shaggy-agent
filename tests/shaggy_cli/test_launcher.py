"""Tests for the top-level `./shaggy` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./shaggy` should use `shaggy_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "shaggy"
    called = []

    fake_main_module = types.ModuleType("shaggy_cli.main")

    def fake_main():
        called.append("shaggy_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "shaggy_cli.main", fake_main_module)
