...
When ``SHAGGY_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.shaggy/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.shaggy            (has its own auth.json fixture)
    * Profile     -> tmp_path/.shaggy/profiles/coder   (active, SHAGGY_HOME points here)
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (SHAGGY_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.shaggy as the "real home" — which is NOT equal
    # to the SHAGGY_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    shaggy_home = tmp_path / "classic"
    shaggy_home.mkdir()
    monkeypatch.setenv("SHAGGY_HOME", str(shaggy_home))

    _write(shaggy_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from shaggy_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: SHAGGY_HOME is set to a custom path that is NOT under
    # ~/.shaggy/profiles/ — get_default_shaggy_root() returns SHAGGY_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from shaggy_cli.auth import read_credential_pool, write_credential_pool
