...
Covers:
- Defense 1: gateway stop/restart refuse when _SHAGGY_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "shaggy  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "SHAGGY GATEWAY RESTART",           # uppercase
    ])
    def test_shaggy_gateway_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "launchctl kickstart gui/501/ai.shaggy.gateway",
        "launchctl unload ~/Library/LaunchAgents/ai.shaggy.gateway.plist",
        "launchctl stop ai.shaggy.gateway",
        "systemctl restart shaggy-gateway",
        "systemctl stop shaggy-gateway.service",
        "systemctl start shaggy-gateway",
    ])
    def test_service_manager_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "kill shaggy gateway process",
        "pkill -f shaggy.*gateway",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "shaggy cron list",
        "shaggy update",
        "shaggy config set model claude",
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_shaggy_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade shaggy then run shaggy gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_launchctl_kickstart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="0 9 * * *",
            prompt="Run launchctl kickstart -k gui/501/ai.shaggy.gateway",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
...
class TestGatewaySelfTargetingGuard:
    """Verify shaggy gateway stop/restart refuse when _SHAGGY_GATEWAY=1."""

    def test_stop_refuses_inside_gateway(self, monkeypatch):
        monkeypatch.setenv("_SHAGGY_GATEWAY", "1")
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_SHAGGY_GATEWAY", raising=False)
