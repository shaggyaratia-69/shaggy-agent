"""Tests for the Shaggy plugin system (shaggy_cli.plugins)."""
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<shaggy_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<shaggy_home>/plugins/``; we derive
    ``<shaggy_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <SHAGGY_HOME>/config.yaml.
        # Config is always read from SHAGGY_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        shaggy_home_str = os.environ.get("SHAGGY_HOME")
        if shaggy_home_str:
            shaggy_home = Path(shaggy_home_str)
        else:
            shaggy_home = base.parent
        shaggy_home.mkdir(parents=True, exist_ok=True)
        cfg_path = shaggy_home / "config.yaml"
        cfg: dict = {}
        if cfg_path.exists():
            try:
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.shaggy/plugins/ are discovered."""
        plugins_dir = tmp_path / "shaggy_test" / "plugins"
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SHAGGY_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".shaggy" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_missing_init(self, tmp_path, monkeypatch):
        ...
        # missing-init error.
        shaggy_home = tmp_path / "shaggy_test"
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under shaggy_plugins.<name>."""
        plugins_dir = tmp_path / "shaggy_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("SHAGGY_HOME", str(tmp_path / "shaggy_test"))

        # Clean up any prior namespace module
        sys.modules.pop("shaggy_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "shaggy_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="shaggy_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPreToolCallBlocking:
    ...
    def test_block_message_returned_for_valid_directive(self, monkeypatch):
        monkeypatch.setattr(
            "shaggy_cli.plugins.invoke_hook",
...
class TestPluginToolVisibility:
    ...
    def test_plugin_tools_in_definitions(self, tmp_path, monkeypatch):
        ...
        import shaggy_cli.plugins as plugins_mod
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("shaggy_cli.plugins._plugin_manager", mgr):
    ...
    def test_get_plugin_context_engine_discovers_plugins_lazily(self, tmp_path, monkeypatch):
        ...
        shaggy_home = tmp_path / "shaggy_test"
        plugins_dir = shaggy_home / "plugins"
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "shaggy_test" / "plugins"
        # Set SHAGGY_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("shaggy_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("shaggy_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("shaggy_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("shaggy_cli.plugins.PluginContext.dispatch_tool.__module__", "shaggy_cli.plugins"):
...
class TestPluginDebugLogging:
    """SHAGGY_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("SHAGGY_PLUGINS_DEBUG", raising=False)
        from shaggy_cli import plugins as plugins_mod
    ...
    def test_debug_handler_installed_when_env_var_set(self, monkeypatch):
        """With SHAGGY_PLUGINS_DEBUG=1, a DEBUG-level stderr handler is attached."""
        monkeypatch.setenv("SHAGGY_PLUGINS_DEBUG", "1")
        from shaggy_cli import plugins as plugins_mod
