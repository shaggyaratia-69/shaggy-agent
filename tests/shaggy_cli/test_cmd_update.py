...
from shaggy_cli.main import cmd_update, PROJECT_ROOT
...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_non_interactive_runs_safe_config_migrations(self, mock_args, capsys):
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "shaggy_cli.config.get_missing_env_vars", return_value=["MISSING_KEY"]
        ), patch(
            "shaggy_cli.config.get_missing_config_fields",
            return_value=[{"key": "new.option", "default": True}],
        ), patch("shaggy_cli.config.check_config_version", return_value=(1, 2)), patch(
            "shaggy_cli.config.migrate_config",
            return_value={"env_added": [], "config_added": ["new.option"]},
        ), patch("shaggy_cli.main.sys") as mock_sys:
            ...
            from shaggy_cli.config import migrate_config
...
class TestCmdUpdateProfileSkillSync:
    ...
    def test_active_profile_included_in_skill_sync(
        ...
        default_p = SimpleNamespace(name="default", path=Path("/fake/.shaggy"))
        active_p = SimpleNamespace(name="bit", path=Path("/fake/.shaggy/profiles/bit"))
        other_p = SimpleNamespace(name="work", path=Path("/fake/.shaggy/profiles/work"))
        ...
        with (
            patch("shaggy_cli.profiles.list_profiles", return_value=all_profiles),
            patch("shaggy_cli.profiles.seed_profile_skills", side_effect=fake_seed),
    ...
    def test_single_profile_default_is_synced(
        ...
        with (
            patch("shaggy_cli.profiles.list_profiles", return_value=[default_p]),
            patch("shaggy_cli.profiles.seed_profile_skills", side_effect=fake_seed),
            patch("tools.skills_sync.sync_skills", return_value=empty_sync),
        ):
            cmd_update(mock_args)
