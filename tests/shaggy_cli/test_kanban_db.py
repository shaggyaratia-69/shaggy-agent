"""Tests for the Kanban DB layer (shaggy_cli.kanban_db)."""
...
from shaggy_cli import kanban_db as kb


@pytest.fixture
def kanban_home(tmp_path, monkeypatch):
    """Isolated SHAGGY_HOME with an empty kanban DB."""
...
def test_claim_uses_env_default_ttl(kanban_home, monkeypatch):
    monkeypatch.setenv("SHAGGY_KANBAN_CLAIM_TTL_SECONDS", "3600")
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x", assignee="a")
...
def test_stale_claim_reclaimed(kanban_home, monkeypatch):
    import signal
    import shaggy_cli.kanban_db as _kb
...
def test_dispatch_skips_nonspawnable_into_separate_bucket(kanban_home, monkeypatch):
    ...
    from shaggy_cli import profiles
...
def test_has_spawnable_ready_true_when_real_profile_present(kanban_home, monkeypatch):
    ...
    has an assignee that maps to a real Shaggy profile — preserves the
    ...
    with kb.connect() as conn:
        ...
        kb.create_task(conn, title="shaggy-task", assignee="daily")
...
def test_scratch_workspace_created_under_shaggy_home(kanban_home):
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x")
        task = kb.get_task(conn, t)
...
# spawned with `shaggy -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's SHAGGY_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's SHAGGY_HOME."""

    def _set_home(self, monkeypatch, tmp_path, shaggy_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SHAGGY_HOME", str(shaggy_home))
        monkeypatch.delenv("SHAGGY_KANBAN_HOME", raising=False)

    def test_default_install_anchors_at_home_dot_shaggy(
        self, tmp_path, monkeypatch
    ):
        # Standard install: SHAGGY_HOME == ~/.shaggy, no profile active.
        default_home = tmp_path / ".shaggy"
        default_home.mkdir()
    ...
    def test_profile_worker_resolves_to_shared_root(
        ...
        # Reproduces the bug: dispatcher uses ~/.shaggy/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.shaggy/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.shaggy/kanban.db.
        default_home = tmp_path / ".shaggy"
        default_home.mkdir()
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # SHAGGY_HOME and confirm equality. This is the property the
        ...
        # Worker's perspective (profile activated by `shaggy -p coder`).
        monkeypatch.setenv("SHAGGY_HOME", str(profile_home))
    ...
    def test_docker_custom_shaggy_home_uses_env_path_directly(
        self, tmp_path, monkeypatch
    ):
        # Docker / custom deployment: SHAGGY_HOME points outside ~/.shaggy.
        # `get_default_shaggy_root()` returns env_home directly when it
        # is not a `<root>/profiles/<name>` shape and not under
        # `Path.home() / ".shaggy"`.
        custom_root = tmp_path / "opt" / "shaggy"
    ...
    def test_docker_profile_layout_uses_grandparent(
        ...
        # Docker profile shape: SHAGGY_HOME=/opt/shaggy/profiles/coder;
        # `get_default_shaggy_root()` walks up to /opt/shaggy because
    ...
    def test_explicit_override_via_shaggy_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # Explicit override: SHAGGY_KANBAN_HOME beats every other
        # resolution rule.
        default_home = tmp_path / ".shaggy"
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SHAGGY_HOME", str(profile_home))
        monkeypatch.setenv("SHAGGY_KANBAN_HOME", str(override))
    ...
    def test_empty_override_falls_through(self, tmp_path, monkeypatch):
        ...
        default_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SHAGGY_HOME", str(default_home))
        monkeypatch.setenv("SHAGGY_KANBAN_HOME", "   ")
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # SHAGGY_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile SHAGGY_HOME and reads.
        monkeypatch.setenv("SHAGGY_HOME", str(profile_home))
        with kb.connect() as conn:
    ...
    def test_shaggy_kanban_db_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # SHAGGY_KANBAN_DB pins the file path directly and beats both
        # SHAGGY_KANBAN_HOME and the `get_default_shaggy_root()` path.
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SHAGGY_HOME", str(default_home))
        monkeypatch.setenv("SHAGGY_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("SHAGGY_KANBAN_DB", str(pinned_db))

        assert kb.kanban_db_path() == pinned_db
        # workspaces_root still follows SHAGGY_KANBAN_HOME -- the pins
        # are independent.
        assert kb.workspaces_root() == umbrella / "kanban" / "workspaces"

    def test_shaggy_kanban_workspaces_root_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # SHAGGY_KANBAN_WORKSPACES_ROOT pins the workspaces root directly.
        default_home = tmp_path / ".shaggy"
        default_home.mkdir()
        umbrella = tmp_path / "umbrella"
        umbrella.mkdir()
        ...
        monkeypatch.setenv("SHAGGY_HOME", str(default_home))
        monkeypatch.setenv("SHAGGY_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("SHAGGY_KANBAN_WORKSPACES_ROOT", str(pinned_ws))

        assert kb.workspaces_root() == pinned_ws
        # kanban_db_path still follows SHAGGY_KANBAN_HOME.
    ...
    def test_empty_per_path_overrides_fall_through(
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SHAGGY_HOME", str(default_home))
        monkeypatch.setenv("SHAGGY_KANBAN_DB", "   ")
        monkeypatch.setenv("SHAGGY_KANBAN_WORKSPACES_ROOT", "")
    ...
    def test_dispatcher_spawn_injects_kanban_db_and_workspaces_root(
        ...
        # The dispatcher's `_default_spawn` must inject SHAGGY_KANBAN_DB
        # and SHAGGY_KANBAN_WORKSPACES_ROOT into the worker env so the
        # worker converges on the dispatcher's paths even when the
        # `-p <profile>` flag rewrites SHAGGY_HOME.
        default_home = tmp_path / ".shaggy"
        default_home.mkdir()
        self._set_home(monkeypatch, tmp_path, default_home)
        ...
        env = captured["env"]
        assert env["SHAGGY_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["SHAGGY_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["SHAGGY_KANBAN_TASK"] == "t_dispatch_env"
        assert env["SHAGGY_KANBAN_BRANCH"] == "wt/t_dispatch_env"
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see shaggy_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(kanban_home, caplog):
    ...
    with _patch("shaggy_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("WARNING", logger="shaggy_state"):
            conn = kb.connect()
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'shaggy kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
...
# ---------------------------------------------------------------------------
# Dispatcher spawn invocation — _resolve_shaggy_argv()
#
# Workers spawned by the dispatcher must use a `shaggy` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["shaggy", ...]`
...
def test_resolve_shaggy_argv_prefers_path_shim(monkeypatch):
    """When `shaggy` is on PATH, use the shim — preserves familiar ps output."""
    import shutil
    import shaggy_cli.kanban_db as kb

    monkeypatch.delenv("SHAGGY_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: "/usr/local/bin/shaggy")
    argv = kb._resolve_shaggy_argv()
    assert argv == ["/usr/local/bin/shaggy"]


def test_resolve_shaggy_argv_absolutizes_relative_exe_shim(monkeypatch, tmp_path):
    """A relative executable override must not remain workspace-cwd-dependent."""
    import shaggy_cli.kanban_db as kb

    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("SHAGGY_BIN", ".\\shaggy.exe")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_shaggy_argv() == [os.path.abspath(".\\shaggy.exe")]


def test_resolve_shaggy_argv_avoids_implicit_windows_batch_shim(monkeypatch, tmp_path):
    ...
    bin_dir.mkdir()
    (bin_dir / "shaggy.CMD").write_text("@echo off\n", encoding="utf-8")
    monkeypatch.delenv("SHAGGY_BIN", raising=False)
    monkeypatch.setenv("PATH", str(bin_dir))
    monkeypatch.setenv("PATHEXT", ".CMD")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_shaggy_argv() == [sys.executable, "-m", "shaggy_cli.main"]


def test_resolve_shaggy_argv_honors_shaggy_bin_path_override(monkeypatch, tmp_path):
    """An explicit path-like SHAGGY_BIN lets service managers pin the executable."""
    import shutil
    import shaggy_cli.kanban_db as kb

    shim = tmp_path / "bin" / "shaggy"
    shim.parent.mkdir()
    shim.write_text("#!/bin/sh\n", encoding="utf-8")
    monkeypatch.setenv("SHAGGY_BIN", str(shim))
    monkeypatch.setattr(shutil, "which", lambda name: None)

    assert kb._resolve_shaggy_argv() == [str(shim)]


def test_resolve_shaggy_argv_shaggy_bin_bare_name_uses_path(monkeypatch, tmp_path):
    """Bare SHAGGY_BIN values keep PATH semantics instead of cwd shadowing."""
    import stat
    import shaggy_cli.kanban_db as kb

    cwd_shaggy = tmp_path / "shaggy"
    cwd_shaggy.write_text("wrong\n", encoding="utf-8")
    cwd_shaggy.chmod(cwd_shaggy.stat().st_mode | stat.S_IXUSR)
    path_shaggy = tmp_path / "bin" / "shaggy"
    path_shaggy.parent.mkdir()
    path_shaggy.write_text("right\n", encoding="utf-8")
    path_shaggy.chmod(path_shaggy.stat().st_mode | stat.S_IXUSR)
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("PATH", str(path_shaggy.parent))
    monkeypatch.setenv("SHAGGY_BIN", "shaggy")

    assert kb._resolve_shaggy_argv() == [str(path_shaggy)]


def test_resolve_shaggy_argv_shaggy_bin_bare_name_ignores_cwd(monkeypatch, tmp_path):
    """Bare SHAGGY_BIN does not accept current-directory shadow executables."""
    import sys
    import shaggy_cli.kanban_db as kb

    (tmp_path / "shaggy.exe").write_text("wrong\n", encoding="utf-8")
    monkeypatch.chdir(tmp_path)
...
def test_resolve_shaggy_argv_shaggy_bin_bare_cmd_uses_module_fallback(monkeypatch, tmp_path):
    """A PATH-resolved SHAGGY_BIN batch shim is not used as worker argv[0]."""
    import sys
    import shaggy_cli.kanban_db as kb
...
def test_resolve_shaggy_argv_shaggy_bin_unresolved_bare_name_falls_back(monkeypatch):
    """Unresolved SHAGGY_BIN command names do not delegate cwd search to Popen."""
    import sys
    import shaggy_cli.kanban_db as kb
...
def test_resolve_shaggy_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m shaggy_cli.main`.

    Pins the correct module name (NOT `shaggy` — there is no top-level
    `shaggy` package). Regression for #23198: the original PR shipped
    `python -m shaggy` which fails with `No module named shaggy` on every
    invocation.
    """
    import shutil
    import sys
    ...
    monkeypatch.setattr(shutil, "which", lambda name: None)
    argv = kb._resolve_shaggy_argv()
    assert argv == [sys.executable, "-m", "shaggy_cli.main"]


def test_resolve_shaggy_argv_module_actually_runs():
    """The fallback module name must be importable + runnable.

    A unit test that pins the literal string is necessary but not
    sufficient — if `shaggy_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m shaggy_cli.main --version`
    ...
    with mock.patch.dict(os.environ, {}, clear=False):
        os.environ.pop("SHAGGY_BIN", None)
    ...
    assert "Shaggy Agent" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
