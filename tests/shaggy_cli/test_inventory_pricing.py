...
columns + Free/Pro badges and gate paid models on free Cherries accounts, the
same way the `shaggy model` CLI picker does.
"""

import shaggy_cli.inventory as inv
...
def test_apply_pricing_nous_free_tier_gates_paid_models(monkeypatch):
    """A free-tier Cherries account marks paid models unavailable and sets the flag."""
    _patch_pricing(
        monkeypatch,
        free_tier=True,
        pricing={
...
def test_apply_pricing_nous_paid_tier_no_gating(monkeypatch):
    """A paid Cherries account gates nothing."""
    _patch_pricing(
        monkeypatch,
        free_tier=False,
