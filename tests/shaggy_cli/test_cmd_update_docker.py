"""Tests for ``shaggy update`` / ``--check`` inside the Docker container.
...
fix, ``shaggy update`` would fall through to ``"✗ Not a git repository.
Please reinstall: curl ... install.sh"`` — that script installs a *new*
host-side Shaggy, not an update to the running container, so the message
...
from shaggy_cli.main import _cmd_update_check, cmd_update
...
@patch("shaggy_cli.config.detect_install_method", return_value="docker")
@patch("subprocess.run")
def test_cmd_update_in_docker_prints_guidance_and_exits(
    mock_run, _mock_method, _mock_managed, capsys
):
    """``shaggy update`` inside Docker → friendly message + exit 1, no git calls."""
    ...
    assert "docker pull nousresearch/shaggy-agent:latest" in out
...
def test_cmd_update_check_in_docker_prints_guidance_and_exits(
    ...
    """``shaggy update --check`` inside Docker → same message + exit 1, no fetch."""
    with pytest.raises(SystemExit) as excinfo:
...
@patch("shaggy_cli.config.detect_install_method", return_value="pip")
@patch("shaggy_cli.banner.check_via_pypi", return_value=0)
...
def test_format_docker_update_message_contents():
    ...
    """
    from shaggy_cli.config import format_docker_update_message

    msg = format_docker_update_message()

    # Primary command — the entire reason this message exists.
    assert "docker pull nousresearch/shaggy-agent:latest" in msg
    ...
    assert "SHAGGY_HOME" in msg or "/opt/data" in msg, (
