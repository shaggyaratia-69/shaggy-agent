"""Regression test: ``shaggy mcp add --command`` must not clobber the
top-level ``args.command`` subparser dest.

The top-level argparse parser uses ``dest="command"`` for its subparsers
(``shaggy_cli/_parser.py``).  The dispatcher in ``shaggy_cli/main.py``
...
result, ``shaggy mcp add foo --url ...`` silently launches chat instead
...
def _build_parser():
    """Minimal replica of the slice of the shaggy parser that exhibits
...
class TestMcpAddCommandDest:
    def test_url_invocation_preserves_top_level_command(self):
        """`shaggy mcp add foo --url ...` must keep args.command == "mcp".
