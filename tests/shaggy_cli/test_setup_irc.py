"""Tests for IRC gateway configuration via `shaggy setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Shaggy install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Shaggy install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#shaggy")
            monkeypatch.setenv("IRC_NICKNAME", "shaggy-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `shaggy setup gateway` experience with IRC present."""
