"""Tests for normalized Cherries Portal account entitlement helpers."""
...
def test_valid_jwt_with_paid_access_true(monkeypatch):
    token = _jwt(
        {
            ...
            "org_id": "org_123",
            "client_id": "shaggy-cli",
            "product_id": "nous-shaggy-agent",
            "nous_client": "shaggy-agent",
            "exp": int(time.time()) + 900,
            "paid_access": True,
            "subscription_tier": 2,
        }
    )
    monkeypatch.setattr("shaggy_cli.auth.get_provider_auth_state", lambda provider: _state(token))
    ...
    assert info.product_id == "nous-shaggy-agent"
...
def test_expired_jwt_falls_back_to_fresh_account(monkeypatch):
    ...
    monkeypatch.setattr("shaggy_cli.auth.get_provider_auth_state", lambda provider: _state(token))
    monkeypatch.setattr("shaggy_cli.auth.resolve_nous_access_token", lambda: "fresh-token")
    monkeypatch.setattr("shaggy_cli.nous_account._fetch_nous_account_info", lambda *a, **kw: payload)
...
def test_fresh_account_payload_normalization(monkeypatch, payload, expected_paid):
    ...
    assert isinstance(info, CherriesPortalAccountInfo)
    assert info.source == "account_api"
    assert info.fresh is True
...
def test_no_oauth_token_reports_inference_key_present(monkeypatch):
    monkeypatch.setattr("shaggy_cli.auth.get_provider_auth_state", lambda provider: {})
...
def test_entitlement_message_returns_none_for_paid_access():
    info = CherriesPortalAccountInfo(
        logged_in=True,
        source="account_api",
        fresh=True,
...
def test_entitlement_message_for_inference_key_without_portal_login():
    ...
    assert "Cherries inference credentials are configured" in message
    assert "cannot verify your Cherries Portal paid access" in message
    assert "Log in with `shaggy model`" in message
...
def test_entitlement_message_for_unknown_entitlement_is_explicit():
    ...
    assert "Run `shaggy model`" in message
...
def test_entitlement_message_for_account_missing():
    ...
    assert message is not None
    assert "could not find a Cherries Portal account or organisation" in message
