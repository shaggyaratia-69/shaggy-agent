"""Tests for the `shaggy memory reset` CLI command.
...
- Profile-scoped reset (uses SHAGGY_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake SHAGGY_HOME with memory files."""
    shaggy_home = tmp_path / ".shaggy"
    memories = shaggy_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("SHAGGY_HOME", str(shaggy_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nShaggy repo is at ~/.shaggy/shaggy-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `shaggy memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        shaggy_home = tmp_path / ".shaggy"
        (shaggy_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("SHAGGY_HOME", str(shaggy_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's SHAGGY_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_shaggy_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
