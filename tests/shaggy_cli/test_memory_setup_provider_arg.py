"""Tests for `shaggy memory setup [provider]` routing.
...
``shaggy memory setup honcho``) without the interactive picker — which matters
because the per-provider ``shaggy <provider>`` subcommand is only registered
...
from shaggy_cli import memory_setup
...
class TestMemorySetupProviderRouting:
    ...
    def test_unknown_provider_reports_and_returns_early(self, capsys):
        ...
        assert "shaggy memory setup" in out
