...
The Python launcher (`shaggy --tui …`) has a ~100–300ms cold-start window
where stdin is still in cooked + echo mode. If a previous Shaggy session
...
from shaggy_cli.main import _suppress_mouse_residue_early
...
class TestEarlyMouseDisable:
    def test_writes_disable_sequence_when_tui_flag_in_argv(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["shaggy", "--tui", "-c", "abc"])
        monkeypatch.delenv("SHAGGY_TUI", raising=False)
        monkeypatch.delenv("SHAGGY_TUI_NO_EARLY_DISABLE", raising=False)
    ...
    def test_writes_disable_sequence_when_shaggy_tui_env_set(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["shaggy"])
        monkeypatch.setenv("SHAGGY_TUI", "1")
        monkeypatch.delenv("SHAGGY_TUI_NO_EARLY_DISABLE", raising=False)
    ...
    def test_no_op_on_non_tui_invocation(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["shaggy", "--version"])
        monkeypatch.delenv("SHAGGY_TUI", raising=False)
        monkeypatch.delenv("SHAGGY_TUI_NO_EARLY_DISABLE", raising=False)
    ...
    def test_respects_diagnostic_escape_hatch(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["shaggy", "--tui"])
        monkeypatch.delenv("SHAGGY_TUI", raising=False)
        monkeypatch.setenv("SHAGGY_TUI_NO_EARLY_DISABLE", "1")
    ...
    def test_skips_when_stdout_is_not_a_tty(self, monkeypatch):
        # `shaggy --tui … >log` or CI capture: pipe is fd 1, not a TTY. The
