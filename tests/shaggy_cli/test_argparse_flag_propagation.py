...
subparser, placing the flag BEFORE the subcommand (e.g. 'shaggy --yolo chat')
...
def _build_parser():
    """Build the shaggy argument parser from the real code.
...
class TestYoloEnvVar:
    """Verify --yolo sets SHAGGY_YOLO_MODE regardless of flag position.
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    parser and `chat`, so `shaggy gateway run --accept-hooks` failed
    ...
    def test_accepted_at_every_position(self, argv):
        """Invoking `shaggy <argv>` must exit 0 (help) rather than
