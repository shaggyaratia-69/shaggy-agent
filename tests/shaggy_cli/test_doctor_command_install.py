"""Tests for the Command Installation check in shaggy doctor."""
...
import shaggy_cli.doctor as doctor_mod


def _setup_doctor_env(monkeypatch, tmp_path, venv_name="venv"):
    """Create a minimal SHAGGY_HOME + PROJECT_ROOT for doctor tests."""
    ...
    shaggy_bin = venv_bin_dir / "shaggy"
    shaggy_bin.write_text("#!/usr/bin/env python\n# entry point\n")
    shaggy_bin.chmod(0o755)
    ...
    return home, project, shaggy_bin
...
class TestDoctorCommandInstallation:
    ...
    def test_correct_symlink_shows_ok(self, monkeypatch, tmp_path):
        home, project, shaggy_bin = _setup_doctor_env(monkeypatch, tmp_path)

        # Create the command link dir with correct symlink
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "shaggy"
        cmd_link.symlink_to(shaggy_bin)
    ...
    def test_fix_creates_missing_symlink(self, monkeypatch, tmp_path):
        ...
        cmd_link = tmp_path / ".local" / "bin" / "shaggy"
        assert cmd_link.is_symlink()
        assert cmd_link.resolve() == shaggy_bin.resolve()
    ...
    def test_wrong_target_symlink_shows_warn(self, monkeypatch, tmp_path):
        ...
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "shaggy"
        wrong_target = tmp_path / "wrong_shaggy"
    ...
    def test_dot_venv_dir_is_found(self, monkeypatch, tmp_path):
        ...
        shaggy_bin = project / ".venv" / "bin" / "shaggy"
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "shaggy"
        cmd_link.symlink_to(shaggy_bin)
        ...
        out = _run_doctor(fix=False)
        assert "Venv entry point exists" in out
        assert ".venv/bin/shaggy" in out

    @pytest.mark.skipif(sys.platform == "win32", reason="Symlink check is Unix-only")
    def test_non_symlink_regular_file_shows_ok(self, monkeypatch, tmp_path):
        """If ~/.local/bin/shaggy is a regular file (not symlink), accept it."""
        home, project, shaggy_bin = _setup_doctor_env(monkeypatch, tmp_path)

        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "shaggy"
        cmd_link.write_text("#!/bin/sh\nexec python -m shaggy_cli.main \"$@\"\n")
