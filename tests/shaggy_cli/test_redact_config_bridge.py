...
`SHAGGY_REDACT_SECRETS` at module-import time. `shaggy_cli/main.py` at
...
in .env), the toggle was silently ignored in both `shaggy chat` and
`shaggy gateway run`.

Fix: bridge `security.redact_secrets` from config.yaml → `SHAGGY_REDACT_SECRETS`
env var in `shaggy_cli/main.py` BEFORE the `setup_logging()` call.
"""
import os
import subprocess
import sys
...
def test_redact_secrets_false_in_config_yaml_is_honored(tmp_path):
    ...
    # Spawn a fresh Python process that imports shaggy_cli.main and checks
    # _REDACT_ENABLED. Must be a subprocess — we need a clean module state.
    probe = textwrap.dedent(
        """\
        import sys, os
        # Make absolutely sure the env var is not pre-set
        os.environ.pop("SHAGGY_REDACT_SECRETS", None)
        sys.path.insert(0, %r)
        import shaggy_cli.main  # triggers the bridge + setup_logging
        import agent.redact
        print(f"REDACT_ENABLED={agent.redact._REDACT_ENABLED}")
        print(f"ENV_VAR={os.environ.get('SHAGGY_REDACT_SECRETS', '<unset>')}")
        """
    ) % str(REPO_ROOT)

    env = dict(os.environ)
    env["SHAGGY_HOME"] = str(shaggy_home)
    env.pop("SHAGGY_REDACT_SECRETS", None)
...
def test_dotenv_redact_secrets_beats_config_yaml(tmp_path):
    """.env SHAGGY_REDACT_SECRETS takes precedence over config.yaml."""
    shaggy_home = tmp_path / ".shaggy"
    shaggy_home.mkdir()
    (shaggy_home / "config.yaml").write_text(
        textwrap.dedent(
    ...
    (shaggy_home / ".env").write_text("SHAGGY_REDACT_SECRETS=true\n")
