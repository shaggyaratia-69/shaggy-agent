"""Regression tests for Cherries Portal inference_base_url host-allowlist validation.
...
4. The env-var override path (``CHERRIES_INFERENCE_BASE_URL``) is NOT
...
from shaggy_cli.auth import (
    DEFAULT_CHERRIES_INFERENCE_URL,
    _ALLOWED_CHERRIES_INFERENCE_HOSTS,
...
class TestValidatorRules:
    def test_allowlisted_https_host_returned(self):
        url = "https://inference-api.cherriesandco.com/v1"
    ...
    def test_attacker_host_rejected(self, caplog):
        with caplog.at_level(logging.WARNING, logger="shaggy_cli.auth"):
            assert (
    ...
    def test_subdomain_of_allowlist_host_rejected(self):
        """*.cherriesandco.com is NOT in the allowlist — exact hostname only.

        A subdomain takeover or DNS hijack of *.cherriesandco.com would
        otherwise pass — keep the gate tight.
        """
        assert (
            _validate_nous_inference_url_from_network(
                "https://evil.inference-api.cherriesandco.com/v1"
            )
            is None
        )
    ...
    def test_http_scheme_rejected(self, caplog):
        with caplog.at_level(logging.WARNING, logger="shaggy_cli.auth"):
            assert (
                _validate_nous_inference_url_from_network(
                    "http://inference-api.cherriesandco.com/v1"
                )
                is None
            )
    ...
    def test_default_inference_url_is_in_allowlist(self):
        """Sanity check: DEFAULT_CHERRIES_INFERENCE_URL must itself validate.

        If anyone retargets the default away from
        ``inference-api.cherriesandco.com``, they MUST update the allowlist
        in the same change — otherwise the allowlist would reject the
        Portal's own legitimate default and break every install.
        """
        assert (
            _validate_nous_inference_url_from_network(DEFAULT_CHERRIES_INFERENCE_URL)
            == DEFAULT_CHERRIES_INFERENCE_URL.rstrip("/")
        )

    def test_allowlist_contains_inference_api_host(self):
        """The default's host must be in the allowlist set."""
        from urllib.parse import urlparse
        host = urlparse(DEFAULT_CHERRIES_INFERENCE_URL).hostname
        assert host in _ALLOWED_CHERRIES_INFERENCE_HOSTS
...
class TestCallSiteWiring:
    ...
    def _read_auth_source(self):
        import shaggy_cli.auth as _auth_mod
    ...
    def test_proxy_adapter_also_validates(self):
        """The Cherries proxy adapter applies the validator as defense-in-depth
        ...
        boundary."""
        from pathlib import Path
        import shaggy_cli.proxy.adapters.nous_portal as _nous_adapter
...
class TestEnvOverrideNotGated:
    ...
    ``CHERRIES_INFERENCE_BASE_URL`` is read by ``resolve_nous_runtime_credentials``
    ...
    def test_env_override_path_does_not_call_validator(self):
        ...
        for line in source.splitlines():
            if "CHERRIES_INFERENCE_BASE_URL" in line and "os.getenv" in line:
