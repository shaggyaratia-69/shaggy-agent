"""Tests for shaggy_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Shaggy entry point
(shaggy, shaggy-agent, shaggy-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Shaggy entry point imports shaggy_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported shaggy_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("shaggy_bootstrap", None)
    import shaggy_bootstrap  # noqa: WPS433
    return shaggy_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Shaggy CLI
...
class TestEntryPointsImportBootstrap:
    """Every Shaggy entry point must import shaggy_bootstrap as its
    ...
    # Entry points that invoke Shaggy as a process.  Each one must
    # import shaggy_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "shaggy_cli/main.py",   # shaggy CLI (console_script)
        "run_agent.py",          # shaggy-agent (console_script)
        "acp_adapter/entry.py",  # shaggy-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import shaggy_bootstrap' and that
        ...
        half-finished ``shaggy update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``shaggy_bootstrap`` as a top-level module) leaves shaggy
        ...
        """
        # Resolve relative to the shaggy-agent repo root.  Tests live
        # at tests/test_shaggy_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # shaggy start even when shaggy_bootstrap hasn't been
        ...
        assert first_import_name == "shaggy_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'shaggy_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import shaggy_bootstrap' line to be the first import."
        )
