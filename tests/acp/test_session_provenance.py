...
def test_root_session_no_compression(db):
    ...
    assert prov["currentShaggySessionId"] == "root1"
    assert prov["rootShaggySessionId"] == "root1"
    assert prov["parentShaggySessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_shaggy_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentShaggySessionId"] == "old"
    assert prov["rootShaggySessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousShaggySessionId"] == "old"
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootShaggySessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootShaggySessionId"] == "p"  # lineage root still walked
...
def test_no_false_rotation_when_head_unchanged(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "s", previous_shaggy_session_id="s"
    )
    assert "reason" not in prov
    assert "creatorKind" not in prov
    assert prov["previousShaggySessionId"] == "s"
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"shaggy"}
    assert "sessionProvenance" in meta["shaggy"]
    assert meta["shaggy"]["sessionProvenance"]["currentShaggySessionId"] == "root1"
