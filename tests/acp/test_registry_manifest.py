"""Tests for ACP Registry metadata shipped with Shaggy."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "shaggy-agent"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "Shaggy Agent"
    assert data["description"]
    assert data["repository"] == "https://github.com/shaggyaratia-69/shaggy-agent"
    assert data["website"].startswith("https://shaggyaratia-69.github.io/shaggy-agent/")
    assert data["authors"] == ["Cherries and Co"]
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"shaggy-agent[acp]=={data['version']}"
    assert uvx["args"] == ["shaggy-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"shaggy-agent[acp]=={_pyproject_version()}"
    )
