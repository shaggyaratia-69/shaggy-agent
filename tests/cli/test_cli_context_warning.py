...
def _isolate(tmp_path, monkeypatch):
    """Isolate SHAGGY_HOME so tests don't touch real config."""
...
def cli_obj(_isolate):
    """Create a minimal ShaggyCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = ShaggyCLI.__new__(ShaggyCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["shaggy-core"]
        obj.compact = False
