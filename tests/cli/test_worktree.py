...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("shaggy/shaggy-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned shaggy/* and pr-* branches."""

    def test_prunes_orphaned_shaggy_branch(self, git_repo):
        """shaggy/shaggy-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "shaggy/shaggy-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "shaggy/shaggy-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "shaggy/shaggy-deadbeef" in result.stdout
        ...
        assert "shaggy/shaggy-deadbeef" in orphaned
        ...
        assert "shaggy/shaggy-deadbeef" not in result.stdout
