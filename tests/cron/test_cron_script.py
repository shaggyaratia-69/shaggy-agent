...
- Path resolution (absolute, relative to SHAGGY_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp SHAGGY_HOME."""
    shaggy_home = tmp_path / ".shaggy"
    shaggy_home.mkdir()
    (shaggy_home / "cron").mkdir()
    (shaggy_home / "cron" / "output").mkdir()
    (shaggy_home / "scripts").mkdir()
    monkeypatch.setenv("SHAGGY_HOME", str(shaggy_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "SHAGGY_DIR", shaggy_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", shaggy_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", shaggy_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", shaggy_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.shaggy/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("SHAGGY_SESSION_PLATFORM") is None
        assert os.environ.get("SHAGGY_SESSION_CHAT_ID") is None
        assert os.environ.get("SHAGGY_SESSION_CHAT_NAME") is None
