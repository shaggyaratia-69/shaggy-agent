"""
Tests for mcp_serve — Shaggy MCP server.
...
def _isolate_shaggy_home(tmp_path, monkeypatch):
    """Redirect SHAGGY_HOME to a temp directory."""
    monkeypatch.setenv("SHAGGY_HOME", str(tmp_path))
    try:
        import shaggy_constants
        monkeypatch.setattr(shaggy_constants, "get_shaggy_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking shaggy_state schema."""
