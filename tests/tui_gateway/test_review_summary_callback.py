...
def server():
    with patch.dict(
        "sys.modules",
        {
            "shaggy_constants": MagicMock(
                get_shaggy_home=MagicMock(return_value="/tmp/shaggy_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'shaggy-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'shaggy-release' patched"
    }
