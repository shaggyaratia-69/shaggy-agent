"""Tests for `shaggy photon setup`'s access auto-configuration.

`_autoconfigure_access` allowlists the operator and points the cron home
channel at their DM, writing to the per-test ~/.shaggy/.env (the hermetic
SHAGGY_HOME fixture isolates this). It must fill only unset keys so a re-run
