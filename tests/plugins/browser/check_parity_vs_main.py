...
Run from the PR worktree:
    ...
    cd ~/.shaggy/shaggy-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.shaggy/shaggy-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.shaggy/shaggy-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a shaggy-agent checkout"
)
...
# Isolated SHAGGY_HOME for the config write.
