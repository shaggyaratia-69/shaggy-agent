"""Tests for shaggy_constants module."""
...
from shaggy_constants import (
    ...
    get_default_shaggy_root,
    is_container,
    parse_reasoning_effort,
)


class TestGetDefaultShaggyRoot:
    """Tests for get_default_shaggy_root() — Docker/custom deployment awareness."""

    def test_no_shaggy_home_returns_native(self, tmp_path, monkeypatch):
        """When SHAGGY_HOME is not set, returns ~/.shaggy."""
        monkeypatch.delenv("SHAGGY_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_shaggy_root() == tmp_path / ".shaggy"

    def test_shaggy_home_is_native(self, tmp_path, monkeypatch):
        """When SHAGGY_HOME = ~/.shaggy, returns ~/.shaggy."""
        native = tmp_path / ".shaggy"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SHAGGY_HOME", str(native))
        assert get_default_shaggy_root() == native

    def test_shaggy_home_is_profile(self, tmp_path, monkeypatch):
        """When SHAGGY_HOME is a profile under ~/.shaggy, returns ~/.shaggy."""
        native = tmp_path / ".shaggy"
    ...
    def test_shaggy_home_is_docker(self, tmp_path, monkeypatch):
        """When SHAGGY_HOME points outside ~/.shaggy (Docker), returns SHAGGY_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SHAGGY_HOME", str(docker_home))
        assert get_default_shaggy_root() == docker_home

    def test_shaggy_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any SHAGGY_HOME outside ~/.shaggy is treated as the root."""
        custom = tmp_path / "my-shaggy-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SHAGGY_HOME", str(custom))
        assert get_default_shaggy_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (SHAGGY_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SHAGGY_HOME", str(profile))
        assert get_default_shaggy_root() == docker_root
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(shaggy_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(shaggy_constants, "_container_detected", True)
        assert is_container() is True
