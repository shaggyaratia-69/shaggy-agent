...
class TestWriteDenyExactPaths:
    ...
    def test_shaggy_env(self):
        # ``.env`` under the active SHAGGY_HOME (profile-aware, not just
        # ``~/.shaggy``) must be write-denied. The hermetic test conftest
        # points SHAGGY_HOME at a tempdir — resolve via get_shaggy_home()
        # to match the denylist.
        from shaggy_constants import get_shaggy_home
        path = str(get_shaggy_home() / ".env")
        assert _is_write_denied(path) is True
...
class TestWriteAllowed:
    ...
    def test_shaggy_config_not_env(self):
        path = os.path.join(str(Path.home()), ".shaggy", "config.yaml")
