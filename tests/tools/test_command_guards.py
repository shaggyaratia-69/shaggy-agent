...
def _clean_state():
    ...
    saved = {}
    for k in ("SHAGGY_INTERACTIVE", "SHAGGY_GATEWAY_SESSION", "SHAGGY_EXEC_ASK", "SHAGGY_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["SHAGGY_INTERACTIVE"] = "1"
        session_key = os.getenv("SHAGGY_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No SHAGGY_INTERACTIVE or SHAGGY_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
