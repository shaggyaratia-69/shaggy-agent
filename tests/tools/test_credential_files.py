...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        shaggy_home.mkdir()
        (shaggy_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(shaggy_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.shaggy/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        shaggy_home = tmp_path / ".shaggy"
        shaggy_home.mkdir()
        (shaggy_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        shaggy_home = tmp_path / ".shaggy"
        shaggy_home.mkdir()
        (shaggy_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        shaggy_home = tmp_path / ".shaggy"
        shaggy_home.mkdir()
        (shaggy_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.shaggy/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        shaggy_home = tmp_path / ".shaggy"
        (shaggy_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"SHAGGY_HOME": str(shaggy_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.shaggy")

        assert mounts[0]["container_path"] == "/home/user/.shaggy/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.shaggy/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.shaggy/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape SHAGGY_HOME."""
        monkeypatch.setenv("SHAGGY_HOME", str(tmp_path / ".shaggy"))
        (tmp_path / ".shaggy").mkdir()

        # Create a sensitive file one level above shaggy_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside shaggy_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside SHAGGY_HOME must still be registered."""
        shaggy_home = tmp_path / ".shaggy"
        shaggy_home.mkdir()
        monkeypatch.setenv("SHAGGY_HOME", str(shaggy_home))
        (shaggy_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_shaggy_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of SHAGGY_HOME must be allowed."""
        shaggy_home = tmp_path / ".shaggy"
        shaggy_home.mkdir()
        subdir = shaggy_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside SHAGGY_HOME pointing outside must be rejected."""
        shaggy_home = tmp_path / ".shaggy"
        shaggy_home.mkdir()
        monkeypatch.setenv("SHAGGY_HOME", str(shaggy_home))

        # Create a sensitive file outside shaggy_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside shaggy_home pointing outside
        symlink = shaggy_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes SHAGGY_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, shaggy_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape SHAGGY_HOME."""
        shaggy_home = tmp_path / ".shaggy"
        shaggy_home.mkdir()
        monkeypatch.setenv("SHAGGY_HOME", str(shaggy_home))
        ...
        sensitive.write_text("{}")
        self._write_config(shaggy_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(shaggy_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside SHAGGY_HOME via config must still mount."""
        shaggy_home = tmp_path / ".shaggy"
        shaggy_home.mkdir()
        monkeypatch.setenv("SHAGGY_HOME", str(shaggy_home))

        (shaggy_home / "oauth.json").write_text("{}")
        self._write_config(shaggy_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        shaggy_home = tmp_path / ".shaggy"
        shaggy_home.mkdir()
        (shaggy_home / "cache" / "documents").mkdir(parents=True)
        (shaggy_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("SHAGGY_HOME", str(shaggy_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.shaggy/cache/documents" in paths
        assert "/root/.shaggy/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.shaggy/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        shaggy_home = tmp_path / ".shaggy"
        shaggy_home.mkdir()
        # Use legacy dir name — get_shaggy_dir prefers old if it exists
        (shaggy_home / "document_cache").mkdir()
        (shaggy_home / "image_cache").mkdir()
        monkeypatch.setenv("SHAGGY_HOME", str(shaggy_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(shaggy_home / "document_cache") in host_paths
        assert str(shaggy_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.shaggy/cache/documents" in container_paths
        assert "/root/.shaggy/cache/images" in container_paths
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        shaggy_home = tmp_path / ".shaggy"
        doc_dir = shaggy_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        shaggy_home = tmp_path / ".shaggy"
        ss_dir = shaggy_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.shaggy/cache/screenshots/session_abc/screen1.png"
