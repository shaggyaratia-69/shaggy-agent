...
When SHAGGY_UID is remapped at container boot, ``usermod -u`` only rewrites
files under the shaggy user's home directory ($SHAGGY_HOME == /opt/data).
Runtime-writable trees under ``/opt/shaggy`` must be explicitly chowned to the
new UID before services drop privileges. ``/opt/shaggy/gateway`` is one such
