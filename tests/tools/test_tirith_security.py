...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._shaggy_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_shaggy_bin(self):
        """After _INSTALL_FAILED, manual install in SHAGGY_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        shaggy_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(shaggy_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(shaggy_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._shaggy_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == shaggy_bin
            assert _tirith_mod._resolved_path == shaggy_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# SHAGGY_HOME isolation
# ---------------------------------------------------------------------------

class TestShaggyHomeIsolation:
    def test_shaggy_bin_dir_respects_shaggy_home(self):
        """_shaggy_bin_dir must use SHAGGY_HOME, not hardcoded ~/.shaggy."""
        from tools.tirith_security import _shaggy_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"SHAGGY_HOME": tmpdir}):
            result = _shaggy_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_shaggy_home(self):
        """_failure_marker_path must use SHAGGY_HOME, not hardcoded ~/.shaggy."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"SHAGGY_HOME": "/custom/shaggy"}):
            result = _failure_marker_path()
        assert result == "/custom/shaggy/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets SHAGGY_HOME; verify it's active."""
        shaggy_home = os.getenv("SHAGGY_HOME")
        assert shaggy_home is not None, "SHAGGY_HOME should be set by conftest"
        assert "shaggy_test" in shaggy_home, "Should point to test temp dir"

    def test_get_shaggy_home_fallback(self):
        """Without SHAGGY_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_shaggy_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove SHAGGY_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".shaggy")
            result = _get_shaggy_home()
