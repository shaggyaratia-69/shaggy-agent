...
def _restore_tool_modules():
    original_shaggy_home = os.environ.get("SHAGGY_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_shaggy_home is None:
            os.environ.pop("SHAGGY_HOME", None)
        else:
            os.environ["SHAGGY_HOME"] = original_shaggy_home
        _reset_modules(("tools", "shaggy_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    shaggy_cli = types.ModuleType("shaggy_cli")
    shaggy_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["shaggy_cli"] = shaggy_cli
    shaggy_home = tmp_path / "shaggy-home"
    os.environ["SHAGGY_HOME"] = str(shaggy_home)
    sys.modules["shaggy_cli.config"] = types.SimpleNamespace(
        get_shaggy_home=lambda: shaggy_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="shaggy-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": shaggy_home / "modal_snapshots.json",
