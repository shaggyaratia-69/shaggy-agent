...
def test_check_website_access_uses_dynamic_shaggy_home(monkeypatch, tmp_path):
    ...
    # Invalidate the module-level cache so the new SHAGGY_HOME is picked up.
    # A prior test may have cached a default policy (enabled=False) under the
    # old SHAGGY_HOME set by the autouse _isolate_shaggy_home fixture.
...
def test_check_website_access_fails_open_on_malformed_config(tmp_path, monkeypatch):
    ...
    # Simulate default path by pointing SHAGGY_HOME to tmp_path
