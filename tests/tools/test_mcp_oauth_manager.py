...
def test_shaggy_provider_subclass_exists():
    """ShaggyMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _SHAGGY_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _SHAGGY_PROVIDER_CLS is not None
    assert issubclass(_SHAGGY_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_shaggy_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns ShaggyMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _SHAGGY_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("SHAGGY_HOME", str(tmp_path))
    ...
    assert _SHAGGY_PROVIDER_CLS is not None
    assert isinstance(provider, _SHAGGY_PROVIDER_CLS)
    assert provider._shaggy_server_name == "srv"
