...
def _enable_managed_nous_tools(monkeypatch):
    ...
    monkeypatch.setattr("shaggy_cli.auth.get_nous_auth_status", lambda: {"logged_in": True})
    monkeypatch.setattr("shaggy_cli.models.check_nous_free_tier", lambda: False)
...
def test_openai_tts_uses_managed_audio_gateway_when_direct_key_absent(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("TOOL_GATEWAY_DOMAIN", "cherriesandco.com")
    monkeypatch.setenv("TOOL_GATEWAY_USER_TOKEN", "nous-token")
    ...
    assert captured["base_url"] == "https://openai-audio-gateway.cherriesandco.com/v1"
...
def test_transcription_uses_model_specific_response_formats(monkeypatch, tmp_path):
    ...
    assert whisper_capture["base_url"] == "https://openai-audio-gateway.cherriesandco.com/v1"
