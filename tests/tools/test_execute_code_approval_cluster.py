...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad SHAGGY_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: SHAGGY_GATEWAY_SESSION set, a bound session
    ...
    monkeypatch.delenv("SHAGGY_CRON_SESSION", raising=False)
...
def test_guard_cron_deny_blocks(monkeypatch):
    monkeypatch.setenv("SHAGGY_CRON_SESSION", "1")
    monkeypatch.delenv("SHAGGY_GATEWAY_SESSION", raising=False)
    monkeypatch.setattr(A, "_get_approval_mode", lambda: "manual")
...
def test_env_scrub_shaggy_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "SHAGGY_HOME": "/h", "SHAGGY_PROFILE": "p",
        "SHAGGY_CONFIG": "/c.yaml", "SHAGGY_ENV": "/e",
        # other SHAGGY_* → dropped (broad prefix removed)
        "SHAGGY_BASE_URL": "https://x", "SHAGGY_INTERACTIVE": "1",
        "SHAGGY_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("SHAGGY_HOME", "SHAGGY_PROFILE", "SHAGGY_CONFIG", "SHAGGY_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "SHAGGY_BASE_URL", "SHAGGY_INTERACTIVE", "SHAGGY_KANBAN_DB",
...
def test_env_scrub_logs_dropped_shaggy_vars(caplog):
    """Dropping a non-allowlisted, non-secret SHAGGY_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset SHAGGY_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "SHAGGY_HOME": "/h",          # allowlisted → kept, not logged
        "SHAGGY_BASE_URL": "https://x",   # dropped → logged
        "SHAGGY_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "SHAGGY_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "SHAGGY_HOME" in out and "PATH" in out
    assert "SHAGGY_BASE_URL" not in out and "SHAGGY_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "SHAGGY_BASE_URL" in msgs and "SHAGGY_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "SHAGGY_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped SHAGGY_* vars."""
    import logging
