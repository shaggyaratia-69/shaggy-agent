...
- Signal forwarding runs through the init so ``docker stop`` triggers
  shaggy's own graceful-shutdown path.
"""
...
def test_dockerfile_installs_an_init_for_zombie_reaping(dockerfile_text):
    ...
    Without a PID-1 init that handles SIGCHLD, shaggy accumulates zombie
    ...
    assert installed, (
        ...
        "orphaned subprocesses, shaggy accumulates zombies in Docker "
...
def test_dockerfile_entrypoint_routes_through_the_init(dockerfile_text):
    ...
    directly, the shell becomes PID 1 and will ``exec`` into shaggy,
    ...
    assert routes_through_init, (
        ...
        "If tini is only installed but not wired into ENTRYPOINT, shaggy "
...
def test_dockerfile_installs_tui_dependencies(dockerfile_text):
    ...
    # resolve dependencies. The bundled ``shaggy-ink`` workspace package is
    ...
    assert "ui-tui/packages/shaggy-ink/" in dockerfile_text
    assert any(
...
def test_dockerfile_materializes_local_tui_ink_package(dockerfile_text):
    # ``shaggy-ink`` is a bundled workspace package referenced from
    # ``ui-tui/package.json`` via ``file:`` — not pulled from the npm
    # registry. The contract this test pins is just that the image
    # actually carries the package source so ``await import('@shaggy/ink')``
    # can resolve at runtime; the previous, much pickier assertion (manual
    # ``rm -rf`` + ``npm install --omit=dev --prefix node_modules/@shaggy/ink``)
    ...
    assert "ui-tui/packages/shaggy-ink/" in dockerfile_text, (
        "Dockerfile must COPY the bundled shaggy-ink workspace package "
        "so ``await import('@shaggy/ink')`` resolves at runtime."
    )
