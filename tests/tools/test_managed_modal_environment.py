...
def _restore_tool_and_agent_modules():
    ...
    original_modules = {
        ...
        if name in {"tools", "agent", "shaggy_cli"}
    ...
    try:
        ...
        _reset_modules(("tools", "agent", "shaggy_cli"))
...
def _install_fake_tools_package(*, credential_mounts=None):
    ...
    sys.modules["shaggy_cli.config"] = types.SimpleNamespace(
        get_shaggy_home=lambda: Path(tempfile.gettempdir()) / "shaggy-home",
    )
...
def test_managed_modal_rejects_host_credential_passthrough():
    _install_fake_tools_package(
        credential_mounts=[{
            ...
            "container_path": "/root/.shaggy/token.json",
        }]
    )
    managed_modal = _load_tool_module("tools.environments.managed_modal", "environments/managed_modal.py")
