...
def test_load_transcript_returns_db_messages_when_no_jsonl(tmp_path, monkeypatch):
    ...
    ~/.shaggy/state.db. (DEFAULT_DB_PATH is a module-level constant computed
    at shaggy_state import time, before pytest's SHAGGY_HOME monkeypatch
    fires — the autouse fixture's SHAGGY_HOME override doesn't help here.)
    """
    import shaggy_state
    monkeypatch.setattr(shaggy_state, "DEFAULT_DB_PATH", tmp_path / "state.db")
