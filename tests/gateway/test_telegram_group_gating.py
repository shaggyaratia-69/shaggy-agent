...
def test_group_messages_can_require_direct_trigger_via_config():
    ...
    assert adapter._should_process_message(_group_message("hi @shaggy_bot", entities=[_mention_entity("hi @shaggy_bot")])) is True
    ...
    assert adapter._should_process_message(
        _group_message(
            "/status@shaggy_bot",
            entities=[_bot_command_entity("/status@shaggy_bot", "/status@shaggy_bot")],
        ),
        is_command=True,
    ) is True
...
def test_raw_bot_mention_fallback_does_not_match_email_or_substring():
    adapter = _make_adapter(require_mention=True, bot_username="shaggy_bot")

    assert adapter._should_process_message(_group_message("email ops@shaggy_bot.example")) is False
    assert adapter._should_process_message(_group_message("prefix@shaggy_bot hi")) is False
    assert adapter._should_process_message(_group_message("hi @shaggy_bot")) is True
...
def test_guest_mode_allows_only_direct_mentions_outside_allowed_chats():
    ...
    mentioned = _group_message(
        "hi @shaggy_bot",
        chat_id=-201,
        entities=[_mention_entity("hi @shaggy_bot")],
    )
...
def test_allowed_topics_drop_other_forum_topics_before_other_gates():
    ...
    assert adapter._should_process_message(
        _group_message("hi @shaggy_bot", chat_id=-100, thread_id=11, entities=[_mention_entity("hi @shaggy_bot")])
    ) is False
