...
class TestHandleUpdateCommand:
    ...
    async def test_no_shaggy_binary(self, tmp_path):
        """Returns error when shaggy is not on PATH and shaggy_cli is not importable."""
        runner = _make_runner()
        event = _make_event()
        ...
        assert "shaggy update" in result

    @pytest.mark.asyncio
    async def test_fallback_to_sys_executable(self, tmp_path):
        """Falls back to sys.executable -m shaggy_cli.main when shaggy not on PATH."""
        runner = _make_runner()
        event = _make_event()
        ...
        # The update_cmd uses sys.executable -m shaggy_cli.main
        joined = " ".join(call_args) if isinstance(call_args, list) else call_args
        assert "shaggy_cli.main" in joined or "bash" in call_args[0]

    @pytest.mark.asyncio
    async def test_resolve_shaggy_bin_prefers_which(self, tmp_path):
        """_resolve_shaggy_bin returns argv parts from shutil.which when available."""
        from gateway.run import _resolve_shaggy_bin

        with patch("shutil.which", return_value="/custom/path/shaggy"):
            result = _resolve_shaggy_bin()

        assert result == ["/custom/path/shaggy"]

    @pytest.mark.asyncio
    async def test_resolve_shaggy_bin_fallback(self):
        """_resolve_shaggy_bin falls back to sys.executable argv when which fails."""
        import sys
        from gateway.run import _resolve_shaggy_bin
        ...
        assert result == [sys.executable, "-m", "shaggy_cli.main"]

    @pytest.mark.asyncio
    async def test_resolve_shaggy_bin_returns_none_when_both_fail(self):
        """_resolve_shaggy_bin returns None when both strategies fail."""
        from gateway.run import _resolve_shaggy_bin
    ...
    async def test_writes_pending_marker(self, tmp_path):
        ...
        pending_path = shaggy_home / ".update_pending.json"
        assert pending_path.exists()
        data = json.loads(pending_path.read_text())
        assert data["platform"] == "telegram"
        assert data["chat_id"] == "99999"
        assert "timestamp" in data
        assert not (shaggy_home / ".update_exit_code").exists()
    ...
    async def test_writes_pending_marker_with_thread_id(self, tmp_path):
        ...
        data = json.loads((shaggy_home / ".update_pending.json").read_text())
    ...
    async def test_popen_failure_cleans_up(self, tmp_path):
        ...
        assert not (shaggy_home / ".update_pending.json").exists()
        assert not (shaggy_home / ".update_exit_code").exists()
...
class TestSendUpdateNotification:
    ...
    async def test_defers_notification_while_update_still_running(self, tmp_path):
        ...
        (shaggy_home / ".update_output.txt").write_text("still running")
    ...
    async def test_recovers_from_claimed_pending_file(self, tmp_path):
        ...
        claimed_path = shaggy_home / ".update_pending.claimed.json"
        claimed_path.write_text(json.dumps({
            "platform": "telegram", "chat_id": "67890", "user_id": "12345",
        }))
        (shaggy_home / ".update_output.txt").write_text("done")
        (shaggy_home / ".update_exit_code").write_text("0")
    ...
    async def test_sends_notification_with_output(self, tmp_path):
        ...
        (shaggy_home / ".update_pending.json").write_text(json.dumps(pending))
        (shaggy_home / ".update_output.txt").write_text(
    ...
    async def test_truncates_long_output(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (shaggy_home / ".update_pending.json").write_text(json.dumps(pending))
        (shaggy_home / ".update_output.txt").write_text("x" * 5000)
        (shaggy_home / ".update_exit_code").write_text("0")
    ...
    async def test_sends_failure_message_when_update_fails(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (shaggy_home / ".update_pending.json").write_text(json.dumps(pending))
        (shaggy_home / ".update_output.txt").write_text("Traceback: boom")
        (shaggy_home / ".update_exit_code").write_text("1")
    ...
    async def test_cleans_up_files_after_notification(self, tmp_path):
        ...
        pending_path = shaggy_home / ".update_pending.json"
        output_path = shaggy_home / ".update_output.txt"
        exit_code_path = shaggy_home / ".update_exit_code"
        pending_path.write_text(json.dumps({
