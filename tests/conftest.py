"""Shared fixtures for the shaggy-agent test suite.
...
2. **Isolated SHAGGY_HOME.** SHAGGY_HOME points to a per-test tempdir so
   code reading ``~/.shaggy/*`` via ``get_shaggy_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".shaggy"`` instead of the canonical
   ``get_shaggy_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No SHAGGY_SESSION_* inheritance** — the agent's current gateway
...
# SHAGGY_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_SHAGGY_BEHAVIORAL_VARS = frozenset({
    "SHAGGY_YOLO_MODE",
    "SHAGGY_INTERACTIVE",
    "SHAGGY_QUIET",
    ...
    "SHAGGY_SESSION_CHAT_ID",
    "SHAGGY_SESSION_CHAT_NAME",
    "SHAGGY_SESSION_THREAD_ID",
    "SHAGGY_SESSION_SOURCE",
    "SHAGGY_SESSION_KEY",
    "SHAGGY_GATEWAY_SESSION",
    "SHAGGY_PLATFORM",
    "SHAGGY_MODEL",
    "SHAGGY_INFERENCE_MODEL",
    "SHAGGY_INFERENCE_PROVIDER",
    "SHAGGY_TUI_PROVIDER",
    "SHAGGY_MANAGED",
    "SHAGGY_DEV",
    "SHAGGY_CONTAINER",
    ...
    "SHAGGY_TIMEZONE",
    "SHAGGY_REDACT_SECRETS",
    "SHAGGY_BACKGROUND_NOTIFICATIONS",
    "SHAGGY_EXEC_ASK",
    "SHAGGY_HOME_MODE",
    "SHAGGY_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.shaggy/kanban.db instead of the per-test SHAGGY_HOME.
    "SHAGGY_KANBAN_DB",
    "SHAGGY_KANBAN_BOARD",
    "SHAGGY_KANBAN_HOME",
    "SHAGGY_KANBAN_WORKSPACES_ROOT",
    "SHAGGY_KANBAN_LOGS_ROOT",
    "SHAGGY_KANBAN_TASK",
    "SHAGGY_KANBAN_WORKSPACE",
    "SHAGGY_KANBAN_RUN_ID",
    "SHAGGY_KANBAN_CLAIM_LOCK",
    "SHAGGY_KANBAN_DISPATCH_IN_GATEWAY",
    "SHAGGY_TENANT",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and SHAGGY_HOME to per-test tempdirs so code that
    reads ``~/.shaggy/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral SHAGGY_* vars that could change test semantics.
    for name in _SHAGGY_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect SHAGGY_HOME to a per-test tempdir. Code that reads
    #    ``~/.shaggy/*`` via ``get_shaggy_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.shaggy/*`` via
    #    ``Path.home() / ".shaggy"`` instead of ``get_shaggy_home()``
    #    is a bug to fix at the callsite.
    fake_shaggy_home = tmp_path / "shaggy_test"
    fake_shaggy_home.mkdir()
    (fake_shaggy_home / "sessions").mkdir()
    (fake_shaggy_home / "cron").mkdir()
    (fake_shaggy_home / "memories").mkdir()
    (fake_shaggy_home / "skills").mkdir()
    monkeypatch.setenv("SHAGGY_HOME", str(fake_shaggy_home))
    ...
    #    ~/.shaggy/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import shaggy_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_shaggy_home(_hermetic_environment):
...
def _reset_module_state():
    ...
    for _logger_name in ("tools", "run_agent", "trajectory_compressor", "cron", "shaggy_cli"):
...
@pytest.fixture()
def mock_config():
    """Return a minimal shaggy config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``shaggy-gateway`` process
...
# correlated with a ``tests/shaggy_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> shaggy-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart shaggy-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting shaggy/python patterns are also blocked.
    """
    ...
    def _matches_shaggy_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _SHAGGY_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_shaggy_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch shaggy-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m shaggy_cli.main".
                if (
                    "shaggy" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live shaggy-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting shaggy/python could hit the live gateway. "
