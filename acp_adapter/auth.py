"""ACP auth helpers — detect and advertise Shaggy authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "shaggy-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Shaggy runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Shaggy can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Shaggy.
    ...
    not have Shaggy provider credentials configured yet, so Shaggy always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Shaggy using the currently configured "
