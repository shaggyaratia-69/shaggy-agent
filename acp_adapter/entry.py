"""CLI entry point for the shaggy-agent ACP adapter.

Loads environment variables from ``~/.shaggy/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from SHAGGY_HOME (default ``~/.shaggy``)."""
    from shaggy_cli.env_loader import load_shaggy_dotenv

    shaggy_home = get_shaggy_home()
    loaded = load_shaggy_dotenv(shaggy_home=shaggy_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", shaggy_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="shaggy-acp",
        description="Run Shaggy Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Shaggy version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
...
def _print_version() -> None:
    ...
    print(shaggy_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import ShaggyACPAgent  # noqa: F401

    print("Shaggy ACP check OK")


def _run_setup() -> None:
    from shaggy_cli.main import main as shaggy_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "shaggy", "model"]
        shaggy_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``shaggy postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting shaggy-agent ACP adapter")
    ...
    import acp
    from .server import ShaggyACPAgent
