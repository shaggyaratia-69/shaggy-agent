...
SHAGGY_HOME_DIR="${SHAGGY_HOME:-$HOME/.shaggy}"
SHAGGY_CFG="${SHAGGY_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Shaggy config has twozero_td MCP entry ──
if [[ ! -f "$SHAGGY_CFG" ]]; then
    echo -e " ${FAIL} Shaggy config not found at ${SHAGGY_CFG}"
    manual_steps+=("Create ${SHAGGY_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$SHAGGY_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Shaggy config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Shaggy config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${SHAGGY_CFG} manually"); }
    manual_steps+=("Restart Shaggy session to pick up config change")
fi
