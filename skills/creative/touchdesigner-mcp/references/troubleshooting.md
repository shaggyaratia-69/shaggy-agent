...
## 5. Shaggy Configuration

### Config location

`$SHAGGY_HOME/config.yaml` (defaults to `~/.shaggy/config.yaml` when `SHAGGY_HOME` is unset)
...
Restart the Shaggy session for changes to take effect. The MCP connection is
