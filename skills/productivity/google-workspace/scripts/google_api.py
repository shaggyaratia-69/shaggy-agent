#!/usr/bin/env python3
"""Google Workspace API CLI for Shaggy Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Shaggy-facing JSON contract and falls back to the Python client
...
from _shaggy_home import get_shaggy_home
...
def _gws_binary() -> str | None:
    override = os.getenv("SHAGGY_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Shaggy Agent")
