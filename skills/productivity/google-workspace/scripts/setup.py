#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Shaggy Agent.
...
# Ensure sibling modules (_shaggy_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _shaggy_home import display_shaggy_home, get_shaggy_home

SHAGGY_HOME = get_shaggy_home()
TOKEN_PATH = SHAGGY_HOME / "google_token.json"
CLIENT_SECRET_PATH = SHAGGY_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = SHAGGY_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Shaggy profile to refresh consent."
    )
...
def install_deps():
    ...
    try:
        ...
        print("  pip install 'shaggy-agent[google]'")
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Shaggy home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_shaggy_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Shaggy")
