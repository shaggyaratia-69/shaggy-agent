...
# 5. Symlinks the 'shaggy' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Shaggy Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/shaggy-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink shaggy into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up shaggy command..."

SHAGGY_BIN="$SCRIPT_DIR/venv/bin/shaggy"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$SHAGGY_BIN" "$COMMAND_LINK_DIR/shaggy"
echo -e "${GREEN}✓${NC} Symlinked shaggy → $COMMAND_LINK_DISPLAY_DIR/shaggy"
...
# ============================================================================
# Seed bundled skills into ~/.shaggy/skills/
# ============================================================================

SHAGGY_SKILLS_DIR="${SHAGGY_HOME:-$HOME/.shaggy}/skills"
mkdir -p "$SHAGGY_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.shaggy/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$SHAGGY_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  shaggy status        # Check configuration"
if is_termux; then
    echo "  shaggy gateway       # Run gateway in foreground"
else
    echo "  shaggy gateway install # Install gateway service (messaging + cron)"
fi
echo "  shaggy cron list     # View scheduled jobs"
echo "  shaggy doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m shaggy_cli.main setup
fi
