"""Persistent slash-command worker — one ShaggyCLI per TUI session.
...
def _run(cli: ShaggyCLI, command: str) -> str:
...
def main():
    ...
    os.environ["SHAGGY_SESSION_KEY"] = args.session_key
    os.environ["SHAGGY_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = ShaggyCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
