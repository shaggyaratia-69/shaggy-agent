# OpenAI-Compatible API Server for Shaggy Agent
...
SSE streaming. By exposing this endpoint, shaggy-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  shaggy-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.shaggy/.env`
2. Run `shaggy gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with shaggy-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns shaggy-agent as a model) |
...
Authorization: Bearer shaggy-api-key-here
...
But shaggy-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "shaggy-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured shaggy-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of shaggy-agent's core prompt
...
Once implemented, shaggy-agent works as a drop-in backend for:
