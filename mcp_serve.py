"""
Shaggy MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Shaggy-specific extra)

Usage:
    shaggy mcp serve
    shaggy mcp serve --verbose
...
logger = logging.getLogger("shaggy.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using SHAGGY_HOME."""
    try:
        from shaggy_constants import get_shaggy_home
        return get_shaggy_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("SHAGGY_HOME", Path.home() / ".shaggy")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from shaggy_constants import get_shaggy_home
        directory_file = get_shaggy_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("SHAGGY_HOME", Path.home() / ".shaggy")
...
class EventBridge:
    ...
    This is the Shaggy equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from shaggy_constants import get_shaggy_home
            db_file = get_shaggy_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("SHAGGY_HOME", Path.home() / ".shaggy")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Shaggy MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "shaggy",
        instructions=(
            "Shaggy Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Shaggy MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
