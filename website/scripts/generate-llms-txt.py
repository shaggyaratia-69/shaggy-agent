#!/usr/bin/env python3
"""Generate llms.txt and llms-full.txt for the Shaggy docs site.
...
SECTIONS: list[tuple[str, list[tuple[str, str, str | None]]]] = [
    ...
    ("Guides & Tutorials", [
        ...
        ("guides/python-library", "Use Shaggy as a Python Library", None),
        ("guides/use-mcp-with-shaggy", "Use MCP with Shaggy", None),
        ("guides/use-voice-mode-with-shaggy", "Use Voice Mode with Shaggy", None),
        ("guides/use-soul-with-shaggy", "Use SOUL.md with Shaggy", None),
        ("guides/build-a-shaggy-plugin", "Build a Shaggy Plugin", None),
    ...
    ("Reference", [
        ...
        ("reference/skills-catalog", "Bundled Skills Catalog", "Table of all ~90 skills bundled with Shaggy"),
...
def emit_llms_index() -> str:
    ...
    lines: list[str] = []
    lines.append("# Shaggy Agent")
    lines.append("")
    lines.append(
        "> The self-improving AI agent built by Cherries and Co. A terminal-native "
...
def emit_llms_full() -> str:
    ...
    seen: set[Path] = set()
    chunks: list[str] = [
        "# Shaggy Agent — Full Documentation\n",
        (
            "This file is the entire Shaggy Agent documentation concatenated for LLM "
