...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/shaggyaratia-69/shaggy-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    shaggy_meta = (fm.get("metadata") or {}).get("shaggy") or {}
    tags = shaggy_meta.get("tags") or []
    related = shaggy_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `shaggy skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Shaggy Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Shaggy ships with a large built-in skill library copied into `~/.shaggy/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Shaggy also syncs bundled skills on `shaggy update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.shaggy/skills/` tree, it is still shipped with Shaggy; restore it with `shaggy skills reset <name> --restore`.",
        "",
