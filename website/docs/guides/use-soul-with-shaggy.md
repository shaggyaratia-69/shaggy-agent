---
sidebar_position: 7
title: "Use SOUL.md with Shaggy"
description: "How to use SOUL.md to shape Shaggy Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Shaggy

`SOUL.md` is the **primary identity** for your Shaggy instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Shaggy to feel like the same assistant every time you talk to it — or if you want to replace the Shaggy persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Shaggy should be
- what Shaggy should avoid stylistically
- how Shaggy should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Shaggy is and how Shaggy speaks
...
Shaggy now uses only the global SOUL file for the current instance:

```text
~/.shaggy/SOUL.md
```

If you run Shaggy with a custom home directory, it becomes:
...
Shaggy automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Shaggy does not overwrite it
- if the file exists but is empty, Shaggy adds nothing from it to the prompt

## How Shaggy uses it

When Shaggy starts a session, it reads `SOUL.md` from `SHAGGY_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Shaggy feels.
...
Shaggy already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Shaggy should sound.

# Avoid
What Shaggy should not do.

# Defaults
How Shaggy should behave when ambiguity appears.
```
...
Then restart Shaggy or start a new session.
...
### I edited SOUL.md but Shaggy still sounds the same

Check:
- you edited `~/.shaggy/SOUL.md` or `$SHAGGY_HOME/SOUL.md`
...
### Shaggy is ignoring parts of my SOUL.md
