---
sidebar_position: 6
title: "Use MCP with Shaggy"
description: "A practical guide to connecting MCP servers to Shaggy Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Shaggy

This guide shows how to actually use MCP with Shaggy Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Shaggy tool
- you want Shaggy to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Shaggy to internal APIs, databases, or company systems without modifying Shaggy core

Do not use MCP when:
- a built-in Shaggy tool already solves the job well
...
- Shaggy remains the agent
- MCP servers contribute tools
- Shaggy discovers those tools at startup or reload time
...
If you installed Shaggy with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Shaggy banner/status should show MCP integration when configured
- ask Shaggy what tools it has available
...
## WSL2: bridge Shaggy in WSL to Windows Chrome

This is the practical setup when:

- Shaggy runs inside WSL2
...
In this setup, Shaggy does **not** connect to Chrome directly. Instead:

- Shaggy runs in WSL
- Shaggy starts a local stdio MCP server
...
```text
Shaggy (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Shaggy stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Shaggy core browser transport
...
```bash
shaggy mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
shaggy mcp test chrome-devtools-win
```

Then start a fresh Shaggy session or run:
...
Once loaded, Shaggy can use the MCP-prefixed browser tools directly. For example:
...
If Shaggy runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Shaggy from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Shaggy from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Shaggy:
...
2. Shaggy-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Shaggy will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Shaggy to reason over a bounded workspace.
...
Now Shaggy can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Shaggy core.
...
Because Shaggy now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
