...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `shaggy send`."
---

# Pipe Script Output to Messaging Platforms

`shaggy send` is a small, scriptable CLI that pushes a message to any
messaging platform Shaggy is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | shaggy send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `shaggy
...
shaggy send --to telegram "deploy finished"

# Pipe in stdout from anything
echo "RAM 92%" | shaggy send --to telegram:-1001234567890

# Send a file
shaggy send --to discord:#ops --file /tmp/report.md

# Attach a subject/header line
shaggy send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
shaggy send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
shaggy send --list

# Filter by platform
shaggy send --list telegram
```
...
Any platform Shaggy ships adapters for works as a target:
...
`shaggy send` resolves the message body in this order:

1. **Positional argument** — `shaggy send --to telegram "hi"`
2. **`--file PATH`** — `shaggy send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | shaggy send --to telegram`

When stdin is a TTY (no pipe), Shaggy does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  shaggy send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `shaggy send` reuses your Shaggy config, the same script works on
any host where Shaggy is installed — no need to export bot tokens into
...
`shaggy send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  shaggy send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | shaggy send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.shaggy/bin/shaggy send \
...
```bash
./train.py --epochs 200 && \
  shaggy send --to telegram "training done" || \
  shaggy send --to telegram "training failed (exit $?)"
```
...
shaggy send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(shaggy send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `shaggy send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `shaggy send` calls
the platform's REST endpoint directly using credentials from
`~/.shaggy/.env` and `~/.shaggy/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `shaggy gateway start` and retry.
...
# Machine-readable
shaggy send --list --json
```

The listing is built from `~/.shaggy/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`shaggy gateway
...
| Approach | Multi-platform | Reuses Shaggy creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `shaggy send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`shaggy send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `shaggy send`.
...
- [Gateway Internals](/docs/developer-guide/gateway-internals) —
  the delivery router that `shaggy send` shares with cron delivery.
