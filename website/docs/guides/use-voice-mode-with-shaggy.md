---
sidebar_position: 8
title: "Use Voice Mode with Shaggy"
description: "A practical guide to setting up and using Shaggy voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Shaggy
...
- you want Shaggy sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Shaggy.
...
## Step 1: make sure normal Shaggy works first
...
Shaggy supports both local and cloud speech stacks.
...
### If you use `shaggy setup`

If you choose NeuTTS in the setup wizard, Shaggy checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Shaggy transcribes and responds
...
- asking Shaggy to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Shaggy loop.

## Tuning CLI behavior

### Silence threshold

If Shaggy starts/stops too aggressively, tune:
...
Shaggy stays a normal chat bot, but can speak replies.
...
- you want Shaggy to function like a portable research or ops assistant
...
Shaggy joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Shaggy detects speech boundaries
- transcripts are posted in the associated text channel
- Shaggy responds in text and audio
...
1. get text Shaggy working
2. install `shaggy-agent[voice]`
