...
```bash
shaggy teams-pipeline validate
```
...
```bash
shaggy teams-pipeline token-health
shaggy teams-pipeline token-health --force-refresh
```
...
```bash
shaggy teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
shaggy teams-pipeline maintain-subscriptions
shaggy teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Shaggy cron (recommended if you already run the Shaggy gateway)

Shaggy ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.shaggy/scripts/`. First create the script:

```bash
mkdir -p ~/.shaggy/scripts
cat > ~/.shaggy/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec shaggy teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.shaggy/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
shaggy cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/shaggy-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Shaggy Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=shaggy
EnvironmentFile=/etc/shaggy/env
ExecStart=/usr/local/bin/shaggy teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/shaggy-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Shaggy Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now shaggy-teams-pipeline-maintain.timer
systemctl list-timers shaggy-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/shaggy teams-pipeline maintain-subscriptions >> /var/log/shaggy/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.shaggy/.env` at the top of a wrapper script that crontab calls.
...
```bash
shaggy teams-pipeline subscriptions   # should show expirationDateTime advanced
shaggy teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
shaggy teams-pipeline list
shaggy teams-pipeline list --status failed
shaggy teams-pipeline show <job-id>
```

### Replay a stored job

```bash
shaggy teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
shaggy teams-pipeline fetch --meeting-id <meeting-id>
shaggy teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `shaggy teams-pipeline maintain-subscriptions --dry-run`
- inspect `shaggy teams-pipeline list --status failed`
...
- run `shaggy teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `shaggy teams-pipeline run`
...
- [ ] `shaggy teams-pipeline validate` returns an OK snapshot
- [ ] `shaggy teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Shaggy cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
