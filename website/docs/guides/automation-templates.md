...
Copy-paste recipes for common automation patterns. Each template uses Shaggy's built-in [cron scheduler](/docs/user-guide/features/cron) for time-based triggers and [webhook platform](/docs/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`shaggy webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `shaggy webhook subscribe`) |
...
```bash
shaggy cron create "0 2 * * *" \
  "You are a project manager triaging the shaggyaratia-69/shaggy-agent GitHub repo.

1. Run: gh issue list --repo shaggyaratia-69/shaggy-agent --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
shaggy webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
shaggy cron create "0 9 * * 1" \
  "Scan the shaggyaratia-69/shaggy-agent repo for documentation drift.

1. Run: gh pr list --repo shaggyaratia-69/shaggy-agent --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (shaggy_cli/commands.py, shaggy_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (shaggy_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
shaggy cron create "0 6 * * *" \
  "Run a dependency security audit on the shaggy-agent project.

1. cd ~/.shaggy/shaggy-agent && source .venv/bin/activate
...
```bash
shaggy webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
shaggy webhook subscribe alert-triage \
...
```python title="~/.shaggy/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Shaggy-Monitor/1.0"})
...
shaggy cron create "every 30m" \
  ...
  --script ~/.shaggy/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
shaggy cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Shaggy Agent development." \
...
```bash
shaggy webhook subscribe github-issues \
  --events "issues" \
...
```bash
shaggy webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
shaggy webhook subscribe stripe-payments \
...
```bash
shaggy cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the shaggy-agent codebase.
