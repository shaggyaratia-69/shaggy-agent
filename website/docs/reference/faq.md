...
description: "Frequently asked questions and solutions to common issues with Shaggy Agent"
---
...
### What LLM providers work with Shaggy?

Shaggy Agent works with any OpenAI-compatible API. Supported providers include:

- **[OpenRouter](https://openrouter.ai/)** — access hundreds of models through one API key (recommended for flexibility)
- **Nous Portal** — Cherries and Co's own inference endpoint
- **OpenAI** — GPT-5.4, GPT-5-codex, GPT-4.1, GPT-4o, etc.
- **Anthropic** — Claude models (direct API, OAuth via `shaggy login anthropic`, OpenRouter, or any compatible proxy)
...
Set your provider with `shaggy model` or by editing `~/.shaggy/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.

### Does it work on Windows?

**Not natively.** Shaggy Agent requires a Unix-like environment. On Windows, install [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) and run Shaggy from inside it. The standard install command works perfectly in WSL2:

```bash
curl -fsSL https://raw.githubusercontent.com/shaggyaratia-69/shaggy-agent/main/scripts/install.sh | bash
```

### I run Shaggy in WSL2. What's the best way to control my normal Windows Chrome?

Prefer an MCP bridge over `/browser connect`.

Recommended pattern:

- run Shaggy inside WSL2
...
- let Shaggy use the resulting MCP browser tools

This is more reliable than trying to force Shaggy core browser transport to attach directly across the WSL2/Windows boundary.

See:

- [Use MCP with Shaggy](../guides/use-mcp-with-shaggy.md#wsl2-bridge-shaggy-in-wsl-to-windows-chrome)
...
Yes — Shaggy now has a tested Termux install path for Android phones.
...
API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). Shaggy Agent does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.shaggy/`.

### Can I use it offline / with local models?

Yes. Run `shaggy model`, select **Custom endpoint**, and enter your server's URL:
...
Shaggy persists the endpoint, provider, and base URL in `config.yaml` so it survives restarts. If your local server has exactly one model loaded, `/model custom` auto-detects it. You can also set `provider: custom` in config.yaml — it's a first-class provider, not an alias for anything else.
...
:::tip Ollama users
If you set a custom `num_ctx` in Ollama (e.g., `ollama run --num_ctx 16384`), make sure to set the matching context length in Shaggy — Ollama's `/api/show` reports the model's *maximum* context, not the effective `num_ctx` you configured.
:::

:::tip Timeouts with local models
Shaggy auto-detects local endpoints and relaxes streaming timeouts (read timeout raised from 120s to 1800s, stale stream detection disabled). If you still hit timeouts on very large contexts, set `SHAGGY_STREAM_READ_TIMEOUT=1800` in your `.env`. See the [Local LLM guide](../guides/local-llm-on-mac.md#timeouts) for details.
:::

### How much does it cost?

Shaggy Agent itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same Shaggy Agent instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
Yes. Import the `AIAgent` class and use Shaggy programmatically:
...
#### `shaggy: command not found` after installation
...
**Cause:** Shaggy requires Python 3.11 or newer.
...
**Cause:** Shaggy builds a per-session environment snapshot by running `bash -l` once at startup. A bash login shell reads `/etc/profile`, `~/.bash_profile`, and `~/.profile`, but **does not source `~/.bashrc`** — so tools that install themselves there (`nvm`, `asdf`, `pyenv`, `cargo`, custom `PATH` exports) stay invisible to the snapshot. This most commonly happens when Shaggy runs under systemd or in a minimal shell where nothing has pre-loaded the interactive shell profile.

**Solution:** Shaggy auto-sources `~/.bashrc` by default. If that's not enough — e.g. you're a zsh user whose PATH lives in `~/.zshrc`, or you init `nvm` from a standalone file — list the extra files to source in `~/.shaggy/config.yaml`:
...
**Solution:** Exit your session and use `shaggy model` from your terminal to add new providers:

```bash
# Exit the Shaggy chat session first (Ctrl+C or /quit)
...
After adding a new provider via `shaggy model`, start a new chat session — `/model` will now show all your configured providers.

:::tip Quick reference
| Want to... | Use |
|-----------|-----|
| Add a new provider | `shaggy model` (from terminal) |
| Enter/change API keys | `shaggy model` (from terminal) |
...
# Or set directly
shaggy config set OPENROUTER_API_KEY sk-or-v1-xxxxxxxxxxxx
```

:::warning
Make sure the key matches the provider. An OpenAI key won't work with OpenRouter and vice versa. Check `~/.shaggy/.env` for conflicting entries.
:::
...
# Set a valid model
shaggy config set SHAGGY_MODEL anthropic/claude-opus-4.7

# Or specify per-session
shaggy chat --model openrouter/meta-llama/llama-3.1-70b-instruct
```
...
- Using `shaggy chat --provider <alternative>` to route to a different backend

#### Context length exceeded

**Cause:** The conversation has grown too long for the model's context window, or Shaggy detected the wrong context length for your model.
...
shaggy chat --model openrouter/google/gemini-3-flash-preview
```

If this happens on the first long conversation, Shaggy may have the wrong context length for your model. Check what it detected:
...
**Cause:** Shaggy detected a potentially destructive command (e.g., `rm -rf`, `DROP TABLE`). This is a safety feature.
...
:::tip
This is working as intended — Shaggy never silently runs destructive commands. The approval prompt shows you exactly what will execute.
:::
...
- Or switch to the terminal interface for administrative tasks: `shaggy chat`
...
cat ~/.shaggy/logs/gateway.log | tail -50
```
...
**Solution:**
- Verify your bot token is valid with `shaggy gateway setup`
- Check gateway logs: `cat ~/.shaggy/logs/gateway.log | tail -50`
...
Configure in `~/.shaggy/config.yaml` under your gateway's settings. See the [Messaging docs](../user-guide/messaging/index.md).
...
pip install "shaggy-agent[messaging]"  # Telegram, Discord, Slack, and shared gateway deps
...
#### WSL: Gateway keeps disconnecting or `shaggy gateway start` fails
...
tmux new -s shaggy 'shaggy gateway run'
# Reattach later: tmux attach -t shaggy

# Option 3: Background via nohup
nohup shaggy gateway run > ~/.shaggy/logs/gateway.log 2>&1 &
```
...
1. Create a task that runs `wsl -d Ubuntu -- bash -lc 'shaggy gateway run'`
...
**Solution:** The gateway captures your shell PATH when you run `shaggy gateway install`. If you installed tools after setting up the gateway, re-run the install to capture the updated PATH:

```bash
shaggy gateway install    # Re-snapshots your current PATH
shaggy gateway start      # Detects the updated plist and reloads
```

You can verify the plist has the correct PATH:
```bash
/usr/libexec/PlistBuddy -c "Print :EnvironmentVariables:PATH" \
  ~/Library/LaunchAgents/ai.shaggy.gateway.plist
```
...
**Solution:**
- Try a faster/smaller model: `shaggy chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `shaggy chat -t "terminal"`
...
cd ~/.shaggy/shaggy-agent && uv pip install -e ".[mcp]"
...
Verify your `~/.shaggy/config.yaml` MCP configuration:
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
shaggy config show | grep -A 12 mcp_servers

# Restart Shaggy or reload MCP after config changes
shaggy chat
```

See also:
- [MCP (Model Context Protocol)](/docs/user-guide/features/mcp)
- [Use MCP with Shaggy](/docs/guides/use-mcp-with-shaggy)
...
:::warning
If an MCP server crashes mid-request, Shaggy will report a timeout. Check the server's own logs (not just Shaggy logs) to diagnose the root cause.
:::
...
No. Each profile has its own memory store, session database, and skills directory. They are completely isolated. If you want to start a new profile with existing memories and sessions, use `shaggy profile create newname --clone-all` to copy everything from the current profile.

### What happens when I run `shaggy update`?

`shaggy update` pulls the latest code and reinstalls dependencies **once** (not per-profile). It then syncs updated skills to all profiles automatically. You only need to run `shaggy update` once — it covers every profile on the machine.


### How many profiles can I run?

There is no hard limit. Each profile is just a directory under `~/.shaggy/profiles/`. The practical limit depends on your disk space and how many concurrent gateways your system can handle (each gateway is a lightweight Python process). Running dozens of profiles is fine; each idle profile uses no resources.
...
**Solution: Delegation config.** Shaggy can route subagents to a different model automatically. Set this in `~/.shaggy/config.yaml`:
...
Now when you tell Shaggy "write me a Twitter thread about X" and it spawns a `delegate_task` subagent, that subagent runs on Gemini instead of your main model. Your primary conversation stays on GPT-5.4.
...
**Scenario:** In OpenClaw, you had multiple independent agents bound to specific WhatsApp chats — one for a family shopping list group, another for your private chat. Can Shaggy do this?

**Current limitation:** Shaggy profiles each require their own WhatsApp number/session. You cannot bind multiple profiles to different chats on the same WhatsApp number — the WhatsApp bridge (Baileys) uses one authenticated session per number.
...
**Scenario:** You see gateway exec logs, Shaggy reasoning, and tool call details in Telegram instead of just the final output.
...
**Scenario:** Telegram has a 100 slash command limit, and your skills are pushing past it. You want to disable skills you don't need on Telegram, but `shaggy skills config` settings don't seem to take effect.

**Solution:** Use `shaggy skills config` to disable skills per-platform. This writes to `config.yaml`:
...
After changing this, **restart the gateway** (`shaggy gateway restart` or kill and relaunch). The Telegram bot command menu rebuilds on startup.
...
**Current behavior:** Shaggy creates sessions keyed by user ID on most platforms, so each person gets their own conversation context. This is by design for privacy and context isolation.
...
### Exporting Shaggy to another machine
...
1. Install Shaggy Agent on the new machine:
   ```bash
   curl -fsSL https://raw.githubusercontent.com/shaggyaratia-69/shaggy-agent/main/scripts/install.sh | bash
   ```
...
2. On the **source machine**, create a full backup:
   ...
   shaggy backup
   ```
   This creates a zip of your entire `~/.shaggy/` directory — config, API keys, memories, skills, sessions, and profiles — saved to your home directory as `~/shaggy-backup-<timestamp>.zip`.

3. Copy the zip to the new machine and import it:
   ```bash
   # On the source machine
   scp ~/shaggy-backup-<timestamp>.zip newmachine:~/

   # On the new machine
   shaggy import ~/shaggy-backup-<timestamp>.zip
   ```

4. On the new machine, run `shaggy setup` to verify API keys and provider config are working.
...
```bash
# On the source machine
shaggy profile export work ./work-backup.tar.gz

# Copy the file to the target machine, then:
shaggy profile import ./work-backup.tar.gz work
```
...
### `shaggy backup` vs `shaggy profile export`

| Feature | `shaggy backup` | `shaggy profile export` |
| :--- | :--- | :--- |
| **Use Case** | **Full machine migration** | **Porting/sharing a specific profile** |
| **Scope** | Global (entire `~/.shaggy` directory) | Local (single profile directory) |
...
```bash
rsync -av --exclude='shaggy-agent' ~/.shaggy/ newmachine:~/.shaggy/
```

:::tip
`shaggy backup` produces a consistent snapshot even while Shaggy is actively running. The restored archive excludes machine-local runtime files like `gateway.pid` and `cron.pid`.
:::

### Permission denied when reloading shell after install

**Scenario:** After running the Shaggy installer, `source ~/.zshrc` gives a permission denied error.

**Cause:** This usually happens when `~/.zshrc` (or `~/.bashrc`) has incorrect file permissions, or when the installer couldn't write to it cleanly. It's not a Shaggy-specific issue — it's a shell config permissions problem.
...
shaggy config show | head -20

# Re-run model selection
shaggy model

# Or test with a known-good model
shaggy chat -q "hello" --model anthropic/claude-opus-4.7
```
...
1. **Search existing issues:** [GitHub Issues](https://github.com/shaggyaratia-69/shaggy-agent/issues)
2. **Ask the community:** [Cherries and Co Discord](https://discord.gg/cherriesandco)
3. **File a bug report:** Include your OS, Python version (`python3 --version`), Shaggy version (`shaggy --version`), and the full error message
