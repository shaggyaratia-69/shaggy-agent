...
This page covers all commands related to [Shaggy profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `shaggy profile` without a subcommand shows help.
...
```bash
shaggy profile use <name>
```

Sets `<name>` as the active profile. All subsequent `shaggy` commands (without `-p`) will use this profile.
...
shaggy profile use default
```

## `shaggy profile create`

```bash
shaggy profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `shaggy profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-skills` marker into the profile so future `shaggy update` runs won't re-seed the bundled set, and refuses to combine with `--clone` / `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. |
...
shaggy profile create mybot

# Clone config only from current profile
shaggy profile create work --clone

# Clone everything from current profile
shaggy profile create backup --clone-all

# Clone config from a specific profile
shaggy profile create work2 --clone --clone-from work
```

## `shaggy profile describe`

```bash
shaggy profile describe [<name>] [options]
```
...
shaggy profile describe researcher

# Set it explicitly
shaggy profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
shaggy profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
shaggy profile describe --all --auto
```

## `shaggy profile delete`

```bash
shaggy profile delete <name> [options]
```
...
```bash
shaggy profile delete mybot
shaggy profile delete mybot --yes
```
...
```bash
shaggy profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Shaggy home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ shaggy profile show work
Profile: work
Path:    ~/.shaggy/profiles/work
...
## `shaggy profile alias`

```bash
shaggy profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Shaggy installation.
...
```bash
shaggy profile alias work
# Creates/updates ~/.local/bin/work

shaggy profile alias work --name mywork
# Creates ~/.local/bin/mywork

shaggy profile alias work --remove
# Removes the wrapper script
```

## `shaggy profile rename`

```bash
shaggy profile rename <old-name> <new-name>
```
...
```bash
shaggy profile rename mybot assistant
# ~/.shaggy/profiles/mybot → ~/.shaggy/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `shaggy profile export`

```bash
shaggy profile export <name> [options]
```
...
```bash
shaggy profile export work
# Creates work.tar.gz in the current directory

shaggy profile export work -o ./work-2026-03-29.tar.gz
```

## `shaggy profile import`

```bash
shaggy profile import <archive> [options]
```
...
```bash
shaggy profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

shaggy profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`shaggy profile export` / `import` are still the right commands for
...
### `shaggy profile install`

```bash
shaggy profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `shaggy -p telemetry`). |
...
shaggy profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
shaggy profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
shaggy profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
shaggy profile install ./telemetry/
```

### `shaggy profile update`

```bash
shaggy profile update <name> [--force-config] [--yes]
```
...
```bash
shaggy profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Shaggy version, author, env var requirements, the source URL/path, and
...
`shaggy profile list` also shows the distribution name and version in a
`Distribution` column, and `shaggy profile show <name>` / `delete <name>`
...
shaggy profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
shaggy profile install https://github.com/your-org/internal-assistant.git
```
...
`shaggy_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Shaggy version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Shaggy can clone from.
3. Tell recipients to run `shaggy profile install <your-repo-url>`.
...
## `shaggy -p` / `shaggy --profile`

```bash
shaggy -p <name> <command> [options]
shaggy --profile <name> <command> [options]
```

Global flag to run any Shaggy command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
shaggy -p work chat -q "Check the server status"
shaggy --profile dev gateway start
shaggy -p personal skills list
shaggy -p work config edit
```

## `shaggy completion`

```bash
shaggy completion <shell>
```
...
- `shaggy profile <TAB>` — subcommands (list, use, create, etc.)
- `shaggy profile use <TAB>` — profile names
- `shaggy -p <TAB>` — profile names
