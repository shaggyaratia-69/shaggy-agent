...
sidebar_position: 1
title: "Architecture"
description: "Shaggy Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Shaggy Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ shaggy_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # ShaggyCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── shaggy_state.py           # SQLite session/state database with FTS5
├── shaggy_constants.py       # SHAGGY_HOME, profile-aware paths
...
│
├── shaggy_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `shaggy` subcommands (large file)
...
│   ├── skills_config.py      # shaggy skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # shaggy tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # shaggy gateway start/stop
│
...
```text
User input → ShaggyCLI.process_input()
...
- **`prompt_builder.py`** — Assembles the system prompt from: personality (SOUL.md), memory (MEMORY.md, USER.md), skills, context files (AGENTS.md, .shaggy.md), tool-use guidance, and model-specific instructions
...
Three discovery sources: `~/.shaggy/plugins/` (user), `.shaggy/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `shaggy plugins` or `config.yaml`.

→ [Plugin Guide](/docs/guides/build-a-shaggy-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Shaggy as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`shaggy -p <name>`) gets its own SHAGGY_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
