...
description: "How Shaggy stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `shaggy_cli/cron.py` | CLI `shaggy cron` subcommands |
...
Jobs are stored in `~/.shaggy/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `shaggy cron` commands are run or during active CLI sessions.
...
```python
# ~/.shaggy/scripts/check_competitors.py
...
2. **Environment variable** — `SHAGGY_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.shaggy/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `shaggy cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `shaggy cron` CLI provides direct job management:

```bash
shaggy cron list                    # Show all jobs
shaggy cron create                  # Interactive job creation (alias: add)
shaggy cron edit <job_id>           # Edit job configuration
shaggy cron pause <job_id>          # Pause a running job
shaggy cron resume <job_id>         # Resume a paused job
shaggy cron run <job_id>            # Trigger immediate execution
shaggy cron remove <job_id>         # Delete a job
```
