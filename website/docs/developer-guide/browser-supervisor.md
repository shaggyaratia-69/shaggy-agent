...
[PR #12550](https://github.com/shaggyaratia-69/shaggy-agent/pull/12550) proposed a
...
(`shaggy-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs
...
One `asyncio.Task` running in a background daemon thread per Shaggy `task_id`.
...
- `toolsets.py` — register `browser_dialog` in `browser`, `shaggy-acp`, `shaggy-api-server`, core toolsets (gated on CDP reachability)
...
- `shaggy_cli/config.py` — add `browser.dialog_policy` and `browser.dialog_timeout_s` to `DEFAULT_CONFIG`
