...
The ACP adapter wraps Shaggy' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
shaggy acp / shaggy-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.shaggy/.env
  -> configure stderr logging
  -> construct ShaggyACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'shaggy-agent[acp]==<version>' shaggy-acp`, pointed at the `shaggy-agent` PyPI release.
...
- `allow_once` -> Shaggy `once`
- `allow_always` -> Shaggy `always`
- reject options -> Shaggy `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Shaggy tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["shaggy-acp"])
...
Instead it reuses Shaggy' runtime resolver:

- `acp_adapter/auth.py`
- `shaggy_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Shaggy provider/credentials. It also always advertises a terminal setup auth method (`shaggy-setup`, args `--setup`) so first-run registry clients can open Shaggy' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.shaggy/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `shaggy-acp` toolset definition
- `shaggy_cli/main.py` — `shaggy acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `shaggy-acp` script
