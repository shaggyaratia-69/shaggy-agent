...
Shaggy Agent uses a SQLite database (`~/.shaggy/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `shaggy_state.py`


## Architecture Overview

```
~/.shaggy/state.db (SQLite, WAL mode)
...
Multiple shaggy processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.shaggy/state.db
...
Default path: `~/.shaggy/state.db`

This is derived from `shaggy_constants.get_shaggy_home()` which resolves to
`~/.shaggy/` by default, or the value of `SHAGGY_HOME` environment variable.
