...
The messaging gateway is the long-running process that connects Shaggy to 20+ external messaging platforms through a unified architecture.
...
1. `resolve_command()` from `shaggy_cli/commands.py` maps input to canonical name (handles aliases, prefix matching)
...
|--------|-----------------|
| `~/.shaggy/.env` | API keys, bot tokens, platform credentials |
| `~/.shaggy/config.yaml` | Model settings, tool configuration, display options |
...
- **Explicit target delivery** — `send_message` tool specifying `telegram:-1001234567890`, or the [`shaggy send` CLI](/docs/guides/pipe-script-output) wrapping the same tool for shell scripts
...
Hooks are discovered from `gateway/builtin_hooks/` (an extension point — currently empty in the shipped distribution; `_register_builtin_hooks()` is a no-op stub) and `~/.shaggy/hooks/` (user-installed). Each hook is a directory with a `HOOK.yaml` manifest and `handler.py`.
...
- `shaggy gateway start` / `shaggy gateway stop` — manual control
- `systemctl` (Linux) or `launchctl` (macOS) — service management
- PID file at `~/.shaggy/gateway.pid` — profile-scoped process tracking

**Profile-scoped vs global**: `start_gateway()` uses profile-scoped PID files. `shaggy gateway stop` stops only the current profile's gateway. `shaggy gateway stop --all` uses global `ps aux` scanning to kill all gateway processes (used during updates).
