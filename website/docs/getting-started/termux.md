...
description: "Run Shaggy Agent directly on an Android phone with Termux"
---

# Shaggy on Android with Termux

This is the tested path for running Shaggy Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Shaggy from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
- links `shaggy` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `shaggy` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/shaggy" "$PREFIX/bin/shaggy"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `shaggy` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.shaggy/.env`.
...
### `shaggy doctor` says ripgrep or Node is missing
