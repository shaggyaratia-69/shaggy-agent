...
If you only run one Shaggy agent, you don't need this page — see
[Profiles](./profiles.md) for the basics.

## When to use this

You want this setup when you have two or more Shaggy agents that should all
...
(`ai.shaggy.gateway-<name>.plist`) or systemd user service
(`shaggy-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
shaggy profile create coder
shaggy profile create personal-bot
shaggy profile create research
...
`~/.local/bin/shaggy-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: shaggy-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    shaggy gateway "$action"
  else
    shaggy -p "$profile" gateway "$action"
  fi
}
...
```bash
shaggy-gateways start      # start every configured profile
shaggy-gateways stop       # stop every configured profile
shaggy-gateways restart    # restart all
shaggy-gateways status     # status across all
shaggy-gateways list       # delegates to `shaggy gateway list`
```

:::tip
The `default` profile is targeted with `shaggy gateway <action>` (no `-p`),
not `shaggy -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `shaggy -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.shaggy.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/shaggy-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.shaggy.gateway.plist` /
`shaggy-gateway.service`.
...
```bash
# Default profile
tail -f ~/.shaggy/logs/gateway.log
tail -f ~/.shaggy/logs/gateway.error.log

# Named profile
tail -f ~/.shaggy/profiles/<name>/logs/gateway.log
tail -f ~/.shaggy/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.shaggy/logs/gateway.log ~/.shaggy/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
shaggy logs -f                  # follow default profile
shaggy -p coder logs -f         # follow one profile
shaggy logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
shaggy profile list             # profiles + model + gateway state
shaggy-gateways status          # full status across every profile
launchctl list | grep shaggy    # macOS — PIDs and labels
systemctl --user list-units 'shaggy-gateway-*'   # Linux — units
```
...
```
~/.shaggy/profiles/<name>/
├── .env              # API keys, bot tokens (chmod 600)
├── config.yaml       # model, provider, toolsets, gateway settings
└── SOUL.md           # personality / system prompt
```

The default profile uses `~/.shaggy/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
shaggy config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.shaggy/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=shaggy --why="gateways running" \
  sleep infinity &
...
`shaggy-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.shaggy/.env ~/.shaggy/profiles/*/.env
```

## Updating the code

`shaggy update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `shaggy gateway start` after a previous `shaggy gateway stop`. The
...
```bash
ps -ef | grep "shaggy_cli.*-p <profile>"
cat ~/.shaggy/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.shaggy.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.shaggy.gateway-<profile>.plist

# Linux
systemctl --user restart shaggy-gateway-<profile>.service
```

### Health check

```bash
shaggy doctor                  # default profile
shaggy -p <profile> doctor     # one profile
```
