...
Connect Shaggy to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Shaggy — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Shaggy runs it in a
...
shaggy photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Shaggy Agent` project on your account.
...
Runtime credentials are written to `~/.shaggy/.env`
...
(device token, dashboard project id) lives in `~/.shaggy/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Shaggy
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Shaggy replies with a pairing code. Approve it with:

```bash
shaggy pairing approve photon <CODE>
```

Use `shaggy pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.shaggy/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.shaggy/.env`):
...
By default Shaggy responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Shaggy` and
`@Shaggy agent` variants. For a custom agent name, set regex patterns:
...
```bash
shaggy gateway start --platform photon
```
...
Send an iMessage to your assigned number and Shaggy will reply.

## Status & troubleshooting

```bash
shaggy photon status
```

Prints saved credentials, sidecar health, your registered number, and the
assigned iMessage line Shaggy uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run shaggy photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `shaggy photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `shaggy photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `shaggy photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Shaggy sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | Shaggy wake words  | JSON list / comma / newline regex patterns for group mentions |
