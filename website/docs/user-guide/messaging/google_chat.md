...
sidebar_position: 12
title: "Google Chat"
description: "Set up Shaggy Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Shaggy Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Shaggy
...
download the file. Save it somewhere only Shaggy can read (e.g.,
`~/.shaggy/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a shaggy restart)
...
- Principal: `shaggy-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Shaggy calls
...
- **App name**: whatever you want users to see ("Shaggy" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/shaggy-chat-events`.
...
`ADDED_TO_SPACE` event that Shaggy uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Shaggy

Add the Google Chat section to `~/.shaggy/.env`:
...
Install the dependencies the Google Chat adapter needs (no Shaggy extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "Shaggy is thinking…" marker, then
...
Thread support: when a user replies inside a thread, Shaggy detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Shaggy session.
...
3. Download the JSON. Move it onto the host that runs Shaggy.
4. On the host, register the client with Shaggy:
...
That writes `~/.shaggy/google_chat_user_client_secret.json`. This is shared
...
The token lands at `~/.shaggy/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.shaggy/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Shaggy isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `shaggy gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Shaggy will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.shaggy/google_chat_user_tokens/<sanitized_email>.json` (filesystem
