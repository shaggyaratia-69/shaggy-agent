...
description: "Chat with Shaggy from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Shaggy from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/docs/user-guide/features/voice-mode) and [Use Voice Mode with Shaggy](/docs/guides/use-voice-mode-with-shaggy).
...
```mermaid
flowchart TB
    subgraph Gateway["Shaggy Gateway"]
...
```bash
shaggy gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
shaggy gateway              # Run in foreground
shaggy gateway setup        # Configure messaging platforms interactively
shaggy gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo shaggy gateway install --system   # Linux only: install a boot-time system service
shaggy gateway start        # Start the default service
shaggy gateway stop         # Stop the default service
shaggy gateway status       # Check default service status
shaggy gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update Shaggy Agent to the latest version |
...
Configure per-platform overrides in `~/.shaggy/gateway.json`:
...
shaggy pairing approve telegram XKGH5N7P

# Other pairing commands:
shaggy pairing list          # View pending + approved users
shaggy pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Shaggy appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.shaggy/config.yaml`:
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.shaggy/config.yaml`:
...
```bash
SHAGGY_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
shaggy gateway install               # Install as user service
shaggy gateway start                 # Start the service
shaggy gateway stop                  # Stop the service
shaggy gateway status                # Check status
journalctl --user -u shaggy-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo shaggy gateway install --system
sudo shaggy gateway start --system
sudo shaggy gateway status --system
journalctl -u shaggy-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Shaggy will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Shaggy installations on the same machine (with different `SHAGGY_HOME` directories), each gets its own systemd service name. The default `~/.shaggy` uses `shaggy-gateway`; other installations use `shaggy-gateway-<hash>`. The `shaggy gateway` commands automatically target the correct service for your current `SHAGGY_HOME`.
:::

### macOS (launchd)

```bash
shaggy gateway install               # Install as launchd agent
shaggy gateway start                 # Start the service
shaggy gateway stop                  # Stop the service
shaggy gateway status                # Check status
tail -f ~/.shaggy/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.shaggy.gateway.plist`. It includes three environment variables:
...
- **SHAGGY_HOME** — scopes the gateway to your Shaggy installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `shaggy gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `SHAGGY_HOME` directory gets its own launchd label. The default `~/.shaggy` uses `ai.shaggy.gateway`; other installations use `ai.shaggy.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `shaggy-cli` | Full access |
| Telegram | `shaggy-telegram` | Full tools including terminal |
| Discord | `shaggy-discord` | Full tools including terminal |
| WhatsApp | `shaggy-whatsapp` | Full tools including terminal |
| Slack | `shaggy-slack` | Full tools including terminal |
| Google Chat | `shaggy-google_chat` | Full tools including terminal |
| Signal | `shaggy-signal` | Full tools including terminal |
| SMS | `shaggy-sms` | Full tools including terminal |
| Email | `shaggy-email` | Full tools including terminal |
| Home Assistant | `shaggy-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `shaggy-mattermost` | Full tools including terminal |
| Matrix | `shaggy-matrix` | Full tools including terminal |
| DingTalk | `shaggy-dingtalk` | Full tools including terminal |
| Feishu/Lark | `shaggy-feishu` | Full tools including terminal |
| WeCom | `shaggy-wecom` | Full tools including terminal |
| WeCom Callback | `shaggy-wecom-callback` | Full tools including terminal |
| Weixin | `shaggy-weixin` | Full tools including terminal |
| BlueBubbles | `shaggy-bluebubbles` | Full tools including terminal |
| QQBot | `shaggy-qqbot` | Full tools including terminal |
| Yuanbao | `shaggy-yuanbao` | Full tools including terminal |
| Microsoft Teams | `shaggy-teams` | Full tools including terminal |
| API Server | `shaggy-api-server` | Full tools (drops `clarify`, `send_message`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `shaggy-webhook` | Full tools including terminal |
...
1. **Gateway log** (`~/.shaggy/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
