...
Connect Shaggy to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Shaggy supports two WeCom integration modes:
...
3. Shaggy decrypts the message, queues it for the agent
...
(Use `shaggy gateway start` only after `shaggy gateway install` has registered the systemd/launchd service.)
