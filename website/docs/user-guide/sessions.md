...
Shaggy Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.shaggy/state.db`) — structured session metadata with FTS5 full-text search
2. **JSONL transcripts** (`~/.shaggy/sessions/`) — raw conversation transcripts including tool calls (gateway)
...
Shaggy stores session history so it can resume conversations, but it does not
...
Shaggy explicitly injects for that turn.
...
For example, if a user sends an image and asks Shaggy to make a meme from it,
Shaggy may inspect that image once with vision and run an image-processing
...
`shaggy sessions prune` only when you want to delete old ended sessions from
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`shaggy` or `shaggy chat`) |
...
shaggy -c "my project"   # → resumes "my project #3"
```
...
shaggy --resume 20250305_091523_a1b2c3d4
shaggy -r 20250305_091523_a1b2c3d4

# Resume by title
shaggy --resume "refactoring auth"

# Or with the chat subcommand
shaggy chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `shaggy sessions list`.

### Conversation Recap on Resume

When you resume a session, Shaggy displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src="/img/docs/session-recap.svg" alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Shaggy session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.shaggy/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `shaggy -r "<title>"` from the shell) and pick up where the platform left off.
...
Shaggy automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `shaggy sessions list` or `shaggy sessions browse`.
...
```bash
shaggy sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Shaggy creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`shaggy -c "my project"`), it automatically picks the most recent session in the lineage.
...
Shaggy provides a full set of session management commands via `shaggy sessions`:
...
# Filter by platform
shaggy sessions list --source telegram

# Show more sessions
shaggy sessions list --limit 50
```
...
shaggy sessions export backup.jsonl

# Export sessions from a specific platform
shaggy sessions export telegram-history.jsonl --source telegram

# Export a single session
shaggy sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
shaggy sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
shaggy sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
shaggy sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
shaggy sessions prune

# Custom age threshold
shaggy sessions prune --older-than 30

# Only prune sessions from a specific platform
shaggy sessions prune --source telegram --older-than 60

# Skip confirmation
shaggy sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`shaggy insights`](/docs/reference/cli-commands#shaggy-insights).
...
When Shaggy cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Shaggy uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Shaggy in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.shaggy/state.db` | All session metadata + messages with FTS5 |
| Gateway transcripts | `~/.shaggy/sessions/` | JSONL transcripts per session + sessions.json index |
| Gateway index | `~/.shaggy/sessions/sessions.json` | Maps session keys to active session IDs |
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Shaggy process in the same `SHAGGY_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.shaggy/config.yaml`:
...
shaggy sessions delete <session_id>
