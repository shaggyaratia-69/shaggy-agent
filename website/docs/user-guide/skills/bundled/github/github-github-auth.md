...
- Give it a name like "shaggy-agent"
- Select scopes:
...
- Give it a title like "shaggy-agent-&lt;machine-name>"
...
elif [ -f ~/.shaggy/.env ] && grep -q "^GITHUB_TOKEN=" ~/.shaggy/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.shaggy/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
