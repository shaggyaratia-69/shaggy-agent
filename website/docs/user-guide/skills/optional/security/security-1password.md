...
| | |
|---|---|
| Source | Optional — install with `shaggy skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Shaggy Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Shaggy terminal calls (desktop app flow only)
...
### Service Account (recommended for Shaggy)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.shaggy/.env` (the skill will prompt for this on first load).
...
## Shaggy Execution Pattern (desktop app flow)

Shaggy terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/shaggy-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/shaggy-op.sock"
