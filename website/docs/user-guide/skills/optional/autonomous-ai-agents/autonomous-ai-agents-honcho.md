...
description: "Configure and use Honcho memory with Shaggy -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Shaggy -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `shaggy skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Shaggy

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Shaggy profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/shaggy#running-honcho-locally-with-shaggy

### Verify

```bash
shaggy honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Shaggy profile's AI peer
...
Honcho models conversations as interactions between **peers**. Shaggy creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Shaggy instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Shaggy run |
| `global` | Single session across all directories |

Manual override: `shaggy honcho map my-project-name`
...
Each Shaggy profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block shaggy.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `shaggy.coder` host block in `honcho.json`
...
```bash
shaggy honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Shaggy when Honcho memory is active.
...
Config file: `$SHAGGY_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `shaggy honcho setup`. Ensure `memory.provider: honcho` is in `~/.shaggy/config.yaml`.

### Memory not persisting across sessions
Check `shaggy honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `shaggy profile create <name> --clone`. For existing profiles: `shaggy honcho sync`.
...
| Command | Description |
|---------|-------------|
| `shaggy honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `shaggy honcho status` | Show resolved config, connection test, peer info for active profile |
| `shaggy honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `shaggy honcho disable` | Disable Honcho for the active profile |
| `shaggy honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `shaggy honcho peers` | Show peer identities across all profiles |
| `shaggy honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `shaggy honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `shaggy honcho sessions` | List known directory-to-session-name mappings |
| `shaggy honcho map <name>` | Map current working directory to a Honcho session name |
| `shaggy honcho identity` | Seed AI peer identity or show both peer representations |
| `shaggy honcho sync` | Create host blocks for all Shaggy profiles that don't have one yet |
| `shaggy honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Shaggy + Honcho |
| `shaggy memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `shaggy memory status` | Show active memory provider and config |
| `shaggy memory off` | Disable external memory provider |
