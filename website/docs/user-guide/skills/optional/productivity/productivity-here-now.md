...
| | |
|---|---|
| Source | Optional — install with `shaggy skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${SHAGGY_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${SHAGGY_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${SHAGGY_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client shaggy
```
...
```bash
PUBLISH="${SHAGGY_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client shaggy
```
...
```bash
DRIVE="${SHAGGY_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `shaggy`)    |
