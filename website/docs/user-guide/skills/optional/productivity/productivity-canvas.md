...
| | |
|---|---|
| Source | Optional — install with `shaggy skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Shaggy Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `~/.shaggy/.env`:
...
```bash
CANVAS="python $SHAGGY_HOME/skills/productivity/canvas/scripts/canvas_api.py"
