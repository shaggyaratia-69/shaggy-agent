...
| | |
|---|---|
| Source | Optional — install with `shaggy skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Shaggy or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Shaggy. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.shaggy/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.shaggy/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Shaggy integration, either:

- configure the server in `~/.shaggy/config.yaml` using the `native-mcp` skill, or
...
### Shaggy cannot see the deployed server

The server-building part may be correct while the Shaggy config is not. Load the `native-mcp` skill and configure the server in `~/.shaggy/config.yaml`, then restart Shaggy.
