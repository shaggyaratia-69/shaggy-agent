...
| | |
|---|---|
| Source | Optional — install with `shaggy skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Shaggy Agent |
| License | MIT |
...
import upstream classes into Shaggy itself.
...
```bash
mkdir -p ~/.shaggy/cache/darwinian-evolver && cd ~/.shaggy/cache/darwinian-evolver
...
```bash
cd ~/.shaggy/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.shaggy/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.shaggy/skills/research/darwinian-evolver
DE_DIR=~/.shaggy/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Shaggy core.
   Custom driver scripts under `~/.shaggy/skills/...` are user-side and fine.
