...
title: "Git Worktrees"
description: "Run multiple Shaggy agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Shaggy Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Shaggy so each session has a clean, isolated working directory.

## Why Use Worktrees with Shaggy?

Shaggy treats the **current working directory** as the project root:

- CLI: the directory where you run `shaggy` or `shaggy chat`
...
git worktree add ../repo-feature feature/shaggy-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/shaggy-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Shaggy there:

```bash
cd ../repo-feature

# Start Shaggy in the worktree
shaggy
```
...
git worktree add ../repo-experiment-a feature/shaggy-a
git worktree add ../repo-experiment-b feature/shaggy-b
```
...
- Works on its own branch (`feature/shaggy-a` vs `feature/shaggy-b`).
...
- Shaggy checkpoint data under `~/.shaggy/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Shaggy experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/shaggy-checkpoints-docs`, `feature/shaggy-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Shaggy from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `shaggy -w` (Automatic Worktree Mode)

Shaggy has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `shaggy/shaggy-<hash>`).
...
```bash
shaggy -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `shaggy -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Shaggy session its own clean checkout.
