...
Shaggy runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Shaggy
...
1. Shaggy captures a baseline of current diagnostics for the file.
...
…). Shaggy auto-detects the binary on PATH or in
`<SHAGGY_HOME>/lsp/bin/`.
...
`node_modules` tree — Shaggy installs both packages together when you
run `shaggy lsp install typescript` or auto-install fires on first
use.

## CLI

```
shaggy lsp status          # service state + per-server install status
shaggy lsp list            # registry, optionally --installed-only
shaggy lsp install <id>    # eagerly install one server
shaggy lsp install-all     # try every server with a known recipe
shaggy lsp restart         # tear down running clients
shaggy lsp which <id>      # print resolved binary path
```

`shaggy lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <SHAGGY_HOME>/lsp/bin
...
When `install_strategy: auto`, Shaggy installs binaries into
`<SHAGGY_HOME>/lsp/bin/`. NPM packages land in
`<SHAGGY_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Shaggy-owned and is
...
Servers are kept alive for the life of the Shaggy process. There's
...
**`shaggy lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<SHAGGY_HOME>/lsp/bin/`. Run
`shaggy lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `shaggy lsp status`**
...
When `shaggy lsp status` shows a `Backend warnings` section, install
...
`~/.shaggy/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.shaggy/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `shaggy lsp restart` to clear the set;
