...
description: "Customize Shaggy Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Shaggy Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `SHAGGY_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Shaggy is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Shaggy now seeds a default `SOUL.md` automatically in:

```text
~/.shaggy/SOUL.md
```

More precisely, it uses the current instance's `SHAGGY_HOME`, so if you run Shaggy with a custom home directory, it will use:
...
- Shaggy creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Shaggy loads `SOUL.md` only from `SHAGGY_HOME`
- Shaggy does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Shaggy falls back to a built-in default identity
...
If Shaggy loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `SHAGGY_HOME`, the personality belongs to the Shaggy instance itself.

That also makes it easier to teach users:
- "Edit `~/.shaggy/SOUL.md` to change Shaggy' default personality."
...
- how Shaggy should handle uncertainty, disagreement, or ambiguity
...
## What Shaggy injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Shaggy falls back to a built-in default identity ("You are Shaggy Agent, an intelligent AI assistant created by Cherries and Co..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Shaggy ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Shaggy, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Shaggy its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.shaggy/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.shaggy/SOUL.md`
...
- [SOUL.md Guide](/docs/guides/use-soul-with-shaggy)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Shaggy speaks
- `display.skin` and `/skin` affect how Shaggy looks in the terminal
