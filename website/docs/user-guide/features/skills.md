...
All skills live in **`~/.shaggy/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.

You can also point Shaggy at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling Shaggy to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.shaggy/plans/` relative to the active workspace/backend working directory.

You can also interact with skills through natural conversation:

```bash
shaggy chat --toolsets skills -q "What skills do you have?"
shaggy chat --toolsets skills -q "Show me the axolotl skill"
```
...
/home/user/.shaggy/cache/chart-q4-2025.png
...
When a missing value is encountered, Shaggy asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `shaggy setup` or `~/.shaggy/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `shaggy config migrate` prompts for unconfigured settings, and `shaggy config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
```text
~/.shaggy/skills/                  # Single source of truth
...
If you maintain skills outside of Shaggy — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell Shaggy to scan those directories too.

Add `external_dirs` under the `skills` section in `~/.shaggy/config.yaml`:
...
- **Read-only**: External dirs are only scanned for skill discovery. When the agent creates or edits a skill, it always writes to `~/.shaggy/skills/`.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, Shaggy ignores it without errors. Useful for optional shared directories that may not be present on every machine.

### Example

```text
~/.shaggy/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
Bundles live in **`~/.shaggy/skill-bundles/<slug>.yaml`** and look like this:
...
- `description` (optional) — short text shown in `/bundles` and `shaggy bundles list`.
...
shaggy bundles create research

# Overwrite an existing bundle
shaggy bundles create backend-dev --skill ... --force

# Delete a bundle
shaggy bundles delete backend-dev

# Re-scan ~/.shaggy/skill-bundles/ and report changes
shaggy bundles reload
```
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.shaggy/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.shaggy/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
```bash
shaggy skills browse                              # Browse all hub skills (official first)
shaggy skills browse --source official            # Browse only official optional skills
shaggy skills search kubernetes                   # Search all sources
shaggy skills search react --source skills-sh     # Search the skills.sh directory
shaggy skills search https://mintlify.com/docs --source well-known
shaggy skills inspect openai/skills/k8s           # Preview before installing
shaggy skills install openai/skills/k8s           # Install with security scan
shaggy skills install official/security/1password
shaggy skills install skills-sh/vercel-labs/json-render/json-render-react --force
shaggy skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
shaggy skills list --source hub                   # List hub-installed skills
shaggy skills check                               # Check installed hub skills for upstream updates
shaggy skills update                              # Reinstall hub skills with upstream changes when needed
shaggy skills audit                               # Re-scan all hub skills for security
shaggy skills uninstall k8s                       # Remove a hub skill
shaggy skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
shaggy skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
shaggy skills publish skills/my-skill --to github --repo owner/repo
shaggy skills snapshot export setup.json          # Export skill config
shaggy skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with Shaggy. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `shaggy skills search <query> --source skills-sh`. Shaggy resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
Shaggy currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the Shaggy repository itself and install with builtin trust.
...
This is Vercel's public skills directory. Shaggy can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
shaggy skills search https://mintlify.com/docs --source well-known
shaggy skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
shaggy skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

Shaggy can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
```bash
shaggy skills install openai/skills/k8s
shaggy skills tap add myorg/skills-repo
```
...
- Shaggy source id: `clawhub`

#### 6. Claude marketplace-style repos (`claude-marketplace`)

Shaggy supports marketplace repos that publish Claude-compatible plugin/marketplace manifests.

Known integrated sources include:
- [anthropics/skills](https://github.com/anthropics/skills)
- [aiskillstore/marketplace](https://github.com/aiskillstore/marketplace)

Shaggy source id: `claude-marketplace`

#### 7. LobeHub (`lobehub`)

Shaggy can search and convert agent entries from LobeHub's public catalog into installable Shaggy skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- Shaggy source id: `lobehub`

#### 8. browse.sh (`browse-sh`)

Shaggy integrates with [browse.sh](https://browse.sh), Browserbase's catalog of 200+ site-specific browser-automation SKILL.md files (Airbnb, Amazon, arXiv, 12306.cn, Etsy, Xero, and many more). Each skill describes how to drive one website end-to-end and is suitable for use with Shaggy' browser tools and any browser-automation skills you already have installed.

- Site: [browse.sh](https://browse.sh/)
- Catalog API: `https://browse.sh/api/skills`
- Shaggy source id: `browse-sh`
- Trust level: `community`

```bash
shaggy skills search airbnb --source browse-sh
shaggy skills inspect browse-sh/airbnb.com/search-listings-ddgioa
shaggy skills install browse-sh/airbnb.com/search-listings-ddgioa
```
...
Install a single-file `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). Shaggy fetches the URL, parses the YAML frontmatter, security-scans it, and installs.

- Shaggy source id: `url`
...
```bash
shaggy skills install https://sharethis.chat/SKILL.md
shaggy skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
shaggy skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `shaggy skills update` re-fetches from the same URL automatically when you want to refresh.
...
`shaggy skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with Shaggy | Always trusted |
...
```bash
shaggy skills check          # Report which installed hub skills changed upstream
shaggy skills update         # Reinstall only the skills with updates available
shaggy skills update react   # Update one specific installed hub skill
```
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other Shaggy users add with `shaggy skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
- Each skill directory must contain a `SKILL.md` with standard [SKILL.md frontmatter](#skillmd-format) (`name`, `description`, plus optional `metadata.shaggy.tags`, `version`, `author`, `platforms`, `metadata.shaggy.config`).
...
Shaggy discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.

#### Minimal tap example

```
my-org/shaggy-skills
└── skills/
    └── deploy-runbook/
        └── SKILL.md
```
...
After pushing that to GitHub, any Shaggy user can subscribe and install:

```bash
shaggy skills tap add my-org/shaggy-skills
shaggy skills search deploy
shaggy skills install my-org/shaggy-skills/deploy-runbook
```

#### Non-default paths

If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.shaggy/.hub/taps.json`:
...
The `shaggy skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `shaggy skills tap list` shows the effective path per tap.
...
```bash
shaggy skills install owner/repo/skills/my-workflow
```
...
Taps are stored in `~/.shaggy/.hub/taps.json` (created on demand).

## Bundled skill updates (`shaggy skills reset`)

Shaggy ships with a set of bundled skills in `skills/` inside the repo. On install and on every `shaggy update`, a sync pass copies those into `~/.shaggy/skills/` and records a manifest at `~/.shaggy/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, Shaggy recomputes the hash of your local copy and compares it to the origin hash:
...
shaggy skills reset google-workspace --restore
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `SHAGGY_HOME`, so `shaggy -p coder skills reset <name>` only affects that profile.
:::
