...
chat completions — use your Shaggy-managed provider subscription as their
...
| Use case | "Use Shaggy as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Shaggy stores
the refresh token in `~/.shaggy/auth.json` — the same place all Shaggy
...
```
Starting Shaggy proxy for Nous Portal
...
Model:      Shaggy-4-70B    # or Shaggy-4.3-36B, Shaggy-4-405B
```
...
If you see `not logged in`, run `shaggy login nous`. If you see
...
supports; check `portal.cherriesandco.com/models`.
...
[portal.cherriesandco.com](https://portal.cherriesandco.com).
...
`shaggy_cli/proxy/adapters/<provider>.py` and registering it in
