...
title: "Plugins"
description: "Extend Shaggy with custom tools, hooks, and integrations via the plugin system"
---

# Plugins

Shaggy has a plugin system for adding custom tools, hooks, and integrations without modifying core code.
...
[Adding Tools](/docs/developer-guide/adding-tools) page is for built-in Shaggy
core tools that live in `tools/` and `toolsets.py`.

**→ [Build a Shaggy Plugin](/docs/guides/build-a-shaggy-plugin)** — step-by-step guide with a complete working example.

## Quick overview

Drop a directory into `~/.shaggy/plugins/` with a `plugin.yaml` and Python code:

```
~/.shaggy/plugins/my-plugin/
...
Start Shaggy — your tools appear alongside built-in tools. The model can call them immediately.

### Minimal working example

Here is a complete plugin that adds a `hello_world` tool and logs every tool call via a hook.

**`~/.shaggy/plugins/hello-world/plugin.yaml`**
...
**`~/.shaggy/plugins/hello-world/__init__.py`**

```python
"""Minimal Shaggy plugin — registers a tool and a hook."""
...
Drop both files into `~/.shaggy/plugins/hello-world/`, restart Shaggy, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

Project-local plugins under `./.shaggy/plugins/` are disabled by default. Enable them only for trusted repositories by setting `SHAGGY_ENABLE_PROJECT_PLUGINS=true` before starting Shaggy.
...
| Add CLI commands | `ctx.register_cli_command(name, help, setup_fn, handler_fn)` — adds `shaggy <plugin> <subcommand>` |
| Inject messages | `ctx.inject_message(content, role="user")` — see [Injecting Messages](#injecting-messages) |
| Ship data files | `Path(__file__).parent / "data" / "file.yaml"` |
| Bundle skills | `ctx.register_skill(name, path)` — namespaced as `plugin:skill`, loaded via `skill_view("plugin:skill")` |
| Gate on env vars | `requires_env: [API_KEY]` in plugin.yaml — prompted during `shaggy plugins install` |
| Distribute via pip | `[project.entry-points."shaggy_agent.plugins"]` |
...
|--------|------|----------|
| Bundled | `<repo>/plugins/` | Ships with Shaggy — see [Built-in Plugins](/docs/user-guide/features/built-in-plugins) |
| User | `~/.shaggy/plugins/` | Personal plugins |
| Project | `.shaggy/plugins/` | Project-specific plugins (requires `SHAGGY_ENABLE_PROJECT_PLUGINS=true`) |
| pip | `shaggy_agent.plugins` entry_points | Distributed packages |
| Nix | `services.shaggy-agent.extraPlugins` / `extraPythonPackages` | NixOS declarative installs — see [Nix Setup](/docs/getting-started/nix-setup#plugins) |

Later sources override earlier ones on name collision, so a user plugin with the same name as a bundled plugin replaces it.

### Plugin sub-categories

Within each source, Shaggy also recognizes sub-category directories that route plugins to specialized discovery systems:
...
User plugins at `~/.shaggy/plugins/model-providers/<name>/` and `~/.shaggy/plugins/memory/<name>/` override bundled plugins of the same name — last-writer-wins in `register_provider()` / `register_memory_provider()`. Drop a directory in, and it replaces the built-in without any repo edits.

Sub-category plugins surface in `shaggy plugins list` and the interactive `shaggy plugins` UI under their **path-derived key** — e.g. `observability/langfuse`, `image_gen/openai`, `platforms/teams`. That key (not the bare manifest `name:`) is the value you pass to `shaggy plugins enable …` / `disable …` and the string to add under `plugins.enabled` in `config.yaml`.

## Plugins are opt-in (with a few exceptions)

**General plugins and user-installed backends are disabled by default** — discovery finds them (so they show up in `shaggy plugins` and `/plugins`), but nothing with hooks or tools loads until you add the plugin's name to `plugins.enabled` in `~/.shaggy/config.yaml`. This stops third-party code from running without your explicit consent.
...
```bash
shaggy plugins                    # interactive toggle (space to check/uncheck)
shaggy plugins enable <name>      # add to allow-list
shaggy plugins disable <name>     # remove from allow-list + add to disabled
```

After `shaggy plugins install owner/repo`, you're asked `Enable 'name' now? [y/N]` — defaults to no. Skip the prompt for scripted installs with `--enable` or `--no-enable`.

### What the allow-list does NOT gate

Several categories of plugin bypass `plugins.enabled` — they're part of Shaggy' built-in surface and would break basic functionality if gated off by default:
...
| **User-installed platforms** (under `~/.shaggy/plugins/platforms/`) | Opt-in via `plugins.enabled` — third-party gateway adapters need explicit consent. |

In short: **bundled "always-works" infrastructure loads automatically; third-party general plugins are opt-in.** The `plugins.enabled` allow-list is the gate specifically for arbitrary code a user drops into `~/.shaggy/plugins/`.

### Migration for existing users

When you upgrade to a version of Shaggy that has opt-in plugins (config schema v21+), any user plugins already installed under `~/.shaggy/plugins/` that weren't already in `plugins.disabled` are **automatically grandfathered** into `plugins.enabled`. Your existing setup keeps working. Bundled standalone plugins are NOT grandfathered — even existing users have to opt in explicitly. (Bundled platform/backend plugins never needed grandfathering because they were never gated.)
...
Shaggy has four kinds of plugins:

| Type | What it does | Selection | Location |
|------|-------------|-----------|----------|
| **General plugins** | Add tools, hooks, slash commands, CLI commands | Multi-select (enable/disable) | `~/.shaggy/plugins/` |
...
The table above shows the four plugin categories, but within "General plugins" the `PluginContext` exposes several distinct extension points — and Shaggy also accepts extensions outside the Python plugin system (config-driven backends, shell-hooked commands, external servers, etc.). Use this table to find the right doc for what you want to build:

| Want to add… | How | Authoring guide |
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a Shaggy Plugin](/docs/guides/build-a-shaggy-plugin) · [Adding Tools](/docs/developer-guide/adding-tools) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](/docs/user-guide/features/hooks) · [Build a Shaggy Plugin](/docs/guides/build-a-shaggy-plugin) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a Shaggy Plugin](/docs/guides/build-a-shaggy-plugin) · [Extending the CLI](/docs/developer-guide/extending-the-cli) |
| A **subcommand** for `shaggy <thing>` | Python plugin — `ctx.register_cli_command()` | [Extending the CLI](/docs/developer-guide/extending-the-cli) |
...
| An **STT backend** (custom whisper binary, local ASR CLI) | Config-driven — set `SHAGGY_LOCAL_STT_COMMAND` env var to a shell template | [Voice Message Transcription (STT)](/docs/user-guide/features/tts#voice-message-transcription-stt) |
| **External tools via MCP** (filesystem, GitHub, Linear, Notion, any MCP server) | Config-driven — declare `mcp_servers.<name>` with `command:` / `url:` in `config.yaml`. Shaggy auto-discovers the server's tools and registers them alongside built-ins. | [MCP](/docs/user-guide/features/mcp) |
| **Additional skill sources** (custom GitHub repos, private skill indexes) | CLI — `shaggy skills tap add <repo>` | [Skills Hub](/docs/user-guide/features/skills#skills-hub) · [Publishing a custom tap](/docs/user-guide/features/skills#publishing-a-custom-skill-tap) |
| **Gateway event hooks** (fire on `gateway:startup`, `session:start`, `agent:end`, `command:*`) | Drop `HOOK.yaml` + `handler.py` into `~/.shaggy/hooks/<name>/` | [Event Hooks](/docs/user-guide/features/hooks#gateway-event-hooks) |
...
On NixOS, plugins can be installed declaratively via the module options — no `shaggy plugins install` needed. See the **[Nix Setup guide](/docs/getting-started/nix-setup#plugins)** for full details.
...
```bash
shaggy plugins                                       # unified interactive UI
shaggy plugins list                                  # table: enabled / disabled / not enabled
shaggy plugins install user/repo                     # install from Git, then prompt Enable? [y/N]
shaggy plugins install user/repo --enable            # install AND enable (no prompt)
shaggy plugins install user/repo --no-enable         # install but leave disabled (no prompt)
shaggy plugins update my-plugin                      # pull latest
shaggy plugins remove my-plugin                      # uninstall
shaggy plugins enable my-plugin                      # add to allow-list (flat plugin)
shaggy plugins enable observability/langfuse         # add to allow-list (sub-category plugin)
shaggy plugins disable my-plugin                     # remove from allow-list + add to disabled
```

For plugins under a sub-category directory (e.g. `plugins/observability/langfuse/`, `plugins/image_gen/openai/`), use the full `<category>/<plugin>` key — that's exactly what `shaggy plugins list` shows in the **Name** column.

### Interactive UI

Running `shaggy plugins` with no arguments opens a composite interactive screen:
...
The default for a newly-installed or bundled plugin is `not enabled`. `shaggy plugins list` shows all three distinct states so you can tell what's been explicitly turned off vs. what's just waiting to be enabled.
...
See the **[full guide](/docs/guides/build-a-shaggy-plugin)** for handler contracts, schema format, hook behavior, error handling, and common mistakes.
