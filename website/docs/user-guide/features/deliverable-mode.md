...
When Shaggy Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
custom instructions in `~/.shaggy/config.yaml` under `agent.custom_instructions`.
...
If you use Shaggy' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.shaggy/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Shaggy Agent's deliverable
