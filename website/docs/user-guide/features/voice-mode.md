...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Shaggy Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Shaggy Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Shaggy](/docs/guides/use-voice-mode-with-shaggy).

## Prerequisites

Before using voice features, make sure you have:

1. **Shaggy Agent installed** — `pip install shaggy-agent` (see [Installation](/docs/getting-started/installation))
2. **An LLM provider configured** — run `shaggy model` or set your preferred provider credentials in `~/.shaggy/.env`
3. **A working base setup** — run `shaggy` to verify the agent responds to text before enabling voice

:::tip
The `~/.shaggy/` directory and default `config.yaml` are created automatically the first time you run `shaggy`. You only need to create `~/.shaggy/.env` manually for API keys.
:::
...
pip install "shaggy-agent[voice]"

# Discord + Telegram messaging (includes discord.py[voice] for VC support)
pip install "shaggy-agent[messaging]"

# Premium TTS (ElevenLabs)
pip install "shaggy-agent[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
pip install "shaggy-agent[all]"
```
...
Voice mode is available in both the **classic CLI** (`shaggy chat`) and the **TUI** (`shaggy --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.shaggy/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
```bash
shaggy                # Start the interactive CLI
```
...
1. Start the CLI with `shaggy` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.shaggy/config.yaml` (default: `ctrl+b`).
:::
...
```bash
shaggy gateway        # Start the gateway (connects to configured platforms)
shaggy gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@shaggybyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.shaggy/.env`:
```bash
...
```bash
shaggy gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.shaggy/.env`
...
- Review gateway logs: `tail -f ~/.shaggy/logs/gateway.log`
