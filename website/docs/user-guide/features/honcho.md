...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Shaggy's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Shaggy instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
shaggy memory setup    # select "honcho" from the provider list
```
...
```bash
echo 'HONCHO_API_KEY=***' >> ~/.shaggy/.env
```
...
Honcho is configured in `~/.honcho/config.json` (global) or `$SHAGGY_HOME/honcho.json` (profile-local). The setup wizard handles this for you.
...
- `per-session` — each `shaggy` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Shaggy syncs them back at session init.
...
The `shaggy honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). Run `shaggy memory setup` and pick Honcho first; the subcommand appears on the next invocation.

```bash
shaggy honcho status          # Connection status, config, and key settings
shaggy honcho setup           # Redirects to `shaggy memory setup`
shaggy honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
shaggy honcho peer            # Show or update peer names + dialectic reasoning level
shaggy honcho mode            # Show or set recall mode (hybrid/context/tools)
shaggy honcho tokens          # Show or set token budget for context and dialectic
shaggy honcho identity        # Seed or show the AI peer's Honcho identity
shaggy honcho sync            # Sync Honcho config to all existing profiles
shaggy honcho peers           # Show peer identities across all profiles
shaggy honcho sessions        # List known Honcho session mappings
shaggy honcho map             # Map current directory to a Honcho session name
shaggy honcho enable          # Enable Honcho for the active profile
shaggy honcho disable         # Disable Honcho for the active profile
shaggy honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `shaggy honcho`

If you previously used the standalone `shaggy honcho setup`:
...
No re-login or re-setup needed. Run `shaggy memory setup` and select "honcho" — the wizard detects your existing config.
