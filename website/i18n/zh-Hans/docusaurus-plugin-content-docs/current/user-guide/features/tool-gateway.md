...
在 `~/.shaggy/config.yaml` 中直接设置 `use_gateway`：
...
网关使用 Nous Portal 凭据认证（在 `shaggy model` 完成后写入 `~/.shaggy/auth.json`）。
...
在 `shaggy tools` 中选择非网关提供商时，`use_gateway` 会自动设为 `false`，避免配置自相矛盾。
...
Modal（无服务器终端后端）可作为 Nous 订阅的可选附加能力，但**不会**由 Tool Gateway 安装向导一并打开——请单独通过 `shaggy setup terminal` 或在 `config.yaml` 中配置。
