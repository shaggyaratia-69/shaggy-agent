"""Windows UTF-8 bootstrap for Shaggy entry points.
...
should be imported at the very top of every Shaggy entry point
(``shaggy``, ``shaggy-agent``, ``shaggy-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import shaggy_bootstrap``
# (or ``from shaggy_bootstrap import apply_windows_utf8_bootstrap``) at
