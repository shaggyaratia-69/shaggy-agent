/**
 * Shaggy Dashboard Plugin SDK — typed contract (SPIKE)
 * ====================================================
 *
 * This is the public type surface for ``window.__SHAGGY_PLUGIN_SDK__`` and
 * ``window.__SHAGGY_PLUGINS__``, the globals the dashboard host exposes to
 ...
 * window.__SHAGGY_PLUGIN_SDK__`` because:
 ...
 *
 * Versioning: bump ``ShaggyPluginSDK["sdkVersion"]`` (and the
 ...
 *   - Ship as a published ``@shaggy/dashboard-plugin-sdk`` types package, or
 ...
 * of calling ``fetch`` with a hand-read ``window.__SHAGGY_SESSION_TOKEN__``.
 */
...
// ---------------------------------------------------------------------------
// Registry surface (window.__SHAGGY_PLUGINS__)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// SDK surface (window.__SHAGGY_PLUGIN_SDK__)
// ---------------------------------------------------------------------------

export interface ShaggyPluginSDK {
  ...
  /**
   * Shared UI primitives (Cherries DS / shadcn). Typed permissively at the
...
declare global {
  interface Window {
    __SHAGGY_PLUGIN_SDK__?: ShaggyPluginSDK;
    __SHAGGY_PLUGINS__?: PluginRegistry;
  }
}
