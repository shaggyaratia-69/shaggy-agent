...
// (e.g. https://mission-control.tilos.com/shaggy/). The Python backend
// injects ``window.__SHAGGY_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__SHAGGY_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const SHAGGY_BASE_PATH = readBasePath();
const BASE = SHAGGY_BASE_PATH;
...
declare global {
  interface Window {
    __SHAGGY_SESSION_TOKEN__?: string;
    __SHAGGY_BASE_PATH__?: string;
  }
}
let _sessionToken: string | null = null;
const SESSION_HEADER = "X-Shaggy-Session-Token";
...
export async function fetchJSON<T>(url: string, init?: RequestInit): Promise<T> {
  ...
  const token = window.__SHAGGY_SESSION_TOKEN__;
  if (token) {
...
async function getSessionToken(): Promise<string> {
  ...
  const injected = window.__SHAGGY_SESSION_TOKEN__;
  if (injected) {
    _sessionToken = injected;
    return _sessionToken;
  }
  throw new Error("Session token not available — page must be served by the Shaggy dashboard server");
}
...
export const api = {
  ...
  updateShaggy: () =>
    fetchJSON<ActionResponse>("/api/shaggy/update", { method: "POST" }),
