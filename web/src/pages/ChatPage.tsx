/**
 * ChatPage — embeds `shaggy --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (shaggy_cli/web_server.py)                          .
 *          ▼                                                              .
...
function buildWsUrl(
  ...
  return `${proto}//${window.location.host}${SHAGGY_BASE_PATH}/api/pty?${qs.toString()}`;
}
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    typeof window !== "undefined" && !window.__SHAGGY_SESSION_TOKEN__
      ? "Session token unavailable. Open this page through `shaggy dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const term = new Terminal({
      ...
      // even when the inner Shaggy TUI has enabled xterm mouse-events
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[shaggy-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // forwarding them into Shaggy. Keyboard input, paste, and resize still
    // behave normally.
