...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - shaggy_home (str): The active SHAGGY_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.shaggy``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "shaggy").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'shaggy memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], shaggy_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'shaggy memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``shaggy_home`` is the active SHAGGY_HOME directory path.
