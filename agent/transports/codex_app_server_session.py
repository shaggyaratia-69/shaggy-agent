...
Owns one Codex thread per Shaggy session. Drives `turn/start`, consumes
...
# Shaggy' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_SHAGGY_TO_CODEX_PERMISSION_PROFILE = {
...
class CodexAppServerSession:
    """One Codex thread per Shaggy session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _SHAGGY_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("SHAGGY_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="shaggy",
            client_title="Shaggy Agent",
            client_version=_get_shaggy_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Shaggy' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Shaggy' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own shaggy-tools callback we
            # auto-accept — the user already approved Shaggy' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Shaggy approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Shaggy returns 'once', 'session', 'always', or 'deny'.
...
def _get_shaggy_version() -> str:
    """Best-effort Shaggy version string for codex's userAgent line."""
    try:
        from importlib.metadata import version

        return version("shaggy-agent")
