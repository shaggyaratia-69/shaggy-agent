...
Storage
-------
Bundles live in ``~/.shaggy/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``SHAGGY_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``shaggy bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under SHAGGY_HOME.

    Honors ``SHAGGY_BUNDLES_DIR`` for tests; falls back to
    ``<SHAGGY_HOME>/skill-bundles``.
    """
    override = os.environ.get("SHAGGY_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_shaggy_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `shaggy bundles` CLI subcommand.
# ---------------------------------------------------------------------------
