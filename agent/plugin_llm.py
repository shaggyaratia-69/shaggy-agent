...
Plugins built on Shaggy Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Shaggy
...
:class:`~shaggy_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Shaggy' main config
...
handles every provider, fallback chain, and per-task override Shaggy
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`shaggy_cli.plugins.PluginContext`
