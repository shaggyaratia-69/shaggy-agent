...
as ``kind: backend``) or ``~/.shaggy/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``shaggy tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``shaggy tools`` model picker.
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``shaggy tools`` picker."""
        return {
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``shaggy tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$SHAGGY_HOME/cache/videos/``, creating parents as needed."""
    from shaggy_constants import get_shaggy_home

    path = get_shaggy_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$SHAGGY_HOME/cache/videos/``.
