...
Every Shaggy request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Shaggy Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=shaggy-agent",
        "client=shaggy-client-v<__version__>",
    ]

The version is sourced live from ``shaggy_cli.__version__`` so it auto-aligns
...
``shaggy_cli.__version__`` is the canonical source of truth.
"""
...
def _shaggy_version() -> str:
    """Return the current Shaggy release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``shaggy_cli`` cannot be imported (should
...
def shaggy_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=shaggy-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=shaggy-client-v{_shaggy_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=shaggy-agent", shaggy_client_tag()]
