...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Shaggy starts and finds ``~/.openclaw/``.

    Points users at ``shaggy claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``shaggy claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Shaggy, run "
        "`shaggy claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `shaggy claw cleanup` (renames it to ~/.openclaw.pre-migration — "
