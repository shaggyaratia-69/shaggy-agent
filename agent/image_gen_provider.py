...
as ``kind: backend``) or ``~/.shaggy/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
def _images_cache_dir() -> Path:
    """Return ``$SHAGGY_HOME/cache/images/``, creating parents as needed."""
    from shaggy_constants import get_shaggy_home

    path = get_shaggy_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$SHAGGY_HOME/cache/images/``.
