...
from shaggy_constants import get_config_path, get_skills_dir
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``SHAGGY_PLATFORM`` or
            ``SHAGGY_SESSION_PLATFORM`` env vars.  Falls back to the
...
# which becomes the dominant cost of ``shaggy`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.shaggy/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``shaggy`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against SHAGGY_HOME, not cwd
        if not p.is_absolute():
            p = (shaggy_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.shaggy/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    shaggy = metadata.get("shaggy") or {}
    if not isinstance(shaggy, dict):
        shaggy = {}
    return {
        "fallback_for_toolsets": shaggy.get("fallback_for_toolsets", []),
        "requires_toolsets": shaggy.get("requires_toolsets", []),
        "fallback_for_tools": shaggy.get("fallback_for_tools", []),
        "requires_tools": shaggy.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    shaggy = metadata.get("shaggy")
    if not isinstance(shaggy, dict):
        return []
    raw = shaggy.get("config")
    if not raw:
        return []
