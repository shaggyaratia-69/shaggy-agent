...
``auth.json``.  They intentionally have no dependency on ``shaggy_cli.auth`` so
...
# Sources Shaggy owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Shaggy-owned OAuth/device-code state)
