#!/usr/bin/env python3
"""Live test harness for Shaggy Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any shaggy imports.
ORIGINAL_HOME = os.environ.get("SHAGGY_HOME")
ORIGINAL_AUTH = Path.home() / ".shaggy" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.shaggy/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.shaggy/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="shaggy_ts_live_"))
    shaggy_home = home_dir / ".shaggy"
    shaggy_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, shaggy_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".shaggy" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, shaggy_home / ".env")
        ...
        load_shaggy_dotenv(shaggy_home=str(Path.home() / ".shaggy"))
    ...
    (shaggy_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return shaggy_home
...
def reset_module_state():
    """Drop cached modules so the new SHAGGY_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "shaggy_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["SHAGGY_HOME"] = str(home)
...
def main():
    ...
    # Restore original SHAGGY_HOME
    if ORIGINAL_HOME is not None:
        os.environ["SHAGGY_HOME"] = ORIGINAL_HOME
    else:
