...
# Bootstrap Open WebUI against Shaggy Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.shaggy/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-shaggy.sh
...
#   SHAGGY_API_PORT=8642
#   SHAGGY_API_HOST=127.0.0.1
#   SHAGGY_API_MODEL_NAME='Shaggy Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Shaggy Agent WebUI}"
...
SHAGGY_ENV_FILE="${SHAGGY_ENV_FILE:-$HOME/.shaggy/.env}"
SHAGGY_API_PORT="${SHAGGY_API_PORT:-8642}"
SHAGGY_API_HOST="${SHAGGY_API_HOST:-127.0.0.1}"
SHAGGY_API_CONNECT_HOST="${SHAGGY_API_CONNECT_HOST:-127.0.0.1}"
SHAGGY_API_MODEL_NAME="${SHAGGY_API_MODEL_NAME:-Shaggy Agent}"
SHAGGY_API_BASE_URL="http://${SHAGGY_API_CONNECT_HOST}:${SHAGGY_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-shaggy.sh"
LOG_DIR="$HOME/.shaggy/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$SHAGGY_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$SHAGGY_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.shaggy.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.shaggy</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.shaggy"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.shaggy"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-shaggy.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Shaggy Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-shaggy.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.shaggy/logs/openwebui.log
StandardError=append:%h/.shaggy/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-shaggy.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$SHAGGY_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Shaggy API server is configured...'
  upsert_env API_SERVER_ENABLED true "$SHAGGY_ENV_FILE"
  upsert_env API_SERVER_HOST "$SHAGGY_API_HOST" "$SHAGGY_ENV_FILE"
  upsert_env API_SERVER_PORT "$SHAGGY_API_PORT" "$SHAGGY_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$SHAGGY_API_MODEL_NAME" "$SHAGGY_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$SHAGGY_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Shaggy gateway so API server settings take effect...'
  shaggy gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${SHAGGY_API_CONNECT_HOST}:${SHAGGY_API_PORT}/health" >/dev/null; then
    log 'Shaggy API server did not answer on the first check. Trying to start gateway in the background...'
    nohup shaggy gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${SHAGGY_API_CONNECT_HOST}:${SHAGGY_API_PORT}/health" >/dev/null
  ...
  log "Shaggy API endpoint: ${SHAGGY_API_BASE_URL}"
