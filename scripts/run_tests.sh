#!/usr/bin/env bash
# Canonical test runner for shaggy-agent. Run this instead of calling
...
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.shaggy/shaggy-agent/venv"; do
...
# Unset SHAGGY_* behavioral vars too.
unset SHAGGY_YOLO_MODE SHAGGY_INTERACTIVE SHAGGY_QUIET SHAGGY_TOOL_PROGRESS \
      SHAGGY_TOOL_PROGRESS_MODE SHAGGY_MAX_ITERATIONS SHAGGY_SESSION_PLATFORM \
      SHAGGY_SESSION_CHAT_ID SHAGGY_SESSION_CHAT_NAME SHAGGY_SESSION_THREAD_ID \
      SHAGGY_SESSION_SOURCE SHAGGY_SESSION_KEY SHAGGY_GATEWAY_SESSION \
      SHAGGY_CRON_SESSION \
      SHAGGY_PLATFORM SHAGGY_INFERENCE_PROVIDER SHAGGY_MANAGED SHAGGY_DEV \
      SHAGGY_CONTAINER SHAGGY_EPHEMERAL_SYSTEM_PROMPT SHAGGY_TIMEZONE \
      SHAGGY_REDACT_SECRETS SHAGGY_BACKGROUND_NOTIFICATIONS SHAGGY_EXEC_ASK \
      SHAGGY_HOME_MODE 2>/dev/null || true
...
# If a system-wide shaggy pytest_live_guard plugin is installed at
# $HOME/.shaggy/pytest_live_guard.py, force-load it here so every test run
...
if [ -f "$HOME/.shaggy/pytest_live_guard.py" ]; then
  case ":${PYTHONPATH:-}:" in
    *":$HOME/.shaggy:"*) ;;
    *) export PYTHONPATH="${PYTHONPATH:+$PYTHONPATH:}$HOME/.shaggy" ;;
  esac
