# ============================================================================
# Shaggy Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$ShaggyHome = "$env:LOCALAPPDATA\shaggy",
    [string]$InstallDir = "$env:LOCALAPPDATA\shaggy\shaggy-agent",
...
$RepoUrlSsh = "git@github.com:shaggyaratia-69/shaggy-agent.git"
$RepoUrlHttps = "https://github.com/shaggyaratia-69/shaggy-agent.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An open source AI agent by Cherries and Co.              |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $ShaggyHome)) {
        New-Item -ItemType Directory -Force -Path $ShaggyHome | Out-Null
    }
    $envFile = Join-Path $ShaggyHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $ShaggyHome "node"
...
function Install-Git {
    ...
    which Shaggy uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\shaggy\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Shaggy needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Shaggy copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\shaggy\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``SHAGGY_GIT_BASH_PATH`` (User scope) so Shaggy can find it in a fresh
    ...
    # Download PortableGit into $ShaggyHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $ShaggyHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Shaggy features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Shaggy needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``SHAGGY_GIT_BASH_PATH`` (User env scope) so Shaggy can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $ShaggyHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $ShaggyHome\git\usr\bin\bash.exe
    $candidates += "$ShaggyHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$ShaggyHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("SHAGGY_GIT_BASH_PATH", $candidate, "User")
            $env:SHAGGY_GIT_BASH_PATH = $candidate
            Write-Info "Set SHAGGY_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Shaggy may not find Git Bash."
    Write-Info "If needed, set SHAGGY_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    $managedNode = "$ShaggyHome\node\node.exe"
    if (Test-Path $managedNode) {
        $version = & $managedNode --version
        $env:Path = "$ShaggyHome\node;$env:Path"
        Write-Success "Node.js $version found (Shaggy-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    # The portable zip path drops node.exe + npm into $ShaggyHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $ShaggyHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\shaggy-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$ShaggyHome\node") { Remove-Item -Recurse -Force "$ShaggyHome\node" }
                Move-Item $extractedDir.FullName "$ShaggyHome\node"
                ...
                $version = & "$ShaggyHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $ShaggyHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
    ...
    if (Test-Path $InstallDir) {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                Write-Info "terminals, running shaggy processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/shaggyaratia-69/shaggy-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/shaggyaratia-69/shaggy-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/shaggyaratia-69/shaggy-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\shaggy-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\shaggy-agent-extract"
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `shaggy.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install shaggy-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and shaggy.exe broken with
    ...
    # here, not 30 seconds later when the user runs `shaggy`.
    if (-not $NoVenv) {
    ...
    # users hit and lazy-import errors from `shaggy dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `shaggy dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `shaggy dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up shaggy command..."
    
    if ($NoVenv) {
        $shaggyBin = "$InstallDir"
    } else {
        $shaggyBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so shaggy is globally available
    # On Windows, the shaggy.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$shaggyBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$shaggyBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $shaggyBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set SHAGGY_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\shaggy instead
    # of the Unix default ~/.shaggy
    $currentShaggyHome = [Environment]::GetEnvironmentVariable("SHAGGY_HOME", "User")
    if (-not $currentShaggyHome -or $currentShaggyHome -ne $ShaggyHome) {
        [Environment]::SetEnvironmentVariable("SHAGGY_HOME", $ShaggyHome, "User")
        Write-Success "Set SHAGGY_HOME=$ShaggyHome"
    }
    $env:SHAGGY_HOME = $ShaggyHome
    
    # Update current session
    $env:Path = "$shaggyBin;$env:Path"
    
    Write-Success "shaggy command ready"
}

function Copy-ConfigTemplates {
    Write-Info "Setting up configuration files..."
    
    # Create ~/.shaggy directory structure
    New-Item -ItemType Directory -Force -Path "$ShaggyHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ShaggyHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ShaggyHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ShaggyHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ShaggyHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ShaggyHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ShaggyHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ShaggyHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ShaggyHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$ShaggyHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.shaggy/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.shaggy/.env"
        }
    } else {
        Write-Info "~/.shaggy/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$ShaggyHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.shaggy/config.yaml from template"
        }
    } else {
        Write-Info "~/.shaggy/config.yaml already exists, keeping it"
    }
    ...
    # (the default PS5 behaviour), and Shaggy's prompt-injection scanner
...
"@
        ...
        Write-Success "Created ~/.shaggy/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.shaggy/"
    
    # Seed bundled skills into ~/.shaggy/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.shaggy/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.shaggy/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$ShaggyHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.shaggy/skills/"
            }
        }
    }
}
...
function Install-NodeDeps {
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'shaggy setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\shaggy-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\shaggy-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\shaggy-npm-tui-$(Get-Random).log"
...
function Install-PlatformSdks {
    ...
    # ~/.shaggy/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'shaggy setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m shaggy_cli.main setup
    } else {
        python -m shaggy_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $shaggyCmd = "$InstallDir\venv\Scripts\shaggy.exe"
    if (-not (Test-Path $shaggyCmd)) {
        $shaggyCmd = "shaggy"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$ShaggyHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'shaggy whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: shaggy gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$ShaggyHome\logs\gateway.log"
            Start-Process -FilePath $shaggyCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$ShaggyHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: shaggy gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: shaggy gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$ShaggyHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$ShaggyHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ShaggyHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ShaggyHome\shaggy-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   shaggy              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   shaggy setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   shaggy config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   shaggy config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   shaggy gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   shaggy update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Shaggy repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
    ...
    @{ Name = "path";             Title = "Adding Shaggy to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/shaggyaratia-69/shaggy-agent/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
