...
Module-level run.py helpers a handler needs (``_shaggy_home``,
...
class GatewaySlashCommandsMixin:
    ...
    def _typed_command_prefix_for(self, platform) -> str:
        """Return the prefix users can always type to reach Shaggy commands.
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        from gateway.run import _shaggy_home
    ...
    async def _handle_version_command(self, event: MessageEvent) -> str:
        """Handle /version — show the running Shaggy Agent version."""
        from shaggy_cli.banner import format_banner_version_label
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        """
        from gateway.run import _shaggy_home, _load_gateway_config
        import yaml
        ...
        if force_refresh:
            try:
                from shaggy_cli.models import clear_provider_models_cache
        ...
        async def _finish_switch() -> str:
            ...
            # Copilot, and Cherries-enforced caps win over the raw models.dev entry.
            mi = result.model_info
            from shaggy_cli.model_switch import resolve_display_context_length
        ...
        _cost_warning = None
        try:
            from shaggy_cli.model_cost_guard import expensive_model_warning
    ...
    async def _handle_reasoning_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _shaggy_home, _platform_config_key
        import yaml
    ...
    async def _handle_memory_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _shaggy_home
        from shaggy_cli.write_approval_commands import handle_pending_subcommand
        ...
        if args and args[0].lower() == "diff" and len(out) > 3000:
            ...
            out = (out[:3000]
                   + f"\n… (truncated — full diff: `/skills diff {pending_id}` "
                     f"on the CLI, or ~/.shaggy/pending/skills/{pending_id}.json)")
        return out

    async def _handle_fast_command(self, event: MessageEvent) -> str:
        """Handle /fast — mirror the CLI Priority Processing toggle in gateway chats."""
        from gateway.run import _shaggy_home, _load_gateway_config, _resolve_gateway_model
        import yaml
    ...
    async def _handle_verbose_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _shaggy_home, _load_gateway_config, _platform_config_key
    ...
    async def _handle_footer_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _shaggy_home, _load_gateway_config, _platform_config_key, _resolve_gateway_model
    ...
    async def _handle_compress_command(self, event: MessageEvent) -> str:
        ...
        from shaggy_cli.partial_compress import (
    ...
    async def _handle_usage_command(self, event: MessageEvent) -> str:
        ...
        # ── Cherries credits magnitudes + monthly-grant % gauge ─────────────
        ...
        # Run off the event loop. The helper gates on "a Cherries account is logged in"
        # — NOT the inference provider and NOT nested under `if provider:` — so a
        # Cherries-credentialled user running inference elsewhere (or with none resident)
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _shaggy_home, _resolve_shaggy_bin
        import json
        import shutil
        import subprocess
        from datetime import datetime
