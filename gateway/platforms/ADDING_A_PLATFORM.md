...
There are two ways to add a platform to the Shaggy gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.shaggy/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Shaggy code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `shaggy gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `shaggy_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Shaggy core.
...
And add it to the `shaggy-gateway` composite:

```python
"shaggy-gateway": {
    "includes": [..., "shaggy-your-platform"]
}
```
...
## 12. Status Display (`shaggy_cli/status.py`)
...
## 13. Gateway Setup Wizard (`shaggy_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ shaggy_cli/ toolsets.py \
  --include="*.py" -l | sort -u
