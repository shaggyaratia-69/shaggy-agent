...
When running Shaggy inside Docker, the default `network_mode: host` gives the
...
The Shaggy [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   SHAGGY_UID=$(id -u) SHAGGY_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=shaggy,shaggy-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://shaggy-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Shaggy trust model and vulnerability reporting
