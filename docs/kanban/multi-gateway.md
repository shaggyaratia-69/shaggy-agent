...
Shaggy supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.shaggy/config.yaml`:
...
Or set the env var: `SHAGGY_KANBAN_DISPATCH_IN_GATEWAY=false`
