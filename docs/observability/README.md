# Shaggy Observer Hooks

Shaggy observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Shaggy' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. Shaggy catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Shaggy only builds
