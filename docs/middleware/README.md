...
Shaggy middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Shaggy'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Shaggy calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Shaggy retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Shaggy guardrails,
...
- `telemetry_schema_version`: currently `shaggy.observer.v1`
- `middleware_schema_version`: currently `shaggy.middleware.v1`
...
Shaggy stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Shaggy runs
them as a nested chain in registration order. Middleware failures are fail-open:
Shaggy logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Shaggy applies middleware in this order:
...
For each tool call, Shaggy applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Shaggy pre-execution path against the effective arguments:
...
```bash
shaggy plugins enable <plugin-name>
```

For isolated local testing, use one `SHAGGY_HOME` for plugin enablement and the
agent run:

```bash
export SHAGGY_HOME=/tmp/shaggy-middleware-test
mkdir -p "$SHAGGY_HOME"
shaggy plugins enable <plugin-name>
shaggy chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run shaggy plugins enable <plugin-name>
uv run shaggy chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["shaggy_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/shaggy-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Shaggy expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Shaggy treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Shaggy preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Shaggy does not convert downstream
