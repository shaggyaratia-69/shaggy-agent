...
# shellcheck shell=sh
# /opt/shaggy/docker/main-wrapper.sh — wraps the container's CMD with
# the same argument-routing logic the pre-s6 entrypoint.sh used. Runs
# as /init's "main program" (Docker CMD) so it inherits stdin/stdout/
# stderr from the container.
#
# Shebang note: /init scrubs env before invoking CMD, so a plain
# `#!/bin/sh` wrapper sees an empty environ and `ENV SHAGGY_HOME=/opt/data`
# from the Dockerfile never reaches `shaggy`. with-contenv repopulates
# the env from /run/s6/container_environment before exec'ing, which is
# what s6-supervised services use too (see main-shaggy/run).
#
# Routing:
#   no args                       → exec `shaggy` (the default)
#   first arg is an executable    → exec it directly (sleep, bash, sh, …)
#   first arg is anything else    → exec `shaggy <args>` (subcommand passthrough)
#
# Drop to shaggy via s6-setuidgid, but skip it when already non-root.
set -e

drop() { [ "$(id -u)" = 0 ] && set -- s6-setuidgid shaggy "$@"; exec "$@"; }
...
# an arbitrary non-root, non-shaggy UID, the bootstrap was skipped and the
# baked image dirs (owned by the shaggy build UID) are unwritable, so fail
...
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u shaggy)" ]; then
    cat >&2 <<EOF
[shaggy] ERROR: container started with --user $cur_uid (an arbitrary, non-shaggy UID) — not supported.
...
# to the shaggy user's home before dropping privileges so libraries that
...
_shaggy_orig_cwd="${SHAGGY_ORIG_CWD:-$PWD}"
...
# the user's command so `shaggy chat` starts in the Docker -w
...
# Shaggy subcommand pass-through.
drop shaggy "$@"
