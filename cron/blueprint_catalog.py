...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``shaggy://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``shaggy://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"shaggy://blueprint/{quote(blueprint.key)}{qs}"
