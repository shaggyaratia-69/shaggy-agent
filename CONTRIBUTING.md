# Contributing to Shaggy Agent

Thank you for contributing to Shaggy Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Shaggy to work everywhere.
...
Bundled skills (in `skills/`) ship with every Shaggy install. They should be **broadly useful to most users**:
...
**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.shaggy/plugins/` (or via a pip entry point).

Standalone memory plugins:

- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(shaggy_home, config)` for setup-wizard integration
...
- Integrate with `shaggy memory setup` via `post_setup()` — no need to touch core code
...
```bash
mkdir -p ~/.shaggy/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.shaggy/config.yaml
touch ~/.shaggy/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.shaggy/.env
```
...
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/shaggy" ~/.local/bin/shaggy
...
├── cli.py                    # ShaggyCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (shaggy-cli, shaggy-telegram, etc.)
├── shaggy_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── shaggy_cli/               # CLI command implementations
...
### User configuration (stored in `~/.shaggy/`)

| Path | Purpose |
|------|---------|
| `~/.shaggy/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.shaggy/.env` | API keys and secrets |
| `~/.shaggy/auth.json` | OAuth credentials (Nous Portal) |
| `~/.shaggy/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.shaggy/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.shaggy/state.db` | SQLite session database |
| `~/.shaggy/sessions/` | JSON session logs |
| `~/.shaggy/cron/` | Scheduled job data |
| `~/.shaggy/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`shaggy_state.py`) with full-text search and unique session titles. JSON logs go to `~/.shaggy/sessions/`.
...
(for example `_SHAGGY_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
Four fields are supported under `metadata.shaggy`:
...
The user may skip setup and keep loading the skill. Shaggy only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `shaggy setup` or update `~/.shaggy/.env` locally.
...
2. **Tools referenced in SKILL.md prose must be native Shaggy tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Shaggy Agent" is the secondary collaborator. If the contributor's commit shows "Shaggy Agent" as author because they used Shaggy to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Shaggy tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `shaggy --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Shaggy uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.shaggy/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `shaggy_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `shaggy_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Shaggy runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the shaggy wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `shaggy_cli/gateway.py::_scan_gateway_pids` for the pattern.
...
10. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `shaggy_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
16. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `shaggy_cli/gateway_windows.py::
...
Shaggy has terminal access. Security matters.
...
2. **Test manually**: Run `shaggy` and exercise the code path you changed
