...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or Shaggy exit
...
# display_shaggy_home imported lazily at call site (stale-module safety during shaggy update)
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of shaggy directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "shaggy-*")):
...
def _get_sudo_password_cache_scope() -> str:
    ...
    try:
        ...
        session_key = get_session_env("SHAGGY_SESSION_KEY", "")
    except Exception:
        session_key = os.getenv("SHAGGY_SESSION_KEY", "")
    if session_key:
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = env_var_enabled("SHAGGY_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (SHAGGY_INTERACTIVE=1).
    ...
    try:
        os.environ["SHAGGY_SPINNER_PAUSE"] = "1"
        time.sleep(0.2)
        ...
        if "SHAGGY_SPINNER_PAUSE" in os.environ:
            del os.environ["SHAGGY_SPINNER_PAUSE"]
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    If SUDO_PASSWORD is not set and in interactive mode (SHAGGY_INTERACTIVE=1):
    ...
    # interactive Shaggy password prompt or the sudo -S password-pipe path.
    ...
    # silently without Shaggy prompting.
    if not has_configured_password and not sudo_password and _sudo_nopasswd_works():
        return command, None

    if not has_configured_password and not sudo_password and env_var_enabled("SHAGGY_INTERACTIVE"):
...
For servers/watchers, do NOT use shell-level background wrappers (nohup/disown/setsid/trailing '&') in foreground mode. Use background=true so Shaggy can track lifecycle and output.
...
def _resolve_container_task_id(task_id: Optional[str]) -> str:
    ...
    Exception: RL / benchmark environments (TerminalBench2, ShaggySweEnv, ...)
...
def _parse_env_var(name: str, default: str, converter=int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.shaggy/.env or environment variables."
        )
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(unquoted):
        return (
            ...
            "Use terminal(background=true) so Shaggy can track the process, then run "
...
def terminal_tool(
    ...
    try:
        ...
        # Product privacy guard: block common shell attempts to print/copy Shaggy internal source.
        if os.getenv("SHAGGY_ALLOW_INTERNAL_SOURCE", "").strip().lower() not in {"1", "true", "yes", "on"}:
            lowered_command = command.lower()
            reads_internal_source = (
                ("site-packages" in lowered_command and any(marker in lowered_command for marker in (
                    "/agent/", "/tools/", "/shaggy_cli/", "/acp_adapter/", "/gateway/", "/tui_gateway/",
                    "cli.py", "run_agent.py", "model_tools.py", "toolsets.py",
                )))
                or "shaggy_agent-" in lowered_command and ".whl" in lowered_command
            )
            source_read_commands = (
                "cat ", "sed ", "awk ", "head ", "tail ", "less ", "more ", "grep ", "rg ",
                "find ", "python -", "python3 -", "unzip ", "zipinfo ", "strings ", "cp ", "open ",
            )
            if reads_internal_source and any(tok in lowered_command for tok in source_read_commands):
                return json.dumps({
                    "output": "",
                    "exit_code": 126,
                    "error": "Access denied: Shaggy Agent internal product files are protected. I can help with normal setup, status, and troubleshooting, but I cannot reveal or copy internal source/package files.",
                    "status": "blocked"
                }, ensure_ascii=False)
        ...
        if background:
            ...
            try:
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    _gw_platform = _gse("SHAGGY_SESSION_PLATFORM", "")
                    if _gw_platform:
                        _gw_chat_id = _gse("SHAGGY_SESSION_CHAT_ID", "")
                        _gw_thread_id = _gse("SHAGGY_SESSION_THREAD_ID", "")
                        _gw_user_id = _gse("SHAGGY_SESSION_USER_ID", "")
                        _gw_user_name = _gse("SHAGGY_SESSION_USER_NAME", "")
                        _gw_message_id = _gse("SHAGGY_SESSION_MESSAGE_ID", "")
            ...
            try:
                from shaggy_cli.plugins import invoke_hook
