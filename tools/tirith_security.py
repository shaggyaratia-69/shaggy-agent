...
it is automatically downloaded from GitHub releases to $SHAGGY_HOME/bin/tirith.
...
def _get_shaggy_home() -> str:
    """Return the Shaggy home directory, respecting SHAGGY_HOME env var."""
    return str(get_shaggy_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_shaggy_home(), ".tirith-install-failed")
...
def _shaggy_bin_dir() -> str:
    """Return $SHAGGY_HOME/bin, creating it if needed."""
    d = os.path.join(_get_shaggy_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $SHAGGY_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_shaggy_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $SHAGGY_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $SHAGGY_HOME/bin/tirith
    ...
    shaggy_bin = os.path.join(_shaggy_bin_dir(), "tirith")
    if os.path.isfile(shaggy_bin) and os.access(shaggy_bin, os.X_OK):
        _resolved_path = shaggy_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return shaggy_bin
