...
This module provides the glue:
    - ``ShaggyTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Shaggy Agent"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses SHAGGY_HOME so each profile gets its own OAuth tokens.
    Layout: ``SHAGGY_HOME/mcp-tokens/``
    """
    try:
        from shaggy_constants import get_shaggy_home
        base = Path(get_shaggy_home())
    except ImportError:
        base = Path(os.environ.get("SHAGGY_HOME", str(Path.home() / ".shaggy")))
...
# ---------------------------------------------------------------------------
# ShaggyTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class ShaggyTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        SHAGGY_HOME/mcp-tokens/<server_name>.json         -- tokens
        SHAGGY_HOME/mcp-tokens/<server_name>.client.json   -- client info
        SHAGGY_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Shaggy records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Shaggy.</p></body></html>"
            ) if code else (
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  Then open the URL above. See: https://shaggyaratia-69.github.io/shaggy-agent/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Shaggy Agent")
    scope = cfg.get("scope")
