...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("SHAGGY_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return os.getenv("SHAGGY_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set SHAGGY_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind SHAGGY_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds SHAGGY_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("SHAGGY_CRON_SESSION"):
        return False
...
# via shell expansions like $HOME or $SHAGGY_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_SHAGGY_ENV_PATH = (
    r'(?:~\/\.shaggy/|'
    r'(?:\$home|\$\{home\})/\.shaggy/|'
    r'(?:\$shaggy_home|\$\{shaggy_home\})/)'
    r'\.env\b'
)
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bshaggy\s+gateway\s+(stop|restart)\b', "stop/restart shaggy gateway (kills running agents)"),
    (r'\bshaggy\s+update\b', "shaggy update (restarts gateway, kills running agents)"),
    # Gateway protection: never start gateway outside systemd management
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart shaggy-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart shaggy-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(shaggy|gateway|cli\.py)\b', "kill shaggy/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill shaggy` but not
    # `kill -9 $(pgrep -f shaggy)` because the substitution is opaque
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("SHAGGY_YOLO_MODE")) or is_current_session_yolo_enabled():
        return {"approved": True, "message": None}
    ...
    is_cli = env_var_enabled("SHAGGY_INTERACTIVE")
    ...
    if is_gateway or env_var_enabled("SHAGGY_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("SHAGGY_YOLO_MODE")) or is_current_session_yolo_enabled() or approval_mode == "off":
        return {"approved": True, "message": None}

    is_cli = env_var_enabled("SHAGGY_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("SHAGGY_EXEC_ASK")
