...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``shaggy tools`` repaint, tool-registration scans,
    ...
    2. ``~/.shaggy/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_shaggy_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.shaggy/.env`` first, then ``os.environ``.

    Wraps :func:`shaggy_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from shaggy_cli.config import get_env_value as _shaggy_get_env_value

        value = _shaggy_get_env_value(name)
...
def shaggy_xai_user_agent() -> str:
    """Return a stable Shaggy-specific User-Agent for xAI HTTP calls."""
    try:
        from shaggy_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Shaggy-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers Shaggy-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``shaggy_cli.config.get_env_value`` so keys
    stored in ``~/.shaggy/.env`` (the standard Shaggy location) are honored —
