...
One ``CDPSupervisor`` runs per Shaggy ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "shaggy-dialog-bridge.invalid"
...
(() => {
  if (window.__shaggyDialogBridgeInstalled) return;
  window.__shaggyDialogBridgeInstalled = true;
  const ENDPOINT = "http://shaggy-dialog-bridge.invalid/";
