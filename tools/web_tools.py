...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.shaggy/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``shaggy tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `shaggy tools` repaint.
        try:
        ...
        if provider is None:
            response_data = {
                ...
                "error": (
                    ...
                    "Run `shaggy tools` to set one up."
                ),
            }
        else:
            logger.info(
