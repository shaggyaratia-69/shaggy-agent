...
`shaggy skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.shaggy.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.shaggy.blueprint`` is a mapping containing
    ...
    shaggy = meta.get("shaggy") if isinstance(meta, dict) else None
    blueprint = shaggy.get("blueprint") if isinstance(shaggy, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.shaggy.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.shaggy.blueprint`` block) they can hand
    to ``shaggy skills publish`` to share. ``body`` is the plain-language
