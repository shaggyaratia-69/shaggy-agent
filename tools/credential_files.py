...
def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.shaggy",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``SHAGGY_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside SHAGGY_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.
    """
    shaggy_home = _resolve_shaggy_home()

    # Reject absolute paths — they bypass the SHAGGY_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to SHAGGY_HOME)",
            relative_path,
        )
        return False

    host_path = shaggy_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape SHAGGY_HOME.
    from tools.path_security import validate_within_dir

    containment_error = validate_within_dir(host_path, shaggy_home)
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.shaggy/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="shaggy-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching shaggy_constants.get_shaggy_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_shaggy_dir()`` for backward compatibility with old directory layouts.
    """
    from shaggy_constants import get_shaggy_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_shaggy_dir(new_subpath, old_name)
        if host_dir.is_dir():
