...
Lets the LLM write a Python script that calls Shaggy tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `shaggy_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `shaggy_tools.py` with file-based RPC stubs
...
# available on every platform Shaggy itself runs on.
...
# ---------------------------------------------------------------------------
# shaggy_tools.py code generator
# ---------------------------------------------------------------------------
...
def generate_shaggy_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the shaggy_tools.py stub module.
...
"""Auto-generated Shaggy tools RPC stubs."""
...
def _connect():
    ...
    SHAGGY_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["SHAGGY_RPC_SOCKET"]
...
"""Auto-generated Shaggy tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("SHAGGY_RPC_DIR") or os.path.join(tempfile.gettempdir(), "shaggy_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated shaggy_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/shaggy_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_shaggy_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/shaggy_tools.py", tools_src)
        ...
        env_prefix = (
            f"SHAGGY_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("SHAGGY_TIMEZONE", "").strip()
        if tz:
            env_prefix += f" TZ={tz}"
...
def execute_code(
    ...
    # --- Set up temp directory with shaggy_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="shaggy_sandbox_")
    ...
    # selector from SHAGGY_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"shaggy_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated shaggy_tools module.
        ...
        tools_src = generate_shaggy_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "shaggy_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  SHAGGY_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["SHAGGY_RPC_SOCKET"] = rpc_endpoint
        ...
        # Ensure the shaggy-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from shaggy_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _shaggy_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _shaggy_root]
        if _existing_pp:
            _pp_parts.append(_existing_pp)
        child_env["PYTHONPATH"] = os.pathsep.join(_pp_parts)
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set —
        # SHAGGY_TIMEZONE is an internal Shaggy setting and must not leak
        # into child processes.
        _tz_name = os.getenv("SHAGGY_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("SHAGGY_TIMEZONE", None)

        # Per-profile HOME isolation: redirect system tool configs into
        # {SHAGGY_HOME}/home/ when that directory exists.
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.shaggy/.env')).
...
def _load_config() -> dict:
    ...
    ``shaggy --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (shaggy-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``shaggy tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and shaggy-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.shaggy/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call Shaggy tools programmatically. "
        ...
        f"Available via `from shaggy_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into shaggy_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from shaggy_tools import {import_str}` "
