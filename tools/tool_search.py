"""Progressive tool disclosure ("tool search") for Shaggy Agent.
...
``tool_call`` — and surfaced on demand. Core Shaggy tools never defer.

Design constraints this module is built around (see ``openclaw-tool-search-report``
for the full rationale):

* Core tools defined in ``toolsets._SHAGGY_CORE_TOOLS`` are *never* deferred.
...
def load_config() -> ToolSearchConfig:
    ...
    try:
        from shaggy_cli.config import load_config as _load
        cfg = _load() or {}
...
def _core_tool_names() -> frozenset[str]:
    ...
    try:
        ...
        return frozenset(_SHAGGY_CORE_TOOLS)
    except Exception:
        return frozenset()
...
def is_deferrable_tool_name(name: str) -> bool:
    ...
    OR it is not in ``_SHAGGY_CORE_TOOLS``. Core tools are never deferred
