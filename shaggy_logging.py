"""Centralized logging setup for Shaggy Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.shaggy/logs/`` (profile-aware via ``get_shaggy_home()``).
...
from shaggy_constants import get_config_path, get_shaggy_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_shaggy_session_injector", False):
    ...
    _session_record_factory._shaggy_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``shaggy logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway", "shaggy_plugins"),
...
def setup_logging(
    *,
    ...
    """Configure the Shaggy logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    shaggy_home
        Override for the Shaggy home directory.  Falls back to
        ``get_shaggy_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_shaggy_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._shaggy_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the shaggy group. However, both _open() (initial creation) and
    ...
    def __init__(self, *args, **kwargs):
        from shaggy_cli.config import is_managed
