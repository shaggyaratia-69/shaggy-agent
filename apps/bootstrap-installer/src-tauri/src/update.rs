...
//!
//! Driven when the installer is launched as `Shaggy-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old Shaggy desktop process to fully exit (so the venv
//!      shim is free; otherwise `shaggy update` aborts with exit code 2),
//!   2. run `shaggy update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in shaggy_cli/main.py),
//!   3. run `shaggy desktop --build-only` (the rebuild step update skips),
...
//!
//! Cross-platform note: `shaggy update` already handles macOS/Linux (git/pip).
//! The only OS-specific bits here are the venv shim path (resolve_shaggy) and
...
/// `shaggy update` exit code meaning "another shaggy process is holding the
/// venv shim open / dirty precondition" — see _cmd_update_impl in
/// shaggy_cli/main.py (sys.exit(2)). We surface a targeted message for this.
const UPDATE_EXIT_CONCURRENT: i32 = 2;

/// How long to wait for the old desktop process to release the venv shim
/// before giving up and letting `shaggy update`'s own guard decide.
...
pub async fn start_update(app: AppHandle) -> Result<(), String> {
        ...
        .is_err()
    {
        ...
        let mut stages = vec![
            stage_info("update", "Updating Shaggy"),
...
async fn run_update(app: AppHandle) -> Result<()> {
    let shaggy_home = crate::paths::shaggy_home();
    let install_root = shaggy_home.join("shaggy-agent");
    ...
    let shaggy = resolve_shaggy(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the shaggy CLI under {}. Is Shaggy installed? \
    ...
    // async on Windows. If it still holds the venv shim, `shaggy update`
    // aborts with exit 2. Give it a bounded window to clear.
    wait_for_venv_free(&install_root, &app).await;

    // ---- stage 1: shaggy update -----------------------------------------
    // Pass --branch so `shaggy update` targets the branch this installer was
    ...
    // without --branch, `shaggy update` switches the checkout to `main` (a
    ...
    // --force skips `shaggy update`'s Windows running-exe guard (which would
    ...
    // time `shaggy update` runs there is no legitimate shaggy.exe to protect,
    // and the guard would only produce a false "Shaggy is still running" stop.
    ...
    // Retry-once for the update-boundary crash. `shaggy update` lazily imports
    ...
    // second `shaggy update` runs clean because the now-current module is loaded
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close Shaggy" state
    // a retry can't fix.
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "Shaggy is still running. Close all Shaggy windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
        ...
        other => {
            let msg = format!(
                "shaggy update failed (exit {:?}). See {} for details.",
                other,
                crate::paths::shaggy_home()
                    .join("logs")
                    .join("update.log")
                    .display()
            );
    ...
    // ---- stage 2: shaggy desktop --build-only ----------------------------
    // `shaggy update` deliberately does NOT build apps/desktop (it installs
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `shaggy desktop` \
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch Shaggy manually."),
            );
        }
    } else if let Err(err) =
        crate::bootstrap::launch_shaggy_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
    {
...
async fn wait_for_venv_free(install_root: &Path, app: &AppHandle) {
    let shim = venv_shaggy(install_root);
    let deadline = Instant::now() + DESKTOP_EXIT_WAIT;

    emit_log(app, Some("update"), LogStream::Stdout, "[update] waiting for Shaggy to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            // Last resort: a backend shaggy.exe (or a grandchild it spawned)
            ...
            // every shaggy.exe except ourselves, then give the OS a beat to
            // unload the image.
            emit_log(
                app,
                Some("update"),
                LogStream::Stdout,
                "[update] Shaggy still holding the venv shim; force-killing stragglers…",
            );
            force_kill_other_shaggy();
...
/// Force-kill any `shaggy.exe` other than this process. Windows-only; a no-op
...
/// knew its children — so we kill the whole `shaggy.exe` image tree via
/// taskkill, excluding our own PID.
///
/// Safe w.r.t. our own update child: this runs inside `wait_for_venv_free`,
/// which completes BEFORE we spawn `venv\Scripts\shaggy.exe update`. At this
/// point no update-driven shaggy.exe exists yet, so the only shaggy.exe images
...
/// Spawn `shaggy <args>` from `cwd`, stream stdout/stderr as Log events on the
...
/// Path to the venv shaggy shim under an install root, regardless of existence.
fn venv_shaggy(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
        install_root.join("venv").join("Scripts").join("shaggy.exe")
    } else {
        install_root.join("venv").join("bin").join("shaggy")
    }
}

/// Resolve the shaggy CLI to drive. Prefer the venv shim in the install we
/// just updated; fall back to `shaggy` on PATH.
fn resolve_shaggy(install_root: &Path) -> Option<PathBuf> {
    let shim = venv_shaggy(install_root);
    if shim.exists() {
        return Some(shim);
    }
    // PATH fallback. which-style probe via env, kept dependency-free.
    let exe = if cfg!(target_os = "windows") { "shaggy.exe" } else { "shaggy" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    let shaggy_home = crate::paths::shaggy_home();
    let mut envs = vec![(
        "SHAGGY_HOME".to_string(),
        shaggy_home.as_os_str().to_os_string(),
    )];
    if let Some(path) = path_with_prepended_entries(&[
        shaggy_home.join("node").join("bin"),
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_shaggy_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no Shaggy.app was found under {}",
    ...
    let tmp = PathBuf::from(format!("{}.shaggy-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.shaggy-update-old", target_app.display()));
    ...
    let ditto = Command::new("/usr/bin/ditto")
        .arg(&rebuilt_app)
        .arg(&tmp)
        .current_dir(crate::paths::shaggy_home())
        .status()
        .await
...
mod tests {
    ...
    #[test]
    fn venv_shaggy_is_under_install_root() {
        let root = Path::new("/x/shaggy-agent");
        let shim = venv_shaggy(root);
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/Shaggy.app"]),
            Some(PathBuf::from("/Applications/Shaggy.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("Shaggy.app");
        let tmp = base.join("Shaggy.app.shaggy-update-new");
        let old = base.join("Shaggy.app.shaggy-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("Shaggy.app");
        let tmp = base.join("Shaggy.app.shaggy-update-new"); // intentionally absent
        let old = base.join("Shaggy.app.shaggy-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("Shaggy.app");
        let tmp = base.join("Shaggy.app.shaggy-update-new"); // absent
        let old = base.join("Shaggy.app.shaggy-update-old");
        write_marker(&target, "OLD");
