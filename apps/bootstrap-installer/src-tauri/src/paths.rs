...
//!
//! Mirrors `shaggy_constants.get_shaggy_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\shaggy
//!   macOS:   ~/.shaggy
//!   Linux:   ~/.shaggy  (override via $SHAGGY_HOME)
//!
//! NOTE (macOS): Python's get_shaggy_home(), scripts/install.sh, and the
//! Electron desktop's resolveShaggyHome() ALL use ~/.shaggy on macOS — there
...
/// Returns the canonical Shaggy home directory, respecting $SHAGGY_HOME if set.
pub fn shaggy_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("SHAGGY_HOME") {
    ...
    {
        // %LOCALAPPDATA%\shaggy — matches scripts/install.ps1's $ShaggyHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("shaggy");
        }
    }

    // macOS + Linux + fallback: ~/.shaggy (matches Python get_shaggy_home(),
    // install.sh, and the Electron desktop's resolveShaggyHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".shaggy");
    }
...
pub fn bootstrap_cache_dir() -> PathBuf {
    shaggy_home().join("bootstrap-cache")
}
...
/// SHAGGY_HOME so it survives repo checkout deletion (unlike anything under
/// shaggy-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\shaggy\shaggy-setup.exe`; on other
...
pub fn copy_self_to_shaggy_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to SHAGGY_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_SHAGGY_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".shaggy-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under SHAGGY_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[shaggy-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("SHAGGY_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_shaggy_home() -> String {
    shaggy_home().to_string_lossy().into_owned()
}
