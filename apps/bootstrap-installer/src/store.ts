...
export const $shaggyHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  try {
    const [logPath, shaggyHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_shaggy_home'),
      invoke<AppMode>('get_mode')
    ])
    $logPath.set(logPath)
    $shaggyHome.set(shaggyHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Shaggy" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  // Update is driven by the desktop handing off (Shaggy-Setup.exe --update);
...
export async function launchShaggyDesktop(): Promise<void> {
  const installRoot = $bootstrap.get().installRoot
  if (!installRoot) throw new Error('no install root')
  await invoke('launch_shaggy_desktop', { installRoot })
}
