...
// no longer bundles the Shaggy The Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Shaggy.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Shaggy'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Shaggy.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'shaggy'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default SHAGGY_HOME for non-sandboxed runs -- matches main.cjs's
// resolveShaggyHome(). On Windows it's %LOCALAPPDATA%\shaggy; elsewhere
// it's ~/.shaggy. The fresh-install sandbox launchFresh() sets its own
// SHAGGY_HOME and never touches this.
const DEFAULT_SHAGGY_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'shaggy')
  }
  return path.join(os.homedir(), '.shaggy')
})()
const VENV_ROOT = path.join(DEFAULT_SHAGGY_HOME, 'shaggy-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'shaggy-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.SHAGGY_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Shaggy-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Shaggy-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Shaggy-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Shaggy-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.SHAGGY_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.SHAGGY_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const shaggyHome = path.join(sandbox, 'shaggy-home')
  const cwd = path.join(sandbox, 'workspace')

  fs.mkdirSync(userDataDir, { recursive: true })
  fs.mkdirSync(shaggyHome, { recursive: true })
  ...
  env.SHAGGY_DESKTOP_CWD = cwd
  env.SHAGGY_DESKTOP_IGNORE_EXISTING = '1'
  env.SHAGGY_DESKTOP_TEST_MODE = 'fresh-install'
  env.SHAGGY_DESKTOP_USER_DATA_DIR = userDataDir
  env.SHAGGY_HOME = shaggyHome
  delete env.SHAGGY_DESKTOP_HERMES
  delete env.SHAGGY_DESKTOP_SHAGGY_ROOT
  ...
  console.log(`  SHAGGY_HOME: ${shaggyHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(shaggyHome, 'shaggy-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Shaggy The Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of shaggy_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'shaggy-agent', 'shaggy_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Shaggy
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + SHAGGY_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  SHAGGY_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
