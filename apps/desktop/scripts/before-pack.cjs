 ...
 * product name (`Shaggy`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Shaggy'
 *
 * This is a hard failure with no obvious cause for the user — `shaggy desktop`
 ...
 * (the mac-unpacked Shaggy.app bundle) and Windows (win-unpacked), so we
