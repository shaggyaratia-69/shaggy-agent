#!/usr/bin/env node
// set-exe-identity.cjs — stamp the Shaggy icon + version metadata onto the
// built Shaggy.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Shaggy.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `shaggy desktop`, the installer's
...
// path (which rebuilds via `shaggy desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.cjs <path-to-Shaggy.exe>
//
...
// Stamp the Shaggy icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = path.resolve(__dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ProductName: 'Shaggy',
      FileDescription: 'Shaggy',
      CompanyName: 'Cherries and Co Research',
      LegalCopyright: 'Copyright (c) 2026 Cherries and Co Research'
    }
  })

  console.log('[set-exe-identity] done — Shaggy icon + identity stamped')
}
