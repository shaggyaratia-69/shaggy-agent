 ...
 *
 * Stamps the Shaggy icon + identity onto the packed Windows Shaggy.exe via
 * rcedit (delegated to set-exe-identity.cjs). This runs for EVERY packed build
 * — first install, `shaggy desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Shaggy')
 */
...
exports.default = async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Shaggy'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Shaggy.exe keeps the stock Electron icon`)
  }
}
