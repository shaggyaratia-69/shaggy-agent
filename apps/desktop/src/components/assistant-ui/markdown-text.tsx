...
async function mediaSrc(path: string): Promise<string> {
  ...
  if (window.shaggyDesktop && ['audio', 'video'].includes(mediaKind(path))) {
  ...
  if (window.shaggyDesktop && isRemoteGateway()) {
    return gatewayMediaDataUrl(path)
  }

  if (!window.shaggyDesktop?.readFileDataUrl) {
    return mediaExternalUrl(path)
  }

  return window.shaggyDesktop.readFileDataUrl(filePathFromMediaPath(path))
}
...
function OpenMediaButton({ kind, path }: { kind: 'audio' | 'video'; path: string }) {
  return (
    <button
      ...
      onClick={() => void window.shaggyDesktop?.openExternal(mediaExternalUrl(path))}
      type="button"
    >
      Open {kind} file
    </button>
...
function MarkdownTextSurface({ containerClassName, containerProps }: MarkdownTextSurfaceProps) {
  const partTextState = useMessagePartText()
  const { status } = partTextState
  const isStreaming = status.type === 'running'
  const danglingLeadingFence = useMemo(() => {
    if (!isStreaming) {
      return null
    }

    const rawText = typeof partTextState.text === 'string' ? partTextState.text : ''
    ...
    return body.trimEnd()
  }, [isStreaming, partTextState.text])
  ...
  if (danglingLeadingFence) {
    return (
      <div className={cn(MARKDOWN_CONTAINER_CLASS_NAME, containerClassName)} {...containerProps}>
        <CodeCard data-streaming="true">
          <CodeCardBody>
            <pre className="m-0 overflow-hidden bg-transparent p-0">
              <code className="block whitespace-pre">{danglingLeadingFence}</code>
            </pre>
          </CodeCardBody>
        </CodeCard>
      </div>
