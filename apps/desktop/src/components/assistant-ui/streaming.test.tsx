...
describe('assistant-ui streaming renderer', () => {
  ...
  it('renders assistant text incrementally before completion', async () => {
    ...
    expect(screen.getByRole('status', { name: 'Shaggy is loading a response' })).toBeTruthy()
    ...
    expect(screen.queryByRole('status', { name: 'Shaggy is loading a response' })).toBeNull()
  ...
  it('renders an incomplete streaming reasoning fenced code block as a code card', async () => {
    ...
    if (thinkingButton.getAttribute('aria-expanded') === 'false') {
    ...
    await waitFor(() => {
      expect(container.querySelector('[data-slot="code-card"]')?.textContent).toContain('const answer = 42')
    })
    expect(container.textContent).not.toContain('```ts')
  })
