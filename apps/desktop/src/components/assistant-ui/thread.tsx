...
import { extractDroppedFiles, isImagePath, partitionDroppedFiles, SHAGGY_PATHS_MIME } from '@/app/chat/hooks/use-composer-actions'
...
const ReasoningTextPart: FC<{ text: string; status?: { type: string } }> = ({ text, status }) => {
  ...
  const messageRunning = useAuiState(s => s.message.status?.type === 'running')
  const isRunning = status?.type === 'running' || messageRunning || hasUnclosedFence(displayText)
