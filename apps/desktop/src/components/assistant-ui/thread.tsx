...
import { extractDroppedFiles, SHAGGY_PATHS_MIME, isImagePath, partitionDroppedFiles } from '@/app/chat/hooks/use-composer-actions'
import { uploadComposerAttachment } from '@/app/session/hooks/use-prompt-actions'
import { ClarifyTool } from '@/components/assistant-ui/clarify-tool'
import { DirectiveContent, shaggyDirectiveFormatter } from '@/components/assistant-ui/directive-text'
...
import type { ShaggyGateway } from '@/shaggy'
...
function danglingLeadingFenceBody(text: string): string | null {
  ...
  const hasClose = body
    ...
    .some(line => {
      ...
      return trimmed[0] === marker[0] && trimmed.length >= marker.length && /^(`+|~+)$/.test(trimmed)
    })

  return hasClose || !body.trim() ? null : body.trimEnd()
}

function runningReasoningFenceBody(content: unknown): string | null {
  ...
  for (const part of content) {
    ...
    if (row.type === 'reasoning' && typeof row.text === 'string') {
      const body = danglingLeadingFenceBody(row.text)
...
const AssistantMessage: FC<{ onBranchInNewChat?: (messageId: string) => void }> = ({ onBranchInNewChat }) => {
  ...
  const reasoningFenceBody = useMemo(
    () => (messageStatus === 'running' ? runningReasoningFenceBody(parts) : null),
    [messageStatus, parts]
  )
  ...
  return (
    ...
    >
      ...
      >
        ...
        {reasoningFenceBody ? (
          <ThinkingDisclosure messageRunning={true} pending={false} timerKey={`reasoning-fence:${messageId}`}>
            <div className="text-xs leading-snug text-muted-foreground/85" data-slot="aui_reasoning-text">
              <CodeCard data-streaming="true">
                <CodeCardBody>
                  <pre className="m-0 overflow-hidden bg-transparent p-0">
                    <code className="block whitespace-pre">{reasoningFenceBody}</code>
                  </pre>
                </CodeCardBody>
              </CodeCard>
            </div>
...
const StreamStallIndicator: FC<{ activity: string }> = ({ activity }) => {
  ...
  return (
    <StatusRow className="mt-1.5" data-slot="aui_stream-stall" label="Shaggy is thinking">
      <span aria-hidden="true" className="dither inline-block size-3 rounded-[2px] text-midground/80 animate-pulse" />
      <ActivityTimerText seconds={elapsed} />
    </StatusRow>
  )
...
const ReasoningTextPart: FC<{ text: string; status?: { type: string } }> = ({ text, status }) => {
  ...
  if (danglingFence) {
    return (
      <div
        className={cn('text-xs leading-snug text-muted-foreground/85', isRunning && 'shimmer text-muted-foreground/55')}
        data-slot="aui_reasoning-text"
      >
        <CodeCard>
          <CodeCardBody>
            <pre className="m-0 overflow-hidden bg-transparent p-0">
              <code className="block whitespace-pre">{danglingFence}</code>
            </pre>
          </CodeCardBody>
        </CodeCard>
      </div>
...
const UserEditComposer: FC<UserEditComposerProps> = ({ cwd, gateway, sessionId }) => {
  ...
  const replaceTriggerWithChip = useCallback(
    (item: Unstable_TriggerItem) => {
      ...
      const serialized = shaggyDirectiveFormatter.serialize(item)
  ...
  const handleDragEnter = (event: ReactDragEvent<HTMLElement>) => {
    if (!dragHasAttachments(event.dataTransfer, SHAGGY_PATHS_MIME)) {
      return
    }
