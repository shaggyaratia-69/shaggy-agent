...
const SHAGGY_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type ShaggyRefType = (typeof SHAGGY_REF_TYPES)[number]
...
const ICON_PATHS: Record<ShaggyRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as ShaggyRefType] ?? ICON_FALLBACK
...
const SHAGGY_DIRECTIVE_RE = new RegExp(
...
export const shaggyDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(SHAGGY_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as ShaggyRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: ShaggyRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
/**
 * Renders text containing Shaggy directives (`@file:...`, `@image:...`) as
...
export function DirectiveContent({ text }: { text: string }) {
  ...
  const segments = useMemo(() => shaggyDirectiveFormatter.parse(cleanedText), [cleanedText])
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.shaggyDesktop && isRemoteGateway()
        ? gatewayMediaDataUrl(id)
        : window.shaggyDesktop?.readFileDataUrl(id)
