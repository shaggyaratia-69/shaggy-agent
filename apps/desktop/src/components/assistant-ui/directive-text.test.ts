...
import { formatRefValue, shaggyDirectiveFormatter } from './directive-text'
...
describe('shaggyDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = shaggyDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = shaggyDirectiveFormatter.parse('@file:src/main.tsx the entry point')
