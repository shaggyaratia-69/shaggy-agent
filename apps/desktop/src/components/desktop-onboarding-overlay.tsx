...
import { getGlobalModelOptions } from '@/shaggy'
...
import type { ModelOptionProvider, OAuthProvider } from '@/types/shaggy'
...
const API_KEY_OPTIONS: ApiKeyOption[] = [
  ...
  },
  {
    ...
    docsUrl: 'https://github.com/shaggyaratia-69/shaggy-agent#bring-your-own-endpoint',
    placeholder: 'http://127.0.0.1:8000/v1'
  }
]

// Build the FULL API-key provider catalog from the backend model options so the
// onboarding / Providers key form lists every `api_key` provider `shaggy model`
...
const PROVIDER_DISPLAY: Record<string, { order: number; title: string }> = {
  nous: { order: 0, title: 'Cherries Portal' },
...
const SHOW_ALL_KEY = 'shaggy-onboarding-show-all-v1'
...
export function Picker({ ctx }: { ctx: OnboardingContext }) {
  ...
  // Collapse the secondary providers behind a disclosure only when Cherries
