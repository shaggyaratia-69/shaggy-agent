...
/** Why a known Shaggy command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Shaggy profile', surface: action('profile') },
  ...
  { name: '/version', description: 'Show Shaggy The Agent version', surface: exec() },
...
function isKnownShaggySlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Shaggy' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownShaggySlashCommand(normalized)
}
