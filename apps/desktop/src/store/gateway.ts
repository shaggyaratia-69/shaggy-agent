import type { ConnectionState, GatewayEvent } from '@shaggy/shared'
...
const isOpen = (gateway: ShaggyGateway | null): boolean => gateway?.connectionState === 'open'
...
export const $gateway = atom<ShaggyGateway | null>(null)
...
let primaryGateway: ShaggyGateway | null = null
let primaryProfile = 'default'

export function setPrimaryGateway(gateway: ShaggyGateway | null, profile = 'default'): void {
...
export function activeGateway(): ShaggyGateway | null {
...
export async function ensureActiveGatewayOpen(): Promise<ShaggyGateway | null> {
  if (activeKey === primaryProfile) {
    return primaryGateway
  }
