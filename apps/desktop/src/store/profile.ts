...
import { getProfiles, setApiRequestProfile } from '@/shaggy'
...
import type { ProfileInfo } from '@/types/shaggy'
...
// /api/profiles/active `current`). "default" is the root ~/.shaggy. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'shaggy.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'shaggy.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.shaggyDesktop.api<ActiveProfileResponse>({ path: '/api/profiles/active' })
...
// Persist the choice and relaunch the backend under the new SHAGGY_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.shaggyDesktop.profile.set(name)
}
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'shaggy.desktop.showAllProfiles'
...
// Switch to the default (root ~/.shaggy) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.shaggyDesktop?.touchBackend?.(target).catch(() => undefined)
}
