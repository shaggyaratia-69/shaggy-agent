...
import type { ModelOptionProvider, OAuthProvider, OAuthStartResponse } from '@/types/shaggy'
...
const CONFIGURED_CACHE_KEY = 'shaggy-desktop-onboarded-v1'
const SKIP_CACHE_KEY = 'shaggy-onboarding-skipped-v1'
const POLL_MS = 2000
...
function notifyReady(provider: string) {
  notify({ kind: 'success', title: 'Shaggy is ready', message: `${provider} connected.` })
}

// Human-friendly labels for tools auto-routed through the Cherries Tool Gateway,
// mirroring shaggy_cli/nous_subscription._GATEWAY_TOOL_LABELS so the GUI and
...
// When switching to Cherries auto-routes unconfigured tools through the Tool
...
function notifyGatewayTools(tools: string[] | undefined) {
  ...
  notify({
    durationMs: 8000,
    kind: 'info',
    message: `${list} now run through your Cherries subscription — no separate API keys needed.`,
...
async function fetchProviderDefaultModel(
  ...
  // `shaggy model` does (for Cherries it honors the user's free/paid tier, so a
...
function providerResolutionFailure(reason: null | string) {
  ...
  return detail
    ? `Connected, but Shaggy still cannot resolve a usable provider. ${detail}`
    : 'Connected, but Shaggy still cannot resolve a usable provider.'
}
...
async function openSignInUrl(url: string) {
  if (window.shaggyDesktop?.openExternal) {
    try {
      await window.shaggyDesktop.openExternal(url)
...
export async function recheckExternalSignin(ctx: OnboardingContext) {
  ...
  await completeWithModelConfirm(ctx, provider.name, [provider.id], reason =>
    setFlow({
      ...
      message:
        reason?.trim() ||
        `Shaggy still cannot reach ${provider.name}. Run \`${provider.cli_command}\` in a terminal first.`
    })
  )
}
...
export async function saveOnboardingLocalEndpoint(baseUrl: string, ctx: OnboardingContext) {
  ...
  try {
    ...
    if (!runtime.ready) {
      ...
      return { ok: false, message: detail || `Saved, but Shaggy still cannot reach ${url}.` }
    }
