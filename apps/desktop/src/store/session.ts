...
import type { SessionInfo, UsageStats } from '@/types/shaggy'

type Updater<T> = T | ((current: T) => T)

const WORKSPACE_CWD_KEY = 'shaggy.desktop.workspace-cwd'

let configuredDefaultProjectDir = ''

function workspaceCwdKey(connection: ShaggyConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(): Promise<string> {
  const settings = window.shaggyDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(): Promise<void> {
  const sanitize = window.shaggyDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<ShaggyConnection | null>(null)
...
export const setConnection = (next: Updater<ShaggyConnection | null>) => updateAtom($connection, next)
