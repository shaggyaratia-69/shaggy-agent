...
export const ja = defineLocale({
  ...
  boot: {
    ready: 'Shaggy The Agent の準備ができました',
    ...
    steps: {
      ...
      startingShaggyDesktop: 'Shaggy The Agent を起動中…'
    },
    errors: {
  ...
  notifications: {
    ...
    errors: {
      ...
      methodNotAllowed:
        'デスクトップバックエンドがそのリクエストを拒否しました (405 Method Not Allowed)。Shaggy The Agent を再起動してください。',
  ...
  settings: {
    ...
    gateway: {
      ...
      intro:
        'Shaggy The Agent はデフォルトで独自のローカルゲートウェイを起動します。別のマシンや信頼できるプロキシの背後で既に動作している Shaggy バックエンドをこのアプリで制御する場合は、リモートゲートウェイを使用してください。以下でプロファイルを選択して、それぞれのリモートホストを設定します。',
      appliesTo: '適用対象',
      ...
      restartingMessage: 'Shaggy The Agent は保存された設定を使用して再接続します。',
  ...
  install: {
    ...
    settingUpTitle: 'Shaggy The Agent を設定中',
    finishingTitle: '仕上げ中',
    failedDesc:
  ...
  onboarding: {
    headerTitle: 'Shaggy The Agent のセットアップをしましょう',
  ...
  desktop: {
    ...
    restartToUseSaveImage: '画像を保存するには Shaggy The Agent を再起動してください。',
    restartToSaveImages: '画像を保存するには Shaggy The Agent を再起動してください',
