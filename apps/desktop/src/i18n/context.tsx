...
import { getShaggyConfigRecord, saveShaggyConfig, type ShaggyConfigRecord } from '@/shaggy'
...
export interface I18nConfigClient {
  getConfig: () => Promise<ShaggyConfigRecord>
  saveConfig: (config: ShaggyConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.shaggyDesktop?.api) {
      return Promise.resolve({})
    }

    return getShaggyConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.shaggyDesktop?.api) {
    ...
    return saveShaggyConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: ShaggyConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: ShaggyConfigRecord, locale: Locale): ShaggyConfigRecord {
