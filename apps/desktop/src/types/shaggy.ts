...
export type ShaggyConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `shaggy model` CLI / onboarding OAuth flow. */
  auth_type?: string
  ...
  /** Cherries only: whether the current account is on the free tier. */
  free_tier?: boolean
  /** Cherries only: paid models a free-tier user cannot select (shown disabled). */
...
export interface ToolProvider {
   ...
   *  CLI `shaggy tools` active-provider detection). */
  is_active: boolean
}
...
/** Shape of `GET /api/shaggy/update/check` — the backend's own update state.
...
export interface ModelAssignmentResponse {
  ...
  base_url?: string
  /** Toolset keys auto-routed through the Cherries Tool Gateway as a result of
   *  switching the main provider to Cherries. Empty unless provider === 'nous'
