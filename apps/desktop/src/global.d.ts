...
declare global {
  interface Window {
    shaggyDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<ShaggyConnection>
      ...
      profile: {
        ...
        // backend under the new SHAGGY_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: ShaggyApiRequest) => Promise<T>
      notify: (payload: ShaggyNotification) => Promise<boolean>
      requestMicrophoneAccess: () => Promise<boolean>
      readFileDataUrl: (filePath: string) => Promise<string>
      readFileText: (filePath: string) => Promise<ShaggyReadFileTextResult>
      ...
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<ShaggyPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<ShaggyPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setTitleBarTheme?: (payload: ShaggyTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<ShaggyReadDirResult>
      gitRoot?: (path: string) => Promise<string | null>
      terminal: {
        dispose: (id: string) => Promise<boolean>
        onData: (id: string, callback: (payload: string) => void) => () => void
        onExit: (id: string, callback: (payload: ShaggyTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<ShaggyTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: ShaggyWindowState) => void) => () => void
      onPreviewFileChanged: (callback: (payload: ShaggyPreviewFileChanged) => void) => () => void
...
export interface ShaggyTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface ShaggyTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `shaggy update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  shaggyRoot?: string
}
...
export interface ShaggyConnection {
  baseUrl: string
...
export interface ShaggyTitleBarTheme {
  background: string
  foreground: string
}

export interface ShaggyWindowState {
  isFullscreen: boolean
  nativeOverlayWidth: number
  windowButtonPosition: { x: number; y: number } | null
}
...
export interface ShaggyApiRequest {
  path: string
  method?: string
  body?: unknown
  timeoutMs?: number
...
export interface ShaggyNotification {
  title?: string
  body?: string
  silent?: boolean
}

export interface ShaggyPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface ShaggyReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface ShaggyPreviewWatch {
  id: string
  path: string
}

export interface ShaggyReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface ShaggyReadDirResult {
  entries: ShaggyReadDirEntry[]
  error?: string
}

export interface ShaggyPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface ShaggySelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
