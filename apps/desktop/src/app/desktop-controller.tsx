...
import { getCronJobs, getSessionMessages, listAllProfileSessions, type SessionInfo, triggerCronJob } from '../shaggy'
...
import { useShaggyConfig } from './session/hooks/use-shaggy-config'
...
export function DesktopController() {
  ...
  useEffect(() => {
    window.shaggyDesktop?.setPreviewShortcutActive?.(Boolean(chatOpen && (filePreviewTarget || previewTarget)))
  }, [chatOpen, filePreviewTarget, previewTarget])

  useEffect(() => {
    startUpdatePoller()
    const unsubscribe = window.shaggyDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  // shaggy:// deep links (e.g. a docs "Send to App" button for an automation blueprint).
  ...
  useEffect(() => {
    const unsubscribe = window.shaggyDesktop?.onDeepLink?.((payload) => {
    ...
    void window.shaggyDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.shaggyDesktop?.onClosePreviewRequested?.(closeActiveRightRailTab)
  ...
  const { refreshShaggyConfig, sttEnabled, voiceMaxRecordingSeconds } = useShaggyConfig({
    activeSessionIdRef,
  ...
  useEffect(() => {
    if (gatewayState === 'open' && !activeSessionId && freshDraftReady) {
      void refreshCurrentModel()
      void refreshShaggyConfig()
    }
  }, [activeSessionId, freshDraftReady, gatewayState, refreshCurrentModel, refreshShaggyConfig])
