...
import type { ShaggyConnection } from '@/global'
import { translateNow } from '@/i18n'
import { desktopDefaultCwd } from '@/lib/desktop-fs'
import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@/lib/gateway-ws-url'
import { ShaggyGateway } from '@/shaggy'
import {
  $desktopBoot,
...
import type { RpcEvent } from '@/types/shaggy'

interface GatewayBootOptions {
  handleGatewayEvent: (event: RpcEvent) => void
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.shaggyDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: ShaggyGateway | null) => void
  refreshShaggyConfig: () => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: ShaggyConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Shaggy...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Shaggy…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to Shaggy gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshShaggyConfig().catch(() => undefined)
        ...
        if (!cancelled && bootCompleted && consecutiveReconnectFailures >= 6 && !$desktopBoot.get().error) {
          failDesktopBoot('Could not reconnect to the Shaggy gateway. Retry, sign in again, or switch to the local gateway.')
        }
      } finally {
        reconnecting = false
    ...
    const gateway = new ShaggyGateway()
    ...
    async function boot() {
      try {
        ...
        await callbacksRef.current.refreshShaggyConfig()
