...
// bug that drives the REAL useGatewayBoot hook + REAL ShaggyGateway through a
...
function Harness() {
  useGatewayBoot({
    ...
    refreshShaggyConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { shaggyDesktop?: unknown }).shaggyDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { shaggyDesktop?: unknown }).shaggyDesktop
})
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForShaggy) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startShaggy()'s remote branch awaits waitForShaggy() for 45s before it
    ...
    ;(window as { shaggyDesktop?: unknown }).shaggyDesktop = desktop
    ...
    // After ~45s waitForShaggy gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Shaggy backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
