...
import type { ShaggyReadDirEntry, ShaggyReadDirResult } from '@/global'

export type ProjectTreeEntry = ShaggyReadDirEntry
...
async function gitRootFor(start: string) {
  const normalizedStart = clean(start)
  const key = `${desktopFsCacheKey()}:${normalizedStart}`
  let cached = gitRootCache.get(key)

  if (!cached) {
    cached = desktopGitRoot(normalizedStart)
...
function ignoredBy(rules: GitignoreRule[], entry: ShaggyReadDirEntry) {
...
async function filterIgnored(entries: ShaggyReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<ShaggyReadDirResult> {
  if (!window.shaggyDesktop) {
