...
import type { ShaggyReadDirResult } from '@/global'

import { clearProjectDirCache, readProjectDir } from './ipc'
import { resetProjectTreeState, useProjectTree } from './use-project-tree'

const readDir = vi.fn<(path: string) => Promise<ShaggyReadDirResult>>()

beforeEach(() => {
  $connection.set(null)
  resetProjectTreeState()
  readDir.mockReset()
  ;(window as unknown as { shaggyDesktop: { readDir: typeof readDir } }).shaggyDesktop = { readDir }
})
...
afterEach(() => {
  ...
  $connection.set(null)
  resetProjectTreeState()
  delete (window as unknown as { shaggyDesktop?: unknown }).shaggyDesktop
})

function ok(entries: { name: string; path: string; isDirectory: boolean }[]): ShaggyReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { shaggyDesktop: unknown }).shaggyDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: ShaggyReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<ShaggyReadDirResult>(resolve => {
  ...
  it('returns no-bridge gracefully when window.shaggyDesktop is missing', async () => {
    delete (window as unknown as { shaggyDesktop?: unknown }).shaggyDesktop
