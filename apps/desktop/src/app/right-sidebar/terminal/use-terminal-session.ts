...
const SHAGGY_PATHS_MIME = 'application/x-shaggy-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(SHAGGY_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(SHAGGY_PATHS_MIME)
...
export function useTerminalSession({ cwd, onAddSelectionToChat }: UseTerminalSessionOptions) {
  ...
  useEffect(() => {
    ...
    const terminalApi = window.shaggyDesktop?.terminal
    ...
    const term = new Terminal({
      ...
      fontSize: 11,
      lineHeight: 1.12,
      // Full-screen TUIs (shaggy --tui, vim) grab the mouse, so a plain drag
    ...
    const mount = () => {
      ...
      try {
        ...
        console.warn('[shaggy-terminal] WebGL unavailable; falling back to DOM', err)
      }
