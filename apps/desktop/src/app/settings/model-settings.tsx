...
import type { AuxiliaryModelsResponse, ModelOptionProvider, StaleAuxAssignment } from '@/shaggy'
...
// backend now surfaces the full `shaggy model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in shaggy_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `shaggy model` curation). Best-effort — fall through to the refreshed
            ...
            <SelectContent>
              {providerOptions.map(provider => (
                <SelectItem key={provider.slug || 'none'} value={provider.slug || 'none'}>
                  {provider.name}
                  {!isProviderReady(provider) && <span className="ml-1 text-muted-foreground">· set up</span>}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
        ...
        {needsSetup && !setupIsApiKey && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — Shaggy runs the flow for you.`}
          </p>
        )}
