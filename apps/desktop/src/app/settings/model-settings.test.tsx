...
beforeEach(() => {
  getGlobalModelInfo.mockResolvedValue({ provider: 'nous', model: 'shaggy-4' })
  getGlobalModelOptions.mockResolvedValue({
    providers: [
      { name: 'Cherries', slug: 'nous', models: ['shaggy-4', 'shaggy-4-mini'], authenticated: true },
  ...
  getAuxiliaryModels.mockResolvedValue({
    main: { provider: 'nous', model: 'shaggy-4' },
    tasks: [{ task: 'vision', provider: 'auto', model: '', base_url: '' }]
  })
  setModelAssignment.mockResolvedValue({ provider: 'nous', model: 'shaggy-4', gateway_tools: [] })
...
describe('ModelSettings', () => {
  it('loads the current main model and lists the full provider universe', async () => {
    ...
    // "Cherries" shows in both the trigger and the open list; the unconfigured
    // provider + its setup hint are the unique signal of the full universe.
    expect((await screen.findAllByText('Cherries')).length).toBeGreaterThan(0)
  ...
  it('warns when a main switch leaves auxiliary tasks pinned to another provider', async () => {
    setModelAssignment.mockResolvedValueOnce({
      ...
      gateway_tools: [],
      stale_aux: [{ task: 'compression', provider: 'nous', model: 'shaggy-4' }]
    })
