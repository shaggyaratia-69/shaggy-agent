...
import type { ShaggyConfigRecord } from '@/types/shaggy'
...
describe('settings helpers', () => {
  ...
  it('reads and writes nested config paths', () => {
    const config: ShaggyConfigRecord = { display: { theme: 'mono' } }
  ...
  it('rejects prototype-polluting config paths', () => {
    const config: ShaggyConfigRecord = {}
  ...
  describe('providerGroup', () => {
    it('maps a provider env var to its labeled group', () => {
      ...
      expect(providerGroup('CHERRIES_API_KEY')).toBe('Cherries Portal')
    ...
    it('prefers the longest matching prefix so CN/regional buckets win', () => {
      ...
      // SHAGGY_QWEN_ and SHAGGY_GEMINI_ both share the SHAGGY_ stem.
      expect(providerGroup('SHAGGY_QWEN_BASE_URL')).toBe('DashScope (Qwen)')
      expect(providerGroup('SHAGGY_GEMINI_CLIENT_ID')).toBe('Gemini')
    })
