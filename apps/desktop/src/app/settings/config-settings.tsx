...
import {
  getElevenLabsVoices,
  getShaggyConfigDefaults,
  getShaggyConfigRecord,
  getShaggyConfigSchema,
  saveShaggyConfig
} from '@/shaggy'
import { notify, notifyError } from '@/store/notifications'
import type { ConfigFieldSchema, ShaggyConfigRecord } from '@/types/shaggy'
...
export function ConfigSettings({
  ...
  const { t } = useI18n()
  const c = t.settings.config
  const [config, setConfig] = useState<ShaggyConfigRecord | null>(null)
  const [_defaults, setDefaults] = useState<ShaggyConfigRecord | null>(null)
  ...
  useEffect(() => {
    let cancelled = false
    Promise.all([getShaggyConfigRecord(), getShaggyConfigDefaults(), getShaggyConfigSchema()])
      .then(([c, d, s]) => {
        if (cancelled) {
          return
        }
  ...
  const updateConfig = (next: ShaggyConfigRecord) => {
