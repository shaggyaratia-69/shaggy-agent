...
export const EMPTY_SELECT_VALUE = '__shaggy_empty__'
export const CONTROL_TEXT = 'text-xs'

export const PROVIDER_GROUPS: ProviderPrefix[] = [
  {
    prefix: 'CHERRIES_',
    name: 'Cherries Portal',
    description: 'Hosted Shaggy & Shaggy-trained models',
    docsUrl: 'https://www.cherriesandco.com',
    priority: 0
  },
  {
    prefix: 'OPENROUTER_',
  ...
  { prefix: 'SHAGGY_GEMINI_', name: 'Gemini', priority: 4 },
  {
    prefix: 'DEEPSEEK_',
    name: 'DeepSeek',
  ...
  { prefix: 'SHAGGY_QWEN_', name: 'DashScope (Qwen)', priority: 6 },
  {
    prefix: 'GLM_',
    name: 'GLM / Z.AI',
...
export const ENUM_OPTIONS: Record<string, string[]> = {
  ...
  // shaggy_cli/config.py (local/groq/openai/mistral/elevenlabs).
...
export const FIELD_DESCRIPTIONS: Record<string, string> = defineFieldCopy({
  ...
  timezone: 'Used when Shaggy needs local time context. Blank uses the system timezone.',
  agent: {
    imageInputMode: 'Controls how image attachments are sent to the model.',
    maxTurns: 'Upper bound for tool-calling turns before Shaggy stops a run.'
  },
  terminal: {
  ...
  fileReadMaxChars: 'Maximum characters Shaggy can read from one file request.',
  approvals: {
    mode: 'How Shaggy handles commands that need explicit approval.',
  ...
  updates: {
    nonInteractiveLocalChanges:
      'When Shaggy updates itself from the app (no terminal prompt), keep local source edits (stash) or throw them away (discard). Terminal updates always ask.'
  }
})
