...
import { shaggyDirectiveFormatter, type SlashChipKind } from '@/components/assistant-ui/directive-text'
...
import { extractDroppedFiles, partitionDroppedFiles, SHAGGY_PATHS_MIME } from '../hooks/use-composer-actions'
...
export function ChatBar({
  ...
  // cold start ("Starting Shaggy...") from a dropped connection we're trying to
  ...
  const handleDragEnter = (event: ReactDragEvent<HTMLFormElement>) => {
    if (!onAttachDroppedItems || !dragHasAttachments(event.dataTransfer, SHAGGY_PATHS_MIME)) {
      return
    }
