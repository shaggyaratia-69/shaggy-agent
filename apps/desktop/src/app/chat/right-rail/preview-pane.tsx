        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.shaggyDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Shaggy to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.shaggyDesktop?.watchPreviewFile ||
      !window.shaggyDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.shaggyDesktop.onPreviewFileChanged(payload => {
    ...
    void window.shaggyDesktop
      .watchPreviewFile(target.url)
      .then(watch => {
        if (!active) {
          void window.shaggyDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.shaggyDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:shaggy-preview')
