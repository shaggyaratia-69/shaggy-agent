...
import { getShaggyConfig, getShaggyConfigDefaults } from '@/shaggy'
...
interface ShaggyConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
  refreshProjectBranch: (cwd: string) => Promise<void>
}

export function useShaggyConfig({ activeSessionIdRef, refreshProjectBranch }: ShaggyConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)

  const refreshShaggyConfig = useCallback(async () => {
    try {
      const [config, defaults] = await Promise.all([getShaggyConfig(), getShaggyConfigDefaults().catch(() => ({}))])
  ...
  return { refreshShaggyConfig, sttEnabled, voiceMaxRecordingSeconds }
}
