...
import { getProfiles, transcribeAudio } from '@/shaggy'
import {
  $composerAttachments,
...
async function readFileDataUrlForAttach(filePath: string): Promise<string | null> {
  const reader = window.shaggyDesktop?.readFileDataUrl
...
export function usePromptActions({
  ...
  // separate `shaggy gateway` process performs the actual transfer, so we
