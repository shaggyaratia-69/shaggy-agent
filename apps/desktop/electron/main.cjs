...
const USER_DATA_OVERRIDE = process.env.SHAGGY_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.SHAGGY_DESKTOP_DEV_SERVER
...
// switches only apply pre-launch. Override with SHAGGY_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[shaggy] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[shaggy] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
          continue
        }
...
if (INSTALL_STAMP) {
  console.log(
    `[shaggy] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[shaggy] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// SHAGGY_HOME — the user-facing root for everything Shaggy-related. Mirrors
// scripts/install.ps1's $ShaggyHome and scripts/install.sh's $SHAGGY_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\shaggy (matches install.ps1)
//   macOS / Linux: ~/.shaggy (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.shaggy directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\shaggy yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// SHAGGY_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// SHAGGY_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.shaggy / %LOCALAPPDATA%\shaggy.
function resolveShaggyHome() {
  if (process.env.SHAGGY_HOME) return path.resolve(process.env.SHAGGY_HOME)
  if (USER_DATA_OVERRIDE) return path.join(path.resolve(USER_DATA_OVERRIDE), 'shaggy-home')
  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'shaggy')
    const legacy = path.join(app.getPath('home'), '.shaggy')
    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.shaggy setup (no LOCALAPPDATA install yet) so users don't lose state.
    if (!directoryExists(localappdata) && directoryExists(legacy)) return legacy
    return localappdata
  }
  return path.join(app.getPath('home'), '.shaggy')
}

const SHAGGY_HOME = resolveShaggyHome()
// ACTIVE_SHAGGY_ROOT — the canonical mutable Shaggy install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_SHAGGY_ROOT = path.join(SHAGGY_HOME, 'shaggy-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_SHAGGY_ROOT, 'venv')
...
// boots, going straight to `resolveShaggyBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_SHAGGY_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_SHAGGY_ROOT, '.shaggy-bootstrap-complete')
const BOOTSTRAP_MARKER_SCHEMA_VERSION = 1

const DESKTOP_CONNECTION_CONFIG_PATH = path.join(app.getPath('userData'), 'connection.json')
const DESKTOP_UPDATE_CONFIG_PATH = path.join(app.getPath('userData'), 'updates.json')
// active-profile.json records which Shaggy profile the desktop launches its
// local backend as. When set, startShaggy() passes `shaggy --profile <name>
// dashboard …`, which deterministically pins SHAGGY_HOME (see
// _apply_profile_override in shaggy_cli/main.py) and bypasses the sticky
// ~/.shaggy/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors shaggy_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// shaggyDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under SHAGGY_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by shaggy_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(SHAGGY_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (shaggy_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.SHAGGY_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.SHAGGY_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
  if (!Number.isFinite(raw) || raw <= 0) return 650
  return Math.max(120, raw)
})()
const APP_NAME = 'Shaggy'
...
// Seed the native About panel with the live Shaggy version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `shaggy update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveShaggyVersion(),
  copyright: 'Copyright © 2026 Cherries and Co Research'
})
...
const MEDIA_PROTOCOL = 'shaggy-media'
// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(shaggy-media://…)`.
...
let mainWindow = null
let shaggyProcess = null
let connectionPromise = null
// Additional per-profile backends, keyed by profile name. The PRIMARY backend
// (the desktop's launch profile) stays managed by shaggyProcess +
// connectionPromise + startShaggy(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.SHAGGY_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.SHAGGY_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startShaggy() calls (e.g. the renderer's
...
let bootProgressState = {
  ...
  message: 'Waiting to start Shaggy backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function rememberLog(chunk) {
  ...
  if (!text) return
  const lines = text.split(/\r?\n/).map(line => `[shaggy] ${line}`)
  shaggyLog.push(...lines)
  if (shaggyLog.length > 300) {
    shaggyLog.splice(0, shaggyLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-shaggy-wsl-windows-fonts.conf')
    let existing = ''
    try {
...
function broadcastBootProgress() {
  ...
  webContents.send('shaggy:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the shaggy:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('shaggy:bootstrap:event', ev)
}
...
function isShaggySourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'shaggy_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.SHAGGY_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Shaggy deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Shaggy' terminal tool requires
...
function findGitBash() {
  ...
  // install.ps1 drops PortableGit at %LOCALAPPDATA%\shaggy\git\... — checked
  // first so users who installed via install.ps1 are detected before we
  // start probing system-wide locations.
  const localAppData = process.env.LOCALAPPDATA || ''
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'shaggy', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'shaggy', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
// install only has PortableGit under %LOCALAPPDATA%\shaggy\git (never on
...
function resolveGitBinary() {
  ...
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'shaggy', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'shaggy', 'git', 'bin', 'git.exe'))
  }
...
function recentShaggyLog() {
  return shaggyLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's shaggy root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_SHAGGY_ROOT.
// SHAGGY_DESKTOP_SHAGGY_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.SHAGGY_DESKTOP_SHAGGY_ROOT && path.resolve(process.env.SHAGGY_DESKTOP_SHAGGY_ROOT),
    !IS_PACKAGED && isShaggySourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isShaggySourceRoot(ACTIVE_SHAGGY_ROOT) ? ACTIVE_SHAGGY_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_SHAGGY_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('shaggy:updates:progress', merged)
  }
}
...
// SHAGGY_HOME/shaggy-setup.exe on a successful install (see
// apps/bootstrap-installer paths::copy_self_to_shaggy_home). That binary owns
// ALL repo mutation — running `shaggy update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'shaggy-setup.exe' : 'shaggy-setup'
  const candidate = path.join(SHAGGY_HOME, name)
...
// Path to the venv shim whose lock decides whether `shaggy update` can write
// fresh entry points. On Windows this is the file the running backend
// `shaggy.exe` holds open; on POSIX it's never mandatory-locked.
function venvShaggyShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'shaggy.exe')
    : path.join(updateRoot, 'venv', 'bin', 'shaggy')
}
...
// only signals the direct child, so on Windows a backend `shaggy.exe` that
// spawned its own grandchildren (a `shaggy` REPL, a pty terminal session, the
...
// `shaggyProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `shaggy.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a shaggy
// REPL, a pty terminal, the gateway) keep `shaggy.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const pids = []
  if (shaggyProcess && Number.isInteger(shaggyProcess.pid)) pids.push(shaggyProcess.pid)
  ...
  if (shaggyProcess && !shaggyProcess.killed) {
    try {
      shaggyProcess.kill('SIGTERM')
    } catch {
      void 0
    }
  }
  ...
  const shim = venvShaggyShimPath(updateRoot)
...
// `shaggy update`). Instead we spawn the staged Shaggy-Setup binary with
// --update and quit, so it can run `shaggy update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri shaggy-setup. Unlike Windows
      ...
      // whole update itself: `shaggy update` (backend) + `shaggy desktop
    ...
    if (!updater) {
      ...
      // `shaggy desktop`, never the Tauri installer that self-copies
      // shaggy-setup.exe into SHAGGY_HOME). They DO have a working `shaggy`
      ...
      // checkout they're on — bare `shaggy update` defaults to main and would
      ...
      const updateRoot = resolveUpdateRoot()
      let command = 'shaggy update'
      try {
        const head = await runGit(['rev-parse', '--abbrev-ref', 'HEAD'], { cwd: updateRoot })
        const current = (head.stdout || '').trim()
        if (head.code === 0 && current && current !== 'HEAD') {
          const branch = await resolveHealedBranch(updateRoot, current)
          if (branch !== 'main') command = `shaggy update --branch ${branch}`
        }
      } catch {
        // Best-effort: fall back to bare `shaggy update` if branch detection fails.
      }
      rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
      emitUpdateProgress({ stage: 'manual', message: command, percent: null })
      return { ok: true, manual: true, command, shaggyRoot: updateRoot }
    }

    emitUpdateProgress({ stage: 'restart', message: 'Handing off to the Shaggy updater…', percent: 100 })
    ...
    // shaggy.exe (held by the backend child / its grandchildren) and the update
    // bricks. See releaseBackendLockForUpdate for the full failure analysis.
    await releaseBackendLockForUpdate(updateRoot)

    // Detached so the updater outlives this process — it needs us GONE before
    // `shaggy update` will run (the venv shim is locked while we live).
    const child = spawn(updater, updaterArgs, {
      cwd: SHAGGY_HOME,
      env: {
        ...process.env,
        SHAGGY_HOME,
        PATH: [path.join(SHAGGY_HOME, 'node', 'bin'), venvBin, process.env.PATH].filter(Boolean).join(path.delimiter)
      },
      detached: true,
      stdio: 'ignore',
      windowsHide: false
...
// Resolve the shaggy CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `shaggy` on PATH.
function resolveShaggyCliBinary(updateRoot) {
  const venvShaggy = path.join(updateRoot, 'venv', 'bin', 'shaggy')
  if (fileExists(venvShaggy)) return venvShaggy
  return findOnPath('shaggy') || null
}
...
// macOS/Linux in-app update: backend (`shaggy update`) + OS-aware GUI rebuild
// (`shaggy desktop --build-only`), then atomically swap the running .app bundle
// with the freshly built one and relaunch. Degrades to "backend updated,
// restart to load the new GUI" if the swap can't be performed.
async function applyUpdatesPosixInApp() {
  const updateRoot = resolveUpdateRoot()
  const shaggy = resolveShaggyCliBinary(updateRoot)
  if (!shaggy) {
    emitUpdateProgress({ stage: 'manual', message: 'shaggy update', percent: null })
    return { ok: true, manual: true, command: 'shaggy update', shaggyRoot: updateRoot }
  }

  // Put the Shaggy-managed Node and the venv on PATH so `shaggy desktop`'s
  // npm build can find them on a machine with no system Node.
  const extraPath = [path.join(SHAGGY_HOME, 'node', 'bin'), path.join(updateRoot, 'venv', 'bin')]
    .filter(Boolean)
    .join(path.delimiter)
  const env = {
    SHAGGY_HOME,
  ...
  // `shaggy update` reaps stale `shaggy dashboard` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads SHAGGY_DESKTOP_CHILD_PID and excludes
  ...
  if (shaggyProcess && Number.isInteger(shaggyProcess.pid)) {
    desktopChildPids.push(shaggyProcess.pid)
  }
  for (const entry of backendPool.values()) {
  ...
  if (desktopChildPids.length) {
    env.SHAGGY_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Shaggy (git + dependencies)…', percent: 10 })
  const updated = await runStreamedUpdate(shaggy, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'shaggy update failed.', error: updated.error || 'update-failed' })
    return { ok: false, error: 'shaggy update failed' }
  }

  emitUpdateProgress({ stage: 'rebuild', message: 'Rebuilding the desktop app…', percent: 60 })
  const rebuilt = await runStreamedUpdate(shaggy, ['desktop', '--build-only'], {
    cwd: updateRoot,
    env,
    stage: 'rebuild'
  })
  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Shaggy to retry.',
  ...
  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Shaggy.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Shaggy.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Shaggy to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.shaggy-update-new"; then
    rm -rf "$DST.shaggy-update-old" 2>/dev/null || true
    mv "$DST" "$DST.shaggy-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.shaggy-update-new" "$DST"
    rm -rf "$DST.shaggy-update-old" 2>/dev/null || true
  fi
fi
...
/usr/bin/open "$DST"
`
  const scriptPath = path.join(app.getPath('temp'), `shaggy-desktop-update-${Date.now()}.sh`)
  try {
    fs.writeFileSync(scriptPath, swapScript, { mode: 0o755 })
  } catch (err) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Shaggy to load the new version.',
      percent: 100
    })
...
function isBootstrapComplete() {
  ...
  // pinned commit -- users update via the in-app update path or `shaggy
  ...
  return isShaggySourceRoot(ACTIVE_SHAGGY_ROOT) && fileExists(getVenvPython(VENV_ROOT))
}
...
function resolveWebDist() {
  const override = process.env.SHAGGY_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set SHAGGY_DESKTOP_WEB_DIST.`
    )
  }
  return fallback
}
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: shaggy desktop --force-build`
  )
  return candidates[0]
}
...
function resolveShaggyCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Shaggy.app/Contents/...`
  ...
  const candidates = [
    ...
    process.env.SHAGGY_DESKTOP_CWD,
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveShaggyCwd(), sanitized: Boolean(trimmed) }
  }
...
function createPythonBackend(root, label, dashboardArgs, options = {}) {
  ...
  return {
    ...
    label,
    command: python,
    args: ['-m', 'shaggy_cli.main', ...dashboardArgs],
    env: {
...
// createActiveBackend — build a backend pointing at ACTIVE_SHAGGY_ROOT, the
...
function createActiveBackend(dashboardArgs) {
  ...
  return {
    kind: 'python',
    label: `Shaggy at ${ACTIVE_SHAGGY_ROOT}`,
    command: fileExists(venvPython) ? venvPython : findSystemPython(),
    args: ['-m', 'shaggy_cli.main', ...dashboardArgs],
    env: {
      PYTHONPATH: [ACTIVE_SHAGGY_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
    },
    root: ACTIVE_SHAGGY_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveShaggyBackend(dashboardArgs) {
  // 1. Explicit override -- SHAGGY_DESKTOP_SHAGGY_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.SHAGGY_DESKTOP_SHAGGY_ROOT && path.resolve(process.env.SHAGGY_DESKTOP_SHAGGY_ROOT)
  if (overrideRoot && isShaggySourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Shaggy source at ${overrideRoot}`, dashboardArgs)
  ...
  //    installed `shaggy` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isShaggySourceRoot.)
  if (!IS_PACKAGED && isShaggySourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Shaggy source at ${SOURCE_REPO_ROOT}`, dashboardArgs)
    if (backend) return backend
  }

  // 3. Bootstrap-complete ACTIVE_SHAGGY_ROOT -- the canonical install at
  //    %LOCALAPPDATA%\shaggy\shaggy-agent (Windows) or ~/.shaggy/shaggy-agent.
  ...
  //    to spawning shaggy. Updates flow through the in-app update path
  //    (applyUpdates -> git pull) or `shaggy update` from the CLI.
  if (isBootstrapComplete()) {
    return createActiveBackend(dashboardArgs)
  }

  // 4. Existing `shaggy` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    SHAGGY_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.SHAGGY_DESKTOP_IGNORE_EXISTING !== '1') {
    let shaggyCommand = null
    const shaggyOverride = process.env.SHAGGY_DESKTOP_HERMES

    if (shaggyOverride) {
      const resolvedOverride = findOnPath(shaggyOverride)
      if (resolvedOverride) {
        shaggyCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(shaggyOverride, { isWsl: IS_WSL })) {
        shaggyCommand = shaggyOverride
      } else {
        rememberLog(`Ignoring Windows Shaggy override under WSL: ${shaggyOverride}`)
      }
    } else {
      shaggyCommand = findOnPath('shaggy')
    }

    if (shaggyCommand) {
      if (looksLikeDesktopAppBinary(shaggyCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Shaggy CLI: ${shaggyCommand}`)
        shaggyCommand = null
      }
    }

    if (shaggyCommand) {
      // Smoke-test the candidate before trusting it. A `shaggy` shim
      ...
      const shellForProbe = isCommandScript(shaggyCommand)
      if (verifyShaggyCli(shaggyCommand, { shell: shellForProbe })) {
        return {
          label: `existing Shaggy CLI at ${shaggyCommand}`,
          command: shaggyCommand,
          args: dashboardArgs,
          bootstrap: false,
          env: {},
          kind: 'command',
      ...
      rememberLog(
        `Ignoring existing Shaggy CLI at ${shaggyCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed shaggy_cli module via system Python.
  ...
  if (python) {
    ...
    // having shaggy_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\shaggy\shaggy-agent\venv.
    if (canImportShaggyCli(python)) {
      return {
        kind: 'python',
        label: `installed shaggy_cli module via ${python}`,
        command: python,
        args: ['-m', 'shaggy_cli.main', ...dashboardArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: shaggy_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveShaggyBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Shaggy The Agent not installed yet; bootstrap required',
    command: null,
    args: dashboardArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveShaggyBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Shaggy install found; starting first-launch bootstrap')
    ...
    const bootstrapResult = await runBootstrap({
      ...
      shaggyHome: SHAGGY_HOME,
      logRoot: path.join(SHAGGY_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Shaggy install was cancelled.')
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Shaggy bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(SHAGGY_HOME, 'logs', 'desktop.log')} for the full transcript.`
      )
      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startShaggy() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // shaggy:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveShaggyBackend(backend.args))
  }
  ...
  if (!isShaggySourceRoot(ACTIVE_SHAGGY_ROOT)) {
    throw new Error(
      `Shaggy install at ${ACTIVE_SHAGGY_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Shaggy' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\shaggy\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `shaggy` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Shaggy on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch Shaggy.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    // isShaggySourceRoot() and the bootstrap writes the marker only after
    // install.ps1 succeeds. If we hit this, the user (or a deleted venv)
    // broke the invariant; tell them to re-run the install.
    throw new Error(
      `Shaggy venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = venvPython
  backend.label = `Shaggy at ${ACTIVE_SHAGGY_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Shaggy runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function fetchJson(url, token, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Shaggy backend URL protocol: ${parsed.protocol}`))
      return
    }
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Shaggy backend.'
              )
            )
            return
          }
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Shaggy backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options = {}) {
  ...
  // NO ``X-Shaggy-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('shaggy:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveShaggyCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('shaggy:preview-file-changed', payload)
}
...
function sendBackendExit(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('shaggy:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('shaggy:close-preview-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Shaggy...".
function sendPowerResume() {
  if (!mainWindow || mainWindow.isDestroyed()) return
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('shaggy:power-resume')
}
...
function sendOpenUpdatesRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('shaggy:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen) {
  ...
  webContents.send('shaggy:window-state-changed', state)
}
...
const ZOOM_STORAGE_KEY = 'shaggy:desktop:zoomLevel'
...
//
// Hosted Shaggy gateways gate the dashboard behind an OAuth provider (e.g.
// Cherries and Co Research) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``shaggy_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / shaggy #37247).
//     Both are set as HttpOnly cookies (``shaggy_session_at`` ~15 min,
//     ``shaggy_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:shaggy-remote-oauth'
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to Shaggy gateway',
...
function fetchJsonViaOauthSession(url, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Shaggy backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// a valid stored value (pins the root SHAGGY_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.SHAGGY_DESKTOP_REMOTE_URL)

  const remoteToken = decryptDesktopSecret(block.token)
  const authMode = normAuthMode(block.authMode)
  const remoteUrl = envOverride ? String(process.env.SHAGGY_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    // scope is never overridden by SHAGGY_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    // Portal issues a 24h rotating refresh token (shaggy #37247), and the
    ...
    if (!(await hasLiveOauthSession(baseUrl))) {
      const err = new Error(
        'Remote Shaggy gateway uses OAuth, but you are not signed in. ' +
  ...
  if (!token) {
    throw new Error(
      'Remote Shaggy gateway is selected, but no session token is saved. ' +
...
//   2. env override (SHAGGY_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.SHAGGY_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.SHAGGY_DESKTOP_REMOTE_TOKEN
  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'SHAGGY_DESKTOP_REMOTE_URL is set but SHAGGY_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Shaggy backend.'
      )
    }
...
function globalRemoteActive() {
  if (process.env.SHAGGY_DESKTOP_REMOTE_URL) {
    return true
  }
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Shaggy
  ...
  if (authRequired) {
    ...
    // button can read "Sign in with Cherries and Co Research" instead of a generic
...
async function testDesktopConnectionConfig(input = {}) {
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startShaggy())
  ...
  // connect to Shaggy gateway". Mirror the renderer's connect here so the test
...
function resetShaggyConnection() {
...
// startShaggy() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait() {
  // Capture the reference before resetShaggyConnection() nulls shaggyProcess.
  const dying = shaggyProcess && !shaggyProcess.killed ? shaggyProcess : null
  resetShaggyConnection()
...
// profile to startShaggy() (the window backend: boot UI, bootstrap, remote
...
// local-spawn portion of startShaggy() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForShaggy(remote.baseUrl, remote.token)
    return {
      ...remote,
      profile,
      logs: shaggyLog.slice(-80),
      ...getWindowState()
    }
  }
  ...
  // --profile wins over the inherited SHAGGY_HOME env (see _apply_profile_override
  // step 3 in shaggy_cli/main.py), so the child re-homes to this profile.
  const dashboardArgs = ['--profile', profile, 'dashboard', '--no-open', '--host', '127.0.0.1', '--port', String(port)]
  let backend
  let shaggyCwd
  let webDist
  try {
    backend = await ensureRuntime(resolveShaggyBackend(dashboardArgs))
    shaggyCwd = resolveShaggyCwd()
  ...
  rememberLog(`Starting Shaggy backend for profile "${profile}" via ${backend.label}`)
  ...
  child.once('error', error => {
    rememberLog(`Shaggy backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    portPool.release(port)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Shaggy backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)
    portPool.release(port)
    if (!ready) {
      rejectStart?.(
        new Error(`Shaggy backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })

  const baseUrl = `http://127.0.0.1:${port}`
  await Promise.race([waitForShaggy(baseUrl, token), startFailed])
  ready = true
  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Shaggy backend for profile "${profile}"`,
    rememberLog
  })
...
async function startShaggy() {
  ...
  // process, every subsequent startShaggy() call re-throws the same error
  ...
  connectionPromise = (async () => {
    await advanceBootProgress('backend.resolve', 'Resolving Shaggy backend', 8)
    // Resolve for the desktop's primary profile so a per-profile remote
    // override on the active profile is honored (falls back to env / global).
    const remote = await resolveRemoteBackend(primaryProfileKey())
    if (remote) {
      await advanceBootProgress('backend.remote', `Connecting to remote Shaggy backend at ${remote.baseUrl}`, 24)
      await waitForShaggy(remote.baseUrl, remote.token)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Shaggy backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    // deterministic (it wins over the sticky ~/.shaggy/active_profile file) and
    // resolves SHAGGY_HOME the same way `shaggy -p <name>` does on the CLI. An
    ...
    await advanceBootProgress('backend.runtime', 'Resolving Shaggy runtime', 28)
    const backend = await ensureRuntime(resolveShaggyBackend(dashboardArgs))
    const shaggyCwd = resolveShaggyCwd()
    const webDist = resolveWebDist()

    await advanceBootProgress('backend.spawn', `Starting Shaggy backend via ${backend.label}`, 84)
    rememberLog(`Starting Shaggy backend via ${backend.label}`)

    shaggyProcess = spawn(backend.command, backend.args, hiddenWindowsChildOptions({
      cwd: shaggyCwd,
      env: {
        ...process.env,
        // Explicitly pin SHAGGY_HOME for the child so Python's get_shaggy_home()
        // resolves to the SAME location our resolveShaggyHome() picked. Without
        // this pin, Python falls back to ~/.shaggy on every platform — fine on
        // mac/linux (where our default matches), but on Windows our default is
        // %LOCALAPPDATA%\shaggy, which differs from C:\Users\<u>\.shaggy.
        // Mismatch would split config / sessions / .env / logs across two
        // directories. install.ps1 sets SHAGGY_HOME via setx; the desktop
    ...
    shaggyProcess.stdout.on('data', rememberLog)
    shaggyProcess.stderr.on('data', rememberLog)
    let backendReady = false
    let rejectBackendStart = null
    const backendStartFailed = new Promise((_resolve, reject) => {
      rejectBackendStart = reject
    })
    shaggyProcess.once('error', error => {
      rememberLog(`Shaggy backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Shaggy backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    shaggyProcess.once('exit', (code, signal) => {
      rememberLog(`Shaggy backend exited (${signal || code})`)
      shaggyProcess = null
      connectionPromise = null
      portPool.release(port)
      sendBackendExit({ code, signal })
      if (!backendReady) {
        const message = `Shaggy backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Shaggy backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentShaggyLog()}`
          )
        )
      }
    })
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Shaggy backend to become ready', 90)
    await Promise.race([waitForShaggy(baseUrl, token), backendStartFailed])
    backendReady = true
    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      // The exit/error handlers null shaggyProcess when the child dies.
      childAlive: () => shaggyProcess !== null && shaggyProcess.exitCode === null && !shaggyProcess.killed,
      rememberLog
    })
    updateBootProgress({
      phase: 'backend.ready',
      message: 'Shaggy backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
function createWindow() {
  ...
  mainWindow.webContents.once('did-finish-load', () => {
    ...
    startShaggy().catch(error => rememberLog(error.stack || error.message))
  })
}

ipcMain.handle('shaggy:connection', async (_event, profile) => ensureBackend(profile))
...
// "Starting Shaggy…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('shaggy:connection:revalidate', async () => {
  ...
  try {
    ...
    // tick rebuilds a fresh, reachable descriptor. resetShaggyConnection only
    // nulls connectionPromise for a remote (no child to SIGTERM).
    rememberLog('Cached remote Shaggy backend failed liveness probe; dropping stale connection.')
    resetShaggyConnection()
    return { ok: true, rebuilt: true }
  }
})
ipcMain.handle('shaggy:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)
  return { ok: true }
})
ipcMain.handle('shaggy:gateway:ws-url', async (_event, profile) => freshGatewayWsUrl(profile))
ipcMain.handle('shaggy:window:openSession', async (_event, sessionId) => {
...
ipcMain.handle('shaggy:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startShaggy() call restarts the
...
ipcMain.handle('shaggy:bootstrap:repair', async () => {
  // Forceful repair: drop the bootstrap-complete marker so the next
  // startShaggy() re-runs the full installer (refreshing a broken/partial
...
ipcMain.handle('shaggy:bootstrap:cancel', async () => {
...
ipcMain.handle('shaggy:boot-progress:get', async () => bootProgressState)
ipcMain.handle('shaggy:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('shaggy:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('shaggy:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('shaggy:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('shaggy:connection-config:oauth-login', async (_event, rawUrl) => {
...
ipcMain.handle('shaggy:connection-config:oauth-logout', async (_event, rawUrl) => {
...
ipcMain.handle('shaggy:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('shaggy:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
...
ipcMain.handle('shaggy:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('shaggy:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new SHAGGY_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('shaggy:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('shaggy:requestMicrophoneAccess', async () => {
...
ipcMain.handle('shaggy:api', async (_event, request) => {
...
ipcMain.handle('shaggy:notify', (_event, payload) => {
  if (!Notification.isSupported()) return false
  new Notification({
    title: payload?.title || 'Shaggy',
    body: payload?.body || '',
...
ipcMain.handle('shaggy:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('shaggy:readFileText', async (_event, filePath) => {
...
ipcMain.handle('shaggy:selectPaths', async (_event, options = {}) => {
...
ipcMain.handle('shaggy:writeClipboard', (_event, text) => {
  clipboard.writeText(String(text || ''))
  return true
})

ipcMain.handle('shaggy:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('shaggy:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('shaggy:saveClipboardImage', async () => {
...
ipcMain.handle('shaggy:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('shaggy:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('shaggy:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))

ipcMain.on('shaggy:titlebar-theme', (_event, payload) => {
...
ipcMain.handle('shaggy:openExternal', (_event, url) => {
...
// it via writeDefaultProjectDir so resolveShaggyCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('shaggy:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveShaggyCwd()
}))

ipcMain.handle('shaggy:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('shaggy:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('shaggy:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('shaggy:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('shaggy:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('shaggy:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: shaggyLog.slice(-200) }))
...
function terminalShellCommand() {
  // SHAGGY_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.SHAGGY_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Shaggy' TUI into
  ...
  env.TERM_PROGRAM = 'Shaggy'
  env.TERM_PROGRAM_VERSION = app.getVersion()

  // Let a shaggy/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from SHAGGY_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.SHAGGY_DESKTOP_TERMINAL = '1'

  return env
}

function terminalChannel(id, suffix) {
  return `shaggy:terminal:${id}:${suffix}`
}
...
ipcMain.handle('shaggy:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

ipcMain.handle('shaggy:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

ipcMain.handle('shaggy:terminal:start', async (event, payload = {}) => {
  if (!nodePty) {
    throw new Error('PTY support is unavailable. Reinstall desktop dependencies and restart Shaggy.')
  }
...
ipcMain.handle('shaggy:terminal:write', (_event, id, data) => {
...
ipcMain.handle('shaggy:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('shaggy:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('shaggy:updates:check', async () =>
...
ipcMain.handle('shaggy:updates:apply', async (_event, payload) =>
...
ipcMain.handle('shaggy:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('shaggy:updates:branch:set', async (_event, name) => {
  const branch = typeof name === 'string' && name.trim() ? name.trim() : DEFAULT_UPDATE_BRANCH
  writeDesktopUpdateConfig({ branch })
  return { branch }
})

// Resolve the canonical Shaggy version (the one `release.py` bumps in
// shaggy_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Shaggy version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveShaggyVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'shaggy_cli', '__init__.py')
...
// Re-resolve the live Shaggy version and push it into the native About panel
// just before showing it, so an in-place `shaggy update` is reflected without
...
ipcMain.handle('shaggy:version', async () => ({
  appVersion: resolveShaggyVersion(),
  electronVersion: process.versions.electron,
  nodeVersion: process.versions.node,
  platform: process.platform,
  shaggyRoot: resolveUpdateRoot()
}))
...
// the actual removal via `shaggy uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (shaggy_cli/uninstall.py + shaggy_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_SHAGGY_ROOT
  ...
  const fallback = () => ({
    shaggy_home: SHAGGY_HOME,
    agent_installed: isShaggySourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(py, ['-m', 'shaggy_cli.main', 'uninstall', '--gui-summary'], hiddenWindowsChildOptions({
        cwd: agentRoot,
        env: { ...process.env, SHAGGY_HOME, NO_COLOR: '1' },
        stdio: ['ignore', 'pipe', 'ignore']
      }))
      child.stdout.on('data', chunk => {
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Shaggy agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import shaggy_cli` resolves from
  ...
  try {
    await releaseBackendLock(ACTIVE_SHAGGY_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `shaggy-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `shaggy-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('shaggy:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('shaggy:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('shaggy:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('shaggy:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// shaggy:// deep links (e.g. shaggy://blueprint/morning-brief?time=08:00).
// A docs/dashboard "Send to App" button opens this URL; we route it into the
// running app's chat composer. Three delivery paths: macOS 'open-url',
// Win/Linux running-app 'second-instance' (argv), Win/Linux cold-start argv.
// ---------------------------------------------------------------------------
const SHAGGY_PROTOCOL = 'shaggy'
let _pendingDeepLink = null
let _rendererReadyForDeepLink = false

function _extractDeepLink(argv) {
  if (!Array.isArray(argv)) return null
  return argv.find((a) => typeof a === 'string' && a.startsWith(`${SHAGGY_PROTOCOL}://`)) || null
}
...
function handleDeepLink(url) {
  ...
  // shaggy://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('shaggy:deep-link', payload)
...
ipcMain.handle('shaggy:deep-link-ready', () => {
  _rendererReadyForDeepLink = true
  if (_pendingDeepLink) {
    const queued = _pendingDeepLink
    _pendingDeepLink = null
    handleDeepLink(
      `${SHAGGY_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(SHAGGY_PROTOCOL, process.execPath, [
        path.resolve(process.argv[1]),
      ])
    } else {
      app.setAsDefaultProtocolClient(SHAGGY_PROTOCOL)
    }
  } catch (err) {
...
// second-instance argv. Without the lock a second `shaggy://` launch spawns a
...
app.whenReady().then(() => {
  ...
  // Win/Linux cold start: the launching shaggy:// URL is in our own argv.
