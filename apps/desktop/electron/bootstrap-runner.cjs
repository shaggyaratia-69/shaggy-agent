 ...
 *
 * Drives apps/desktop's first-launch install of Shaggy The Agent by spawning
...
function bootstrapCacheDir(shaggyHome) {
  return path.join(shaggyHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.shaggy/shaggy-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(shaggyHome) {
  if (!shaggyHome) return null
  const candidate = path.join(shaggyHome, 'shaggy-agent', 'scripts', installScriptName())
  try {
    fs.accessSync(candidate, fs.constants.R_OK)
    return candidate
  } catch {
...
function cachedScriptPath(shaggyHome, commit) {
  return path.join(bootstrapCacheDir(shaggyHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(commit, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/shaggyaratia-69/shaggy-agent/${commit}/scripts/${scriptName}`
...
async function resolveInstallScript({ installStamp, sourceRepoRoot, shaggyHome, emit, _download = downloadInstallScript }) {
  ...
  const cached = cachedScriptPath(shaggyHome, installStamp.commit)
  try {
  ...
  try {
    ...
    const installed = installedAgentInstallScript(shaggyHome)
    if (installed) {
      emit({
        type: 'log',
        line:
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, shaggyHome } = {}) {
  return new Promise((resolve, reject) => {
    ...
    const child = spawn(ps, fullArgs, hiddenWindowsChildOptions({
      ...
      env: {
        ...process.env,
        // Pass SHAGGY_HOME through so install.ps1 respects the caller's
        // choice rather than re-computing the default.
        SHAGGY_HOME: shaggyHome || process.env.SHAGGY_HOME || ''
      }
    }))
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, shaggyHome } = {}) {
  return new Promise((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, shaggyHome }) {
  const args = ['--dir', activeRoot, '--shaggy-home', shaggyHome]
  if (installStamp && installStamp.branch) {
    args.push('--branch', installStamp.branch)
  }
  if (installStamp && installStamp.commit) {
...
async function fetchManifest({ scriptPath, installerKind, emit, shaggyHome, activeRoot, installStamp }) {
  const isPosix = installerKind === 'posix'
  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, shaggyHome })]
...
async function runStage({ scriptPath, installerKind, stage, emit, shaggyHome, activeRoot, abortSignal, installStamp }) {
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, shaggyHome })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(shaggyHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, shaggyHome, emit })
