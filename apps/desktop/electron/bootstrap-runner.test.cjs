...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'shaggy-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/shaggy-runner-test',
    sourceRepoRoot: null,
    shaggyHome: '/tmp/shaggy-runner-test',
    logRoot: '/tmp/shaggy-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'shaggy-agent', 'scripts')
