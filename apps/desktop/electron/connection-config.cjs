 ...
 *     `X-Shaggy-Session-Token` header; WS uses `?token=`.
...
// shaggy_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - shaggy_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - shaggy_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / shaggy #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-shaggy_session_at', '__Secure-shaggy_session_at', 'shaggy_session_at']
const RT_COOKIE_VARIANTS = ['__Host-shaggy_session_rt', '__Secure-shaggy_session_rt', 'shaggy_session_rt']
...
/**
 * True if any cookie in `cookies` is a shaggy session ACCESS-token cookie
