...
React + Ink terminal UI for Shaggy. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `SHAGGY_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.shaggy/.shaggy_history` or under `SHAGGY_HOME`.
...
```text
ui-tui/
  packages/shaggy-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      shaggy-ink.d.ts    type declarations for @shaggy/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent ShaggyCLI subprocess for slash commands
```
