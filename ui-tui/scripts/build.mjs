...
await build({
  ...
  // Skip the prebuilt @shaggy/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@shaggy/ink': resolve(root, 'packages/shaggy-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The shaggy_cli launcher
