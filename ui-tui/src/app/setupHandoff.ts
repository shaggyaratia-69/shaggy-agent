import type { RunExternalProcess } from '@shaggy/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`shaggy ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching shaggy: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`shaggy ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
