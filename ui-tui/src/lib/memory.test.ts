...
const ENV_KEYS = ['SHAGGY_AUTO_HEAPDUMP', 'SHAGGY_HEAPDUMP_DIR', 'SHAGGY_HEAPDUMP_MAX_BYTES'] as const
...
describe('performHeapDump auto opt-in gate (#21767)', () => {
  ...
  beforeEach(() => {
    ...
    dir = mkdtempSync(join(tmpdir(), 'shaggy-heapdump-test-'))
    process.env.SHAGGY_HEAPDUMP_DIR = dir
  })
  ...
  it('writes diagnostics only for auto-high without SHAGGY_AUTO_HEAPDUMP', async () => {
  ...
  it('writes diagnostics only for auto-critical without SHAGGY_AUTO_HEAPDUMP', async () => {
  ...
  it('writes both diagnostics and snapshot for auto-high when SHAGGY_AUTO_HEAPDUMP=1', async () => {
    process.env.SHAGGY_AUTO_HEAPDUMP = '1'
  ...
  it('accepts truthy spellings (true|yes|on, case-insensitive) as opt-in', async () => {
    for (const value of ['true', 'YES', 'On']) {
      process.env.SHAGGY_AUTO_HEAPDUMP = value
      const result = await performHeapDump('auto-high')
  ...
  it('writes both for manual triggers regardless of SHAGGY_AUTO_HEAPDUMP', async () => {
...
describe('heapdump retention guard (#21767)', () => {
  ...
  beforeEach(() => {
    savedDir = process.env.SHAGGY_HEAPDUMP_DIR
    savedMax = process.env.SHAGGY_HEAPDUMP_MAX_BYTES
    delete process.env.SHAGGY_AUTO_HEAPDUMP
    dir = mkdtempSync(join(tmpdir(), 'shaggy-heapdump-prune-'))
    process.env.SHAGGY_HEAPDUMP_DIR = dir
  })

  afterEach(() => {
    if (savedDir === undefined) {delete process.env.SHAGGY_HEAPDUMP_DIR}
    else {process.env.SHAGGY_HEAPDUMP_DIR = savedDir}

    if (savedMax === undefined) {delete process.env.SHAGGY_HEAPDUMP_MAX_BYTES}
    else {process.env.SHAGGY_HEAPDUMP_MAX_BYTES = savedMax}
  ...
  it('evicts oldest files when total bytes exceed the cap, retaining the newest', async () => {
    ...
    process.env.SHAGGY_HEAPDUMP_MAX_BYTES = String(2 * 1024)
