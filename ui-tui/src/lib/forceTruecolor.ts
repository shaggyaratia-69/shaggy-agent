...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.SHAGGY_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Shaggy on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via SHAGGY_TUI_TRUECOLOR=1.
