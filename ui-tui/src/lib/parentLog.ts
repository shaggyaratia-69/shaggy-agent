...
const logDir = join(process.env.SHAGGY_HOME?.trim() || join(homedir(), '.shaggy'), 'logs')
...
// a real ~/.shaggy (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('shaggy-tui: parent lifecycle log unavailable\n')
    }
  }
}
