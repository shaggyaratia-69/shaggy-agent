"""
Single source of truth for provider identity in Shaggy Agent.
...
2. **Shaggy overlays** — transport type, auth patterns, aggregator flags,
...
# -- Shaggy overlay ----------------------------------------------------------
# Shaggy-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class ShaggyOverlay:
    """Shaggy-specific provider metadata layered on top of models.dev."""
...
SHAGGY_OVERLAYS: Dict[str, ShaggyOverlay] = {
    "openrouter": ShaggyOverlay(
    ...
    "nous": ShaggyOverlay(
        ...
        base_url_override="https://inference-api.cherriesandco.com/v1",
    ),
    "openai-codex": ShaggyOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "xai-oauth": ShaggyOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": ShaggyOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="SHAGGY_QWEN_BASE_URL",
    ),
    "google-gemini-cli": ShaggyOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="cloudcode-pa://google",
    ),
    "lmstudio": ShaggyOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": ShaggyOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": ShaggyOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": ShaggyOverlay(
    ...
    "kimi-for-coding": ShaggyOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": ShaggyOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": ShaggyOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": ShaggyOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": ShaggyOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": ShaggyOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": ShaggyOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": ShaggyOverlay(
        transport="openai_chat",
        base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL",
    ),
    "vercel": ShaggyOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ),
    "opencode": ShaggyOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": ShaggyOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "huggingface": ShaggyOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="HF_BASE_URL",
    ),
    "novita": ShaggyOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "nvidia": ShaggyOverlay(
        transport="openai_chat",
        base_url_override="https://integrate.api.nvidia.com/v1",
        base_url_env_var="NVIDIA_BASE_URL",
    ),
    "xiaomi": ShaggyOverlay(
        transport="openai_chat",
        base_url_env_var="XIAOMI_BASE_URL",
    ),
    "tencent-tokenhub": ShaggyOverlay(
        transport="openai_chat",
    ...
    "azure-foundry": ShaggyOverlay(
        transport="openai_chat",  # default; overridden by api_mode in config
        base_url_env_var="AZURE_FOUNDRY_BASE_URL",
    ),
    "bedrock": ShaggyOverlay(
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "shaggy", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Shaggy overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Shaggy overlay
    ...
    overlay = SHAGGY_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + shaggy extra
    ...
    if overlay is not None:
        # Shaggy-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in SHAGGY_OVERLAYS
    if provider == "bedrock":
