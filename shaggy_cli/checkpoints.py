"""`shaggy checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.shaggy/checkpoints/``.  Actions:

    shaggy checkpoints               # same as `status`
    shaggy checkpoints status        # total size, project count, breakdown
    shaggy checkpoints list          # per-project checkpoint counts + workdir
    shaggy checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    shaggy checkpoints clear [-f]    # nuke the entire base (asks first)
    shaggy checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    shaggy checkpoints
    shaggy checkpoints prune --retention-days 3 --max-size-mb 200
    shaggy checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: shaggy checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``shaggy checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `shaggy checkpoints` → status
