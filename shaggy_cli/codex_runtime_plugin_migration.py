"""Migrate Shaggy' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Shaggy' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Shaggy mcp_servers.<n>.command/args/env  → codex stdio transport
  Shaggy mcp_servers.<n>.url/headers       → codex streamable_http transport
  Shaggy mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Shaggy mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Shaggy-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Shaggy' own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Shaggy config.")
...
# Shaggy keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # Shaggy' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, shaggy_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Shaggy MCP server config to the codex inline-table dict
    ...
    if not isinstance(shaggy_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(shaggy_cfg.get("command"))
    has_url = bool(shaggy_cfg.get("url"))
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (SHAGGY_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Shaggy)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Shaggy' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Shaggy' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Shaggy
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="shaggy-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a SHAGGY_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed shaggy-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.shaggy`` symlink that happens to live under ``/private/var/folders``
...
def _build_shaggy_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Shaggy' own
    ...
    so a shaggy installed under /opt/, /usr/local/, or a venv all work.
    SHAGGY_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # SHAGGY_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_shaggy_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever SHAGGY_HOME its launcher sets
    ...
    # — that would override the launcher's SHAGGY_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("SHAGGY_HOME", tmp_path) would
    ...
    shaggy_home = os.environ.get("SHAGGY_HOME") or ""
    if shaggy_home and _looks_like_test_tempdir(shaggy_home):
        shaggy_home = ""
    if shaggy_home:
        env["SHAGGY_HOME"] = shaggy_home
    # PYTHONPATH passes through so a worktree-launched shaggy finds the
    ...
    env["SHAGGY_QUIET"] = "1"
    env["SHAGGY_REDACT_SECRETS"] = env.get("SHAGGY_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    expose_shaggy_tools: bool = True,
) -> MigrationReport:
    """Translate Shaggy mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        shaggy_config: full ~/.shaggy/config.yaml dict
        ...
        expose_shaggy_tools: when True (default), register Shaggy' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Shaggy for tools
    ...
    shaggy_servers = (shaggy_config or {}).get("mcp_servers") or {}
    if not isinstance(shaggy_servers, dict):
        report.errors.append(
            "mcp_servers in Shaggy config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in shaggy_servers.items():
    ...
    # Inject Shaggy' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Shaggy for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/shaggy_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_shaggy_tools:
        translated["shaggy-tools"] = _build_shaggy_tools_mcp_entry()
        if "shaggy-tools" not in report.migrated:
            report.migrated.append("shaggy-tools")
