"""CLI subcommand: ``shaggy send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(f"shaggy send: cannot read {file_path}: {exc}", file=sys.stderr)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"shaggy send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"shaggy send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"shaggy send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"shaggy send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `shaggy gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.shaggy/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_shaggy_env() -> None:
    """Populate ``os.environ`` from ``~/.shaggy/.env`` AND bridge top-level
    ...
    startup that ``shaggy send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.shaggy/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.shaggy/config.yaml`` into
       ...
       via ``shaggy config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``shaggy send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.shaggy/.env and ~/.shaggy/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "shaggy send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  shaggy send --to telegram \"hello\"\n"
            "  shaggy send --to discord:#ops --file report.md\n"
            "  shaggy send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "shaggy send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `shaggy send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Shaggy "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.shaggy/.env + ~/.shaggy/config.yaml) — no LLM, "
        ...
        epilog=(
            ...
            "  shaggy send --to slack:#eng --subject \"[CI]\" --file build.log\n"
            "  shaggy send --list                  # all platforms\n"
            "  shaggy send --list telegram         # filter by platform\n"
            "\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `shaggy send --list telegram`.",
    )
