...
- ``shaggy_cli.providers``        -- canonical provider identity + overlays
- ``shaggy_cli.model_normalize``  -- per-provider name formatting
...
_SHAGGY_MODEL_WARNING = (
    "Cherries and Co Shaggy 3 & 4 models are NOT agentic and are not designed "
    "for use with Shaggy Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Cherries and Co Shaggy 3 / Shaggy 4 chat families.
# The previous substring check (`"shaggy" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``shaggy-brain:qwen3-14b-ctx16k`` that just
...
def is_nous_shaggy_non_agentic(model_name: str) -> bool:
    ...
    if not model_name:
        return False
    return bool(_NOUS_SHAGGY_NON_AGENTIC_RE.search(model_name))
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``shaggy config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from shaggy_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'shaggy model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'shaggy doctor' — config issues detected:"
    ...
    # Mirrors the same logic in shaggy_cli.runtime_provider.resolve_runtime_provider;
    ...
    shaggy_warn = _check_shaggy_model_warning(new_model)
    if shaggy_warn:
        warnings.append(shaggy_warn)
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from shaggy_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from shaggy_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by shaggy provider ID
    ...
    # --- 1. Check Shaggy-mapped providers ---
    for shaggy_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        pconfig = PROVIDER_REGISTRY.get(shaggy_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (SHAGGY_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from shaggy_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and store.get("credential_pool", {}).get(shaggy_id):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # show up in the picker without requiring a Shaggy release.
        model_ids = curated.get(shaggy_id, [])
        if shaggy_id in _MODELS_DEV_PREFERRED:
            model_ids = _merge_with_models_dev(shaggy_id, model_ids)
        total = len(model_ids)
    ...
    # --- 2. Check Shaggy-only providers (nous, openai-codex, copilot, opencode-go) ---
    from shaggy_cli.providers import SHAGGY_OVERLAYS
    from shaggy_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Shaggy provider ID.
    # SHAGGY_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Shaggy IDs ("copilot").
    _mdev_to_shaggy = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in SHAGGY_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Shaggy slug — e.g. "github-copilot" → "copilot"
        shaggy_slug = _mdev_to_shaggy.get(pid, pid)
        if shaggy_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(shaggy_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, shaggy_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or shaggy_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(shaggy_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", shaggy_slug, exc)
        ...
        # configured.
        if not has_creds and shaggy_slug == "anthropic":
            try:
                ...
                if (shaggy_creds and shaggy_creds.get("accessToken")) or \
        ...
        if shaggy_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = provider_model_ids(shaggy_slug)
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                ...
                model_ids = _ids if _ids is not None else (curated.get(shaggy_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(shaggy_slug, []) or curated.get(pid, [])
        else:
            # Use curated list — look up by Shaggy slug, fall back to overlay key
            model_ids = curated.get(shaggy_slug, []) or curated.get(pid, [])
            # Merge with models.dev for preferred providers (same rationale as above).
            if shaggy_slug in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(shaggy_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": shaggy_slug,
            "name": get_label(shaggy_slug),
            "is_current": shaggy_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "shaggy",
        ...
        seen_slugs.add(shaggy_slug.lower())
        _record_builtin_endpoint(shaggy_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or SHAGGY_OVERLAYS (keeps /model in sync
    # with `shaggy model`).
    try:
        from shaggy_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Shaggy's canonical write key (matches
            ...
            # Shaggy writes ``models:`` as a dict keyed by model id
            # (see shaggy_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Shaggy's own writer uses; a hyphen variant is accepted
            ...
            # active model. Shaggy's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, shaggy_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`shaggy_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from shaggy_cli.models import fetch_openrouter_models
