"""Profile distributions — shareable, packaged Shaggy profiles via git.

A distribution is a Shaggy profile published as a git repository (or
...
* ``shaggy profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``shaggy skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``shaggy profile``, not a parallel tree):

    shaggy profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "shaggy-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            shaggy_requires=str(data.get("shaggy_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.shaggy_requires:
            out["shaggy_requires"] = self.shaggy_requires
        if self.author:
...
def check_shaggy_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Shaggy {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``shaggy profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Shaggy profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Shaggy distribution."
        )

    # Version check up-front so we fail fast
    check_shaggy_requires(manifest.shaggy_requires, shaggy_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.shaggy).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="shaggy_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `shaggy profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`shaggy profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="shaggy_dist_update_") as tmp:
        plan = plan_install(
