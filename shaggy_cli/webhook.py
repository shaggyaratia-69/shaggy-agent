"""shaggy webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.shaggy/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _shaggy_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: shaggy gateway run
"""
...
def webhook_command(args):
    """Entry point for 'shaggy webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: shaggy webhook {subscribe|list|remove|test}")
        print("Run 'shaggy webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (shaggy gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: shaggy webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from shaggy webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (shaggy gateway run)")
