...
``cli.py``'s ``ShaggyCLI`` class. ``ShaggyCLI`` inherits ``CLICommandsMixin`` so
...
from shaggy_constants import display_shaggy_home, is_termux as _is_termux_environment
...
class CLICommandsMixin:
    ...
    def _handle_suggestions_command(self, cmd: str):
        ...
        args = " ".join(tokens)
        try:
            from shaggy_cli.suggestions_cmd import handle_suggestions_command
    ...
    def _handle_blueprint_command(self, cmd: str):
        ...
        try:
            from shaggy_cli.blueprint_cmd import handle_blueprint_command
