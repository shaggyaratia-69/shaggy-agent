...
Profile-aware location: ``$SHAGGY_HOME/logs/dashboard-auth.log``.
...
from ``shaggy_constants`` or other shaggy_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$SHAGGY_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``shaggy_constants.get_shaggy_home`` semantics: env var wins,
    else ``~/.shaggy``. A local copy avoids an import cycle with the
    middleware which lives below ``shaggy_cli``.
    """
    home = os.environ.get("SHAGGY_HOME") or str(Path.home() / ".shaggy")
