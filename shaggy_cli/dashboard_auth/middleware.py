...
from shaggy_cli.dashboard_auth import list_providers
from shaggy_cli.dashboard_auth.audit import AuditEvent, audit_log
from shaggy_cli.dashboard_auth.base import ProviderError, RefreshExpiredError
from shaggy_cli.dashboard_auth.cookies import read_session_cookies
from shaggy_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS
...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    Under a reverse proxy with ``X-Forwarded-Prefix: /shaggy``, the
    ``login_url`` is prefixed (``/shaggy/login?next=...``) so the
    ...
    """
    from shaggy_cli.dashboard_auth.prefix import prefix_from_request
...
def _safe_next_target(request: Request) -> str:
    ...
    # the SPA's own ``sessionStorage["shaggy.lastLocation"]`` fallback
...
async def gated_auth_middleware(
    ...
    # sends only ``shaggy_session_rt``. If we bailed on ``not at`` here we'd
    ...
    if at:
        ...
        # tries to reach Cherries Portal's JWKS; if that's unreachable it raises,
    ...
    if session is None:
        ...
        if refreshed is not None:
            ...
            from shaggy_cli.dashboard_auth.cookies import (
                detect_https,
                set_session_cookies,
            )
            from shaggy_cli.dashboard_auth.prefix import prefix_from_request
        ...
        from shaggy_cli.dashboard_auth.cookies import clear_session_cookies
        from shaggy_cli.dashboard_auth.prefix import prefix_from_request
