"""
Dump command for shaggy CLI.

Outputs a compact, plain-text summary of the user's Shaggy setup
...
from shaggy_cli.config import get_shaggy_home, get_env_path, get_project_root, load_config
...
def _redact(value: str) -> str:
    ...
    ``shaggy dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from shaggy_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(shaggy_home: Path) -> int:
...
def _cron_summary(shaggy_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = shaggy_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"shaggy_home:      {display_shaggy_home()}")
    ...
    toolsets = config.get("toolsets", ["shaggy-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(shaggy_home)}")
    lines.append(f"  skills:             {_count_skills(shaggy_home)}")
