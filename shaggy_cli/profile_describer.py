...
Used by ``shaggy profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``shaggy_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Shaggy Agent kanban board.
...
Rules:
  ...
  - Never write "Shaggy Agent profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_shaggy_home())
        else:
