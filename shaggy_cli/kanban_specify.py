...
Used by ``shaggy kanban specify [task_id | --all]``. Takes a task that
...
* This module intentionally mirrors ``shaggy_cli/goals.py`` — same aux
...
SHAGGY_KANBAN_SPECIFY_MAX_TOKENS = max(
    1500,
    int(os.getenv("SHAGGY_KANBAN_SPECIFY_MAX_TOKENS", "6000")),
)

logger = logging.getLogger(__name__)


_SYSTEM_PROMPT = """You are the Kanban triage specifier for the Shaggy Agent board.
...
def _profile_author() -> str:
    """Mirror of ``shaggy_cli.kanban._profile_author``. Kept local to
    avoid a circular import when kanban.py imports this module."""
    return (
        os.environ.get("SHAGGY_PROFILE")
    ...
    *,
    ...
    try:
        resp = client.chat.completions.create(
            ...
            temperature=0.3,
            max_tokens=SHAGGY_KANBAN_SPECIFY_MAX_TOKENS,
