...
``<root>`` is the **shared Shaggy root** (the parent of any active
...
``shaggy -p <profile>`` joins the same board as the dispatcher that
...
the profile's Shaggy home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``SHAGGY_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``SHAGGY_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``shaggy kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.shaggy``. In Docker / custom
deployments where ``SHAGGY_HOME`` points outside ``~/.shaggy`` (e.g.
``/opt/shaggy``), ``<root>`` is ``SHAGGY_HOME``. Legacy env-var
overrides still work:

* ``SHAGGY_KANBAN_DB`` — pin the database file path directly.
* ``SHAGGY_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``SHAGGY_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``SHAGGY_KANBAN_DB``,
``SHAGGY_KANBAN_WORKSPACES_ROOT``, and ``SHAGGY_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/shaggy-kanban-v1-spec.pdf`` for the full
...
# ``SHAGGY_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``SHAGGY_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("SHAGGY_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
# enough that kebab-case names like ``atm10-server`` or ``shaggy-agent``
...
def kanban_home() -> Path:
    """Return the shared Shaggy root that anchors the kanban board.

    Resolution order:

    1. ``SHAGGY_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_shaggy_root()``, which already returns ``<root>``
       when ``SHAGGY_HOME`` is ``<root>/profiles/<name>``, and returns
       ``SHAGGY_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``SHAGGY_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("SHAGGY_KANBAN_HOME", "").strip()
    if override:
        return Path(override).expanduser()
    from shaggy_constants import get_default_shaggy_root
    return get_default_shaggy_root()
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``shaggy kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``SHAGGY_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``shaggy kanban boards
    ...
    """
    env = os.environ.get("SHAGGY_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``shaggy kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``SHAGGY_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("SHAGGY_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``SHAGGY_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("SHAGGY_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``shaggy kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # within an agent loop that propagated ``SHAGGY_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    max_retries          INTEGER,
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``SHAGGY_SESSION_ID``. NULL
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``SHAGGY_KANBAN_DB`` env → ``SHAGGY_KANBAN_BOARD`` env →
    ...
    try:
        ...
        with _INIT_LOCK:
            ...
            # See shaggy_state._WAL_INCOMPAT_MARKERS for detection logic.
            from shaggy_state import apply_wal_with_fallback
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``shaggy kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``SHAGGY_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from shaggy_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``shaggy --skills ...`` alongside the built-in
    ...
    # `shaggy --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``shaggy kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `shaggy kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``shaggy kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``shaggy kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``shaggy kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``shaggy kanban complete <id>``) works without requiring
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Shaggy
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``shaggy kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Shaggy profile.
    ...
    if not rows:
        return False
    try:
        from shaggy_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
    ...
    *,
    ...
    for row in ready_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        # Skip ready tasks whose assignee is not a real Shaggy profile.
        # `_default_spawn` invokes ``shaggy -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Shaggy
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `shaggy kanban tail` — without
...
def _module_shaggy_argv() -> list[str]:
    """Return the interpreter-bound Shaggy CLI invocation."""
    # ``shaggy_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``shaggy`` package — there is no
    # ``shaggy`` package to import.
    return [sys.executable, "-m", "shaggy_cli.main"]


def _absolute_shaggy_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Shaggy shim."""
...
def _shaggy_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Shaggy executable path.
    ...
    if _IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_shaggy_argv()
    return [_absolute_shaggy_path(path)]


def _resolve_shaggy_argv() -> list[str]:
    """Resolve the ``shaggy`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``$SHAGGY_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("shaggy")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\shaggy.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m shaggy_cli.main`` — fallback for setups where
       Shaggy is launched from a venv and the ``shaggy`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_shaggy_bin`` for the same reason. Kept
    local (not imported from gateway) because ``shaggy_cli`` sits below
    ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("SHAGGY_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _shaggy_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _shaggy_path_argv(resolved_env_bin)
        return _module_shaggy_argv()

    shaggy_bin = _safe_which_no_cwd("shaggy") if _IS_WINDOWS else shutil.which("shaggy")
    if shaggy_bin:
        return _shaggy_path_argv(shaggy_bin)
    return _module_shaggy_argv()


def _kanban_worker_skill_available(shaggy_home: Optional[str]) -> bool:
    """True if the bundled ``kanban-worker`` skill resolves for the home the
    spawned worker will run under.

    The dispatcher injects ``--skills kanban-worker`` into every worker. When
    the worker activates a profile (``shaggy -p <name>``), its ``SKILLS_DIR``
    ...
    # An unset SHAGGY_HOME means the worker falls back to the default root
    # home (``~/.shaggy``), which ships the bundled skill.
    base = _Path(shaggy_home) if shaggy_home else (_Path.home() / ".shaggy")
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``shaggy -p <profile> chat -q ...`` subprocess.
    ...
    ``SHAGGY_KANBAN_DB`` / ``SHAGGY_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject SHAGGY_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `shaggy -p <name>` the
    # _apply_profile_override() runs *before* shaggy_constants is imported.
    # If SHAGGY_HOME is absent from the child's env, get_shaggy_home() falls
    # back to Path.home() / ".shaggy" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from shaggy_cli.profiles import resolve_profile_env
    try:
        env["SHAGGY_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via SHAGGY_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # SHAGGY_HOME never had profiles created.
        pass
    if task.tenant:
        env["SHAGGY_TENANT"] = task.tenant
    env["SHAGGY_KANBAN_TASK"] = task.id
    env["SHAGGY_KANBAN_WORKSPACE"] = workspace
    if task.branch_name:
        env["SHAGGY_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["SHAGGY_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["SHAGGY_KANBAN_CLAIM_LOCK"] = task.claim_lock
    ...
    # that even when the worker activates a profile (`shaggy -p <name>`
    # rewrites SHAGGY_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_shaggy_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["SHAGGY_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["SHAGGY_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["SHAGGY_KANBAN_BOARD"] = resolved_board
    # SHAGGY_PROFILE is the author the kanban_comment tool defaults to.
    # `shaggy -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["SHAGGY_PROFILE"] = profile_arg

    cmd = [
        *_resolve_shaggy_argv(),
        "-p", profile_arg,
        # Worker subprocesses switch to a profile-scoped SHAGGY_HOME above,
    ...
    if _kanban_worker_skill_available(env.get("SHAGGY_HOME")):
    ...
    # `shaggy kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`shaggy` executable not found on PATH. "
            "Install Shaggy Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``shaggy kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled SHAGGY_PROFILE values like "shaggy-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Shaggy root exists

    Reads profile paths directly so this module has no import dependency on
    ``shaggy_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from shaggy_constants import get_default_shaggy_root
        default_root = get_default_shaggy_root()
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``shaggy kanban assignees`` for the terminal.
