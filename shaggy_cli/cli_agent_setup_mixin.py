"""Agent-construction and session-resume display methods for ``ShaggyCLI``.

Extracted from ``cli.py`` as part of the god-file decomposition campaign
(``~/.shaggy/plans/god-file-decomposition.md``, Phase 4 step 2). This mixin holds
...
Behavior-neutral: every method is lifted verbatim from ``ShaggyCLI``. ``self.*``
...
class CLIAgentSetupMixin:
    """Agent construction + session-resume display methods for ``ShaggyCLI``."""
    ...
    def _init_agent(self, *, model_override: str = None, runtime_override: dict = None, request_overrides: dict | None = None) -> bool:
        ...
        from shaggy_cli.mcp_startup import wait_for_mcp_discovery
        ...
        if self._resumed and self._session_db and not self.conversation_history:
            ...
            if not session_meta:
                if _quiet_mode:
                    ...
                    print(
                        "Use a session ID from a previous CLI run (shaggy sessions list).",
                        file=sys.stderr,
                    )
                else:
        ...
        try:
            ...
            # + fail-open inside the helper; harmless for non-Cherries providers.
            try:
