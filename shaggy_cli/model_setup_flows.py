"""Per-provider model-selection wizard flows for ``shaggy setup`` / ``shaggy model``.

Extracted from ``shaggy_cli/main.py`` as part of the god-file decomposition
campaign (``~/.shaggy/plans/god-file-decomposition.md``, Phase 2 — splitting
...
in main.py re-imports them (``from shaggy_cli.model_setup_flows import *``-style
explicit import) so existing call sites — and test monkeypatches that target
``shaggy_cli.main._model_flow_*`` — keep resolving against main.py's namespace.
...
imported lazily inside the flows (``from shaggy_cli.main import ...`` resolves at
call time, when main.py is fully loaded) so this module never imports
``shaggy_cli.main`` at import time -> no import cycle.
"""
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from shaggy_cli.main import _prompt_api_key
...
def _model_flow_nous(config, current_model="", args=None):
    """Cherries Portal provider: ensure logged in, then pick model."""
    from shaggy_cli.auth import (
    ...
    if not state or not state.get("access_token"):
        print("Not logged into Cherries Portal. Starting login...")
        print()
        try:
    ...
    if not model_ids:
        print("No curated models available for Cherries Portal.")
        return
    ...
    try:
        ...
        if relogin:
            ...
            print("Re-authenticating with Cherries Portal...\n")
            try:
                mock_args = argparse.Namespace(
                    portal_url=None,
                    inference_url=None,
    ...
    if free_tier:
        try:
            from shaggy_cli.nous_account import (
            ...
            unavailable_message = (
                format_nous_portal_entitlement_message(
                    _account_info,
                    capability="paid Cherries models",
                )
                or ""
            )
        except Exception:
    ...
    if not model_ids and not unavailable_models:
        print("No models available for Cherries Portal after filtering.")
        return

    if free_tier and not model_ids:
        print("No free models currently available.")
        if unavailable_models:
            from shaggy_cli.auth import DEFAULT_CHERRIES_PORTAL_URL

            _url = (_nous_portal_url or DEFAULT_CHERRIES_PORTAL_URL).rstrip("/")
    ...
    if selected:
        _save_model_choice(selected)
        # Reactivate Cherries as the provider and update config
        ...
        save_config(config)
        print(f"Default model set to: {selected} (via Cherries Portal)")
        # Offer Tool Gateway enablement for paid subscribers
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from shaggy_cli.main import _DEFAULT_QWEN_PORTAL_MODELS
    from shaggy_cli.auth import (
...
def _model_flow_custom(config):
    ...
    """
    from shaggy_cli.main import _auto_provider_name, _prompt_custom_api_mode_selection, _save_custom_provider
    ...
    from shaggy_cli.secret_prompt import masked_secret_prompt
...
def _model_flow_named_custom(config, provider_info):
    ...
    """
    from shaggy_cli.main import _custom_provider_api_key_config_value, _custom_provider_base_url_config_value, _save_custom_provider
    from shaggy_cli.auth import _save_model_choice, deactivate_provider
    from shaggy_cli.config import load_config, save_config
    from shaggy_cli.models import fetch_api_models
...
def _model_flow_copilot(config, current_model=""):
    ...
    from shaggy_cli.main import _current_reasoning_effort, _prompt_reasoning_effort_selection, _set_reasoning_effort
    from shaggy_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    if not api_key:
        ...
        if source in {"GITHUB_TOKEN", "GH_TOKEN"}:
            from shaggy_cli.env_loader import format_secret_source_suffix
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from shaggy_cli.main import _infer_stepfun_region, _prompt_api_key, _prompt_provider_choice, _stepfun_base_url_for_region
    from shaggy_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    from shaggy_cli.models import _PROVIDER_MODELS, fetch_api_models
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    ...
                    "OpenRouter (free models), Groq, Cherries."
                )
                print()
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from shaggy_cli.main import _run_anthropic_oauth_flow
    from shaggy_cli.auth import (
        _prompt_model_selection,
        _save_model_choice,
        deactivate_provider,
