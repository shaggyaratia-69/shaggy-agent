"""
Shaggy Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m shaggy_cli.main web          # Start on http://127.0.0.1:9119
    python -m shaggy_cli.main web --port 8080
"""
...
try:
    ...
    # running `shaggy dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["SHAGGY_WEB_DIST"]) if "SHAGGY_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
_log = logging.getLogger(__name__)

app = FastAPI(title="Shaggy Agent", version=__version__)
...
# or SHAGGY_DASHBOARD_TUI=1.  Set from :func:`start_server`.
...
# streamed to a per-action log file under ``~/.shaggy/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_shaggy_home() / "logs"

# Short ``name`` (from the URL) → absolute log file path.
_ACTION_LOG_FILES: Dict[str, str] = {
    "gateway-restart": "gateway-restart.log",
    "shaggy-update": "shaggy-update.log",
}
...
def _spawn_shaggy_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``shaggy <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``shaggy_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "shaggy_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "SHAGGY_NONINTERACTIVE": "1"},
    }
...
async def restart_gateway():
    """Kick off a ``shaggy gateway restart`` in the background."""
    try:
        proc = _spawn_shaggy_action(["gateway", "restart"], "gateway-restart")
...
@app.post("/api/shaggy/update")
async def update_shaggy():
    """Kick off ``shaggy update`` in the background."""
    try:
        proc = _spawn_shaggy_action(["update"], "shaggy-update")
    except Exception as exc:
        _log.exception("Failed to spawn shaggy update")
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Shaggy supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# in shaggy_cli/config.py — listed here for deterministic ordering in the UI.
...
async def set_model_assignment(body: ModelAssignment):
    ...
    Writes to ``~/.shaggy/config.yaml`` — applies to **new** sessions only.
...
# the canonical ``shaggy auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    Shaggy resolves Anthropic creds in this order at runtime:
    1. ``~/.shaggy/.anthropic_oauth.json`` — Shaggy-managed PKCE flow
    ...
    try:
        from agent.anthropic_adapter import (
            read_shaggy_oauth_credentials,
            read_claude_code_credentials,
            _SHAGGY_OAUTH_FILE,
        )
    except ImportError:
        read_claude_code_credentials = None  # type: ignore
        read_shaggy_oauth_credentials = None  # type: ignore
        _SHAGGY_OAUTH_FILE = None  # type: ignore

    shaggy_creds = None
    if read_shaggy_oauth_credentials:
        try:
            shaggy_creds = read_shaggy_oauth_credentials()
        except Exception:
            shaggy_creds = None
    if shaggy_creds and shaggy_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "shaggy_pkce",
            "source_label": f"Shaggy PKCE ({_SHAGGY_OAUTH_FILE})",
            "token_preview": _truncate_token(shaggy_creds.get("accessToken")),
            "expires_at": shaggy_creds.get("expiresAt"),
            "has_refresh_token": bool(shaggy_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Shaggy even
    when they also have a separate Shaggy-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "pkce",
        "cli_command": "shaggy auth add anthropic",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "shaggy auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "shaggy auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "shaggy auth add minimax-oauth",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from shaggy_cli import auth as hauth
        if provider_id == "nous":
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("shaggy_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same Shaggy-managed PKCE file
    # AND forget the Claude Code import. We don't touch ~/.claude/* directly
    # — that's owned by the Claude Code CLI; users can re-auth there if they
    # want to undo a disconnect.
    if provider_id in {"anthropic", "claude-code"}:
        try:
            from agent.anthropic_adapter import _SHAGGY_OAUTH_FILE
            if _SHAGGY_OAUTH_FILE.exists():
                _SHAGGY_OAUTH_FILE.unlink()
        except Exception:
            pass
        # Also clear the credential pool entry if present.
        try:
            from shaggy_cli.auth import clear_provider_auth
...
#          → persists to ~/.shaggy/.anthropic_oauth.json AND credential pool
...
# Guarded so shaggy web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Shaggy file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``shaggy auth add anthropic``.
    """
    from agent.anthropic_adapter import _SHAGGY_OAUTH_FILE
    payload = {
    ...
    _SHAGGY_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    _SHAGGY_OAUTH_FILE.write_text(json.dumps(payload, indent=2), encoding="utf-8")
...
def _submit_anthropic_pkce(session_id: str, code_input: str) -> Dict[str, Any]:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "shaggy-dashboard/1.0",
        },
        method="POST",
    )
    try:
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            os.getenv("SHAGGY_PORTAL_BASE_URL")
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        from shaggy_cli.auth import persist_nous_credentials
...
async def get_logs(
    ...
    from shaggy_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_shaggy_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from shaggy_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_dicts() -> List[Dict[str, Any]]:
    ...
    from shaggy_cli import profiles as profiles_mod
    try:
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, SHAGGY_HOME)."""
    from shaggy_cli import profiles as profiles_mod
...
def _annotate_cron_job(job: Dict[str, Any], profile: str, home: Path) -> Dict[str, Any]:
    ...
    annotated["shaggy_home"] = str(home)
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process SHAGGY_HOME at import time. The dashboard is a single
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_shaggy_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "shaggy setup" if name == "default" else f"{name} setup"
...
async def get_skills():
    ...
    from shaggy_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from shaggy_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
async def get_toolsets():
    from shaggy_cli.tools_config import (
...
#
# The endpoint spawns the same ``shaggy --tui`` binary the CLI uses, behind
...
try:
    from shaggy_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
def _resolve_chat_argv(
    ...
    Default: whatever ``shaggy --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``SHAGGY_TUI_RESUME`` env var —
    matching what ``shaggy_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    `sidecar_url` (when set) is forwarded as ``SHAGGY_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).
    """
    from shaggy_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("SHAGGY_TUI_DISABLE_MOUSE", "1")
    env.setdefault("SHAGGY_TUI_INLINE", "1")

    if resume:
        latest_resume, _latest_path = _session_latest_descendant(resume)
        if latest_resume:
            resume = latest_resume
        env["SHAGGY_TUI_RESUME"] = resume

    if sidecar_url:
        env["SHAGGY_TUI_SIDECAR_URL"] = sidecar_url
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Shaggy inside WSL2 to use the dashboard's /chat "
...
# SHAGGY_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Returns a string like ``"/shaggy"`` (no trailing slash) or ``""`` when
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/shaggy/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /shaggy`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__SHAGGY_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/shaggy``)
        or empty string when served at root.
        """
        html = _index_path.read_text()
        chat_js = "true" if _DASHBOARD_EMBEDDED_CHAT_ENABLED else "false"
        token_script = (
            f'<script>window.__SHAGGY_SESSION_TOKEN__="{_SESSION_TOKEN}";'
            f"window.__SHAGGY_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
            f'window.__SHAGGY_BASE_PATH__="{prefix}";</script>'
        )
        if prefix:
    ...
    # under ``/shaggy`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Shaggy backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Cherries Cyanotype",  "description": "Cream paper, cobalt ink, and printed Cherries and Co texture"},
    {"name": "default-large", "label": "Cherries Cyanotype (Large)", "description": "Cherries Cyanotype with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.shaggy/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.shaggy/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_shaggy_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.shaggy/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as shaggy_cli.plugins):
    1. User plugins:    ~/.shaggy/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.shaggy/plugins/  (only if SHAGGY_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from shaggy_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_shaggy_home() / "plugins", "user"),
        (bundled_root / "memory", "bundled"),
        (bundled_root, "bundled"),
    ]
    if os.environ.get("SHAGGY_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".shaggy" / "plugins", "project"))
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from shaggy_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_shaggy_home() / "plugins").resolve()
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"shaggy auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from shaggy_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from shaggy_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from shaggy_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from shaggy_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"shaggy_dashboard_plugin_{plugin['name']}"
...
def start_server(
    ...
    *,
    ...
    print(f"  Shaggy Web UI → http://{host}:{port}")
