"""
Shaggy Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``shaggy desktop`` / ``shaggy gui``)
     Built inside the agent checkout under ``$SHAGGY_HOME/shaggy-agent/``:
       ...
       - ``$SHAGGY_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Shaggy.app`` or ``~/Applications/Shaggy.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Shaggy`` (NSIS per-user)
...
the app name ("Shaggy"), separate from ``$SHAGGY_HOME``:
  - macOS:   ``~/Library/Application Support/Shaggy``
  - Windows: ``%APPDATA%\\Shaggy``
  - Linux:   ``$XDG_CONFIG_HOME/Shaggy`` (default ``~/.config/Shaggy``)
...
``shaggy uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(shaggy_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return shaggy_home / "shaggy-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Shaggy"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Shaggy"
...
def source_built_gui_artifacts(shaggy_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``shaggy desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``shaggy-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(shaggy_home)
    ...
    return [
        ...
        shaggy_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Shaggy" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Shaggy.app"),
            home / "Applications" / "Shaggy.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Shaggy).
            local_base / "Programs" / "Shaggy",
            # Older / alternate layout some builds used.
            local_base / "shaggy-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Shaggy")
    else:
        ...
        paths += [
            data_base / "applications" / "shaggy.desktop",
            data_base / "applications" / "Shaggy.desktop",
        ]
    return paths


def agent_is_installed(shaggy_home: Path) -> bool:
    """Return True when a usable Python agent install exists under SHAGGY_HOME.
    ...
    if (agent_root / "shaggy_cli").is_dir():
        return True
...
def gui_is_installed(shaggy_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(shaggy_home):
        if p.exists():
            return True
...
def gui_install_summary(shaggy_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = shaggy_home if shaggy_home is not None else get_shaggy_home()
...
def uninstall_gui(shaggy_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``shaggy-agent/shaggy_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$SHAGGY_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove shaggy' or "
            "'sudo dnf remove shaggy'). AppImage builds are a single file you "
