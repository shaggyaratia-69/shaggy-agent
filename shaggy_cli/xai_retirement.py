...
Pure logic: walks a Shaggy config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `shaggy doctor` and a future `shaggy migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Shaggy config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Shaggy config and return retirement issues.
