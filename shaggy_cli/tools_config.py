"""
Unified tool configuration for Shaggy Agent.

`shaggy tools` and `shaggy setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.shaggy/config.yaml under
...
from shaggy_cli.nous_subscription import (
...
from shaggy_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _SHAGGY_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `shaggy tools` → Video Generation, which walks
...
# `shaggy tools` → X (Twitter) Search setup walks users through credential
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from shaggy_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `shaggy tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``shaggy tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from shaggy_cli.plugins import discover_plugins, get_plugin_toolsets
...
from shaggy_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "web": {
        ...
        #   - "Cherries Subscription" — managed Firecrawl billed via Nous
    ...
    "x_search": {
        ...
        "setup_note": (
            "Shaggy routes X searches through xAI's built-in x_search "
    ...
    "browser": {
        ...
        #   - "Cherries Subscription (Browser Use cloud)" — managed Browser Use
        #     billed via Cherries subscription (requires_nous_auth +
        ...
        "providers": [
            {
                "name": "Cherries Subscription (Browser Use cloud)",
                "badge": "subscription",
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. SHAGGY_CUA_DRIVER_VERSION is an
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``shaggy update`` and
      by ``shaggy computer-use install --upgrade``.
    ...
    if _plat.system() != "Darwin":
        if upgrade:
            # Silent on non-macOS — `shaggy update` calls this for every
    ...
    try:
        ...
        if result.returncode == 0 and shutil.which("cua-driver"):
            if verbose:
                ...
                _print_info("      System Settings > Privacy & Security > Accessibility")
                _print_info("      System Settings > Privacy & Security > Screen Recording")
                _print_info("    Both must allow the terminal / Shaggy process.")
            return True
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_shaggy_home()}/shaggy-agent && npm install")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in shaggy-agent directory)")
            return
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/nousresearch/shaggy-agent:latest"
            )
            return
        ...
        if not camofox_dir.exists() and _npm_bin:
            ...
            # package.json so that `shaggy update` does not silently pull
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.shaggy/config.yaml")
        ...
        # Run the full `shaggy auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.shaggy/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from shaggy_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: shaggy auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `shaggy auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: shaggy auth spotify")
        except Exception as exc:
        ...
        try:
            ...
            _print_info("    Run later: shaggy auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `shaggy auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: shaggy auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: shaggy auth add xai-oauth"
                )
        else:
...
def _get_platform_tools(
    ...
    *,
    ...
    # "shaggy-cli" (which include all _SHAGGY_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[shaggy-cli, spotify]`` after enabling Spotify via ``shaggy
        ...
        # (e.g. "shaggy-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``shaggy tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `shaggy tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"shaggy-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("shaggy-")}
    ...
    # shipped as a bundled plugin but user must opt in via `shaggy tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `shaggy tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by shaggy tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (shaggy-cli, shaggy-telegram, etc.)
    ...
    # Opening `shaggy tools` is the user's opt-in to reconfigure tools, so treat
...
# ("Cherries Subscription" managed-gateway entry, "Firecrawl Self-Hosted")
...
# ("Cherries Subscription", "Local Browser", "Camofox") — see the comment block
...
def _visible_providers(cat: dict, config: dict) -> list[dict]:
    ...
    # later) so the picker lists them alongside FAL / Cherries Subscription.
    ...
    # remaining hardcoded rows ("Cherries Subscription", "Firecrawl
    ...
    # the hardcoded "Cherries Subscription" / "Local Browser" / "Camofox" rows
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `shaggy tools` gets a silent no-op
...
def _configure_provider(provider: dict, config: dict):
    ...
    if provider.get("requires_nous_auth"):
        ...
        if not features.nous_auth_present:
            _print_warning("  Cherries Subscription is only available after logging into Nous Portal.")
            return
    ...
    if not env_vars:
        ...
        if managed_feature:
            _print_info("  Requests for this tool will be billed to your Cherries subscription.")
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `shaggy tools` and `shaggy setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Shaggy Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://shaggyaratia-69.github.io/shaggy-agent/docs/user-guide/features/tools", Colors.DIM))
    print()
        ...
        for pkey in enabled_platforms:
            ...
            if managed_nous_tools_enabled():
                for ts_key in sorted(auto_configured):
                    ...
                    print(color(f"  ✓ {label}: using your Cherries subscription defaults", Colors.GREEN))
    ...
    print()
    from shaggy_constants import display_shaggy_home
    print(color(f"  Tool configuration saved to {display_shaggy_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'shaggy' or gateway restart.", Colors.DIM))
    print()
