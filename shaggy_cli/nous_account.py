"""Normalized Cherries Portal account entitlement helpers."""
...
CherriesAccountInfoSource = Literal["jwt", "account_api", "inference_key", "none", "error"]
...
_account_info_cache: tuple[str, float, "CherriesPortalAccountInfo"] | None = None
_ACCOUNT_INFO_CACHE_LOCK = threading.Lock()


@dataclass(frozen=True)
class CherriesPortalSubscriptionInfo:
...
@dataclass(frozen=True)
class CherriesPaidServiceAccessInfo:
...
@dataclass(frozen=True)
class CherriesPortalAccountInfo:
    logged_in: bool
    source: CherriesAccountInfoSource
    fresh: bool
    ...
    subscription: Optional[CherriesPortalSubscriptionInfo] = None
    paid_service_access: Optional[bool] = None
    paid_service_access_info: Optional[CherriesPaidServiceAccessInfo] = None
    tool_access: Optional[CherriesToolAccessInfo] = None
...
def nous_portal_billing_url(account_info: Optional[CherriesPortalAccountInfo] = None) -> str:
    """Return the billing URL for a normalized Cherries account snapshot."""
    try:
        from shaggy_cli.auth import DEFAULT_CHERRIES_PORTAL_URL
    except Exception:
        DEFAULT_CHERRIES_PORTAL_URL = "https://www.cherriesandco.com"

    base = None
    if account_info is not None:
        base = account_info.portal_base_url
    if not isinstance(base, str) or not base.strip():
        base = DEFAULT_CHERRIES_PORTAL_URL
    return f"{base.rstrip('/')}/billing"


def format_nous_portal_entitlement_message(
    account_info: Optional[CherriesPortalAccountInfo],
    *,
    capability: str = "this feature",
    include_refresh_hint: bool = True,
    coverage_category: Optional[str] = None,
) -> Optional[str]:
    """Return user-facing guidance for a missing Cherries tool-gateway entitlement.
    ...
    if account_info is not None:
        if coverage_category is not None:
            ...
            if account_info.tool_gateway_entitled:
                ...
                return (
                    f"{capability} isn't included with your current Cherries Portal "
    ...
    if account_info is None:
        return (
            f"Shaggy could not verify your Cherries Portal entitlement, so {capability} "
            f"is unavailable. Run `shaggy model` to refresh your login, or check "
    ...
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                f"Cherries inference credentials are configured, but Shaggy cannot verify "
                f"your Cherries Portal paid access for {capability}. Log in with "
                f"`shaggy model` to enable Portal-managed features. Billing and "
                f"credits are managed at {billing_url}."
            )
        return (
            f"Log in to Cherries Portal to use {capability}: run `shaggy model`. "
            f"Billing and credits are managed at {billing_url}."
        )

    if account_info.paid_service_access is None:
        detail = (
            f"Shaggy could not verify your Cherries Portal paid access, so {capability} "
            f"is unavailable."
        )
        if account_info.error:
            detail += f" Account lookup failed: {account_info.error}."
        if include_refresh_hint:
            detail += " Run `shaggy model` to refresh your session."
    ...
    if reason == "account_missing":
        return (
            f"Shaggy could not find a Cherries Portal account or organisation for this "
            f"login, so {capability} is unavailable. Run `shaggy model` to "
            f"authenticate again; if the problem persists, contact Cherries support."
        )
    ...
    if reason == "no_usable_credits" or account_info.paid_service_access is False:
        ...
        if include_refresh_hint and not account_info.fresh:
            message += " If you recently bought credits, run `shaggy model` to refresh Shaggy."
        return message

    return (
        f"Your Cherries Portal account does not currently have paid service access, "
        f"so {capability} is unavailable. Add credits or update billing at {billing_url}."
    )


def _no_paid_access_message(
    account_info: CherriesPortalAccountInfo,
    capability: str,
    billing_url: str,
) -> str:
    access = account_info.paid_service_access_info
    ...
    if has_active_subscription and active_subscription_is_paid:
        ...
        return (
            f"Your Cherries Portal credits are exhausted{credit_detail}, so {capability} "
    ...
    if has_active_subscription and active_subscription_is_paid is False:
        return (
            f"Your current Cherries Portal plan does not include paid service access, "
    ...
    if has_active_subscription is False:
        credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
        return (
            f"Your Cherries Portal account has no active subscription or usable credits"
    ...
    credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
    return (
        f"Your Cherries Portal account has no usable paid credits{credit_detail}, so "
    ...
    *,
...
) -> CherriesPortalAccountInfo:
    """Return normalized Cherries Portal account entitlement information.
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        ...
        if pool_info is not None:
            return pool_info
        return CherriesPortalAccountInfo(
            logged_in=False,
            source="none",
            fresh=False,
            portal_base_url=portal_base_url,
...
def _fresh_account_info(
    *,
    ...
    try:
        from shaggy_cli.auth import get_provider_auth_state, resolve_nous_access_token
...
def _info_from_inference_key_pool(
    portal_base_url: Optional[str],
) -> Optional[CherriesPortalAccountInfo]:
    """Return an explicit unknown-entitlement snapshot for opaque Cherries keys."""
    try:
...
def _fetch_nous_account_info(
    ...
    base = (portal_base_url or "https://www.cherriesandco.com").rstrip("/")
...
def _info_from_valid_jwt(
    ...
    *,
    ...
    try:
        from shaggy_cli.auth import _decode_jwt_claims
    except Exception:
        return None
    ...
    access_info = CherriesPaidServiceAccessInfo(
        allowed=paid_access,
...
def _tool_access_from_value(value: Any) -> Optional[CherriesToolAccessInfo]:
    """Parse a Portal ``tool_access`` object (from the JWT claim or the account
    API) into :class:`CherriesToolAccessInfo`. Fails closed: a non-object value
    ...
    return CherriesToolAccessInfo(enabled=enabled, coverage=coverage)


def _subscription_from_payload(value: Any) -> Optional[CherriesPortalSubscriptionInfo]:
    if not isinstance(value, dict):
        return None
    return CherriesPortalSubscriptionInfo(
...
def _paid_service_access_from_payload(value: Any) -> Optional[CherriesPaidServiceAccessInfo]:
    if not isinstance(value, dict):
        return None
    allowed = _coerce_bool(value.get("allowed"))
    paid_access = _coerce_bool(value.get("paid_access"))
    return CherriesPaidServiceAccessInfo(
        allowed=allowed,
        paid_access=paid_access,
