"""``shaggy mcp`` subcommand parser.

Extracted from ``shaggy_cli/main.py:main()`` (god-file Phase 2 follow-up).
...
from shaggy_cli.subcommands._shared import add_accept_hooks_flag
...
def build_mcp_parser(subparsers, *, cmd_mcp: Callable) -> None:
    ...
    # ── Catalog (Cherries-approved MCPs shipped with the repo) ─────────────────
    mcp_sub.add_parser(
        "picker",
        help="Interactive catalog picker (also the default for `shaggy mcp`)",
    )
    mcp_sub.add_parser(
        "catalog",
        help="List Cherries-approved MCPs available for one-click install",
    )
    mcp_install_p = mcp_sub.add_parser(
        "install",
        help="Install a catalog MCP by name (e.g. `shaggy mcp install n8n`)",
    )
