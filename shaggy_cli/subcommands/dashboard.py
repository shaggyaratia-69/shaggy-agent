"""``shaggy dashboard`` subcommand parser.
...
def build_dashboard_parser(
    ...
    # Backward-compat shim: older Shaggy desktop app shells (<= 0.15.x) spawn the
    # backend as `shaggy dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Shaggy couldn't start" with no actionable cause. Accept and silently
    ...
    # `shaggy dashboard register` — register a self-hosted dashboard OAuth
    # client with Cherries Portal and write the client_id into ~/.shaggy/.env.
    # Nested subparser so bare `shaggy dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        "register",
        help="Register a self-hosted dashboard with Cherries Portal (writes the OAuth client ID to .env)",
        description=(
            "Register this install as a self-hosted dashboard with your Cherries "
            "Portal account. Creates an OAuth client, writes "
            "SHAGGY_DASHBOARD_OAUTH_CLIENT_ID into ~/.shaggy/.env, and prints "
            "how to engage the login gate. Requires being logged in (shaggy setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://shaggy.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        default=None,
        help=(
            "Override the Cherries Portal base URL for registration (default: the "
            "portal you logged into). The access token must be valid at this "
            "portal. Also settable via SHAGGY_DASHBOARD_PORTAL_URL. Mainly for "
