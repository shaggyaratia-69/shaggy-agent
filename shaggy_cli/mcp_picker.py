"""MCP picker — interactive `shaggy mcp picker` (also the default `shaggy mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``shaggy mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `shaggy plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from shaggy_cli.cli_output import prompt_yes_no
from shaggy_cli.curses_ui import curses_single_select
from shaggy_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Shaggy session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from shaggy_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `shaggy mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: shaggy mcp install <name>    Picker: shaggy mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Shaggy is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Shaggy — run `shaggy update` "
...
def show_catalog() -> None:
    """`shaggy mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`shaggy mcp picker` (and default `shaggy mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`shaggy mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `shaggy mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
