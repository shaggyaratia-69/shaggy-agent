"""
Profile management for multiple isolated Shaggy instances.

Each profile is a fully independent SHAGGY_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.shaggy/profiles/<name>/`` by default.

The "default" profile is ``~/.shaggy`` itself — backward compatible,
zero migration needed.

Usage::

    shaggy profile create coder          # fresh profile + bundled skills
    shaggy profile create coder --clone  # also copy config, .env, SOUL.md, skills
    shaggy profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    shaggy -p coder chat                 # or via flag
    shaggy profile use coder             # set as sticky default
    shaggy profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See shaggy_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# default profile (``~/.shaggy``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   shaggy-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
# Marker file written by `shaggy profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `shaggy
...
# `shaggy skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Shaggy infrastructure directories that only the default profile
         (``~/.shaggy``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_shaggy_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.shaggy) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "shaggy-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "shaggy", "default", "test", "tmp", "root", "sudo",
})

# Shaggy subcommands that cannot be used as profile names/aliases
_SHAGGY_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the shaggy root, NOT to the current SHAGGY_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where SHAGGY_HOME points outside
    ``~/.shaggy``, profiles live under ``SHAGGY_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_shaggy_home() / "profiles"


def _get_default_shaggy_home() -> Path:
    """Return the default (pre-profile) SHAGGY_HOME path.

    In standard deployments this is ``~/.shaggy``.
    In Docker/custom deployments where SHAGGY_HOME is outside ``~/.shaggy``
    (e.g. ``/opt/data``), returns SHAGGY_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_shaggy_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Shaggy installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its SHAGGY_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_shaggy_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, shaggy subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _SHAGGY_SUBCOMMANDS:
        return f"'{canon}' conflicts with a shaggy subcommand"
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``shaggy profile list`` for an unrelated profile.
    """
...
# ``config.yaml``. ``config.yaml`` is the user-facing Shaggy config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_shaggy_home()
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``shaggy update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.shaggy)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.shaggy/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Write the opt-out marker so seed_profile_skills() and `shaggy update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`shaggy profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `shaggy update`.\n",
                encoding="utf-8",
            )
        except OSError:
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `shaggy profile describe`
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches SHAGGY_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``shaggy profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "SHAGGY_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.shaggy).\n"
            "To remove everything, use: shaggy uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set SHAGGY_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("SHAGGY_HOME")
    try:
        os.environ["SHAGGY_HOME"] = str(profile_dir)
        from shaggy_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["SHAGGY_HOME"] = old_home
        elif "SHAGGY_HOME" in os.environ:
            del os.environ["SHAGGY_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.shaggy/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: shaggy profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from SHAGGY_HOME.

    Returns ``"default"`` if SHAGGY_HOME is not set or points to ``~/.shaggy``.
    Returns the profile name if SHAGGY_HOME points into ``~/.shaggy/profiles/<name>``.
    Returns ``"custom"`` if SHAGGY_HOME is set to an unrecognized path.
    """
    from shaggy_constants import get_shaggy_home
    shaggy_home = get_shaggy_home()
    resolved = shaggy_home.resolve()

    default_resolved = _get_default_shaggy_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.shaggy itself — its parent is ~/ and its
        # directory name is ".shaggy", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: shaggy profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.shaggy itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.shaggy). "
            "Specify a different name: shaggy profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="shaggy_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"shaggy.{old_name}"
    new_host = f"shaggy.{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_shaggy_home() / "honcho.json",
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a SHAGGY_HOME path string.

    Called early in the CLI entry point, before any shaggy modules
    are imported, to set the SHAGGY_HOME environment variable.
    """
