...
on Japanese locales, etc.).  Shaggy's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``shaggy_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Shaggy is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``SHAGGY_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("SHAGGY_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Shaggy-managed Git install directories so
    ...
    # already-running shells don't see the change — which means shaggy
    ...
    # install without Shaggy-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Shaggy's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Shaggy's
    ``shaggy config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Shaggy (User env var in Windows Settings, or
    export in a PowerShell profile) and Shaggy picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Shaggy-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\shaggy\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``shaggy`` from right after install) retain their old PATH.

    Any subprocess Shaggy spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Shaggy-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "shaggy", "git", "cmd"),
        os.path.join(local_appdata, "shaggy", "git", "bin"),
        os.path.join(local_appdata, "shaggy", "git", "usr", "bin"),
        # Shaggy venv Scripts directory — host of the shaggy.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes shaggy, but harmless to include.
        os.path.join(local_appdata, "shaggy", "shaggy-agent", "venv", "Scripts"),
