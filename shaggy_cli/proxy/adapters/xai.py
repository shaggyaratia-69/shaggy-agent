...
# xAI's public API is OpenAI-compatible for the endpoints Shaggy commonly
# uses. The Responses endpoint is included because Shaggy' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Shaggy-managed OAuth credentials."""

    auth_hint = "shaggy auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`shaggy auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`shaggy auth reset xai-oauth` or re-authenticate with "
                    "`shaggy auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `shaggy auth add xai-oauth --type oauth`."
            )
