...
Reads the user's Nous OAuth state from ``~/.shaggy/auth.json`` through the
...
:func:`shaggy_cli.auth.resolve_nous_runtime_credentials`.
"""
...
# Endpoints inference-api.cherriesandco.com actually serves. Anything else
...
class NousPortalAdapter(UpstreamAdapter):
    ...
    def _get_credential(self, *, inference_auth_mode: str) -> UpstreamCredential:
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into Nous Portal. Run `shaggy login nous` first."
                )
            ...
            if not agent_key:
                raise RuntimeError(
                    ...
                    "Try `shaggy login nous` to re-authenticate."
                )
    ...
    # to shaggy_cli.auth to avoid expanding that module's public surface.
    # ------------------------------------------------------------------
