"""Windows ConPTY bridge for the `shaggy dashboard` chat tab.

Drop-in counterpart to ``shaggy_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``shaggy_cli.web_server`` consumes: ``spawn``, ``read``,
