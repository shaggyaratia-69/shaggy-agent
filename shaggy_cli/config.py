"""
Configuration management for Shaggy Agent.

Config files are stored in ~/.shaggy/ for easy access:
- ~/.shaggy/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.shaggy/.env         - API keys and secrets

This module provides:
- shaggy config          - Show current configuration
- shaggy config edit     - Open config in editor
- shaggy config set      - Set a specific value
- shaggy config wizard   - Re-run setup wizard
"""
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.shaggy/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``shaggy logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  shaggy config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `shaggy plugins enable
    # observability/langfuse`); credentials gate the plugin at runtime.
    "SHAGGY_LANGFUSE_ENV",
    "SHAGGY_LANGFUSE_RELEASE",
    "SHAGGY_LANGFUSE_SAMPLE_RATE",
    "SHAGGY_LANGFUSE_MAX_CHARS",
    "SHAGGY_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from shaggy_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("SHAGGY_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_shaggy_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Shaggy is running in package-manager-managed mode.

    Two signals: the SHAGGY_MANAGED env var (set by the systemd service),
    or a .managed marker file in SHAGGY_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade shaggy-agent"
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Shaggy was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Stamped ``~/.shaggy/.install_method`` file (written by installers)
    2. SHAGGY_MANAGED env / .managed marker (NixOS, Homebrew)
    3. Container detection (/.dockerenv, /run/.containerenv, cgroup)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'
    """
    stamp = get_shaggy_home() / ".install_method"
    try:
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.shaggy/.install_method."""
    stamp = get_shaggy_home() / ".install_method"
    try:
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade shaggy-agent"
    if method == "docker":
        return "docker pull nousresearch/shaggy-agent:latest"
    if method == "pip":
        import shutil
        uv = shutil.which("uv")
        if uv:
            return "uv pip install --upgrade shaggy-agent"
        return "pip install --upgrade shaggy-agent"
    return "shaggy update"
...
def format_managed_message(action: str = "modify this Shaggy installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("SHAGGY_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Shaggy installation is managed by NixOS "
            f"(SHAGGY_MANAGED={env_hint}).\n"
            "Edit services.shaggy-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Shaggy installation is managed by Homebrew "
            f"(SHAGGY_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade shaggy-agent"
        )

    return (
        f"Cannot {action}: this Shaggy installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Shaggy."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from SHAGGY_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, shaggy_bin
    or None if container mode is not active, we're already inside the
    container, or SHAGGY_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("SHAGGY_DEV") == "1":
        return None
    ...
    container_mode_file = get_shaggy_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "shaggy-agent")
    exec_user = info.get("exec_user", "shaggy")
    shaggy_bin = info.get("shaggy_bin", "/data/current-package/bin/shaggy")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "shaggy_bin": shaggy_bin,
    }
...
# Re-export from shaggy_constants — canonical definition lives there.
from shaggy_constants import get_shaggy_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_shaggy_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_shaggy_home() / ".env"
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the shaggy group can
    share state with the gateway service.

    The mode can be overridden via the SHAGGY_HOME_MODE environment variable
    (e.g. SHAGGY_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse SHAGGY_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("SHAGGY_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When Shaggy runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("SHAGGY_CONTAINER") or os.environ.get("SHAGGY_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set SHAGGY_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into SHAGGY_HOME if the user doesn't have one yet."""
...
def ensure_shaggy_home():
    """Ensure ~/.shaggy directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_shaggy_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_shaggy_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"SHAGGY_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Shaggy-level retry loop that wraps the whole call.  Lower
    ...
    "terminal": {
        ...
        # skipped. When empty, Shaggy auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Shaggy sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Shaggy
        ...
        # bundled Shaggy image, which drops to the `shaggy` user via gosu).
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Shaggy sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Shaggy should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``shaggy chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.shaggy/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: shaggy sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, Shaggy truncates the
    ...
    "auxiliary": {
        ...
        # Invoked by ``shaggy kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``shaggy kanban decompose`` and the kanban
        ...
        # ``shaggy profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `shaggy model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # When true, `shaggy --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `shaggy -c` muscle memory.  Default off so existing
        # users aren't surprised.  SHAGGY_TUI_RESUME=<id> always wins.
        ...
        # e.g. `model · 68% · ~/projects/shaggy`. Per-platform overrides go under
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.shaggy/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.shaggy/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, Shaggy feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Shaggy auto-pauses the goal and
    ...
    # always goes to ~/.shaggy/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${SHAGGY_SKILL_DIR} and ${SHAGGY_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `shaggy curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # skipped), snapshot ~/.shaggy/skills/ into
        # ~/.shaggy/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `shaggy curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for shaggy-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Shaggy Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (SHAGGY_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.shaggy/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or SHAGGY_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `shaggy doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``shaggy_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Shaggy to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Also overridable via SHAGGY_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `shaggy -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # falls back to the default profile (the one `shaggy` launches with
        ...
        # decomposition is manual via `shaggy kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     shaggy-agent's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.shaggy/logs/.
    ...
    # update model picker lists without shipping a shaggy-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://shaggyaratia-69.github.io/shaggy-agent/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `shaggy model` invocation; network failures
    ...
    # Session storage — controls automatic cleanup of ~/.shaggy/state.db.
    ...
    "sessions": {
        ...
        # default of ``shaggy sessions prune``.
        "retention_days": 90,
    ...
    # ``shaggy update`` behaviour.
    "updates": {
        # Run a full ``shaggy backup``-style zip of SHAGGY_HOME before every
        # ``shaggy update``.  Backups land in ``<SHAGGY_HOME>/backups/`` and
        # can be restored with ``shaggy import <path>``.  Off by default —
        # on large SHAGGY_HOME directories the zip can add minutes to every
    ...
    "lsp": {
        ...
        #              ``<SHAGGY_HOME>/lsp/bin/`` on first use.
    ...
    # `shaggy tools`. These settings tune the backing Responses API call.
    "x_search": {
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'shaggy model' for endpoint-specific config)",
    ...
    "FIRECRAWL_GATEWAY_URL": {
        "description": "Exact Firecrawl tool-gateway origin override for Cherries subscribers only (optional)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for Cherries subscribers only, used to derive vendor hosts, e.g. cherriesandco.com -> firecrawl-gateway.cherriesandco.com",
    ...
    "TOOL_GATEWAY_SCHEME": {
        "description": "Shared tool-gateway URL scheme for Cherries subscribers only, used to derive vendor hosts (`https` by default, set `http` for local gateway testing)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Cherries subscriber access token for tool-gateway requests (optional; otherwise read from the Shaggy auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @shaggy:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. SHAGGY_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #shaggy)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: shaggy-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'shaggy-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Shaggy API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Shaggy API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Shaggy API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # SHAGGY_TOOL_PROGRESS and SHAGGY_TOOL_PROGRESS_MODE are deprecated —
    # now configured via display.tool_progress in config.yaml (off|new|all|verbose).
    # Gateway falls back to these env vars for backward compatibility.
    "SHAGGY_TOOL_PROGRESS": {
    ...
    "SHAGGY_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.shaggy.config`` entries, then checks
    ...
    try:
        ...
        # should never break `shaggy update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Shaggy versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``shaggy_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'shaggy setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Shaggy won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'shaggy doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("SHAGGY_HOME", "~/.shaggy")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("SHAGGY_TOOL_PROGRESS")
            old_mode = get_env_value("SHAGGY_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from SHAGGY_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from SHAGGY_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("SHAGGY_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from SHAGGY_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$SHAGGY_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_shaggy_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `shaggy plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `shaggy curator status` has no
    ...
    #   3. Creates `~/.shaggy/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_shaggy_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_shaggy_home() / "logs" / "curator"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: shaggy config set <key> <value>")
    ...
    # metadata.shaggy.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _shaggy_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.shaggy/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.shaggy/config.yaml.
    ...
    (which change ``SHAGGY_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
#   openai-codex (OAuth — shaggy auth) — OpenAI Codex
#   nous         (OAuth — shaggy auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.shaggy/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.shaggy/.env.
    ...
    interactive menu render (`shaggy tools`, `shaggy setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `shaggy tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Shaggy env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.shaggy/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.shaggy/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.shaggy/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.shaggy/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Shaggy — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Shaggy vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.shaggy/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Shaggy Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  shaggy config edit     # Edit config file", Colors.DIM))
    print(color("  shaggy config set <key> <value>", Colors.DIM))
    print(color("  shaggy setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: shaggy config set <key> <value>")
            print()
            print("Examples:")
            print("  shaggy config set model anthropic/claude-sonnet-4")
            print("  shaggy config set terminal.backend docker")
            print("  shaggy config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'shaggy config migrate' to add them")
        ...
        print("  shaggy config           Show current configuration")
        print("  shaggy config edit      Open config in editor")
        print("  shaggy config set <key> <value>   Set a config value")
        print("  shaggy config check     Check for missing/outdated config")
        print("  shaggy config migrate   Update config with new options")
        print("  shaggy config path      Show config file path")
        print("  shaggy config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``shaggy config`` UI so users
