...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or shaggy -c.",
    ...
    "Ctrl+Z suspends Shaggy to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "shaggy -c resumes your most recent CLI session. shaggy -c \"project name\" resumes by title.",
    "shaggy -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "shaggy -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "shaggy chat -t web,terminal enables only specific toolsets for a focused session.",
    "shaggy chat -s github-pr-workflow preloads a skill at launch.",
    "shaggy chat -q \"query\" runs a single non-interactive query and exits.",
    "shaggy chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "shaggy chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "shaggy --yolo bypasses all dangerous command approval prompts for the entire session.",
    "shaggy chat --source telegram tags the session for filtering in shaggy sessions list.",
    "shaggy -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "shaggy doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "shaggy dump outputs a compact setup summary — great for bug reports.",
    "shaggy config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "shaggy config edit opens config.yaml in your default editor.",
    "shaggy config check scans for missing or stale configuration options.",
    "shaggy sessions browse opens an interactive session picker with search.",
    "shaggy sessions stats shows session counts by platform and database size.",
    "shaggy sessions prune --older-than 30 cleans up old sessions.",
    "shaggy skills search react --source skills-sh searches the skills.sh public directory.",
    "shaggy skills check scans installed hub skills for upstream updates.",
    "shaggy skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "shaggy skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "shaggy mcp add github --command npx adds MCP servers from the command line.",
    "shaggy mcp serve runs Shaggy itself as an MCP server for other agents.",
    "shaggy auth add lets you add multiple API keys for credential pool rotation.",
    "shaggy completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "shaggy logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "shaggy backup creates a zip backup of your entire Shaggy home directory.",
    "shaggy profile create coder creates an isolated profile that becomes its own command.",
    "shaggy profile create work --clone copies your current config and keys to a new profile.",
    "shaggy update syncs new bundled skills to ALL profiles automatically.",
    "shaggy gateway install sets up Shaggy as a system service (systemd/launchd).",
    "shaggy memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "shaggy webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: shaggy tools disables unused tools, shaggy skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "shaggy models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as shaggy -w).",
    ...
    "SHAGGY_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Shaggy tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'shaggy profile create coder' creates the 'coder' command.",
    "shaggy profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Shaggy prints a resume command with session ID and stats.",
    "shaggy sessions export backup.jsonl exports all sessions for backup or analysis.",
    "shaggy -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "shaggy skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .shaggy/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: shaggy cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Shaggy runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "shaggy gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Shaggy via a local macOS server.",
    ...
    "Context files (.shaggy.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.shaggy/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "Shaggy loads project context from .shaggy.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — shaggy tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.shaggy/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "shaggy chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "shaggy plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "shaggy acp runs Shaggy as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "SHAGGY_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "shaggy login supports OAuth-based auth for Nous and OpenAI Codex providers.",
    ...
    "shaggy status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.shaggy/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Shaggy your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.shaggy/SOUL.md to customize.",
    ...
    "Before a gateway session reset, Shaggy auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.shaggy/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at SHAGGY_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "SHAGGY_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in SHAGGY_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.shaggy/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'shaggy profile create ls' would be rejected.",
    "shaggy profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "shaggy claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "SHAGGY_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via shaggy tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.shaggy/.env.",
    "Skills from trusted repos (shaggyaratia-69) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.shaggy/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Shaggy auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Shaggy config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Shaggy is working.',
    ...
    '/reload reloads ~/.shaggy/.env into the running session — pick up new API keys without restarting.',
    ...
    "SHAGGY_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.shaggy/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.shaggy/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'shaggy curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "shaggy curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'shaggy curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'shaggy auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    'credential_pool_strategies.<provider>: round_robin cycles keys evenly instead of the fill_first default.',
    'use_gateway: true per-tool routes web, image, tts, or browser through your Cherries subscription — no extra keys.',
    ...
    # --- TUI & Dashboard ---
    'SHAGGY_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "SHAGGY_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'shaggy dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'shaggy dashboard --tui embeds the full Shaggy TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.shaggy/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.shaggy/dashboard-plugins/ — no npm build required.',
    ...
    'SHAGGY_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'SHAGGY_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',
    'SHAGGY_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'SHAGGY_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.shaggy/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.shaggy/skills/ so drop-in skills appear without restarting the session.',
    ...
    'shaggy -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'shaggy chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'shaggy chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'shaggy chat --ignore-user-config skips ~/.shaggy/config.yaml — reproducible bug reports and CI runs.',
    "shaggy chat --source tool tags programmatic chats so they don't clutter shaggy sessions list.",
    'shaggy dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'shaggy sessions rename <ID> "new title" renames any past session; shaggy sessions delete <ID> removes one.',
    'shaggy import restores a session export or profile archive produced by sessions export or profile export.',
    'shaggy fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'shaggy pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'shaggy setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'shaggy status --deep runs the full health sweep across every component; plain shaggy status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'SHAGGY_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'SHAGGY_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.shaggy/plugins/ — trust-gated by design.',
    "SHAGGY_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'SHAGGY_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'SHAGGY_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'SHAGGY_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'SHAGGY_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'SHAGGY_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'SHAGGY_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'SHAGGY_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'SHAGGY_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'SHAGGY_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'SHAGGY_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Shaggy block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from shaggy sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.shaggy/dashboard-plugins/.',
]
