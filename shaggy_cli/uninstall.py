"""
Shaggy Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.shaggy/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Shaggy PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing shaggy-agent or shaggy PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Shaggy Agent" comment and following line
                if '# Shaggy Agent' in line or '# shaggy-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('shaggy' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing shaggy
                if 'shaggy' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the shaggy wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "shaggy",
        Path("/usr/local/bin/shaggy"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains shaggy_cli reference)
                content = wrapper.read_text()
                if 'shaggy_cli' in content or 'shaggy-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``shaggy gateway run`` processes
    ...
    try:
        from shaggy_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from shaggy_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``SHAGGY_HOME`` and ``SHAGGY_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\shaggy\git\cmd``, ``%LOCALAPPDATA%\shaggy\git\bin``,
#      ``%LOCALAPPDATA%\shaggy\git\usr\bin``, ``%LOCALAPPDATA%\shaggy\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\shaggy\git\`` and Node to
#      ``%LOCALAPPDATA%\shaggy\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``shaggy dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\shaggy\gateway-service\`` and sometimes
...
def _shaggy_path_markers(shaggy_home: Path) -> list[str]:
    """Path-entry substrings that identify Shaggy-owned User-PATH entries."""
    root = str(shaggy_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare shaggy-agent install dir.
    markers = [root + "\\shaggy-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if SHAGGY_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "shaggy".  We don't want to catch
    # unrelated entries like "cshaggy-foo" or "ephermeral", so we look for
    # backslash-shaggy as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(shaggy_home: Path) -> list[str]:
    """Strip Shaggy-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _shaggy_path_markers(shaggy_home)
            kept: list[str] = []
...
def remove_shaggy_env_vars_windows() -> list[str]:
    """Delete SHAGGY_HOME and SHAGGY_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("SHAGGY_HOME", "SHAGGY_GIT_BASH_PATH"):
                try:
...
def _is_default_shaggy_home(shaggy_home: Path) -> bool:
    """Return True when ``shaggy_home`` points at the default (non-profile) root."""
    try:
        from shaggy_constants import get_default_shaggy_root
        return shaggy_home.resolve() == get_default_shaggy_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its SHAGGY_HOME directory.

    We shell out to ``shaggy -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current SHAGGY_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m shaggy_cli.main` so we don't depend on a `shaggy`
    #    wrapper that may be half-removed mid-uninstall.
    shaggy_invocation = [_sys.executable, "-m", "shaggy_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                shaggy_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's SHAGGY_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.shaggy/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.shaggy/ for future reinstall
    """
    project_root = get_project_root()
    shaggy_home = get_shaggy_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie SHAGGY_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_shaggy_home(shaggy_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Shaggy Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {shaggy_home / 'config.yaml'}")
    print(f"  Secrets: {shaggy_home / '.env'}")
    print(f"  Data:    {shaggy_home / 'cron/'}, {shaggy_home / 'sessions/'}, {shaggy_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their SHAGGY_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Shaggy data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Shaggy code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in shaggy_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\shaggy\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(shaggy_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Shaggy-owned PATH entries in User environment")

        log_info("Removing SHAGGY_HOME / SHAGGY_GIT_BASH_PATH User env vars...")
        removed_env = remove_shaggy_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Shaggy-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing shaggy command...")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.shaggy/, just remove the shaggy-agent subdir
            if shaggy_home in project_root.parents or project_root.parent == shaggy_home:
    ...
    #     under SHAGGY_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(shaggy_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(shaggy_home)
    ...
    # 5. Optionally remove ~/.shaggy/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile SHAGGY_HOME dirs live under
        ...
        try:
            if shaggy_home.exists():
                shutil.rmtree(shaggy_home)
                log_success(f"Removed {shaggy_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {shaggy_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {shaggy_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/shaggyaratia-69/shaggy-agent/main/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/shaggyaratia-69/shaggy-agent/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Shaggy Agent! ⚕")
    print()
