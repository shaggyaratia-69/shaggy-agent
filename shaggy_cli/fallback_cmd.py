"""
shaggy fallback — manage the fallback provider chain.
...
Subcommands:
  shaggy fallback [list]   Show the current fallback chain (default when no subcommand)
  shaggy fallback add      Pick provider + model via the same picker as `shaggy model`,
                           then append the selection to the chain
  shaggy fallback remove   Pick an entry to delete from the chain
  shaggy fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.shaggy/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from shaggy_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `shaggy model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  shaggy fallback add")
        print()
        return
    ...
    print("  Docs: https://shaggyaratia-69.github.io/shaggy-agent/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `shaggy model`, then append the selection to the chain."""
    from shaggy_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `shaggy model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `shaggy fallback list` to view, or `shaggy fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``shaggy fallback [subcommand]``."""
