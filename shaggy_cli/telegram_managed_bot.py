...
manual BotFather token copy-paste. Shaggy talks only to the Cherries onboarding
...
# Default pairing API base URL (Cherries-hosted Cloudflare Worker).
# Override for PoC/staging with TELEGRAM_ONBOARDING_URL.
DEFAULT_API_URL = "https://setup.shaggy-agent.cherriesandco.com"
TELEGRAM_ONBOARDING_URL_ENV = "TELEGRAM_ONBOARDING_URL"

# The Cherries-hosted manager bot username (without @). The backend returns the
# actual deep link, so this is only used by local helpers/tests.
DEFAULT_MANAGER_BOT = "ShaggySetupBot"

DEFAULT_BOT_NAME = "Shaggy Agent"
...
def generate_username_slug(length: int = 16) -> str:
    ...
    keeping ``shaggy_<slug>_bot`` under Telegram's 32-character username limit.
    """
...
def generate_bot_username(profile_name: Optional[str] = None) -> str:
    """Generate a secure suggested bot username like ``shaggy_<slug>_bot``.
    ...
    """
    _ = profile_name
    return f"shaggy_{generate_username_slug()}_bot"
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting Shaggy Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the Shaggy Telegram onboarding service.")
