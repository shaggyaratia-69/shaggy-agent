"""Shell completion script generation for shaggy CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_shaggy_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Shaggy Agent bash completion
# Add to ~/.bashrc:
#   eval "$(shaggy completion bash)"

_shaggy_profiles() {{
    local profiles_dir="$HOME/.shaggy/profiles"
...
_shaggy_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_shaggy_profiles)" -- "$cur"))
        return
    fi
...
complete -F _shaggy_completion shaggy
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef shaggy
# Shaggy Agent zsh completion
# Add to ~/.zshrc:
...
_shaggy_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.shaggy/profiles" ]]; then
        profiles+=("${{(@f)$(ls $HOME/.shaggy/profiles 2>/dev/null)}}")
    fi
...
_shaggy() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_shaggy_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'shaggy command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Shaggy Agent fish completion",
        "# Add to your config:",
        "#   shaggy completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __shaggy_profiles",
        "    echo default",
        "    if test -d $HOME/.shaggy/profiles",
        "        ls $HOME/.shaggy/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c shaggy -f -s p -l profile"
        " -d 'Profile name' -xa '(__shaggy_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__shaggy_profiles)' -d 'Profile name'"
                )
