"""``shaggy dashboard register`` — register a self-hosted dashboard OAuth client.

Automates what a user otherwise does by hand: open the Cherries Portal
``/local-dashboards`` page in a browser, click "register", copy the
resulting ``agent:{id}`` OAuth client ID, and paste it into ``~/.shaggy/.env``
as ``SHAGGY_DASHBOARD_OAUTH_CLIENT_ID``.

This command:
  1. Resolves a fresh Cherries Portal access token from the existing login
     (``~/.shaggy/auth.json``), refreshing it if needed. Fails fast with a
     "run `shaggy setup`" hint when the user isn't logged in.
  ...
  3. Writes ``SHAGGY_DASHBOARD_OAUTH_CLIENT_ID`` and (if absent)
     ``SHAGGY_DASHBOARD_PORTAL_URL`` into ``~/.shaggy/.env`` idempotently.
...
def _resolve_portal_base_url(override: Optional[str] = None) -> str:
    ...
    Precedence:
      1. ``override`` — explicit ``--portal-url`` flag or
         ``SHAGGY_DASHBOARD_PORTAL_URL`` env (used for testing against a
      ...
      2. The ``portal_base_url`` stored on the Cherries login — this is the
    ...
    try:
        from shaggy_cli.auth import DEFAULT_CHERRIES_PORTAL_URL, get_provider_auth_state

        state = get_provider_auth_state("nous") or {}
        base = state.get("portal_base_url")
        if isinstance(base, str) and base.strip():
            return base.rstrip("/")
        return str(DEFAULT_CHERRIES_PORTAL_URL).rstrip("/")
    except Exception:
        return "https://www.cherriesandco.com"
...
def _register_self_hosted_client(
    *,
    ...
    makes re-running ``shaggy dashboard register`` idempotent. The portal
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                "Cherries Portal rejected the access token (401). "
                "Try `shaggy auth login nous` to re-authenticate."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
        ...
        raise RuntimeError(
            f"Could not reach Cherries Portal at {portal_base_url}: {exc.reason}"
        ) from exc
...
def _print_post_register_hint(
    *,
    ...
    from shaggy_cli.config import get_env_path

    env_path = get_env_path()
    _cid = client_id
    print()
    print(f"  Wrote to {env_path}:")
    print("    SHAGGY_DASHBOARD_OAUTH_CLIENT_ID=" + str(_cid))
    if wrote_portal_url:
        print("    SHAGGY_DASHBOARD_PORTAL_URL=" + str(portal_base_url))
    if public_url:
        print("    SHAGGY_DASHBOARD_PUBLIC_URL=" + str(public_url))
    print()
    print(
        "  Heads up — Cherries login only *engages* on a non-loopback bind. A plain\n"
        "  `shaggy dashboard` (localhost) leaves the gate off and serves locally\n"
    ...
    if custom_redirect_uri:
        ...
        try:
            ...
            host = "your-host"
        print("  To require Cherries login on your registered host, run the dashboard")
        print(f"  bound publicly (it must be reachable at https://{host}) and log in")
        print("  at its /login page.")
    else:
        print("  To require Cherries login (e.g. exposing on your LAN or a public host):")
        print("    shaggy dashboard --host 0.0.0.0")
...
def cmd_dashboard_register(args) -> None:
    """Register a self-hosted dashboard OAuth client with Cherries Portal."""
    from shaggy_cli.auth import AuthError, resolve_nous_access_token
    from shaggy_cli.config import get_env_value, is_managed, save_env_value

    # Managed (Docker/hosted) installs get their dashboard OAuth client_id
    # stamped in by the orchestrator (NAS sets SHAGGY_DASHBOARD_OAUTH_CLIENT_ID
    ...
    if is_managed():
        print(
            "✗ `shaggy dashboard register` is not available in a managed/hosted "
            "install.\n"
            "  The dashboard OAuth client is provisioned by the hosting platform."
        )
        sys.exit(1)

    # 1. Resolve a fresh Cherries access token (refreshes if near expiry). Fail fast
    #    with a setup hint when the user isn't logged in.
    try:
        access_token = resolve_nous_access_token()
    except AuthError as exc:
        if getattr(exc, "relogin_required", False):
            print("✗ You're not logged into Cherries Portal.")
            print("  Run `shaggy setup` (or `shaggy auth login nous`) first, then retry.")
        else:
            print(f"✗ Could not resolve a Cherries Portal access token: {exc}")
        sys.exit(1)
    ...
    # SHAGGY_DASHBOARD_PORTAL_URL env var, else the stored login's portal.
    #
    ...
    # client_id locally (SHAGGY_DASHBOARD_OAUTH_CLIENT_ID). Re-send it so the
    ...
    try:
        existing_client_id = get_env_value("SHAGGY_DASHBOARD_OAUTH_CLIENT_ID")
    except Exception:
        existing_client_id = None
    ...
    try:
        save_env_value("SHAGGY_DASHBOARD_OAUTH_CLIENT_ID", client_id)
    except Exception as exc:
        print(f"✗ Failed to write SHAGGY_DASHBOARD_OAUTH_CLIENT_ID to .env: {exc}")
        print(f"  Set it manually:  SHAGGY_DASHBOARD_OAUTH_CLIENT_ID={client_id}")
        sys.exit(1)

    # Persist the portal URL. Two cases:
    #   a) The user explicitly supplied a custom portal (--portal-url flag or
    #      SHAGGY_DASHBOARD_PORTAL_URL env). That's an intentional choice we
    ...
    default_portal = "https://www.cherriesandco.com"
    existing_portal = None
    try:
        existing_portal = get_env_value("SHAGGY_DASHBOARD_PORTAL_URL")
    except Exception:
        existing_portal = None
    ...
    if should_write_portal:
        try:
            save_env_value("SHAGGY_DASHBOARD_PORTAL_URL", portal_base_url)
    ...
    # the portal, e.g. https://shaggy.example.com/auth/callback. At serve time
    # the dashboard auth layer (dashboard_auth/routes._redirect_uri) reconstructs
    # that same callback by taking SHAGGY_DASHBOARD_PUBLIC_URL and appending
    ...
    # the supplied redirect URI and persist it as SHAGGY_DASHBOARD_PUBLIC_URL so
    ...
    if public_url:
        ...
        try:
            existing_public_url = get_env_value("SHAGGY_DASHBOARD_PUBLIC_URL")
        except Exception:
            existing_public_url = None
        if existing_public_url != public_url:
            try:
                save_env_value("SHAGGY_DASHBOARD_PUBLIC_URL", public_url)
