"""
Gateway subcommand for shaggy CLI.

Handles: shaggy gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_shaggy_home is imported lazily at call sites to avoid ImportError
# when shaggy_constants is cached from a pre-update version during `shaggy update`.
from shaggy_cli.setup import (
    print_header, print_info, print_success, print_warning, print_error,
    prompt, prompt_choice, prompt_yes_no,
)
from shaggy_cli.colors import Colors, color
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "shaggy-gateway*",
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "shaggy CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``shaggy gateway status`` from falsely counting the ``shaggy`` CLI that
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``shaggy gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    exclude_pids = exclude_pids | _get_ancestor_pids()
    pids: list[int] = []
    patterns = [
        "shaggy_cli.main gateway",
        "shaggy_cli.main --profile",
        "shaggy_cli.main -p",
        "shaggy_cli/main.py gateway",
        "shaggy_cli/main.py --profile",
        "shaggy_cli/main.py -p",
        "shaggy gateway",
        "gateway/run.py",
    ]
    current_home = str(get_shaggy_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        if current_profile_name:
            return (
                ...
                or f"SHAGGY_HOME={current_home}" in command
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. SHAGGY_HOME
        ...
        # SHAGGY_HOME= in argv is.
        if "--profile " in command or " -p " in command:
            return False
        if "SHAGGY_HOME=" in command and f"SHAGGY_HOME={current_home}" not in command:
            return False
        return True
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``shaggy update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Shaggy profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Shaggy profiles via PID files."""
    _exclude = set(exclude_pids or set())
    processes: list[ProfileGatewayProcess] = []
    try:
        from gateway.status import get_running_pid
        from shaggy_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "shaggy_cli.main"]
    if profile != "default":
...
def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    ...
    # when the user closes the terminal, leaving ``shaggy update`` users
    # with no running gateway until they re-invoke ``shaggy gateway``
...
def _sync_shaggy_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``SHAGGY_HOME``.

    Under ``sudo``, ``SHAGGY_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_shaggy_home` falls back to ``/root/.shaggy`` — the wrong
    profile. The unit file pins ``SHAGGY_HOME`` for the actual gateway
    ...
    unit_home = env.get("SHAGGY_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("SHAGGY_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["SHAGGY_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}shaggy gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}shaggy gateway restart{scope_flag}")
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    from shaggy_constants import is_container
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `shaggy gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``shaggy gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from shaggy_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (SHAGGY_HOME-scoped).
...
from shaggy_constants import is_container, is_termux, is_wsl
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``shaggy gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``SHAGGY_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("SHAGGY_GATEWAY_DETACHED", "").strip().lower()
...
_SERVICE_BASE = "shaggy-gateway"
SERVICE_DESCRIPTION = "Shaggy Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current SHAGGY_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (SHAGGY_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from shaggy_constants import get_default_shaggy_root
    home = get_shaggy_home().resolve()
    default = get_default_shaggy_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary SHAGGY_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(shaggy_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when SHAGGY_HOME is a named profile.

    For ``~/.shaggy/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        shaggy_home: Optional explicit SHAGGY_HOME path. Defaults to the current
            ``get_shaggy_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
    """
    import re
    from shaggy_constants import get_default_shaggy_root
    home = Path(shaggy_home or str(get_shaggy_home())).resolve()
    default = get_default_shaggy_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this SHAGGY_HOME.

    Default ``~/.shaggy`` returns ``shaggy-gateway`` (backward compatible).
    Profile ``~/.shaggy/profiles/coder`` returns ``shaggy-gateway-coder``.
    Any other SHAGGY_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Shaggy installs that predate the
# shaggy-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (shaggy-gateway-*.service) and unrelated third-party
# "shaggy" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("shaggy.service",)
...
def _find_legacy_shaggy_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Shaggy gateway units.

    Detects unit files installed by older Shaggy versions that used a
    different service name (e.g. ``shaggy.service`` before the rename to
    ``shaggy-gateway.service``). When both a legacy unit and the current
    ``shaggy-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``shaggy-gateway-coder.service`` and unrelated third-party
      ``shaggy-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``shaggy.service`` running an unrelated
...
def has_legacy_shaggy_units() -> bool:
    """Return True when any legacy Shaggy gateway unit files exist."""
    return bool(_find_legacy_shaggy_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Shaggy gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_shaggy_units()
    if not legacy:
        return
    print_warning("Legacy Shaggy gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current shaggy-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    shaggy gateway migrate-legacy")


def remove_legacy_shaggy_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Shaggy gateway unit files.

    Iterates over whatever ``_find_legacy_shaggy_units()`` returns — which is
    ...
    legacy = _find_legacy_shaggy_units()
    if not legacy:
        print("No legacy Shaggy gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Shaggy gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: shaggy gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo shaggy gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    shaggy gateway uninstall")
    print_info("    sudo shaggy gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning("  System service install requires sudo, so Shaggy can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo shaggy gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo shaggy gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo shaggy gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Shaggy often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.shaggy`` → ``ai.shaggy.gateway.plist`` (backward compatible).
    Profile ``~/.shaggy/profiles/coder`` → ``ai.shaggy.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.shaggy.gateway-{suffix}" if suffix else "ai.shaggy.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.shaggy/shaggy-agent  -> /home/alice/.shaggy/shaggy-agent
      /opt/shaggy                 -> /opt/shaggy  (kept as-is)
...
def _shaggy_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current SHAGGY_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_shaggy_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.shaggy                    → /home/alice/.shaggy
      /root/.shaggy/profiles/coder     → /home/alice/.shaggy/profiles/coder
      /opt/custom-shaggy               → /opt/custom-shaggy  (kept as-is)
    """
    current_shaggy = get_shaggy_home().resolve()
    current_default = (Path.home() / ".shaggy").resolve()
    target_default = Path(target_home_dir) / ".shaggy"

    # Default ~/.shaggy → remap to target user's default
    if current_shaggy == current_default:
        return str(target_default)

    # Profile or subdir of ~/.shaggy → preserve the relative structure
    try:
        relative = current_shaggy.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.shaggy) — keep as-is
        return str(current_shaggy)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    shaggy_home = get_shaggy_home()
    shaggy_node = shaggy_home / "node" / "bin"
    if _is_dir(shaggy_node):
        candidates.append(str(shaggy_node))
    shaggy_nm = shaggy_home / "node_modules" / ".bin"
    if _is_dir(shaggy_nm):
        candidates.append(str(shaggy_nm))
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        shaggy_home = _shaggy_home_for_target_user(home_dir)
        profile_arg = _profile_arg(shaggy_home)
...
User={username}
Group={group_name}
ExecStart={python_path} -m shaggy_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
Environment="SHAGGY_HOME={shaggy_home}"
Restart=always
RestartSec=5
...
"""
    ...
    shaggy_home = str(get_shaggy_home().resolve())
    profile_arg = _profile_arg(shaggy_home)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # sandboxed by the test conftest (only SHAGGY_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp SHAGGY_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../shaggy_test`` path into
    # ``Environment="SHAGGY_HOME=..."``. Writing that to the developer's
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Shaggy install")
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo shaggy gateway uninstall --system")
    print_info("         shaggy gateway install")
    print_info("         shaggy gateway start")
...
def systemd_install(
    ...
    # Offer to remove legacy units (shaggy.service from pre-rename installs)
    # before installing the new shaggy-gateway.service. If both remain, they
    ...
    if has_legacy_shaggy_units():
        print()
        print_legacy_unit_warning()
        print()
        if prompt_yes_no("Remove the legacy unit(s) before installing?", True):
            remove_legacy_shaggy_units(interactive=False)
            print()
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}shaggy gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}shaggy gateway status{scope_flag}             # Check status")
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}shaggy gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_stop(system: bool = False):
    ...
    _sync_shaggy_home_from_systemd_unit(system=system)
    try:
    ...
    try:
        ...
        print(
            ...
            "check `shaggy gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `shaggy gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}shaggy gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}shaggy gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(f"  Run after the start-limit window expires: {'sudo ' if system else ''}shaggy gateway restart{scope_flag}")
        ...
        print(f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}shaggy gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.shaggy.gateway-{suffix}" if suffix else "ai.shaggy.gateway"
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current Shaggy install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  shaggy gateway status             # Check status")
    from shaggy_constants import display_shaggy_home as _dhh
...
def launchd_stop():
    ...
    # `shaggy gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Shaggy install")
    else:
        print("⚠ Service definition is stale relative to the current Shaggy install")
        print("  Run: shaggy gateway start")
    ...
    if deep:
        log_file = get_shaggy_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("SHAGGY_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the Shaggy gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'shaggy' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/shaggy/docker/entrypoint.sh before the Shaggy command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$SHAGGY_HOME and break later non-root dashboard/gateway runs."
    )
    print("  Set SHAGGY_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk.")
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    # from sibling CLI processes, but foreground `shaggy gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set SHAGGY_GATEWAY_DETACHED=1; older wrappers
    ...
    # /restart) rather than through `shaggy gateway restart` which already
    ...
    # `shaggy gateway start/restart` — leaving the gateway vulnerable to
    ...
    # SHAGGY_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("SHAGGY_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Shaggy will log in directly",
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @shaggy:matrix.example.org"},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. shaggy) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where Shaggy delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Shaggy agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address Shaggy will use (e.g., shaggy@gmail.com)."},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Shaggy connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: shaggy pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Shaggy will connect automatically over WebSocket",
        ],
        "vars": [
...
def _all_platforms() -> list[dict]:
    ...
    ``shaggy setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run shaggy under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.shaggy/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_shaggy_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'shaggy pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: shaggy pairing approve <platform> <code>")
                else:
                    print_info("  Skipped — configure later with 'shaggy gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from shaggy_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from shaggy_cli.dingtalk_auth import dingtalk_qr_auth
...
def _is_service_installed() -> bool:
    ...
    if supports_systemd_services():
        ...
        from shaggy_cli import gateway_windows
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Shaggy will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info("  3. Shaggy will store the returned account_id/token in ~/.shaggy/.env.")
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `shaggy gateway setup`.")
        return
    ...
    import asyncio
    try:
        credentials = asyncio.run(qr_login(str(get_shaggy_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown DM users can request access and you approve them with `shaggy pairing approve`.")
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info("  Unknown users can request access; approve with `shaggy pairing approve`.")
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"ShaggyAgent\"")
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``shaggy_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from shaggy_cli import setup as _s
    return {
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.shaggy/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.shaggy/.env: {', '.join(required)}")
    else:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: shaggy gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: shaggy gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: shaggy gateway install")
                    if supports_systemd_services():
                        print_info("  Or as a boot-time service: sudo shaggy gateway install --system")
                    print_info("  Or run in foreground:  shaggy gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: shaggy gateway run")
                print_info("  For persistence:   tmux new -s shaggy 'shaggy gateway run'")
                print_info("  To enable systemd: add systemd=true to /etc/wsl.conf, then 'wsl --shutdown'")
            elif is_termux():
                from shaggy_constants import display_shaggy_home as _dhh
                print_info("  Termux does not use systemd/launchd services.")
                print_info("  Run in foreground: shaggy gateway run")
                print_info(f"  Or start it manually in the background (best effort): nohup shaggy gateway run >{_dhh()}/logs/gateway.log 2>&1 &")
            else:
                print_info("  Service install not supported on this platform.")
                print_info("  Run in foreground: shaggy gateway run")
    else:
        print()
        print_info("No platforms configured. Run 'shaggy gateway setup' when ready.")
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``shaggy gateway install|uninstall|start|stop|restart``
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: shaggy gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning("WSL detected — systemd services may not survive WSL restarts.")
                print_info("  Consider running in foreground instead: shaggy gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s shaggy 'shaggy gateway run'")
                print()
            ...
            print()
            print("  shaggy gateway run                              # direct foreground")
            print("  tmux new -s shaggy 'shaggy gateway run'         # persistent via tmux")
            print("  nohup shaggy gateway run > ~/.shaggy/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
            ...
            print()
            print("To run the gateway: shaggy gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: shaggy gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: shaggy gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: shaggy gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: shaggy gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  shaggy gateway install")
                else:
                    print("To install as a service:")
                    print("  shaggy gateway install")
                    print("  sudo shaggy gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  shaggy gateway run      # Run in foreground")
                if is_termux():
                    print("  nohup shaggy gateway run > ~/.shaggy/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s shaggy 'shaggy gateway run'         # persistent via tmux")
                    print("  nohup shaggy gateway run > ~/.shaggy/logs/gateway.log 2>&1 &  # background")
                elif is_windows():
                    print("  shaggy gateway install  # Install as Windows Scheduled Task (auto-start on login)")
                else:
                    print("  shaggy gateway install  # Install as user service")
                    print("  sudo shaggy gateway install --system  # Install as boot-time system service")
        ...
        # Stop, disable, and remove legacy Shaggy gateway unit files from
        # pre-rename installs (e.g. shaggy.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_shaggy_units(interactive=not yes, dry_run=dry_run)
