"""shaggy hooks — inspect and manage shell-script hooks.

Usage::

    shaggy hooks list
    shaggy hooks test <event> [--for-tool X] [--payload-file F]
    shaggy hooks revoke <command>
    shaggy hooks doctor

Consent records live under ``~/.shaggy/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.shaggy/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``shaggy hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: shaggy hooks {list|test|revoke|doctor}")
        print("Run 'shaggy hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.shaggy/config.yaml.")
        print("See `shaggy hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `shaggy hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `shaggy hooks test` and `shaggy hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Shaggy wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `shaggy hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `shaggy hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `shaggy hooks doctor`.")
