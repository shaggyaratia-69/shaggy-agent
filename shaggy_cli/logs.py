"""``shaggy logs`` — view and filter Shaggy log files.
...
Usage examples::
    ...
    shaggy logs                    # last 50 lines of agent.log
    shaggy logs -f                 # follow agent.log in real time
    shaggy logs errors             # last 50 lines of errors.log
    shaggy logs gateway -n 100    # last 100 lines of gateway.log
    shaggy logs --level WARNING    # only WARNING+ lines
    shaggy logs --session abc123   # filter by session ID substring
    shaggy logs --component tools  # only tool-related lines
    shaggy logs --since 1h         # lines from the last hour
    shaggy logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_shaggy_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Shaggy runs — try 'shaggy chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_shaggy_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_shaggy_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_shaggy_home()}/logs/")
        return

    print(f"Log files in {display_shaggy_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'shaggy chat' to generate logs)")
