# Shaggy Agent Security Policy

This document describes Shaggy Agent's trust model, names the one
...
or **security@cherriesandco.com**. Do not open public issues for
...
- Environment details (`shaggy version`, commit SHA, OS, Python
  version).
...
Shaggy Agent is a single-tenant personal agent. Its posture is
layered, and the layers are not equally load-bearing. Reporters and
operators should reason about them in the same terms.

### 2.1 Definitions

- **Agent process.** The Python interpreter running Shaggy Agent,
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Shaggy Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Shaggy Agent's documentation
...
Shaggy Agent supports two OS-level isolation postures. They address
...
Shaggy Agent supports this in two ways:

- **Shaggy Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Shaggy Agent's in-process heuristics
...
Shaggy Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Shaggy Agent
itself. Bugs in Shaggy Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Shaggy Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Shaggy Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  Shaggy Agent has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  shaggy-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Shaggy Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Shaggy Agent. Bugs in Shaggy
...
- Shaggy Agent includes supply-chain guards for MCP server
